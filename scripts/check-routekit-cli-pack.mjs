...
try {
  ...
  const qualificationObserverUrl = pathToFileURL(
    join(
      install,
      "node_modules",
      "@velum-labs",
  ...
  execFileSync(
    ...
    [
      ...
      [
        ...
        `import { qualificationGatewayCallObserver } from ${JSON.stringify(qualificationObserverUrl)};`,
        'const suiteRoot = join(process.cwd(), "packed-qualification");',
        "await mkdir(suiteRoot, { recursive: true });",
        'await writeFile(join(suiteRoot, "package.json"), "{\\"type\\":\\"module\\"}\\n");',
        'await writeFile(join(suiteRoot, "routekit.eval-manifest.json"), JSON.stringify({',
        '  version: 1, profileId: "pack-smoke", candidateModels: ["openai/candidate"],',
        '  judgeModel: "openai/judge", caseCount: 1, caseIds: ["case-1"],',
        "  maxOutputTokens: 128, expectedCallCount: 2",
        "}));",
        'const suitePath = join(suiteRoot, "pack-smoke.eval.ts");',
        "await writeFile(suitePath, [",
        '  "import { test } from \\"node:test\\";",',
        '  "import { setupAgent, setupJudge } from \\"routekit/eval\\";",',
        '  "const judge = setupJudge({ agent: setupAgent({ model: \\"openai/judge\\" }) });",',
        '  "test(\\"case-1\\", async () => {",',
        '  "  const run = await setupAgent({ model: \\"openai/candidate\\" }).run({ prompt: \\"Help\\", caseId: \\"case-1\\" });",',
        '  "  run.toComplete();",',
        ...
        "      return yield* (yield* EvalService).runComparison({",
        '        version: 1, profileId: "pack-smoke", suitePath,',
        '        candidateModels: ["openai/candidate"], judgeModel: "openai/judge",',
        "        gatewayUrl: `http://127.0.0.1:${String(address.port)}`, timeoutMs: 15_000",
        '      }, "full");',
        "    }).pipe(",
        "      Effect.provide(makeRouteKitEvalServiceLayer({}, {",
        '        bearerCredential: "pack-secret", observeGatewayCall, timeoutMs: 15_000',
        "      })),",
        "      Effect.provideService(HttpClient.HttpClient, yield* HttpClient.HttpClient)",
        "    );",
        "    return { comparison, observed: yield* Ref.get(observed) };",
        "  }).pipe(Effect.provide(NodeHttpClient.layerUndici)));",
        "  if (result.comparison.models.length !== 1 || result.observed.length !== 2) {",
        "    throw new Error(`packed qualification observation failed: ${JSON.stringify(result)}`);",
        "  }",
        "  if (result.observed.some((call) => call.callId === undefined)) {",
        "    throw new Error(`packed qualification missed call ids: ${JSON.stringify(result.observed)}`);",
        "  }",
        "} finally {",
