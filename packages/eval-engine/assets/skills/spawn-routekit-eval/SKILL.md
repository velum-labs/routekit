---
name: spawn-routekit-eval
description: Run the self-contained RouteKit Eval eval workflow, relay its one question at a time, and return the real eval results.
metadata:
  protocol: routekit-eval-eval-spawn-v1
  run_model: openai/gpt-5.6-terra
  judge_model: openai/gpt-5.6-terra
  harness: pi
---

# Spawn RouteKit Eval Eval

Use the executable that printed this skill. The executable owns the run
directory, task replay, production RouteKit Eval subprocess, answer/error files, locking,
...
1. From the repository root run:
   ...
   ```bash
   routekit-eval-engine spawn prepare --request-file <file>
   ```
...
3. Run:
   ...
   ```bash
   routekit-eval-engine spawn run
   ```
...
5. Pass the answer back without rewriting it:
   ...
   ```bash
   routekit-eval-engine spawn answer --answer-file <file>
   ```

   If the JSON has `accepted: false`, append nothing and do not restart RouteKit Eval.
...
- Never bypass `routekit-eval auth`, candidate approval, or any question emitted by RouteKit Eval.
...
- Never write "bakeoff" to the user. Replace it with "model comparison" every
  time you relay, quote, or summarise RouteKit Eval's text.
