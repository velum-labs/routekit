...
//
// THE RULE: the column reads each run's own `outcome`, and only RouteKit Eval's matchers
...
// flattering half alone is the defect this module exists to prevent (RK-EVAL-911).
//
...
// model, and node --test does not hand the SDK its enclosing test (RK-EVAL-760). Matching a
...
import type { EvalTestRow } from "../routekit-eval/junit.js";
...
import {
  ...
  runModel,
  totalCostUsd
} from "../routekit-eval/results.js";
import { UNMEASURED } from "./format.js";
...
const meanDurationMs = (rows: readonly EvalResultRow[]): number | undefined => {
  const timed = rows.flatMap((row) => (row.durationMs === undefined ? [] : [row.durationMs]));
 ...
 * ranks the models, and ranking is the one thing this report does not do (RK-EVAL-780
 * owns picking a winner). The reader draws their own conclusion from the numbers.
 */
export const rollUpModels = (results: readonly EvalResultRow[]): readonly ModelRollup[] => {
 ...
 * would be noise on the common case where RouteKit Eval's assertions did the asserting.
 *
...
export const contradictingTestFailures = (input: {
  ...
  const graded = input.rollups.some((rollup) => rollup.passedRuns + rollup.rejectedRuns > 0);
  const accounted = input.rollups.some((rollup) => rollup.rejectedRuns > 0);
  return graded && !accounted ? input.tests.filter((test) => test.status === "fail").length : 0;
};
 ...
 * argument: a ratio that counts only RouteKit Eval's own assertions can read as a clean pass
 ...
 */
export const formatCorrectness = (rollup: ModelRollup, contradicted: boolean): string => {
  ...
  const qualifiers = [
    ...
    ...(rollup.unmeasuredRuns === 0 ? [] : [`${rollup.unmeasuredRuns} ${UNMEASURED}`])
  ];
  const ratio = `${rollup.passedRuns}/${judged} passed`;
  return qualifiers.length === 0 ? ratio : `${ratio} (${qualifiers.join(", ")})`;
};
 ...
 */
export const contradictionNote = (contradictingFailures: number): readonly string[] => {
  ...
  return [
    "",
    `Marked \`${CONTRADICTED}\`: ${contradictingFailures} test${plural} failed and no run recorded a failing assertion, so the failure cannot be attributed to a model. Correctness counts the assertions RouteKit Eval graded, and a plain \`expect()\` inside a test is not one of them: it fails the test without ever reaching the run. The figures above are what RouteKit Eval measured; \`## Tests\` below is what the test file decided, and here they disagree. Assert through RouteKit Eval's matchers to have a failure land on the model that caused it.`
  ];
};
