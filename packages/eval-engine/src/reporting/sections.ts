...
import type { ModelRollup } from "./correctness.js";
import {
  CONTRADICTED,
...
import { cell, fileName, formatCost, formatDuration, table, UNMEASURED } from "./format.js";
...
const spendRuns = (rows: readonly EvalResultRow[]): string => {
  ...
  return cutOff === 0 ? String(rows.length) : `${rows.length} (${cutOff} cut off)`;
};
...
const cutOffNote = (rollups: readonly ModelRollup[]): readonly string[] => {
  ...
  const named = missing.map((rollup) => `${cell(rollup.model)} (${rollup.cutOffRuns})`).join(", ");
  return [
    "",
 ...
 *
 * Correctness is read off the row's own `outcome` (RK-EVAL-760), so it holds for the
 ...
 * never reached RouteKit Eval's channel failed the test without failing any run, and the
 * column would otherwise report a clean pass in a document whose own `## Tests`
 * says `fail` (RK-EVAL-911).
 */
...
export const testSection = (tests: readonly EvalTestRow[]): readonly string[] => {
  if (tests.length === 0) {
    return ["## Tests", "", `No per-test outcome was reported (${UNMEASURED}).`];
  }
  return [
    "## Tests",
    "",
...
const verdictLine = (candidates: readonly EvalResultRow[]): string => {
  ...
  const quoted = rejected.filter((row) => (row.outcomeDetail?.trim().length ?? 0) > 0);
 ...
 * one surface not reading it (RK-EVAL-794).
 */
export const judgeSection = (results: readonly EvalResultRow[]): readonly string[] => {
  ...
  return [
    ...
    ...table(
      ...
      ["---", "---:", "---:"],
      [
        ["Models under test", spendRuns(candidates), formatCost(totalCostUsd(candidates))],
...
export const footerSection = (input: {
  ...
  return [
    ...
    ...(contradicted
      ? [
          `- \`${CONTRADICTED}\` beside a correctness figure means the runs passed the assertions RouteKit Eval graded, and a test in this run failed anyway. The figure is measured, and it is not the last word: read \`## Tests\`.`
        ]
      : []),
