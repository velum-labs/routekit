// Keeping `.routekit/eval/history.jsonl` bounded without ever handing back a history
...
// sibling belongs to exactly one prune. Two `routekit-eval eval` runs finishing in the same
...
import { EVAL_HISTORY_MAX_RUNS, readEvalHistory, renderLines } from "../history/entry.js";

/** Marks a file in `.routekit/eval/` as a prune's scratch copy of the history. */
...
const pruneSiblingPrefix = (historyName: string): string => `${historyName}${PRUNE_SUFFIX}.`;
 ...
 */
const pruneSiblingPath = Effect.fn("EvalHistory.pruneSibling")(function* (historyPath: string) {
  ...
  return path.join(path.dirname(historyPath), `${prefix}${token.toString(PRUNE_TOKEN_RADIX)}`);
});
 ...
 */
const removeAbandonedSibling = Effect.fn("EvalHistory.removeAbandoned")(function* (input: {
  readonly now: number;
  ...
  if (Option.isNone(mtime) || input.now - mtime.value.getTime() < PRUNE_ORPHAN_MAX_AGE_MS) {
    return;
  }
 ...
 * `.routekit/eval/`, so the prune that used to do the overwriting clears them instead.
 */
const removeAbandonedSiblings = Effect.fn("EvalHistory.sweep")(function* (historyPath: string) {
  const fs = yield* FileSystem.FileSystem;
  const path = yield* Path.Path;
 ...
 */
export const pruneEvalHistory = Effect.fn("EvalHistory.prune")(function* (historyPath: string) {
