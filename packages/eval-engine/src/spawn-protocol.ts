...
export const cheaperRerunLine = (input: {
  ...
  if (input.totalCostUsd === undefined && input.evalCostUsd === undefined) {
    return "A rerun costs only the amount shown in Ori's closing table; this run's total is unmeasured.";
  }
  if (input.totalCostUsd === undefined) {
    return `A rerun costs only ${formatCost(input.evalCostUsd)}.`;
  }
  if (input.evalCostUsd === undefined) {
    return `The run cost ${formatCost(input.totalCostUsd)} in total, and a rerun costs only the amount shown in Ori's closing table.`;
  }
