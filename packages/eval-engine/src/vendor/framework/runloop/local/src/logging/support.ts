 ...
 * `GET /api/logs/stream` and the durable `.ori/logs` NDJSON without corrupting a
 ...
 * reach `ori logs` (RFC 0011). Records below `minLevel` are dropped.
 */
 ...
 * scheduler loop) can still feed `ori logs`.
 */
 ...
 * level from `ORI_LOG_LEVEL`. Records render to lines that flow to `publish`
 * (typically {@link makeDaemonHubPublish}), so diagnostics reach `ori logs`.
 */
...
/**
 * Publish the process-global `log` (RFC 0011, `ori/logger`) by bridging the
 ...
 * under `ori dev`) each release cleanly without blanking the other's install.
 *
 ...
 * layer and the daemon observability layer — so `log` routes to `ori logs` /
 ...
 * `.ori/logs` NDJSON, so diagnostics survive without touching stdout. Requires
...
> => {
  ...
  // Install the process-global `log` (RFC 0011, `ori/logger`) from the daemon's
  // hub-backed Logger, so a feature's bare `log.*` reaches `ori logs` /
  // `.ori/logs` NDJSON in daemon mode too — not just CLI stderr. `provideMerge`
