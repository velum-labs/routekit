...
export const recordFireFailure = (input: {
  ...
  const failure =
    input.failure ??
    agentFailure({
      code: "ORI_SCHEDULE_FIRE_FAILED",
      message: `The schedule fire failed before ORI received a structured agent failure: ${formatSafeErrorDiagnostic(input.error)}`,
      remediation: "Check the ORI logs for the underlying error.",
      stage: "runtime",
    });
  return recordRun(input.runtime, {
    durationMs,
