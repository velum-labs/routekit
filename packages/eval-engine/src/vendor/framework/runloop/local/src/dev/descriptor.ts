...
const DEV_DESCRIPTOR_RELATIVE_PATH = ".ori/dev.json";
...
interface ResolveLocalRuntimeEndpointInput {
   ...
   * the global workspace (`~/.ori/global`) so `ori tui`/`ori logs`/etc. can
...
const discoverDevDescriptor = Effect.fn("DevDescriptor.discover")(function* (
  ...
  // `~/.ori/global`) is provided, attach to its running daemon so companion
...
/**
 * Descriptor lookup shared by both explicit-port branches below, so `ori tui
 * --host H --port P` (as launched by `ori code`) inherits the daemon's real
 ...
 * `.ori/dev.json` (a different `ori dev` session, or the fallback global
...
const resolveDiscoveredEndpoint = Effect.fn(
  ...
  if (Option.isNone(input.host)) {
    if (discovery.descriptor === null) {
      ...
      return yield* new CliFailureError({
        detail: `No local Ori dev runtime found at ${devDescriptorPath(path, searchedRoot)}. Start \`ori dev\` or pass --host and/or --port.`,
      });
    }
  ...
  if (discovery.descriptor === null) {
    return yield* new CliFailureError({
      detail:
        "No local Ori dev runtime port found. Start `ori dev` or pass --port.",
    });
  }
