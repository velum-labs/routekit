...
// only shapes Ori materializes, so such a link is safe to adopt and rewrite.
 ...
 * Adopts a stale link Ori owns (recorded in the manifest or resolving into the
...
const prepareSymlinkSlot = Effect.fn(
  ...
  if (Option.isSome(currentLink)) {
    ...
    // A link Ori may rewrite: one the manifest recorded, or one that still
    ...
    if (!reclaimable) {
      return {
        action: "skip",
        warning: `skipped materializing skill link ${input.linkPath}: a non-Ori symlink already points to ${currentLink.value}; leaving it untouched`,
      };
    }
    yield* fs
...
const removeStaleLinks = Effect.fn(
  ...
  for (const relativePath of candidates) {
    ...
    // Only reclaim links Ori owns: ones the manifest recorded, or ones that still
