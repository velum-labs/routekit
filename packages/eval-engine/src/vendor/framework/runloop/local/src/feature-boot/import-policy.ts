...
const OPTIONAL_DEVTOOLS_STUB_NAMESPACE = "ori-optional-devtools-stub";
...
interface FreshModuleBuildRuntime {
  // True when ori is running as a compiled standalone binary, where fresh
...
const optionalReactDevtoolsCoreStubPlugin: FreshModuleBuildPlugin = {
  name: "ori-optional-react-devtools-core-stub",
  setup(build) {
...
// bundling it into a fresh `ori-fresh-*` temp dir, so `import.meta.dir(name)` and
...
export class FeatureImportPolicy extends Context.Service<
  FeatureImportPolicy,
  FreshModuleBuildPolicy
>()("ori/runtime/FeatureImportPolicy") {
