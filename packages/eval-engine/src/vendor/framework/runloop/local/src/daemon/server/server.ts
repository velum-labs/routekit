...
/**
 * Whether the daemon prints its `[ori-runtime] listening …` boot line to stdout.
 * Suppressed only by `ori code` (inline/main-screen TUI), where a boot line on
 * stdout is painted over into a stray `[`. Kept everywhere else — `ori dev`/`ori
...
const logDaemonServer = Effect.fn("RuntimeHttp.logServer")(function* (
  ...
  yield* Stream.succeed(
    `[ori-runtime] listening on http://${hostname}:${port}\n`
 ...
 *   (`ori start`/`ori eval` keep refusing `POST /api/dev/schedules/:name`).
 */
...
const handleRuntimeRequest = Effect.fn("RuntimeHttp.handleRequest")(function* (
  ...
  // falling through to the final 404 — `ori start`/`ori eval` keep refusing it.
  if (routing === "mount") {
