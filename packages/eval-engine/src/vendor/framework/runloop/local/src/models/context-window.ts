...
// Pi routes through OpenRouter with an `openrouter/` prefix on the slug, and
// OpenRouter's `~` shorthand pins a provider (the claude harness's default
// model uses it); the catalog keys models by the bare `<vendor>/<model>` id.
const OPENROUTER_ROUTING_PREFIX = "openrouter/";
const OPENROUTER_PIN_PREFIX = "~";

const catalogModelId = (model: string): string => {
  const routed = model.startsWith(OPENROUTER_ROUTING_PREFIX)
    ? model.slice(OPENROUTER_ROUTING_PREFIX.length)
    : model;
  return routed.startsWith(OPENROUTER_PIN_PREFIX)
    ? routed.slice(OPENROUTER_PIN_PREFIX.length)
    : routed;
};
...
interface ContextWindowLookupShape {
  readonly lookup: (
    model?: string | null
  ) => Effect.Effect<number | undefined, never, OpenRouterModels>;
  /**
...
const buildFacts = (models: readonly OpenRouterModel[]): CatalogFacts => {
...
const loadFacts = OpenRouterModels.pipe(
...
const refresh = (
  cache: CatalogCache,
  now: number
): Effect.Effect<Fiber.Fiber<CatalogFacts>, never, OpenRouterModels> =>
...
const factsEffect = (
  cache: CatalogCache,
  now: number
): Effect.Effect<CatalogFacts, never, OpenRouterModels> =>
...
/**
 * A daemon-lifetime cached view of the OpenRouter catalog's `context_length`
...
>()("ori/runloop/ContextWindowLookup") {
