 ...
 * `.ori/logs/sessions/`. Each entry is a `<sessionId>/` directory holding a
...
/**
 * List every persisted session sidecar under `.ori/logs/sessions/`, newest
 ...
 * `.ori/logs/sessions/`, or `None` when there is nothing to resume. Reads go
 * through the disk reader (not the in-memory registry), so it works before any
 * daemon boots — which is what `ori code --resume` needs: it resolves the id to
 * hand to the child `ori tui` before starting its own runtime. Best-effort: an
 ...
 * disk-reader context from the workspace's `.ori/logs` directory. This is the
 * seam `ori code --resume` calls: it owns the launch cwd (not a daemon
