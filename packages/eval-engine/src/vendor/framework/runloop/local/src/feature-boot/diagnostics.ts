...
// A default-exported FeatureModule (formerly ORI-167) gets the sniffer-case
// `ORI_BOOT_IMPORT_FEATURE_MODULE` code instead of the generic
// `ORI_BOOT_FEATURE_DISABLED`, so the migration report (RFC 0002
...
// already does for `ORI_BOOT_IMPORT_SCHEDULE`/`ORI_BOOT_IMPORT_API`.
const disabledFeatureDiagnosticCode = (
  ...
  diagnostic._tag === "DefaultExportFeatureModuleDiagnostic"
    ? "ORI_BOOT_IMPORT_FEATURE_MODULE"
    : "ORI_BOOT_FEATURE_DISABLED";
...
const FEATURE_WARNING_CODE = {
  HollowFeatureDiagnostic: "ORI_BOOT_FEATURE_HOLLOW",
  InvalidFeatureDirectoryWarning: "ORI_BOOT_FEATURE_ID_INVALID",
  ReservedRootSkillsFileWarning: "ORI_BOOT_FEATURE_SKILLS_RESERVED",
  UnrecognizedSkillPathWarning: "ORI_BOOT_FEATURE_SKILL_PATH_UNRECOGNIZED",
  UnrecognizedFeatureModuleExportDiagnostic:
    "ORI_BOOT_FEATURE_EXPORT_UNRECOGNIZED",
} as const;
  ...
  features.flatMap((feature) =>
    (feature.deferredContributions ?? []).map((contribution) =>
      makeContributionBootDiagnostic({
        code: "ORI_BOOT_CONTRIBUTION_DISCOVERED_ONLY",
...
const dependencyDiagnosticCode = (diagnostic: DependencyDiagnostic): string =>
  diagnostic._tag === "CycleDiagnostic"
    ? "ORI_BOOT_DEPENDENCY_CYCLE"
    : "ORI_BOOT_DEPENDENCY_INVALID";
...
const formatHarnessRegistryDiagnostics = (input: {
  ...
  if (input.records.length === EMPTY_COUNT) {
    return [
      {
        code: "ORI_BOOT_HARNESS_REGISTRY_EMPTY",
  ...
  if (input.builtInDefaultHarnessName === undefined) {
    return [
      {
        code: "ORI_BOOT_HARNESS_DEFAULT_MISSING",
        contributionName: "harness",
        entryKey: "harness",
        level: "error",
...
const formatHarnessRegistryWarnings = (input: {
  ...
  return projectRecords.length > 1
    ? [
        {
          code: "ORI_BOOT_HARNESS_DEFAULT_AMBIGUOUS",
          contributionName: "harness",
          entryKey: "harness",
          level: "warning",
...
const makeBuiltInHarnessAvailabilityDiagnostics = (
  ...
  messages.map((message) => ({
    code: "ORI_BOOT_BUILT_IN_HARNESS_UNAVAILABLE",
    contributionName: "harness",
    entryKey: "harness",
    level: "warning",
    message,
 ...
 * turn runs. `ori features validate` and `ori ci boot-check` still fail on any
...
const RUNTIME_FATAL_DIAGNOSTIC_CODES: ReadonlySet<string> = new Set([
  "ORI_BOOT_DEPENDENCY_CYCLE",
  "ORI_BOOT_HARNESS_DEFAULT_MISSING",
  "ORI_BOOT_HARNESS_REGISTRY_EMPTY",
]);
