...
export const DEV_LOG_DIR_SEGMENTS = [".ori", "logs"] as const;
export const SESSIONS_DIR_SEGMENT = "sessions";
export const SESSION_METADATA_FILE = "metadata.json";
// Run files are named `<label>-<stamp>.jsonl`. `ori dev` writes `dev-…`,
// `ori start` writes `start-…`, and `ori code` writes `code-…`; the reader
// accepts any known label so a single `.ori/logs` directory (and `ori logs`)
...
/**
 * Bounds for retaining `.ori/logs` runs. We own this policy (rather than leaning
 ...
 * services plus the resolved `.ori/logs` directory. These three always travel
...
/** `.ori/logs/sessions`, holding the per-session metadata sidecars. */
export const sessionsDir = (logsDir: string, path: Path.Path): string =>
  path.join(logsDir, SESSIONS_DIR_SEGMENT);

/** `.ori/logs/sessions/<sessionId>/metadata.json` for one session's sidecar. */
 ...
 * chronological. `label` defaults to `dev`; `ori start` passes `start`. Shared
