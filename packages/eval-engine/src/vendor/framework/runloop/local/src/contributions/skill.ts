...
const importManagedSkillEntry = Effect.fn("ContributionLoader.managedSkill")(
  function* (input: {
    ...
    const resolution = yield* resolveManagedSkill({
      ...
      pinnedVersion: input.frontmatter.metadata?.["openrouter-skill-version"],
...
// The workspace model is owned by the root `ori.md` alone (ORI-488), so a
...
const inertFrontmatterWarnings = (
  ...
  "model" in frontmatter
    ? [
        `skill "${file}" for feature "${feature.id}" frontmatter \`model\` is ignored: the workspace model is declared only in the root ori.md frontmatter`,
      ]
    : [];
...
export const importSkillContributionsFromFeatures = Effect.fn(
  ...
  // holding `.ori/`), which by convention is the features root's parent.
