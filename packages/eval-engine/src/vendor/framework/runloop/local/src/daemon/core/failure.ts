...
type RuntimeCommand = Parameters<OriDaemonShape["invoke"]>[0];
...
interface RuntimeFailureEventsInput {
   ...
   * looks model-less (ORI-846). Absent, a fresh run is announced as before, so
...
const runtimeInvocationFailure = (error: unknown): AgentFailure =>
  Schema.is(RuntimeValidationError)(error)
    ? agentFailure({
        code: "ORI_RUNTIME_VALIDATION_FAILED",
        kind: "invalid-input",
        message: error.detail,
        stage: "runtime",
      })
    : agentFailure({
        code: "ORI_RUNTIME_INVOKE_FAILED",
        stage: "runtime",
      });
