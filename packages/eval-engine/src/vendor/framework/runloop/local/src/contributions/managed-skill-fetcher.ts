 ...
 * and `fetchBundle` reach the OpenRouter managed skills API; the live HTTP adapter
 * is {@link ManagedSkillFetcherLive} (managed-skill-fetcher-live.ts), which
 * authenticates with `OPENROUTER_API_KEY` or the shared credentials written by
 * `ori login`. Consumers `yield* ManagedSkillFetcher`; tests inject a fake via
...
>()("ori/runloop/ManagedSkillFetcher") {
  /**
