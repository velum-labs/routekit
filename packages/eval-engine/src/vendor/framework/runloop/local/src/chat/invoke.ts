  ...
  Stream.unwrap(
    ...
    Stream.mapError((cause) => {
      if (HttpClientError.isHttpClientError(cause)) {
        return new Error(
          `Runtime event stream from ${runtimeUrl(options, INVOKE_PATH)} failed mid-turn (${cause.message}). The local Ori runtime may have crashed or restarted; check ori start logs and retry.`,
          { cause }
        );
      }
