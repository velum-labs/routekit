...
/**
 * Run the first production boot slice for an Ori feature tree.
 *
...
export const bootFeatureProject = Effect.fn("FeatureBoot.bootProject")(
  function* (options: FeatureBootOptions) {
    ...
    // The root-persona `ori.md` (RFC 0002 root-persona.md) is not a `features/*` entry, so it is
    ...
    // Layer the `ori.md` `harness` preference (if any) ahead of the bundled harness
