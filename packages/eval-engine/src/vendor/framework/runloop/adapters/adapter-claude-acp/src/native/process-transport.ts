...
// A single spawned Claude Code process. A new ORI session maps to a fresh
...
// Resolves the OpenRouter credential. Kept separate from version verification
...
const resolveCredential = Effect.gen(function* () {
  ...
  if (Option.isNone(secret)) {
    return yield* new ClaudeCredentialError({
      detail: "OpenRouter credential is not configured",
    });
  }
  return secret.value;
});
...
const spawnClaudeProcess = Effect.fn("ClaudeProcess.spawn")(
  function* (launchInput: {
    ...
    const config = buildClaudeProcess({
      ...
      openRouterApiKey: launchInput.credential,
