...
import { evalApiBaseUrl } from "../../../../../../../host-env.ts";
...
interface ClaudeProcessInput {
  ...
  readonly openRouterApiKey: Redacted.Redacted;
...
function buildClaudeProcess(
  ...
  const [input, openRouterApiKey, startup] = args;
  let normalized: ClaudeProcessInput;
  if ("config" in input) {
    normalized = input;
  } else {
    if (openRouterApiKey === undefined || startup === undefined) {
  ...
  const sessionId = config.openRouterSessionId ?? sessionStartup.sessionId;
