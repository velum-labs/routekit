...
// Mint an opaque, ORI-owned ACP session id so Pi's native session id never
// leaves the adapter (ORI-423: keep native IDs private). The Node crypto module
...
// `sessionId` is the opaque, ORI-owned ACP session id; `sessionFile` is Pi's
...
const restoreSession = Effect.fn(function* (
  ...
  if (sessionFile === undefined) {
    ...
    // session-file path (ORI-423 keeps native IDs/paths adapter-private).
