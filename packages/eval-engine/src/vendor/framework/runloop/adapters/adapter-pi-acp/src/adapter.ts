 ...
 * wire (per ORI-423: those observations never join `AcpSessionUpdate`). */
  ...
  (effect, state, event) =>
    effect.pipe(
      ...
      Effect.catch((error) => {
        ...
        // a silently dropped blocking request is the exact failure ORI-423
...
const projectEvent = Effect.fn("PiAdapter.projectEvent")(function* (
  ...
  if ("_tag" in event) {
    ...
    // An error-tailed assistant message ends the ATTEMPT, not the turn (ORI-869):
