...
const BUILT_IN_HARNESSES = [
  {
    featureId: "@ori-builtins/harness-pi",
    sourcePath: "@ori-builtins/harness-pi/feature.ts",
    harness: piHarness,
  ...
  {
    featureId: "@ori-builtins/harness-claude",
    sourcePath: "@ori-builtins/harness-claude/feature.ts",
  ...
  {
    featureId: "@ori-builtins/harness-codex",
    sourcePath:
      "@ori-runloop/builtins-catalog/selected-adapter-contributions/codex.ts",
...
// earlier entries are absent. An `ori.md` `harness` preference is layered
...
const formatOpenRouterKeyDiagnostic = Effect.fn(
  "BuiltInHarnessCatalog.formatOpenRouterKeyDiagnostic"
  ...
  return Option.isSome(key)
    ? Option.none<string>()
    : Option.some(OPENROUTER_API_KEY_MISSING_MESSAGE);
});

const readOpenRouterKeyDiagnostic = (
  env: NodeJS.ProcessEnv
): Effect.Effect<Option.Option<string>, Error> => {
  const secrets = makeLocalRuntimeSecretStore(makeRuntimeEnvironment(env));
  return formatOpenRouterKeyDiagnostic(secrets);
};

const readLiveOpenRouterKeyDiagnostic = Effect.fn(
  "BuiltInHarnessCatalog.openRouterKeyDiagnostic"
)(function* () {
  const secrets = yield* RuntimeSecretStore;
  return yield* formatOpenRouterKeyDiagnostic(secrets);
});
...
const readBuiltInHarnessAvailability = Effect.fn(
  ...
  const openRouterKeyDiagnostic = yield* readLiveOpenRouterKeyDiagnostic();
  const availableNames = BUILT_IN_HARNESSES.flatMap((entry, index) =>
    Option.isNone(harnessDiagnostics[index]) ? [entry.name] : []
  );
  const diagnostics = [...harnessDiagnostics, openRouterKeyDiagnostic].flatMap(
...
>()("ori/runtime/BuiltInHarnessCatalog") {
