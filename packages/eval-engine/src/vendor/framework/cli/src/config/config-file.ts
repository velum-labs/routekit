 ...
 * the source of truth for the config path helpers the `ori update` blocks reuse.
 */
export const ORI_CONFIG_FILE_NAME = "config.json";
const JSON_INDENT = 2;
...
export const globalOriConfigPath = (path: Path.Path, homeDir: string): string =>
  path.join(homeDir, ORI_DIRECTORY_NAME, ORI_CONFIG_FILE_NAME);

export const localOriConfigPath = (
  path: Path.Path,
  workspaceRoot: string
): string => path.join(workspaceRoot, ORI_DIRECTORY_NAME, ORI_CONFIG_FILE_NAME);
    ...
    const localObject =
      input.workspaceRoot === undefined
        ...
        : yield* readConfigObject(
            localOriConfigPath(path, input.workspaceRoot)
          );
