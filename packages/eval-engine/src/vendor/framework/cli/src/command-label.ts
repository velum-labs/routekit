...
// tokens, so positional argument values — including an unknown `ori code`
...
import { stripLeadingOutputGlobalFlags } from "./ori-report.ts";

/** Label for an invocation with no recognized subcommand. */
export const ROOT_COMMAND_LABEL = "ori";
 ...
 * a bare `ori "fix the auth bug"` reports `code`, not the prompt.
 */
