...
export const HumanOutputFlag = GlobalFlag.setting("output-human")({
  flag: Flag.boolean("human").pipe(
    Flag.withAlias("tty"),
    Flag.withDescription(
      "Force human-readable output. Redirects and pipes are JSON by default, so `ori eval skill --human > skill.md` is how you capture the text"
    )
  ),
});
...
/** The run's output mode, from the parsed global flags, `ORI_OUTPUT`, and the TTY. */
