...
/** Which headless-deploy artifacts `ori init` materializes. */
...
// RFC 0004: the base unit is operator-owned once scaffolded — `ori update
// --sync-systemd` never rewrites it — so it must not carry the managed stamp.
const OPERATOR_OWNED_FILES = new Set<string>(["ori.service"]);
 ...
 * `Dockerfile` and `compose.yaml`; `systemd` writes the `ori.service` unit. The
 ...
 * re-run (for example a `ori init .` refresh).
 */
...
const COMPOSE_HINT =
  "Build and run it locally with:\n  OPENROUTER_API_KEY=sk-or-... docker compose -f deploy/compose.yaml up --build";
const SYSTEMD_HINT =
...
export const formatDeployScaffoldSummary = (
  ...
  const wroteUnit = result.outcomes.some((outcome) =>
    outcome.file.endsWith("ori.service")
  );
  if (wroteCompose) {
 ...
 * A no-op when neither flag is set, so the default `ori init` scaffold stays
