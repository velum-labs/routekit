...
//
// THE RULE: the column reads each run's own `outcome`, and only Ori's matchers
...
// flattering half alone is the defect this module exists to prevent (ORI-911).
//
...
// model, and node --test does not hand the SDK its enclosing test (ORI-760). Matching a
 ...
 * ranks the models, and ranking is the one thing this report does not do (ORI-780
 ...
 * would be noise on the common case where Ori's assertions did the asserting.
 *
 ...
 * argument: a ratio that counts only Ori's own assertions can read as a clean pass
...
export const contradictionNote = (
  ...
  return [
    "",
    `Marked \`${CONTRADICTED}\`: ${contradictingFailures} test${plural} failed and no run recorded a failing assertion, so the failure cannot be attributed to a model. Correctness counts the assertions Ori graded, and a plain \`expect()\` inside a test is not one of them: it fails the test without ever reaching the run. The figures above are what Ori measured; \`## Tests\` below is what the test file decided, and here they disagree. Assert through Ori's matchers to have a failure land on the model that caused it.`,
  ];
};
