...
import { readOriMdFeatureSources } from "../dev/ori-md-feature-sources.ts";
...
/**
 * Toolchain-side linking for `ori.md`-declared remote feature sources. The
 * runtime composes remote features at boot (`.ori/composed`), but npm and tsc
 ...
 * each one is symlinked under `.ori/linked-features/<name>` and registered as
...
const linkedFeaturesEntry = (name: string): string =>
  `${ORI_DIRECTORY_NAME}/${LINKED_FEATURES_DIR}/${name}`;

const isLinkedFeaturesEntry = (entry: string): boolean =>
  entry.startsWith(`${ORI_DIRECTORY_NAME}/${LINKED_FEATURES_DIR}/`);

/**
 * Rewrite the root package.json `workspaces` so its managed
 * `.ori/linked-features/<name>` entries match `linkedNames` exactly, leaving
...
/**
 * Materialize every remote source declared in the workspace's `ori.md`
 ...
 * directories under `.ori/linked-features/`, and register them as workspace
...
export const linkDeclaredRemoteFeatures = Effect.fn(
  ...
  const declared = yield* readOriMdFeatureSources(projectRoot);
  ...
  for (const value of remoteSources) {
    ...
    for (const [name, target] of dirs) {
      if (localNames.has(name)) {
        yield* cliIo
          .writeStderr(
            `ori init: local feature "${name}" shadows the one declared from ${value}.\n`
          )
          .pipe(Effect.ignore);
        continue;
      }
  ...
  if (linked.length > 0) {
    yield* writeProgressNotice(
      `Linked ${linked.length} remote feature${linked.length === 1 ? "" : "s"} (${linked.join(", ")}) into ${ORI_DIRECTORY_NAME}/${LINKED_FEATURES_DIR}/.\n`
    ).pipe(Effect.ignore);
  }
  return linked;
});
