...
type DevCommandAuthSource = Option.Option<OpenRouterAuthSource>;
...
const startDevDaemon = Effect.fn("DevCommand.startDaemon")(function* (
  ...
  config: {
    readonly authSource: DevCommandAuthSource;
    /** Expose dev-only routes (e.g. schedule dispatch); off for `ori start`. */
    readonly enableDevRoutes?: boolean;
    /** Root containing this daemon's persisted dev logs. */
    readonly devLogWorkspaceRoot?: string | undefined;
    /** Launch cwd used for `ori code` external skill discovery. */
    readonly externalSkillsRoot?: string | undefined;
    readonly featuresRoot: string | undefined;
    readonly host: string;
    ...
    /** Keep built-in daemon diagnostics off the terminal while `ori code` owns it. */
    readonly suppressTuiLogs?: boolean;
    /**
     * Keep the daemon's `[ori-runtime] listening …` boot line off stdout. Only
     * `ori code` sets this because its standalone chat normally renders inline,
...
interface HeadlessRuntimeSessionConfig {
   ...
   * session. `ori start` always sets this; `ori dev` leaves it off by default
   ...
   * the intern is reachable on its chat surface(s). `ori start` sets this (live
   * deployment without a second `ori tui`); `ori dev` leaves it off and attaches
   * the terminal TUI instead. The terminal TUI built-in (`@ori-builtins/chat-tui`)
   * is always skipped here — it cannot run headless; attach it with `ori tui`.
   */
  readonly startChats: boolean;
  /**
   * Expose the dev-only `POST /api/dev/schedules/:name` manual dispatch route on
   * the daemon. Per RFC 0008, `ori dev` sets this `true` (so a developer can fire
   * a schedule on demand) and `ori start` leaves it off. It is independent of
   * {@link armSchedules}: `ori dev` exposes the manual route whether or not it
   * auto-fires; `ori start` auto-fires but does NOT expose the route.
   */
  readonly enableDevRoutes: boolean;
  /**
   * Verb shown in the boot banner and the event-log run-file label — "dev"
   * for `ori dev`, "start" for `ori start`, "code" for `ori code`.
   */
  ...
  readonly eventLog?:
    ...
    | undefined;
  /**
   * Resolved auto-update policy. When present and not `off`, `ori start` runs a
   * periodic update checker inside the session scope so it is torn down on
   * shutdown. `ori dev` leaves this unset — only the long-lived `ori start`
   * server self-updates.
   */
  readonly autoUpdate?: ResolvedAutoUpdateConfig | undefined;
  /** `ori code` override for the descriptor + event log location; see {@link publishDevDescriptor}. */
  readonly descriptorWorkspaceRoot?: string;
  /**
   * Overrides the workspace root that anchors author-contracts and
   * `ORI_INTERN_WORKSPACE_ROOT` in {@link prepareDevFeaturesRoot}. `ori start`
   * sets this when it consolidates several `--features` sources into a composed
   * root under `.ori/composed`, so the anchor stays on the real project rather
...
const formatHeadlessRuntimeMessage = (
  ...
  const watchLine = config.watchReloads
    ...
    : "";
  return (
    `Ori ${config.label} runtime listening at http://${config.host}:${config.port}\n` +
    `${watchLine}Attach the chat with \`ori tui\` or stream events with \`ori logs --live\`. Press Ctrl+C to stop.\n`
  );
};
...
const startReloadWatcherIfEnabled = Effect.fn("DevCommand.startReloadWatcher")(
  function* (input: {
    ...
    // the headless `ori start` path (watchReloads=false) never touches; a top-level
  ...
  function* (
    ...
    return yield* Effect.scoped(
      Effect.gen(function* () {
        ...
        // Anchor precedence: an explicit caller override (`ori code`'s launch
        // cwd) > the resolved workspace (honours the `ori.md`-composition anchor
        // so a composed `.ori/composed` root still writes the descriptor into the
...
/**
 * Boot the daemon in this process, publish the dev descriptor so `ori tui` and
 * `ori logs` can attach from another terminal, optionally watch for reloads,
 * print a banner, and block until Ctrl+C. There is no TUI and no events pane:
 * this is the bare bot server. Shared by `ori start` (always) and `ori dev`'s
...
export const runHeadlessRuntimeSession = Effect.fn(
  ...
  // `performDaemonShutdown` owns the re-exec/exit via the `OriCliExit` teardown
