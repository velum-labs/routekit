// The `ori eval` flag surface. Split out of `command.ts` so that file stays about
...
// `ori eval <file>` is what people reach for first, because it reads like
...
export const hostFlag = Flag.string("host").pipe(
  Flag.withDescription(
    `Host to bind the temporary runtime to (default: ${DEFAULT_DAEMON_HOST}). A plain run does not need this: ori eval starts and tears down that runtime itself`
  ),
...
export const noHistoryFlag = Flag.boolean("no-history").pipe(
  Flag.withDescription(
    "Do not append this run's summary to .ori/eval/history.jsonl"
  )
);
...
export const baselineFlag = Flag.string("baseline").pipe(
  ...
  Flag.withDescription(
    'Run to compare this one against: "last" (default), "best", or "model:<slug>". Needs an Ori workspace, because the run history it reads is only kept inside one'
  ),
...
// file they chose the name of is one they can find again. It also keeps `ori eval`
...
// `ori eval` is meant to run in CI, where a real model call is expected. The
...
export const allowNoKeyFlag = Flag.boolean("allow-no-key").pipe(
  Flag.withDescription(
    "Do not fail when no credential is available from OPENROUTER_API_KEY or ori login (useful with --list; --dry-run already needs no credential)"
  )
);
