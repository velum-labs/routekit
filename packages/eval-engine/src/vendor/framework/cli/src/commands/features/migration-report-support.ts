...
/**
 * Read the generating CLI version a workspace recorded in its `ori.md`
 * frontmatter `version` stamp (written by `ori init`, RFC 0002). Returns
 * `undefined` when there is no `ori.md`, no `version` key, or the frontmatter
 ...
 *
 * `workspaceRoot` is the directory that holds `ori.md` — the parent of
 ...
 * live loader diagnostics with the version gap between the workspace's `ori.md`
 * `version` stamp and the running CLI. `featuresRoot` is the `features/` dir;
 * the `ori.md` stamp lives in its parent (the workspace root).
 *
 * `runningVersion` defaults to the in-process CLI version (`readVersionInfo`),
 * which is correct for `ori features validate` — that command runs entirely on
 * the binary producing the report. `ori update` MUST pass an explicit override:
...
/**
 * Advance the workspace baseline (RFC 0002): rewrite the `ori.md` frontmatter
 ...
 * re-reports until the author migrates. A missing `ori.md` is a no-op — there is
 * no persona to stamp, and `validate`/`update` already surface that separately.
 *
 * `workspaceRoot` is the directory that holds `ori.md` (the parent of `features/`).
 */
 ...
 *   - `ori features validate` passes `false` (the default): it already prints
 ...
 *   - `ori update` passes `true`: it has no separate raw-diagnostics loop, so
