...
// `ori code` and `ori tui` share these definitions so their help text and
...
export const unexpectedArguments = (): Argument.Argument<readonly string[]> =>
  Argument.string("argument").pipe(
    Argument.withDescription(
      "Unexpected positional argument; use --prompt/-p or --prompt-file (see `ori --help`)"
    ),
    Argument.variadic()
  );

/** The parsed initial-prompt flags shared by `ori code` and `ori tui`. */
  ...
  function* (config: InitialPromptConfig, commandName: string) {
    if (config.unexpectedArguments.length > 0) {
      return yield* new CliFailureError({
        detail: `Unexpected positional argument. Use --prompt/-p or --prompt-file with \`${commandName}\` for initial prompt text, or \`ori --help\` for the command list.`,
