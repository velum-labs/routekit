...
const dryRunFailure = (input: {
  ...
  new CliFailureError({
    detail: `\`ori eval --dry-run\` could not confirm that ${input.files.length === 1 ? "the eval" : "every eval"} it discovered loaded.`,
    hint: UNINSTALLED_DEPENDENCY.test(input.output)
      ? "That package is not installed, so this is probably the workspace rather than the eval: run `npm install`, then `ori eval --dry-run` again. A full `ori eval` installs the workspace itself; a dry run does not, because it is meant to cost nothing."
 ...
 * `files` key every other `ori eval` envelope carries, plus what the dry run
 * learned.
 */
  ...
  if (input.mode === "json") {
    ...
    return yield* ok
      ...
      : new CliOutputAlreadyReported({
          cause:
            "`ori eval --dry-run` could not load every eval it discovered.",
        });
  }
