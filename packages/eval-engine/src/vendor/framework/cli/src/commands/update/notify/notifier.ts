 ...
 * are surfaced there with an `ori update` instruction.
 */
interface Notifier {
...
export const formatHeldUpdateMessage = (notice: HeldUpdateNotice): string =>
  `Ori ${notice.severity} update available: ${versionLabel(notice.currentVersion)} -> ${notice.latestVersion}. ` +
  "This exceeds the auto-update threshold and needs approval. Run `ori update` to install it.";

export const formatAppliedMessage = (version: string): string =>
  `Ori updated to ${version}. Restarting the runtime to apply it.`;

export const formatFailureMessage = (detail: string): string =>
  `Ori auto-update failed: ${detail}`;
