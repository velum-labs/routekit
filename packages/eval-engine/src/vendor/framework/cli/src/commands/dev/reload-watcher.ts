...
const ORI_DEV_IGNORE_ENV = "ORI_DEV_IGNORE";
const ORI_DEV_RELOAD_DRAIN_TIMEOUT_MS_ENV = "ORI_DEV_RELOAD_DRAIN_TIMEOUT_MS";
const ORI_DEV_WATCH_POLL_ENV = "ORI_DEV_WATCH_POLL";
...
const readDevReloadConfig = Effect.fn("DevReloadWatcher.readConfig")(
  function* () {
    const poll = yield* readOptionalConfigString(ORI_DEV_WATCH_POLL_ENV);
    const drainTimeout = yield* readOptionalConfigString(
      ORI_DEV_RELOAD_DRAIN_TIMEOUT_MS_ENV
    );
    const ignore = yield* readOptionalConfigString(ORI_DEV_IGNORE_ENV);
    return {
...
export const startDevReloadWatcher = Effect.fn("DevReloadWatcher.start")(
  function* (
    ...
    // Surface failures on the detached reload fiber instead of dying silently (ORI-248).
    yield* events.pipe(
