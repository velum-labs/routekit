...
/**
 * `ori`'s optional configuration file. It lives at two scopes — a global
 * `~/.ori/config.json` and a workspace-local `<workspace>/.ori/config.json` —
...
const ORI_AUTO_UPDATE_ENV = "ORI_AUTO_UPDATE";
const ORI_UPDATE_INTERVAL_ENV = "ORI_UPDATE_INTERVAL";
const ORI_DRAIN_TIMEOUT_ENV = "ORI_DRAIN_TIMEOUT";
const ORI_UPDATE_RESTART_ENV = "ORI_UPDATE_RESTART";
/** Reserved opt-out emitted by the installer; when truthy it forces auto-update off. */
const ORI_NO_UPDATE_CHECK_ENV = "ORI_NO_UPDATE_CHECK";
 ...
 * subscription. It is set only via `ori start --alpha` and resolved through
...
/**
 * The `ori code` early-access opt-in lives at the top level of `config.json` as
 * `earlyAccess`. Its read/write pair lives in `ori-early-access.ts`; the field
 * is modelled here too so {@link loadOriConfigFiles} surfaces it alongside the
 * `autoUpdate` block.
 */
const OriConfigFileSchema = Schema.Struct({
...
const decodeOriConfigFile = decodeJsonString(OriConfigFileSchema);

type AutoUpdateFileConfig = typeof AutoUpdateConfigSchema.Type;
type OriConfigFile = typeof OriConfigFileSchema.Type;
 ...
 */
const readOriConfigFile = Effect.fn("OriConfig.readFile")(function* (
  filePath: string
) {
  ...
  if (!exists) {
    return Option.none<OriConfigFile>();
  }
  const decoded = yield* fs
    .readFileString(filePath)
    .pipe(Effect.flatMap(decodeOriConfigFile), Effect.option);
  return decoded;
});
 ...
 * be resolved).
 */
const loadOriConfigFiles = Effect.fn("OriConfig.load")(function* (input: {
  readonly homeDir: string;
  readonly workspaceRoot: string | undefined;
}) {
  const path = yield* Path.Path;
  const globalConfig = yield* readOriConfigFile(
    globalOriConfigPath(path, input.homeDir)
  );
  const localConfig =
    input.workspaceRoot === undefined
      ? Option.none<OriConfigFile>()
      : yield* readOriConfigFile(localOriConfigPath(path, input.workspaceRoot));
  return {
    global: Option.getOrElse(globalConfig, () => ({}) as OriConfigFile),
    local: Option.getOrElse(localConfig, () => ({}) as OriConfigFile),
  };
});
 ...
 * lenient env parsers first; a malformed `ORI_AUTO_UPDATE=bogus` or a blank
...
const providerFromEnv = (
  ...
  ConfigProvider.fromUnknown({
    drainTimeoutMs: parseIntegerEnv(env[ORI_DRAIN_TIMEOUT_ENV]),
    intervalMs: parseIntegerEnv(env[ORI_UPDATE_INTERVAL_ENV]),
    level: parseLevel(env[ORI_AUTO_UPDATE_ENV]),
    restart: parseRestart(env[ORI_UPDATE_RESTART_ENV]),
  });

/**
 * The reserved `ORI_NO_UPDATE_CHECK` opt-out forces the level to `off`. It sits
...
const providerFromNoUpdateOptOut = (
  ...
  const forced = isTruthyEnv(env[ORI_NO_UPDATE_CHECK_ENV]);
 ...
 *
 * The reserved `ORI_NO_UPDATE_CHECK` env opt-out forces the level to `off`
 * unless an explicit `--auto-update` flag is supplied.
 */
const resolveAutoUpdateConfig = Effect.fn("OriConfig.resolve")(
  function* (input: {
    ...
    // Precedence, highest first: CLI flag > ORI_NO_UPDATE_CHECK force-off > env
...
export const resolveAutoUpdateRuntimeConfig = Effect.fn(
  ...
  const files = yield* loadOriConfigFiles({
    homeDir,
...
export {
  ...
  ORI_AUTO_UPDATE_ENV,
  ORI_UPDATE_INTERVAL_ENV,
  ORI_DRAIN_TIMEOUT_ENV,
  ORI_UPDATE_RESTART_ENV,
  ORI_NO_UPDATE_CHECK_ENV,
