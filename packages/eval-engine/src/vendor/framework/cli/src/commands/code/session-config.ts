...
// Session-input resolution for `ori code`: the config builders plus the
 ...
 * `resolvable`, which is exactly what `ori login` does: it resolves env →
 * workspace `.ori/credentials.json` → global `~/.ori/credentials.json` →
 * `~/.openrouter/credentials.json`, and — crucially for a command that runs in
 * arbitrary directories — when no credential is found it runs interactive login
 * at `workspace-preferred` scope, the same scope `ori login` (no `--local`)
 * uses. `workspace-with-global-fallback` (used by `ori dev`) would instead drive
 * login at `workspace` scope, which fails with WorkspaceRootNotFound outside an
 * Ori workspace — the very directories `ori code` targets. `--no-global-auth`
 * opts into the strict `workspace` mode (matches `ori login --local`).
 */
const codeCredentialMode = (globalAuth: boolean): "resolvable" | "workspace" =>
  globalAuth ? "resolvable" : "workspace";

// `ori code` keeps the default event-log retention (flag > env > built-in
...
/** Inputs for `ori code` session config, resolved before daemon boot. */
export interface CodeSessionInputs {
  readonly authSource: DevCommandAuthSource;
  readonly featuresRoot: string;
  /** Optional harness id from `--harness`, forwarded to the child `ori tui`. */
  readonly harness?: string;
  readonly host: string;
  readonly launchCwd: string;
  /** Optional model slug from `--model`, forwarded to the child `ori tui`. */
  readonly model?: string;
  readonly port: Option.Option<number>;
  /**
   * Optional session id to resume (from `--resume` or `--session`), forwarded
   * to the child `ori tui` as `--session`.
   */
  readonly sessionId?: string;
}

/**
 * Build the argv for the child `ori tui` that `ori code` attaches. `ori code`
 ...
 * `ori dev`'s scriptable fallback. `ori code` is a develop-style session (attach
 * the chat with `ori tui`), not a headless bot bridge, so it does not arm
...
export const codeHeadlessConfig = (
  ...
  // Publish `.ori/dev.json` (and the event log) into the user's launch cwd, not
  // the built-in bundle's temp dir, so an `ori tui`/`ori logs` run from their
...
/**
 * Build the one-shot invoke command for a headless `ori code` turn.
 *
 ...
 * executes in `~/.ori/global` while the audit log still records the caller's
