// How `ori eval` gets a runtime for the child to talk to: the ephemeral daemon
// and its boot config.
//
...
const RUNTIME_HOST_ENV = "ORI_RUNTIME_HOST";
const RUNTIME_PORT_ENV = "ORI_RUNTIME_PORT";
...
// `OriCliExit` comes from prepareDevFeaturesRoot: an `install: true` boot may
// install workspace deps and re-exec, propagating the child's exit code as a
// deliberate `OriCliExit` rather than a manual exit syscall.
...
/**
 * The daemon boot config `ori eval` uses. Exported so a test can assert the
...
export const evalDaemonBootConfig = (
  ...
  featuresRoot,
  host: config.host,
  // No `--port`: bind an ephemeral open port so concurrent `ori eval` runs never
  // collide on the default daemon port.
  port: Option.none(),
  suppressAuditStdout: true,
  // The `[ori-runtime] listening …` boot line would otherwise land on stdout
  ...
  // through ORI_RUNTIME_HOST/PORT. It is noise for a human too, since the port is
