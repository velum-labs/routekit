...
/**
 * Set on a child (the relaunched `ori code` and the spawned `ori tui`) so it
 * never re-runs the launch-time update check that its parent already ran.
 */
const ORI_CODE_UPDATE_RELAUNCHED_ENV = "ORI_CODE_UPDATE_RELAUNCHED";

// `ori code` is always argv[2]+ (node, the CLI entry, then the command); `oriChildArgv` rebuilds the executable
...
/**
 * Whether an `ori code` launch should run the interactive update check: only
...
export const shouldCheckForCodeUpdate = (
  ...
  input.env[ORI_CODE_UPDATE_RELAUNCHED_ENV] !== "1";
...
const promptCodeUpdateSelection = Effect.fn("CodeUpdate.prompt")(function* (
  ...
  return yield* Prompt.select<CodeUpdateSelection>({
    ...
    message: `Ori ${latestVersion} is available. Update before launching?`,
  });
});
...
const relaunchOriCode = Effect.fn("CodeUpdate.relaunch")(function* () {
  ...
  if (argv.length === 0) {
    return yield* new CliFailureError({
      detail:
        "Could not relaunch `ori code` after updating: no executable command was available.",
    });
  }
  ...
  const exitCode = yield* spawner.exitCode(
    makeInteractiveChildCommand(command, args, {
      env: {
        ...env,
        [ORI_CODE_UPDATE_RELAUNCHED_ENV]: "1",
      },
    })
  );
  // Mirror the dev dependency-install re-exec: carry the child's exact code out
  // through `OriCliExit` so runtime teardown applies it after finalizers run.
...
const applyCodeUpdate = Effect.fn("CodeUpdate.apply")(function* (input: {
  ...
  yield* cliIo
    .writeStdout(`Updating Ori to ${input.latestVersion} before launch...\n`)
    .pipe(Effect.ignore);
  yield* runUpdateFromExecutablePath(input.executablePath, input.channel, {});
  return yield* relaunchOriCode();
});

/**
 * Best-effort launch-time update flow for `ori code`, run before booting the
...
export const runCodeUpdateLaunch = Effect.fn("CodeUpdate.run")(function* () {
  ...
  if (
    ...
    env[ORI_CODE_UPDATE_RELAUNCHED_ENV] === "1"
  ) {
    return;
  }
  ...
  yield* hostProcess.setEnv(ORI_CODE_UPDATE_RELAUNCHED_ENV, "1");
...
export { ORI_CODE_UPDATE_RELAUNCHED_ENV };
