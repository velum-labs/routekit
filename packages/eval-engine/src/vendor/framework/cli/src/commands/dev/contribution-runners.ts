 ...
 * runner backs off, and an invalid boot's error diagnostics (a broken `ori.md`
 * frontmatter that dropped the workspace `model:`, a rejected feature) are
 * printed so the session doesn't silently run on built-in defaults (ORI-466).
 */
const inspectBootLoudly = Effect.fn("DevCommand.inspectBootLoudly")(function* (
  ...
  const boot = yield* runtime.inspect(featuresRoot).pipe(
    Effect.tapError((error) =>
      log(
        `[ori-runtime] feature boot failed for ${featuresRoot}: ${formatUnknownError(error)}`
      )
    ),
    Effect.option
  );
  if (boot._tag === "Some" && !boot.value.valid) {
    ...
    yield* log(
      `[ori-runtime] feature boot has ${errors.length} error diagnostic(s)`
    );
    yield* Effect.forEach(
      errors,
      (diagnostic) =>
        log(`[ori-runtime] boot: ${formatBootDiagnostic(diagnostic)}`),
      { discard: true }
    );
  }
  return boot;
...
const armSchedules = Effect.fn("DevCommand.armSchedules")(function* (input: {
  ...
  yield* input.log(
    `[ori-runtime] arming ${input.schedules.length} schedule(s)`
  );
 ...
 * by `ori start` and `ori dev`'s headless fallback via `runHeadlessRuntimeSession`.
 *
...
const startScheduleContributions = Effect.fn("DevCommand.schedules")(
  function* (input: {
    readonly cwd: string;
    /** Daemon runtime whose log hub `ori logs` reads; scheduler diagnostics publish here (RFC 0011). */
    ...
    if (store._tag === "None") {
      yield* log(
        "[ori-runtime] schedules registered but no state store is available; skipping scheduler"
      );
      return;
    }
...
// explicitly with `ori tui`. Every other chat contribution (a Slack/HTTP bridge
...
const TERMINAL_TUI_CHAT_FEATURE_ID = "@ori-builtins/chat-tui";
...
const forkChatContributionStart = Effect.fn("DevCommand.chatStart")(
  function* (input: {
    ...
    // the session scope closes, mirroring `ori tui`'s acquire/release lifecycle.
    ...
    yield* Effect.forkScoped(
      Effect.tryPromise(() => input.contribution.start(input.chat)).pipe(
        Effect.tapError((error) =>
          input.log(
            `[ori-runtime] chat "${input.name}" failed: ${formatUnknownError(error)}`
          )
        ),
        Effect.ignore
      )
 ...
 * daemon's `/api/invoke` path through the same `makeChat` handle `ori tui` uses,
 * so they work whether or not a TUI is attached. Shared by `ori start` via
...
const startChatContributions = Effect.fn("DevCommand.chats")(function* (input: {
  ...
  yield* log(`[ori-runtime] starting ${chats.length} chat surface(s)`);
