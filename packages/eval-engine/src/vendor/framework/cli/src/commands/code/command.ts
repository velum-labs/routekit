...
import { ORI_PERSONA_ENV } from "../../../../contracts/internal/src/cli/intern-launcher-env.ts";
...
import { oriChildArgv } from "../dev/split/child-argv.ts";
...
// `ori code` has no model registry of its own, so `--model` is forwarded to the
// child `ori tui`, which threads it into each turn's invoke. Without it the
// runtime's built-in default model is used.
const modelFlag = Flag.string("model").pipe(
  Flag.withDescription(
    "Model ID to use for turns (OpenRouter slug, e.g. anthropic/claude-sonnet-4.6)"
  ),
  Flag.optional
);

// `ori code` has no harness registry of its own, so `--harness` is forwarded to
// the child `ori tui`, which threads it into each turn's invoke (same as the
...
// `ori code` is meant to "just work" in any local repo, so it allows global auth
// by DEFAULT — the inverse of `ori dev`, which defaults to workspace-only auth.
// A plain repo has no repo-local `.ori/credentials.json`, so workspace-only auth
...
const globalAuthFlag = Flag.boolean("global-auth").pipe(
  ...
  Flag.withDescription(
    "Allow OPENROUTER_API_KEY and global ~/.ori/credentials.json (default: on; use --no-global-auth for workspace-only auth)"
  )
);

// `ori code` writes durable per-session metadata sidecars under the launch
// cwd's `.ori/logs/sessions/`, so the latest one is resolvable from disk before
...
const sessionFlag = Flag.string("session").pipe(
  Flag.withDescription(
    "Resume a specific chat session by id (see `ori sessions`)"
  ),
  Flag.optional
);
...
const attachCodeTui = Effect.fn("CodeCommand.attachTui")(function* (input: {
  ...
  if (argv.length === 0) {
    return yield* new CliFailureError({
      detail:
        "Could not launch `ori tui`: no executable command was available.",
    });
  }
  const [command, ...args] = argv;

  const spawner = yield* ChildProcessSpawner.ChildProcessSpawner;
  // `ori tui` is the interactive foreground UI — it must stay in this terminal's
  ...
  const exitCode = yield* spawner.exitCode(
    makeInteractiveChildCommand(command, args, {
      ...
      env: {
        ...childEnv,
        ORI_TELEMETRY_SURFACE: "code",
      },
    })
  );
  if (Number(exitCode) !== 0) {
    return yield* new CliFailureError({
      detail: `\`ori tui\` exited with code ${Number(exitCode)}.`,
      hint: "Run `ori code` again to restart the daemon and UI.",
    });
  }
});
...
const runCodeCommand = Effect.fn("CodeCommand.run")(function* (
  ...
  const initialPrompt = yield* resolveInitialPrompt(config, "ori code");
  ...
  // On a TTY with nothing found, this runs the same interactive login `ori
  // login` would, so a single `ori login` lets `ori code` run anywhere.
  ...
  // `ori code` boots against the global workspace (`~/.ori/global/features`), so
  ...
  // (see `BuiltInPromptCatalog`), gated on `ORI_PERSONA=code`. Each chat turn runs
  ...
  yield* hostProcess.setEnv(ORI_PERSONA_ENV, "code");
...
> =>
  Command.make(
    ...
    Command.withDescription(
      "Run Ori as a local coding agent in the current directory"
    )
  );
