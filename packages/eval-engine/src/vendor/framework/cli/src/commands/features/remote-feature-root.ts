 ...
 * `github.com/OpenRouterInterns/features[/sub/dir][@ref]` anywhere it accepts
 * a local directory. Downloaded as a tarball (HTTPS + `tar`) into the
 * `.ori/remote-features` cache, with a `git`-over-SSH backup on HTTPS failure
 * (see `remote-feature-ssh-fallback.ts`). Spec: docs/rfcs/0004-cli/features.md.
 */

/** Subdirectory of `.ori/` holding materialized remote feature roots. */
const REMOTE_FEATURES_CACHE_DIR = "remote-features";

const ARCHIVE_TEMP_PREFIX = "ori-remote-features-";
...
const cacheDirFor = Effect.fn("RemoteFeatures.cacheDir")(function* (
  ...
  // Inside a workspace the cache lives in the workspace's own `.ori/` (the
  // node_modules-shaped spot, already gitignored). Outside one it falls back
  // to `~/.ori` so `ori features list --features=github.com/...` works anywhere.
...
/**
 * Materialize a remote source into the `.ori/remote-features` cache and return
