...
import { ORI_UPDATE_BASE_URL } from "../update/release-channel.ts";
...
const ORI_RELEASES_REPO = "OpenRouterLabs/ori-releases";
/**
 ...
 */
const DOCS_BUNDLE_ENV = "ORI_DOCS_BUNDLE";
/**
 * Latest stable asset, resolved through the `openrouter.ai/labs/ori/*` proxy →
 ...
 */
const DOCS_BUNDLE_LATEST_URL = `${ORI_UPDATE_BASE_URL}/${DOCS_BUNDLE_ASSET}`;
/**
...
const pinnedBundleUrl = (version: string): string =>
  `https://github.com/${ORI_RELEASES_REPO}/releases/download/cli-${version.replaceAll(RELEASE_VERSION_MARKER, "-")}/${DOCS_BUNDLE_ASSET}`;
...
const cachePathFor = Effect.fn("DocsBundleSource.cachePath")(function* (
  ...
  return oriDirectory.docsBundleCachePath(homeDir, version);
});
 ...
 * `~/.ori/cache/docs/<version>/docs-bundle.json`; later runs (including offline
 * ones) read the cache. `ORI_DOCS_BUNDLE` short-circuits both steps with a local
