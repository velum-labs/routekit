 ...
 *
 * Correctness is read off the row's own `outcome` (ORI-760), so it holds for the
 ...
 * never reached Ori's channel failed the test without failing any run, and the
 ...
 * one surface not reading it (ORI-794).
 */
...
export const footerSection = (input: {
  ...
  return [
    ...
    ...(contradicted
      ? [
          `- \`${CONTRADICTED}\` beside a correctness figure means the runs passed the assertions Ori graded, and a test in this run failed anyway. The figure is measured, and it is not the last word: read \`## Tests\`.`,
        ]
      : []),
