...
// unreadable entry never aborts the whole snapshot (ORI-248).
...
export const logWatcherStopped =
  ...
  (cause: Cause.Cause<unknown>): Effect.Effect<void> =>
    Effect.gen(function* () {
      // A clean `ori dev` exit disposes the watcher's scope, which interrupts this
      // fiber. That is not a failure, so stay silent and only report genuine
      // stream/fiber deaths (ORI-248) instead of logging on every shutdown.
