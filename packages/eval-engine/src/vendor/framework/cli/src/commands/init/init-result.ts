...
import { hasResolvableOpenRouterCredential } from "../login/credentials.ts";

/**
 * What `ori init` did, for a caller that reads the outcome instead of the
...
const describeInitResult = Effect.fn("ProjectInit.describeResult")(
  function* (input: {
    ...
    const signedIn = yield* hasResolvableOpenRouterCredential({
