// Making an `ori eval` run outlive the command. The results channel
...
// `.ori/eval/history.jsonl` in the workspace, so a pass rate can be watched over
 ...
 * all, which keeps `ori eval` in a bare directory exactly as silent as it was
 * before this file existed. The history lives in the workspace rather than in
 * `~/.ori` because a pass rate is a fact about one project's evals, and a shared
...
export const recordEvalRunForWorkspace = Effect.fn("EvalHistory.recordForRun")(
  function* (input: {
    ...
    yield* recordEvalRun({
      entry: input.entry,
      historyPath: oriDirectory.evalHistoryPath(workspaceRoot.value),
    });
  }
);
