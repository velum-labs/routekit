...
const TEMPLATES_REPO_ENV = "ORI_TEMPLATES_REPO";
/** Override the branch, tag, or commit fetched from the templates repository. */
const TEMPLATES_REF_ENV = "ORI_TEMPLATES_REF";
...
// a moving branch, so a plain `ori init` scaffolds the same content until the
// pin is bumped; `ORI_TEMPLATES_REF` overrides it for callers who want `main`
// or another ref.
const DEFAULT_TEMPLATES_REPO = "OpenRouterIncubator/templates";
...
const ARCHIVE_TEMP_PREFIX = "ori-template-";
...
export const downloadTemplatesArchive = Effect.fn(
  ...
  if (!REPO_SLUG_PATTERN.test(repo)) {
    return yield* new ProjectInitError({
      detail: `Invalid templates repository "${repo}". Use a public "owner/repo" slug, e.g. "OpenRouterIncubator/templates".`,
      exitCode: TEMPLATE_RESOLVE_EXIT_CODE,
      operation: "fetching templates",
    });
  }
