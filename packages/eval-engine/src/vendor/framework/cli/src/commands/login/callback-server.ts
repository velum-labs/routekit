...
const SUCCESS_HTML = buildPage({
  heading: "You're signed in to OpenRouter.",
...
const ERROR_HTML = buildPage({
  ...
  body: "Return to your terminal and run <code>ori login</code> again.",
});
...
const handleCallbackRequest = (
  ...
  if (error !== null) {
    Deferred.doneUnsafe(
      codeDeferred,
      new CliFailureError({
        detail: `OpenRouter authorization failed: ${error}`,
      })
    );
  }
