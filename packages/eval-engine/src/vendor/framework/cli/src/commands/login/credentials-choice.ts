...
const WORKSPACE_CREDENTIAL_CHOICE_FILE_NAME = "openrouter-auth.json";
const JSON_INDENT = 2;
...
const chooseProjectOrStoredCredential = Effect.fn(
  ...
  readonly projectSource: Option.Option<OpenRouterAuthSource>;
  readonly startDir: string;
  readonly startupProjectSource: Option.Option<OpenRouterAuthSource>;
  readonly workspaceRoot: string;
}) {
  ...
  if (Option.isSome(recorded)) {
    ...
    choice = yield* Prompt.select<WorkspaceCredentialChoice["choice"]>({
      choices: [
        ...
        {
          title: "Stored Ori credential",
          value: "stored",
        },
      ],
      message:
        "A project dotenv credential and a stored Ori credential are available. Which should this workspace use?",
    });
  ...
  if (choice === "project") {
    if (Option.isSome(input.startupProjectSource)) {
      return yield* restoreDotenvOpenRouterCredentialAtStartup(
        input.workspaceRoot
      );
    }
...
const resolveProjectCredentialConflict = Effect.fn(
  ...
  readonly existingSource: Option.Option<OpenRouterAuthSource>;
  readonly interactive: boolean;
  readonly startDir: string;
}) {
  const workspaceRoot = yield* (yield* OriDirectory).workspaceRootFrom(
    input.startDir
  );
  const startupProjectSource = yield* projectOpenRouterAuthSourceAtStartup();
  let projectSource = Option.none<OpenRouterAuthSource>();
  if (
