 ...
 * `autoUpdate` policy is present and not `off`: only the long-lived `ori start`
...
interface StartDaemonContributionsInput {
  ...
  /** Run-file label for the event log (`dev` for `ori dev`, `start` for `ori start`). */
  readonly runLabel: RunLabel;
...
export const startDaemonContributions = Effect.fn(
  ...
  // so `ori/state`/`Chat.stores` and `ori/config`/`Chat.config` resolve in-process
  ...
  if (input.armSchedules) {
    yield* startScheduleContributions({
      // Scheduler diagnostics flow to the daemon log hub `ori logs` reads (RFC
