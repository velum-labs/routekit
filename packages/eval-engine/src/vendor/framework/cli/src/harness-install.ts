...
/**
 * The coding agents `ori` can launch, and can offer to install. This is the one
...
export interface HarnessInstallRecipe {
  ...
  /**
   * The method Ori offers to run for the user. Every recipe's script installs
   ...
   * terminal and degrades when refused, so Ori never elevates on its own.
   */
...
/**
 * The install block every `ori <harness>` prints when the binary is missing:
...
export const harnessInstallHint = (recipe: HarnessInstallRecipe): string =>
  `Install ${recipe.displayName} with \`${recipe.recommended.command}\`, then run \`ori ${recipe.kind}\` again. Docs: ${recipe.docsUrl}`;
