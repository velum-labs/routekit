...
import type { StateStore } from "../../ori/src/index.ts";
...
const DEFAULT_STATE_DIR = ".ori";
const FOREIGN_KEYS_PRAGMA = "PRAGMA foreign_keys = ON";
const JOURNAL_MODE_PRAGMA = "PRAGMA journal_mode = WAL";
const ORI_SQLITE_PATH_ENV = "ORI_SQLITE_PATH";
const ORI_STATE_DIR_ENV = "ORI_STATE_DIR";
const RESERVED_KV_TABLE_SQL = `
CREATE TABLE IF NOT EXISTS ori_kv (
  key TEXT PRIMARY KEY,
  value TEXT NOT NULL
)`;
const READ_KV_SQL = "SELECT value FROM ori_kv WHERE key = ?";
const UPSERT_KV_SQL = `
INSERT INTO ori_kv (key, value)
VALUES (?, ?)
...
const readSqliteStateStoreConfig = (
  ...
  const explicitPath = normalizeEnvValue(env[ORI_SQLITE_PATH_ENV]);
  if (explicitPath !== undefined) {
    return { path: explicitPath };
  }

  const stateDir =
    normalizeEnvValue(env[ORI_STATE_DIR_ENV]) ?? join(cwd, DEFAULT_STATE_DIR);
...
const readSqliteStateStoreConfigFromConfig = Effect.fn(
  ...
  const explicitPath = yield* readOptionalConfigString(ORI_SQLITE_PATH_ENV);
  if (explicitPath !== undefined) {
    return { path: explicitPath };
  }

  const stateDir =
    (yield* readOptionalConfigString(ORI_STATE_DIR_ENV)) ??
...
// The reserved `ori_kv` table is `value TEXT NOT NULL` and only ever written a
...
// Project the `ori_kv` read to its value. An absent row is a genuine key miss.
// A present row that fails to decode is out-of-band (`ori_kv.value` is
...
const readKvValue = (
  ...
  row === undefined
    ...
    : Option.match(decodeKvRowOption(row), {
        onNone: () =>
          Effect.logDebug(
            `sqlite ori_kv row for "${key}" failed to decode`
...
export class SqliteStateStore extends Context.Service<
  SqliteStateStore,
  StateStore
>()("ori/builtins/db-sqlite/SqliteStateStore") {
  /**
