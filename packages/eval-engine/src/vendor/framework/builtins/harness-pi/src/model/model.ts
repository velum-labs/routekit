...
// credentials), so the harness pins routing to OpenRouter in two ways:
//
...
//   2. It forces every slug through the `openrouter/` provider segment (see
//      `forceOpenRouterModelSlug`), so neither a bare slug nor the default can
//      resolve against a non-OpenRouter provider.
//
...
const ORI_PI_DEFAULT_MODEL_ENV = "ORI_PI_DEFAULT_MODEL";

const ORI_PI_MAX_TOKENS_ENV = "ORI_PI_MAX_TOKENS";

const RADIX_DECIMAL = 10;

// OpenRouter's 402 credit/budget rejection ("This request requires more credits,
...
// out. See ORI-351 and https://openrouter.ai/docs.
...
const OPENROUTER_HOSTNAME_PATTERN = /(?:^|\.)openrouter\.ai$/iu;
...
/**
 * Force a model slug to route through the OpenRouter provider by prepending the
 * `openrouter/` provider segment. Callers hand this OpenRouter catalog ids
 * (`x-ai/grok-4.5`, `openrouter/fusion`), so an `openrouter/`-leading value is
 * treated as already forced only when the remainder still looks like a catalog
 * id (contains a `/`): `openrouter/x-ai/grok-4.5` passes through unchanged,
 * while the first-party catalog id `openrouter/fusion` gains the provider
 * segment (`openrouter/openrouter/fusion`) so pi routes it through OpenRouter
 * rather than treating `fusion` as the model.
 */
const forceOpenRouterModelSlug = (model: string): string => {
  if (!model.startsWith(OPENROUTER_MODEL_PREFIX)) {
    return `${OPENROUTER_MODEL_PREFIX}${model}`;
  }
  const remainder = model.slice(OPENROUTER_MODEL_PREFIX.length);
  return remainder.includes("/") ? model : `${OPENROUTER_MODEL_PREFIX}${model}`;
};

const readPiDefaultModel = (env: NodeJS.ProcessEnv): string =>
  normalizeEnvValue(env[ORI_PI_DEFAULT_MODEL_ENV]) ?? PI_FALLBACK_DEFAULT_MODEL;
...
const resolvePiModel = (
  ...
  return forceOpenRouterModelSlug(model);
};

/**
 * The pi-side model id for a forced `openrouter/...` slug: pi keys its
 * `providers.openrouter.modelOverrides` by the id WITHIN the provider, so the
 * leading `openrouter/` segment is dropped (`openrouter/deepseek/r1:free` →
 * `deepseek/r1:free`).
 */
const piOpenRouterModelId = (forcedSlug: string): string =>
  forcedSlug.startsWith(OPENROUTER_MODEL_PREFIX)
    ? forcedSlug.slice(OPENROUTER_MODEL_PREFIX.length)
    : forcedSlug;

/**
 * Whether a pi model slug resolves to an Anthropic model, in either the forced
 * (`openrouter/~anthropic/claude-opus-latest`) or provider-stripped
 * (`~anthropic/claude-opus-latest`) form.
 *
 * The leading `~` is OpenRouter's "latest model resolution" alias and does not
 ...
 * (`model.id.startsWith("anthropic/")`) is defeated by the same `~`, so ori
 * decides vendor identity itself. See ORI-950.
 */
const isAnthropicPiModelSlug = (slug: string): boolean => {
  const providerScoped = piOpenRouterModelId(slug);
  const withoutAlias = providerScoped.startsWith(OPENROUTER_LATEST_ALIAS_PREFIX)
    ? providerScoped.slice(OPENROUTER_LATEST_ALIAS_PREFIX.length)
    : providerScoped;
...
/** The max_tokens cap ori writes into pi's models.json for the invoked model. */
 ...
 * `openrouter/...`) model, or `undefined` when none is configured. Driven only by
 * `ORI_PI_MAX_TOKENS`; without it, the harness applies no up-front cap and clamps
...
const resolvePiMaxTokensCap = (
  ...
  const envCap = parsePositiveInt(env[ORI_PI_MAX_TOKENS_ENV]);
  return envCap === undefined
    ? undefined
    : {
        modelId: piOpenRouterModelId(resolvedModel),
        maxTokens: envCap,
      };
};

/**
 * Parse the affordable-tokens figure from an OpenRouter 402 error message ("...
...
const parseRemediationUrl = (errorMessage: string): string | undefined => {
  ...
  return (url.protocol === "http:" || url.protocol === "https:") &&
    OPENROUTER_HOSTNAME_PATTERN.test(url.hostname) &&
...
/**
 * A plain one-line rendering of an OpenRouter 402 credit rejection, or
 * `undefined` when the message is not one. The raw 402 body is journalled and
 * streamed to JSONL consumers, so ORI-882 still forbids echoing it; the only
 * field lifted from it is one openrouter.ai URL that is http(s), userinfo-free,
...
const describeCreditShortfall = (errorMessage: string): string | undefined => {
  ...
  const remediationUrl =
    parseRemediationUrl(errorMessage) ?? OPENROUTER_CREDITS_URL;
  return `Insufficient OpenRouter credits:${need} the balance affords only ${afford} output tokens. ori retried with a lower max_tokens and still could not fit. Raise the limit at ${remediationUrl}, or cap the request with ${ORI_PI_MAX_TOKENS_ENV}.`;
};
