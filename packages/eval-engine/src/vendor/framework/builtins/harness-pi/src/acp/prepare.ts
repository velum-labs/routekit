import type { AgentParameters } from "../../../ori/src/index.ts";

import { join } from "node:path";

import { layer as NodeServicesLayer } from "@effect/platform-node/NodeServices";
import { Effect, Result } from "effect";
import { harnessEffortFlag } from "../../../ori/src/index.ts";
...
const buildPiAcpEnvironment = (
  ...
  env.ORI_OPENROUTER_SESSION_ID ??= crypto.randomUUID();
  return env;
};
...
const piRetrySupport = {
  ...
  resolveModelId: (
    ...
    piOpenRouterModelId(
      forceOpenRouterModelSlug(resolvePiRuntimeModel(requested, env))
    ),
};
...
const preparePiRuntime = Effect.fn("PiAcp.prepareRuntime")(function* (input: {
  ...
  const forcedModel = forceOpenRouterModelSlug(input.model);
  ...
  const attributionExtensionPath = yield* Effect.promise(() =>
    materializeOpenRouterSessionAttributionExtension(input.env)
  );
  return {
  ...
  try {
    ...
    return {
      ...
      env: {
        ...
        ORI_OPENROUTER_SESSION_ID: env.ORI_OPENROUTER_SESSION_ID,
      },
    };
  } catch (error) {
