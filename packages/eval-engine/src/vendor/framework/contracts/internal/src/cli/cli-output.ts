...
interface CliEnvelopeError {
  ...
  /** The failure's tagged discriminant (`_tag`) when it is a known ORI error, for machine branching. */
...
// The diagnosable fields ORI's tagged errors carry. Decoding the thrown value
 ...
 * line reports overall success or failure. A chat-turn stream (`ori code`'s
 ...
 * A command with its own machine-mode escape hatch (e.g. `ori code`'s headless
...
const interactiveCommandError = (
  ...
  new CliFailureError({
    detail: `\`ori ${command}\` is interactive and needs a terminal; it can't produce machine/JSON output.`,
    hint:
      hint ??
      "Run it in a TTY, or use the JSON sub-APIs (`ori sessions`, `ori logs`, `ori schedules`) for machine output.",
  });
