...
// `openrouter-skill-id`) or by its workspace-scoped slug (`pdf-extract`, via
// `openrouter-skill-slug`). Pointers are used as cache directory segments, so
...
const SkillMetadataSchema = Schema.Struct({
  "openrouter-skill-id": Schema.optionalKey(ManagedSkillPointerSchema),
  "openrouter-skill-slug": Schema.optionalKey(ManagedSkillPointerSchema),
  "openrouter-skill-version": Schema.optionalKey(ManagedSkillVersionSchema),
  "command-aliases": Schema.optionalKey(Schema.Array(SkillCommandAliasSchema)),
}).check(
  Schema.makeFilter((metadata) =>
    metadata["openrouter-skill-id"] !== undefined &&
    metadata["openrouter-skill-slug"] !== undefined
      ? "a managed skill pointer must set at most one of `openrouter-skill-id` and `openrouter-skill-slug`, not both"
      : undefined
  )
);
 ...
 * (`openrouter-skill-id`, globally unique) or the workspace-scoped slug
 * (`openrouter-skill-slug`). The `kind` is retained past resolution because it
...
const managedSkillPointerFromMetadata = (
  ...
  const id = metadata?.["openrouter-skill-id"];
  if (id !== undefined) {
    return {
      kind: "id",
      value: id,
  ...
  const slug = metadata?.["openrouter-skill-slug"];
  if (slug !== undefined) {
    return {
      kind: "slug",
      value: slug,
