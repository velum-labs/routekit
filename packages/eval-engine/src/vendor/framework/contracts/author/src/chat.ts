...
export interface Chat {
  ...
  /**
   * The directory `ori tui`/`ori code` was launched in — the same value sent as
   ...
   * `import { config } from "ori/config"` is the equivalent for contribution code
   ...
   * runs in-daemon (`ori start`), and a key-value client proxy when it runs in a
   * client process (`ori tui`). Optional only so lightweight `Chat` mocks need
   * not supply it; a surface that persists state uses this (or the process-global
   * `db` from `ori/state`) and MUST NOT open its own database.
   */
