...
export interface AgentFailure {
  /** Stable ORI-owned lookup key from `AGENT_FAILURE_CODES`. */
  ...
  /** Safe next action when ORI can offer one. */
  ...
  /** Boundary where ORI observed the failure. */
...
/**
 * Enforce the public diagnostic bounds on an already-safe ORI-owned failure.
 *
 ...
 * the boundary says where ORI stood when it saw the fault, while a remediation
