 ...
 *   }
 *
 * Two paths fill it. Workspace features are generated into the `.ori/` cache
 * from their own source. Built-ins have no file in the consumer's workspace, so
 * their provider type is baked into the generated `ori` SDK itself during
 * `contracts:generate` — that is how `use("slack")` resolves to the slack
 * built-in's `postMessage` shape without depending on `@ori-builtins/slack`.
 *
