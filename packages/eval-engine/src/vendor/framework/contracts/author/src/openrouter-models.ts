...
/**
 * Public OpenRouter catalog endpoint; needs no auth. `sort=top-weekly` ranks
...
const OpenRouterPricingSchema = Schema.Struct({
...
const OpenRouterArchitectureSchema = Schema.Struct({
...
const OpenRouterAliasTargetSchema = Schema.Struct({
  name: Schema.String,
  slug: Schema.String,
});

const OpenRouterArtificialAnalysisSchema = Schema.Struct({
  agentic_index: NullableNumber,
  coding_index: NullableNumber,
  intelligence_index: NullableNumber,
});

const OpenRouterBenchmarksSchema = Schema.Struct({
  artificial_analysis: Schema.optionalKey(
    Schema.NullOr(OpenRouterArtificialAnalysisSchema)
  ),
  design_arena: Schema.optionalKey(
    Schema.NullOr(Schema.Array(OpenRouterDesignArenaSchema))
  ),
});

const OpenRouterModelEntrySchema = Schema.Struct({
  alias_target: Schema.optionalKey(Schema.NullOr(OpenRouterAliasTargetSchema)),
  architecture: Schema.optionalKey(Schema.NullOr(OpenRouterArchitectureSchema)),
  benchmarks: Schema.optionalKey(Schema.NullOr(OpenRouterBenchmarksSchema)),
  ...
  name: NullableString,
  pricing: Schema.optionalKey(Schema.NullOr(OpenRouterPricingSchema)),
  reasoning: Schema.optionalKey(Schema.NullOr(OpenRouterReasoningSchema)),
  supported_parameters: NullableStrings,
  top_provider: Schema.optionalKey(Schema.NullOr(OpenRouterTopProviderSchema)),
});

// Entries are decoded one at a time (see `decodeOpenRouterModels`), so the
...
interface OpenRouterAliasTarget {
...
interface OpenRouterModelReasoning {
...
interface OpenRouterDesignArenaEntry {
 ...
 * genuinely weak one.
 */
interface OpenRouterModelBenchmarks {
  readonly agenticIndex?: number | undefined;
  readonly codingIndex?: number | undefined;
  readonly designArena?: readonly OpenRouterDesignArenaEntry[] | undefined;
  readonly intelligenceIndex?: number | undefined;
}

/**
 * A projected OpenRouter model. Absent stays absent: the API omits or nulls a
...
interface OpenRouterModel {
  readonly aliasTarget?: OpenRouterAliasTarget | undefined;
  readonly architecture?: OpenRouterModelArchitecture | undefined;
  readonly benchmarks?: OpenRouterModelBenchmarks | undefined;
  ...
  readonly name: string;
  readonly pricing?: OpenRouterModelPricing | undefined;
   ...
   * unknown case.
   */
  readonly reasoning?: OpenRouterModelReasoning | null | undefined;
  readonly supportedParameters?: readonly string[] | undefined;
}

/** Capability for lazily resolving the OpenRouter model catalog. */
interface ModelCatalog {
  readonly listModels: () => Promise<readonly OpenRouterModel[]>;
}

const decodeResponse = Schema.decodeUnknownSync(OpenRouterModelsResponseSchema);
const decodeEntry = Schema.decodeUnknownSync(OpenRouterModelEntrySchema);

type DecodedEntry = typeof OpenRouterModelEntrySchema.Type;
...
): OpenRouterModelReasoning | null | undefined => {
...
): OpenRouterModelBenchmarks | undefined => {
...
const projectEntry = (entry: DecodedEntry): OpenRouterModel => {
 ...
 * because this module is merged verbatim into the generated `ori` SDK, which is
 * handed `Schema` from `effect` and nothing else.
 */
const decodeOpenRouterModels = (
  payload: unknown
  ...
  if (entries.length > 0 && models.length === 0) {
    throw new Error(
      `OpenRouter models response carried ${entries.length} entries and none could be decoded`,
      { cause: firstFailure }
    );
  }
...
/** Fetch and decode an OpenRouter catalog response from a supplied URL. */
export const fetchOpenRouterModelsRequest = async (
  ...
  if (!response.ok) {
    const authHint =
      ...
      response.status === HTTP_FORBIDDEN
        ? "; your OpenRouter API key may be invalid or expired"
        : "";
    throw new Error(
      `OpenRouter models request failed with HTTP ${response.status}${authHint}`
    );
  }
  const payload: unknown = await response.json();
  return decodeOpenRouterModels(payload);
};

/** Fetch the public OpenRouter catalog. `fetchImpl` is injectable for tests. */
export const fetchOpenRouterModels = (
  fetchImpl: ModelsFetch = fetch
): Promise<readonly OpenRouterModel[]> =>
  fetchOpenRouterModelsRequest(fetchImpl, evalModelsCatalogUrl());

export { OPENROUTER_MODELS_URL, decodeOpenRouterModels };
export type {
  ModelsHttpResponse,
  ModelCatalog,
  OpenRouterAliasTarget,
  OpenRouterDesignArenaEntry,
  OpenRouterModel,
  OpenRouterModelArchitecture,
  OpenRouterModelBenchmarks,
