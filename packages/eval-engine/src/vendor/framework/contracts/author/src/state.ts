...
//
// Unlike `ori/logger`, the pre-install default REJECTS rather than no-ops: a
 ...
 */
const FEATURE_STATE_GLOBAL_KEY = Symbol.for("ori/feature-state/v1");
...
const NO_RESOLVER_MESSAGE =
  "no state store is available in this context: ori/state has no resolver installed (call db() from a running feature, not at import time)";
 ...
 * daemon under `ori dev`) each release cleanly without blanking the other's
 * install. This is a host-only hook, kept off the bare `ori` surface so a
