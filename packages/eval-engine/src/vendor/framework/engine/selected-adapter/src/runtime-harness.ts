...
const ADAPTER_REASON_CODES: Record<
  ...
  AgentFailureCode
> = {
  connection: "ORI_ADAPTER_CONNECTION",
  "invalid-state": "ORI_ADAPTER_INVALID_STATE",
  "malformed-input": "ORI_ADAPTER_MALFORMED_INPUT",
  "peer-exit": "ORI_ADAPTER_PEER_EXIT",
};
    ...
    return terminalFailureOutput(
      new SelectedAdapterError({
        ...
        safeFailure: agentFailure({
          ...
          message: "ORI could not find the session requested for resume.",
...
// itself — filling it in here is the only place that can (ORI-969).
...
/**
 * The adapter's `detail` is peer text ORI did not author, so it is redacted on
