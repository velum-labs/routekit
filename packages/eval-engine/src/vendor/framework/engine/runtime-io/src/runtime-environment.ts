...
const makeLocalRuntimeSecretStore = (
  ...
  > => {
    ...
    // `name === RuntimeSecretName.OpenRouterApiKey` compare narrows to one literal
    ...
    if (requestedName === RuntimeSecretName.OpenRouterApiKey) {
      return environment
        .get(OPENROUTER_API_KEY_ENV)
 ...
 * {@link RuntimeEnvironment} (e.g. the OpenRouter key from `OPENROUTER_API_KEY`),
