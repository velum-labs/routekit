...
const ROOT_SCOPE = "ori";
const ORI_LOG_LEVEL_ENV = "ORI_LOG_LEVEL";

const isLogLevel = (value: string): value is LogLevel =>
  Object.hasOwn(LOG_LEVEL_RANK, value);

/** Parse an `ORI_LOG_LEVEL` string into a {@link LogLevel}, falling back to `info`. */
const parseLogLevel = (value: string): LogLevel => {
  const normalized = value.trim().toLowerCase();
  return isLogLevel(normalized) ? normalized : DEFAULT_LOG_LEVEL;
};

/** Reads the minimum log level from `ORI_LOG_LEVEL`, defaulting to `info`. */
 ...
 * context and the `ORI_LOG_LEVEL` minimum level.
 */
