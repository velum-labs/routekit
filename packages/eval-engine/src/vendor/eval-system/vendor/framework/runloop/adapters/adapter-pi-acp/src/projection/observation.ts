...
type CompactionTrigger = NonNullable<CompactionEnd["routeKitEvalTrigger"]>;
type CompactionCause = NonNullable<CompactionEnd["routeKitEvalCause"]>;
...
const projectCompactionEnd = function* (
  ...
  const common = compactionFields(
    event.reason,
    event.routeKitEvalTrigger,
    event.routeKitEvalCause
  );
  if (event.aborted) {
    yield {
      ...common,
...
const projectPiObservation = function* (
  ...
  if (guards.compaction_start(event)) {
    yield {
      ...compactionFields(event.reason, event.routeKitEvalTrigger, event.routeKitEvalCause),
