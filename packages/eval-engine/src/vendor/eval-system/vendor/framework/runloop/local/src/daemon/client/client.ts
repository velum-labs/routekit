...
>()("routekit-eval/runtime/RuntimeClientConfig") {
  /**
   ...
   * `ROUTEKIT_EVAL_RUNTIME_HOST` / `ROUTEKIT_EVAL_RUNTIME_PORT` lives in the `RuntimeClientConfigLive`
...
const formatInvokeNotFoundError = (
  ...
  `Runtime invoke failed with HTTP 404 at ${makeRuntimeUrl(options, INVOKE_PATH)}. The runtime is reachable, but it does not expose the invoke endpoint; restart routekit-eval start from the current checkout and retry the local TUI. Response: ${body}`;
  ...
  function* (options: RuntimeClientOptions, command: RuntimeCommand) {
    ...
    const response = yield* client
      ...
      .pipe(
        Effect.mapError(
          makeRuntimeClientErrorFromCause(
            `Failed to invoke local RouteKitEval runtime at ${makeRuntimeUrl(options, INVOKE_PATH)}`
          )
        )
      );
    ...
    if (isOkStatus(response.status)) {
      return response.stream.pipe(
        Stream.mapError(
          makeRuntimeClientErrorFromCause(
            `Failed to read local RouteKitEval runtime stream at ${makeRuntimeUrl(options, INVOKE_PATH)}`
          )
        )
      );
    }
...
export class RuntimeClient extends Context.Service<
  RuntimeClient,
  RuntimeClientShape
>()("routekit-eval/runtime/RuntimeClient") {
  /**
