...
const SQLITE_DB_FEATURE_ID = "@routekit-eval-builtins/db-sqlite";
...
>()("routekit-eval/runtime/BuiltInDbCatalog") {
 ...
 * `.routekit-eval/state.sqlite` path against the workspace root rather than the process
 ...
 * `ROUTEKIT_EVAL_INTERN_WORKSPACE_ROOT`; fall back to CWD when it is absent. `ROUTEKIT_EVAL_SQLITE_PATH`
 * / `ROUTEKIT_EVAL_STATE_DIR` still take precedence (resolved inside
...
export const builtInSqliteStateStoreLayer = Layer.unwrap(
  Effect.gen(function* () {
    ...
    const baseDir =
      nonEmptyWorkspaceRoot(env[ROUTEKIT_EVAL_INTERN_WORKSPACE_ROOT_ENV]) ??
      path.resolve();
