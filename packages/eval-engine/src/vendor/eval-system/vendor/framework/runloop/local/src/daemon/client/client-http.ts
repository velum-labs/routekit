...
import type { RouteKitEvalError } from "../../../../../contracts/internal/src/errors.ts";
...
interface DaemonErrorParts {
  ...
  readonly cause?: RouteKitEvalError;
}
...
/**
 * Execute a request against the local RouteKitEval runtime through the injectable
    ...
    effect.pipe(
      Effect.mapError(
        makeRuntimeClientErrorFromCause(
          `Could not reach local RouteKitEval runtime at ${makeRuntimeUrl(options, "")}`
        )
      )
    )
);
...
export const readResponseText = (
  ...
  response.text.pipe(
    Effect.mapError(
      makeRuntimeClientErrorFromCause(
        `Failed to read local RouteKitEval runtime response at ${makeRuntimeUrl(options, path)}`
      )
    )
  );
