 ...
 * daemon is reachable but predates this endpoint (an old `routekit-eval dev`), so it maps
    ...
    if (HttpClientError.isHttpClientError(error)) {
      ...
      if (status === NOT_FOUND_STATUS) {
        return new RuntimeClientError({
          cause: error,
          detail: `Runtime ${describe} are unavailable at ${url}. The runtime is reachable, but it does not expose the ${describe} endpoint; restart \`routekit-eval dev\` from the current checkout and retry.`,
        });
      }
 ...
 * the `.routekit-eval/logs` runs filtered by `sessionId`. Helps agents discover the
 * `sessionId`s to pass to `routekit-eval logs --session` or `routekit-eval tui --session`.
 */
 ...
 * `GET /api/logs/runs`. The remote counterpart of reading `.routekit-eval/logs/` from
 * disk; used by `routekit-eval logs` when targeting a `--host`/`--port` daemon.
 */
...
const fetchRuntimeStreamBody = Effect.fn(
  ...
  if (isOkStatus(response.status)) {
    return response.stream.pipe(
      Stream.mapError(
        makeRuntimeClientErrorFromCause(
          `Failed to read local RouteKitEval runtime ${target.describe} at ${makeRuntimeUrl(options, target.path)}`
        )
      )
    );
  }
  ...
  return yield* new RuntimeClientError({
    detail:
      response.status === NOT_FOUND_STATUS
        ? `Runtime ${target.describe} is unavailable at ${makeRuntimeUrl(options, target.path)}. The runtime is reachable, but it does not expose the ${target.describe} endpoint; restart \`routekit-eval dev\` from the current checkout and retry. Response: ${body}`
