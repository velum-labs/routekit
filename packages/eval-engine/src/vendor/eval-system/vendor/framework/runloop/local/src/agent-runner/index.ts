...
const resolveAssembledSystemPrompt = Effect.fn(
  ...
  return assembleSystemPrompt({
    ...
    // itself (built-in-catalog/prompt.ts): only `routekit-eval code` gets it, so a
    ...
    // interns and `routekit-eval code` have the feature-development skill disabled, so the
...
const makeCompactionInvoker =
  (input: {
    ...
    readonly deps: HarnessRuntimeDeps;
    readonly gatewayModels: GatewayModels["Service"];
  ...
  (command) => {
    ...
    return compaction.pipe(
      Effect.provideService(GatewayModels, input.gatewayModels),
  ...
  Effect.gen(function* () {
    ...
    const invokeRuntime: AgentRunnerShape["invokeRuntime"] = (command) =>
      Stream.unwrap(invokeHarnessRuntime(deps, command)).pipe(
        Stream.provideService(GatewayModels, gatewayModels),
