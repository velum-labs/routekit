...
// Outgoing (ROUTEKIT_EVAL -> Codex) request params, keyed by JSON-RPC method.
...
// Push notifications (Codex -> ROUTEKIT_EVAL) carrying no id; these stream during a turn.
...
const CompactionRouteKitEvalgin = Schema.Literals(["threshold", "overflow"]);
...
const RetryEvent = Schema.Struct({
  method: Schema.Literal("routekit-eval/retry"),
  params: RetryParams,
});
const CompactionParams = Schema.Struct({
  ...
  origin: Schema.optionalKey(CompactionRouteKitEvalgin),
...
const CompactionEvent = Schema.Struct({
  method: Schema.Literal("routekit-eval/compaction"),
...
// Replies to a request ROUTEKIT_EVAL sent Codex, correlated back to the pending map by id.
