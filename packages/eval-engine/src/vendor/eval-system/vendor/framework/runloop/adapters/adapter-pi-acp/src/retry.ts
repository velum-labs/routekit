  ...
  return described === undefined
    ...
    : new SelectedAdapterError({
        ...
        safeFailure: agentFailure({
          code: "ROUTEKIT_EVAL_GATEWAY_CREDITS_EXHAUSTED",
          message: described,
          stage: "adapter",
        }),
      });
...
// pi discards its own partial, but routekit-eval has already streamed that output, so
...
const clampRetryScheduled = (
  ...
  event: {
    payload: {
      ...
      message: `Gateway credit limit reached; retrying with max_tokens<=${maxTokens}. Output from the abandoned attempt precedes this line.`,
    },
...
const makeRetryingPrompt = (input: RetryContext) => {
  ...
  // headless run outright (ROUTEKIT_EVAL-882): the turn's work is already forfeit, so
