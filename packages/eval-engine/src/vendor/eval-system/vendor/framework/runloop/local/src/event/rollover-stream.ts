...
/**
 * Rollover orchestration (ROUTEKIT_EVAL-471): riding INSIDE the user's next turn, the
...
export const resolveRollover = Effect.fn("Daemon.resolveRollover")(function* (
  services: RouteKitEvalDaemonServices,
  ...
  const contextWindow =
    (yield* services.contextWindowLookup
      ...
      .pipe(
        Effect.provideService(GatewayModels, services.gatewayModels)
      )) ??
    (yield* services.contextWindowLookup
...
export interface RolloverStreamServices {
  readonly crypto: RouteKitEvalDaemonServices["crypto"];
  readonly daemonAddress: RouteKitEvalDaemonServices["daemonAddress"];
  readonly journal: Pick<RouteKitEvalDaemonServices["journal"], "entries">;
}
    ...
    ? emitCompaction(input, {
        payload: {
          ...
          failure: agentFailure({
            code: "ROUTEKIT_EVAL_COMPACTION_SUMMARY_FAILED",
            message:
 ...
 * (not before it) so the ROUTEKIT_EVAL-450 conversion renders it as terminal runtime
...
export const guardUnresolvedForcedRollover = (
  ...
  command.prompt.trim().length === 0
    ? new RuntimeValidationError({
        ...
        detail:
          mode === "off"
            ? "nothing to compact: compaction rollover is disabled (ROUTEKIT_EVAL_COMPACTION_ROLLOVER=off)"
