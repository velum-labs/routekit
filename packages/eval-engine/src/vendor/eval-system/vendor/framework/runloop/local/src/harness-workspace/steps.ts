...
const CODE_SKILLS_ROOT = ".routekit-eval/code-skills";
const MATERIALIZED_SKILLS_MANIFEST = ".routekit-eval/materialized-skills.json";
 ...
 * by default (the `routekit-eval dev`/`routekit-eval start` shape, where `featuresRoot` is
 ...
 * tell `routekit-eval dev`'s nested-intern shape (`cwd=/repo`,
 * `featuresRoot=/repo/features/toney/features`) apart from `routekit-eval code`
 * launched from an ancestor of its global workspace (`cwd=~`,
 * `featuresRoot=~/.routekit-eval/global/features`); and a `featuresRoot`-equality check
 * can't tell `routekit-eval code` apart from `routekit-eval dev`'s own fallback to that identical
 * `~/.routekit-eval/global/features` (RFC 0004 dev.md), which wants the opposite
