...
const DEFAULT_SKILLS_API_URL = "http://127.0.0.1:8080/v1/skills";
const SKILLS_API_URL_ENV = "ROUTEKIT_EVAL_SKILLS_API_URL";
...
const SHARED_CREDENTIAL_PATH_SEGMENTS = [
  [".routekit-eval", "credentials.json"],
...
// Mirrors the credential file written by `routekit-eval login` (framework/cli
...
// Excess properties are tolerated so a newer `routekit-eval login` adding fields does
...
const requireGatewayApiKey = Effect.fn("ManagedSkill.requireApiKey")(
  function* () {
    const envKey = yield* readOptionalEnv(ROUTEKIT_EVAL_BEARER_TOKEN_ENV);
    if (envKey !== undefined) {
      return envKey;
    }
    ...
    return yield* new ManagedSkillFetchError({
      detail: `no Gateway key found in ${ROUTEKIT_EVAL_BEARER_TOKEN_ENV} or the shared credentials.json; run \`routekit-eval login\` or export the key to resolve managed skills`,
    });
  }
);
...
const fetchManagedSkillBundle = (
  ...
  Effect.fn("ManagedSkill.fetchBundle")(function* (
    ...
    const apiKey = yield* requireGatewayApiKey();
    const baseUrl =
...
/**
 * Live {@link ManagedSkillFetcher}: the Gateway managed-skills HTTP client
 * (RFC 0002 skill.md), authenticating via `ROUTEKIT_EVAL_BEARER_TOKEN` or the shared
 * `routekit-eval login` credentials. `HttpClient` is captured once here at layer build and
 ...
 * and `routekit-eval pack`) with no shared root to discharge it once, so a self-contained
