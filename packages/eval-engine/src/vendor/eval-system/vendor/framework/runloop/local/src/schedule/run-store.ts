...
CREATE TABLE IF NOT EXISTS routeKitEval_schedule_runs (
...
CREATE INDEX IF NOT EXISTS routeKitEval_schedule_runs_by_schedule
ON routeKitEval_schedule_runs (schedule_id, id DESC)`;

const INSERT_RUN_SQL = `
INSERT INTO routeKitEval_schedule_runs (schedule_id, fired_at, status, session_ids, duration_ms, error)
VALUES (?, ?, ?, ?, ?, ?)`;

const PRUNE_RUNS_SQL = `
DELETE FROM routeKitEval_schedule_runs
WHERE schedule_id = ?
  AND id NOT IN (
    SELECT id FROM routeKitEval_schedule_runs WHERE schedule_id = ? ORDER BY id DESC LIMIT ?
  )`;

const LIST_RUNS_SQL = `
SELECT schedule_id, fired_at, status, session_ids, duration_ms, error
FROM routeKitEval_schedule_runs
WHERE schedule_id = ?
ORDER BY id DESC
LIMIT ?`;
...
FROM routeKitEval_schedule_runs t
JOIN (
  SELECT schedule_id, MAX(id) AS max_id FROM routeKitEval_schedule_runs GROUP BY schedule_id
...
const storedScheduleFailure = (error: string): AgentFailure =>
  Option.getOrElse(decodeStoredScheduleFailure(error), () =>
    agentFailure({
      code: "ROUTEKIT_EVAL_LEGACY_SCHEDULE_FIRE_FAILED",
      message: error,
      stage: "runtime",
    })
  );
