 ...
 * carries an `gateway-skill-id` or `gateway-skill-slug` pointer resolves
 * its body and support files from the Gateway managed skills API at the run
 ...
 *
 * The resolved version is recorded in `.routekit-eval/skills-lock.json` so unpinned
...
const SKILLS_LOCK_PATH_SEGMENTS = [".routekit-eval", "skills-lock.json"] as const;
const JSON_INDENT = 2;
...
const fallbackResolution = (
  ...
  // feature stays valid and `routekit-eval features validate` passes (RFC 0002 skill.md).
