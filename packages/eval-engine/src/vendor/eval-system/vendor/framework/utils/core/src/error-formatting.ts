...
const GATEWAY_KEY_PATTERN = /\bsk-(?:or-)?[A-Za-z0-9_-]+\b/gu;

/**
 * Vendor key formats that announce themselves with a fixed prefix. ROUTEKIT_EVAL does
 ...
 *
 * This is a safety net over text ROUTEKIT_EVAL did not author, not a guarantee: it
...
export const formatSafeErrorDiagnostic = (error: unknown): string =>
  formatUnknownError(error)
    ...
    .replaceAll(GATEWAY_KEY_PATTERN, "<secret>")
