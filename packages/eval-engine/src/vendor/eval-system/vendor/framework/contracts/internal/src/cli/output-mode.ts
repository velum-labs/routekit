...
interface OutputModeShape {
   ...
   * `ROUTEKIT_EVAL_OUTPUT` or merely inferred from a non-TTY stdout. A piped caller and a
   * caller that asked for a machine envelope are NOT the same thing: `routekit-eval code`
...
class OutputMode extends Context.Service<OutputMode, OutputModeShape>()(
  "routekit-eval/runtime/OutputMode"
) {
  /**
...
const ROUTEKIT_EVAL_OUTPUT_ENV = "ROUTEKIT_EVAL_OUTPUT";
...
interface ResolveOutputModeInput {
  ...
  /** Raw `ROUTEKIT_EVAL_OUTPUT` env value, second precedence. */
 ...
 * `ROUTEKIT_EVAL_OUTPUT` env, then auto-detection (human on a TTY, machine when piped).
 */
...
export { OutputMode, currentOutputMode, logToStderrLayer, ROUTEKIT_EVAL_OUTPUT_ENV };
export type {
  OutputModeValue,
  OutputModeShape,
  OutputModeSource,
