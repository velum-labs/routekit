 ...
 * `@routekit-eval-engine/runtime-io` adapters implement the service ports (now in
 * `runtime/runtime-environment.ts` and `runtime/runtime-secret-store.ts`);
 * consumers such as `@routekit-eval-engine/harness` depend only on those ports so the
