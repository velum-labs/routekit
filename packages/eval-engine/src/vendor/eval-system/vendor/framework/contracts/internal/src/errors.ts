...
// it through the public `routekit-eval` SDK. Imported here for the local type unions
 ...
 * a normal end to `routekit-eval … | head`, not a failure, but the platform buries the
...
/**
 * Schema union of every {@link RouteKitEvalError}. Each tagged-error class is itself a
 ...
 */
const RouteKitEvalErrorSchema = Schema.Union([
  CliFailureError,
  CliIoError,
  FeatureLoaderError,
  HarnessCapabilityError,
...
// Compile-time drift guard: the union and the `RouteKitEvalError` type must stay in
// lockstep, so every decoded member is an `RouteKitEvalError` and every `RouteKitEvalError`
// member is covered by the schema.
type _RouteKitEvalErrorSchemaDecodesToRouteKitEvalError = AssertAssignable<
  typeof RouteKitEvalErrorSchema.Type,
  RouteKitEvalError
>;
type _RouteKitEvalErrorIsCoveredBySchema = AssertAssignable<
  RouteKitEvalError,
  typeof RouteKitEvalErrorSchema.Type
>;
