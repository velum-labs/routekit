 ...
 * reader-side filters (`routekit-eval logs --type`, the daemon's log routes), so a new
...
/**
 * One on-disk NDJSON line written by the `routekit-eval dev` event-log file: a
...
const legacyFailure = (
  code: "ROUTEKIT_EVAL_LEGACY_TURN_FAILED" | "ROUTEKIT_EVAL_LEGACY_RUNTIME_DIAGNOSTIC",
 ...
 *
 * Without this, `routekit-eval logs <run from before the upgrade>` renders every event
...
const upgradeLegacyFailureLine = (value: unknown): unknown => {
  if (isLegacyFailureLine(value)) {
    ...
    return failure === undefined
      ? withUpgradedPayload(value, {
          ...payload,
          failure: legacyFailure("ROUTEKIT_EVAL_LEGACY_TURN_FAILED", error),
        })
      : value;
  }
  if (isLegacyDiagnosticLine(value)) {
    ...
    return failure === undefined
      ? withUpgradedPayload(value, {
          ...payload,
          failure: legacyFailure("ROUTEKIT_EVAL_LEGACY_RUNTIME_DIAGNOSTIC", message),
        })
      : value;
  }
  return value;
...
/** Metadata describing one persisted `routekit-eval dev` run file under `.routekit-eval/logs/`. */
