...
>()("routekit-eval/selected-adapter/SelectedAdapterCoordinator") {}
...
const openAdapterSession = Effect.fn(
  ...
  const launchId = key.options.env?.ROUTEKIT_EVAL_GATEWAY_SESSION_ID;
  if (launchId !== undefined) {
    log.info("Gateway launch mapped to session", {
      launchId,
      sessionId,
    });
  }
