...
const projectionFailure = (error: HarnessError): AgentFailure =>
  Match.value(error).pipe(
    Match.tag("HarnessProcessError", (failure) =>
      agentFailure({
        code: "ROUTEKIT_EVAL_HARNESS_PROCESS_FAILED",
    ...
    Match.tag("HarnessCapabilityError", (failure) =>
      agentFailure({
        code: "ROUTEKIT_EVAL_HARNESS_CAPABILITY_UNSUPPORTED",
    ...
    Match.tag("HarnessProtocolError", () =>
      agentFailure({
        code: "ROUTEKIT_EVAL_HARNESS_PROTOCOL_FAILED",
    ...
    Match.tag("HarnessValidationError", () =>
      agentFailure({
        code: "ROUTEKIT_EVAL_HARNESS_VALIDATION_FAILED",
    ...
    Match.tag("RuntimeEnvironmentError", () =>
      agentFailure({
        code: "ROUTEKIT_EVAL_RUNTIME_ENVIRONMENT_FAILED",
        message: "ROUTEKIT_EVAL could not prepare the harness environment.",
        stage: "runtime",
      })
    ),
    Match.tag("RuntimeSecretError", () =>
      agentFailure({
        code: "ROUTEKIT_EVAL_RUNTIME_SECRET_FAILED",
        message: "ROUTEKIT_EVAL could not resolve a required harness secret.",
        stage: "runtime",
      })
    ),
    Match.exhaustive
...
export class AgentHarnessEventProjector extends Context.Service<
  ...
  AgentHarnessEventProjectorShape
>()("routekit-eval/harness/AgentHarnessEventProjector") {
