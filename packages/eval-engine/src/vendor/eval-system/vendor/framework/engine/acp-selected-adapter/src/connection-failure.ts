...
const remoteFailure = (
  ...
  if (isUpstreamAuthRejection(error.message)) {
    return agentFailure({
      code: "ROUTEKIT_EVAL_ADAPTER_UNAUTHORIZED",
      message: `${displayName}'s provider rejected the credential.`,
      remediation:
        "Run `routekit-eval login`, or check the ROUTEKIT_EVAL_BEARER_TOKEN in this environment.",
      stage: "adapter",
      upstreamCode: error.code,
    });
  }
  return agentFailure({
    code: "ROUTEKIT_EVAL_ADAPTER_REMOTE_ERROR",
...
const peerExitFailure = (displayName: string): AgentFailure =>
  agentFailure({
    code: "ROUTEKIT_EVAL_ADAPTER_PEER_EXIT",
...
const protocolFailure = (
  ...
  Match.value(error).pipe(
    Match.tag("AcpProtocolVersionError", (version) =>
      agentFailure({
        code: "ROUTEKIT_EVAL_ADAPTER_PROTOCOL_VERSION",
        message: `${displayName} speaks ACP version ${version.received}; ROUTEKIT_EVAL requires ${version.expected}.`,
    ...
    Match.tag("AcpInitializationError", (initialization) =>
      agentFailure({
        code: "ROUTEKIT_EVAL_ADAPTER_INITIALIZATION",
    ...
    Match.orElse((protocol) =>
      agentFailure({
        code: "ROUTEKIT_EVAL_ADAPTER_PROTOCOL",
...
const safeAcpConnectionFailure = (
  ...
  return Match.value(error).pipe(
    ...
    Match.tag("AcpTransportError", (transport) =>
      agentFailure({
        code: "ROUTEKIT_EVAL_ADAPTER_TRANSPORT",
    ...
    Match.tag("AcpRequestCancelledError", () =>
      agentFailure({
        code: "ROUTEKIT_EVAL_REQUEST_CANCELLED",
    ...
    Match.tag("AcpConnectionClosedError", () =>
      agentFailure({
        code: "ROUTEKIT_EVAL_ADAPTER_CLOSED",
    ...
    Match.tag("AcpConnectionConfigError", () =>
      agentFailure({
        code: "ROUTEKIT_EVAL_ADAPTER_CONFIG",
    ...
    Match.tag(
      ...
      (capacity) =>
        agentFailure({
          code: "ROUTEKIT_EVAL_ADAPTER_CAPACITY",
          message: `An ROUTEKIT_EVAL queue for ${displayName} reached its ${capacity.capacity}-entry limit.`,
          stage: "adapter",
