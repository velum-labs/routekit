import type { AgentFailure } from "../../routekit-eval/src/index.ts";

import { agentFailure, isContextOverflowMessage } from "../../routekit-eval/src/index.ts";

import { isClaudeMissingSessionMessage } from "./failure.ts";

/**
 * What ROUTEKIT_EVAL knows about a finished Claude process.
 *
...
export const claudeProcessFailure = (
  ...
  if (outcome.missingBinary === true) {
    return agentFailure({
      code: "ROUTEKIT_EVAL_CLAUDE_BINARY_UNAVAILABLE",
      message: "The Claude executable was not found.",
      remediation:
        "Install it with `bun add -g --trust @anthropic-ai/claude-code`, or set ROUTEKIT_EVAL_CLAUDE_BIN.",
      stage: "harness",
    });
  }
  if (outcome.timedOut) {
    return agentFailure({
      code: "ROUTEKIT_EVAL_CLAUDE_PROCESS_TIMEOUT",
      message: "Claude timed out before completing the request.",
      remediation: "Raise ROUTEKIT_EVAL_CLAUDE_TIMEOUT_MS or simplify the request.",
      stage: "harness",
  ...
  if (NO_RESULT_PATTERN.test(outcome.message)) {
    return agentFailure({
      code: "ROUTEKIT_EVAL_CLAUDE_NO_RESULT_EVENT",
      stage: "harness",
      ...(upstreamCode === undefined ? {} : { upstreamCode }),
    });
  }
  ...
  return agentFailure({
    code: "ROUTEKIT_EVAL_CLAUDE_PROCESS_FAILED",
    message:
