// ROUTEKIT_EVAL-owned local install of the `pi` runtime.
//
...
// path entirely, so ROUTEKIT_EVAL installs its pinned pi into a private versioned
// directory (`~/.routekit-eval/pi-runtime/<version>`) and spawns that `dist/cli.js`
// directly. This also makes the version pin real: a user-installed global pi
// on PATH no longer shadows it. `ROUTEKIT_EVAL_PI_BIN` remains the authoritative
...
export const ROUTEKIT_EVAL_PI_BIN_ENV = "ROUTEKIT_EVAL_PI_BIN";
const ROUTEKIT_EVAL_PI_INSTALL_DIR_ENV = "ROUTEKIT_EVAL_PI_INSTALL_DIR";
...
// The pinned `pi` version routekit-eval supports. Auto-install resolves to this exact
...
// issue: see the module header. The `@routekit-eval/gateway-agent` SDK that the
...
const PI_RUNTIME_BASE_DIR = ".routekit-eval";
...
// Only surfaced when an explicit ROUTEKIT_EVAL_PI_BIN probe fails
...
export const PI_BINARY_REQUIREMENT = {
  binaryEnvVar: ROUTEKIT_EVAL_PI_BIN_ENV,
  installCommand: `npm install --ignore-scripts ${PI_PACKAGE_SPEC} (routekit-eval normally auto-installs pi)`,
...
export const resolvePiRuntimeInstallDir = (env: NodeJS.ProcessEnv): string =>
  join(
    normalizeEnvValue(env[ROUTEKIT_EVAL_PI_INSTALL_DIR_ENV]) ??
...
// hardlink-installed by an older routekit-eval under proot self-heals via reinstall).
...
const formatPiInstallSpawnError = (error: PiInstallError): string =>
  `automatic pi install failed: ${error.message}. Retry, or install pi yourself and set ${ROUTEKIT_EVAL_PI_BIN_ENV}.`;
...
const formatFailedPiInstallResult = (result: PiInstallResult): string => {
  ...
  return `${detail}. Retry, or install pi yourself and set ${ROUTEKIT_EVAL_PI_BIN_ENV}.`;
};

const formatMissingInstalledCli = (cliPath: string): string =>
  `Automatic pi install completed, but the pi entrypoint was not found (or is unusable) at ${cliPath}. Set ${ROUTEKIT_EVAL_PI_BIN_ENV} to a working pi.`;
...
// ROUTEKIT_EVAL-owned manifest. Two concurrent invokes may race this install in the
...
//   * `ROUTEKIT_EVAL_PI_BIN` set (autoInstall false): the override is authoritative —
//     probe it like any harness binary and pass it through. No l2s logic; the
//     runtime stderr hint covers a mangled override.
//   * Auto-managed (default): PATH is never consulted. Use the ROUTEKIT_EVAL-owned
