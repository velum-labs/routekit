...
import type { JsonObject } from "../gateway-attribution/gateway-attribution.ts";
...
// Marks a `compaction` block as routekit-eval-derived so a later invoke may re-derive it
// for a different model. A block without the marker is user-authored and is
// never touched.
const ROUTEKIT_EVAL_DERIVED_FIELD = "routeKitEvalDerived";
...
// provider rejects the prompt (ROUTEKIT_EVAL-300/ROUTEKIT_EVAL-408).
 ...
 * existing `compaction` block is user-authored (no {@link ROUTEKIT_EVAL_DERIVED_FIELD}
...
// A block is skippable when it is user-authored (no ROUTEKIT_EVAL_DERIVED_FIELD marker
...
const compactionBlockNeedsUpdate = (
  ...
  if (hasCompactionBlock && compaction[ROUTEKIT_EVAL_DERIVED_FIELD] !== true) {
    return false;
  }
  return (
...
export const mergePiCompactionSettings = (
  ...
  compaction[PI_RESERVE_TOKENS_FIELD] = sizing.reserveTokens;
  compaction[PI_KEEP_RECENT_TOKENS_FIELD] = sizing.keepRecentTokens;
  compaction[ROUTEKIT_EVAL_DERIVED_FIELD] = true;
...
// Best-effort like ensurePiGatewayAttribution: never block a run on a
