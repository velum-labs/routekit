// oxlint-disable no-barrel-file -- the `routekit-eval` SDK alias is, by design (RFC 0002), a pure re-export of the author contract surface; a barrel is the intended shape.
// RFC 0002 (Harness Authoring Surface): the in-repo `routekit-eval` SDK alias. Builtins
// import this package exactly as an external feature project imports the
// generated `.routekit-eval/sdk`, so the built-in harnesses stay exemplary user-space
...
// projection's pure functions ship through the type-merged `routekit-eval` entry per
// RFC 0002.
