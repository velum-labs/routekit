...
// The web-tools extension dynamically imports `@routekit-eval/gateway-agent` (callModel +
...
const WEB_TOOLS_SDK_PACKAGE = "@routekit-eval/gateway-agent";
...
const WEB_TOOLS_PACKAGE_JSON = `${JSON.stringify(
  {
    ...
    name: "routekit-eval-pi-web-tools",
    private: true,
    type: "module",
    version: "0.0.0",
  },
...
// Ensure `@routekit-eval/gateway-agent` resolves from a `node_modules` sibling of the
// materialized `web-tools.ts`. pi loads the extension via jiti and does NOT
// expose `@routekit-eval/gateway-agent` as a virtual module or alias, so the extension's
// dynamic `import("@routekit-eval/gateway-agent")` resolves against the filesystem, walking
