...
import { agentFailure, hasEmittedTerminalEvent } from "../../../routekit-eval/src/index.ts";
...
const EMPTY_COUNT = 0;
const ROUTEKIT_EVAL_PI_EXTRA_ARGS_ENV = "ROUTEKIT_EVAL_PI_EXTRA_ARGS";
const ROUTEKIT_EVAL_PI_TIMEOUT_MS_ENV = "ROUTEKIT_EVAL_PI_TIMEOUT_MS";
const ROUTEKIT_EVAL_GATEWAY_SESSION_ID_ENV = "ROUTEKIT_EVAL_GATEWAY_SESSION_ID";

const WHITESPACE_PATTERN = /\s+/u;

// How many times to re-run pi with a lower max_tokens after an Gateway 402
// credit/budget rejection before surfacing the failure. pi builds the request,
// so routekit-eval cannot clamp in place — it lowers the models.json cap and re-invokes.
const MAX_CLAMP_RETRIES = 3;
const materializePiExtensions = async (
  env: NodeJS.ProcessEnv
): Promise<{
  readonly mcpExtensionPath?: string | undefined;
  readonly gatewaySessionAttributionExtensionPath?: string | undefined;
  readonly webToolsExtensionPath?: string | undefined;
}> => {
  const gatewaySessionAttributionExtensionPath =
    await materializeGatewaySessionAttributionExtension(env);
  return {
    mcpExtensionPath: await setUpMcpExtension(env),
    gatewaySessionAttributionExtensionPath,
...
const readPiHarnessConfig = (env: NodeJS.ProcessEnv): PiHarnessConfig => {
  const configuredBinary = normalizeEnvValue(env[ROUTEKIT_EVAL_PI_BIN_ENV]);
  return {
    ...
    extraArgs: splitWhitespaceArgs(env[ROUTEKIT_EVAL_PI_EXTRA_ARGS_ENV] ?? ""),
    timeoutMs: parseTimeoutMs(env[ROUTEKIT_EVAL_PI_TIMEOUT_MS_ENV]),
  };
};
...
const readPiHarnessAvailabilityDiagnostic = (
  ...
  // Auto-managed pi (no ROUTEKIT_EVAL_PI_BIN) is boot-available by construction: the
  ...
  // with a precise message. Only an explicit ROUTEKIT_EVAL_PI_BIN override is probed.
  ...
  Stream.scoped(
    Stream.unwrap(
      Effect.gen(function* () {
        ...
        // ROUTEKIT_EVAL_PI_BIN override verbatim, or the ROUTEKIT_EVAL-owned local install's cli.js.
            ...
            finalizePiNormalizeState(
              initialPiNormalizeState({ prompt: input.prompt }),
              {
                failure: agentFailure({
                  code: "ROUTEKIT_EVAL_PI_BINARY_UNAVAILABLE",
                  message: "ROUTEKIT_EVAL could not start its managed Pi installation.",
                  remediation:
                    "Set ROUTEKIT_EVAL_PI_BIN to a working Pi executable or reinstall Pi.",
                  stage: "harness",
                }),
                ok: false,
              }
...
const nextClampedCap = (
  ...
  log
    .child("pi")
    .warn(
      `Gateway rejected max_tokens (402); retrying with max_tokens<=${clampTo} (attempt ${attempt + 1}/${MAX_CLAMP_RETRIES})`
    );
  return {
    modelId,
    maxTokens: clampTo,
...
const promptPi = async function* (
  ...
  env[ROUTEKIT_EVAL_GATEWAY_SESSION_ID_ENV] =
    options.sessionId ??
    env[ROUTEKIT_EVAL_GATEWAY_SESSION_ID_ENV] ??
    crypto.randomUUID();
  if (normalizeEnvValue(env[ROUTEKIT_EVAL_BEARER_TOKEN_ENV]) === undefined) {
  ...
  const modelId = piGatewayModelId(resolvedModel);
...
export {
  ROUTEKIT_EVAL_PI_BIN_ENV,
  buildPiProcessEnv,
  ROUTEKIT_EVAL_GATEWAY_SESSION_ID_ENV,
  processResultError,
