...
// Bundle-wide `--define` injected by the compile step (`ROUTEKIT_EVAL_CLI_COMPILED=true`
// when compiling a single-file binary, `false` otherwise). Declared ambient
// here — mirroring `@routekit-eval-runtime/cli/build-info` — rather than imported,
...
// non-dependency; the underlying `ROUTEKIT_EVAL_CLI_COMPILED` define is the same one
// build-info reads. Wave 2 sets the define false for this product.
export const isCompiledRouteKitEvalBuild = (): boolean =>
...
// The Node that runs `pi`: this process's interpreter (routekit-eval is itself Node),
