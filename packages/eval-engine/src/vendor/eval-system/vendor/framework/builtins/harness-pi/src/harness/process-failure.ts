...
import type { HarnessProcessResult } from "../../../routekit-eval/src/process.ts";
...
import { stderrTail } from "../../../routekit-eval/src/process.ts";
...
/**
 * What ROUTEKIT_EVAL knows about a finished Pi process.
 *
...
export const piProcessFailure = (outcome: PiProcessOutcome): AgentFailure => {
  ...
  if (outcome.missingBinary === true) {
    return agentFailure({
      ...
      remediation: "Set ROUTEKIT_EVAL_PI_BIN to a working Pi executable or reinstall Pi.",
      stage: "harness",
    });
  }
  if (outcome.timedOut) {
    return agentFailure({
      code: "ROUTEKIT_EVAL_PI_PROCESS_TIMEOUT",
      message: "Pi timed out before completing the request.",
      remediation: "Raise ROUTEKIT_EVAL_PI_TIMEOUT_MS or simplify the request.",
      stage: "harness",
  ...
  return agentFailure({
    code: "ROUTEKIT_EVAL_PI_PROCESS_FAILED",
    message:
