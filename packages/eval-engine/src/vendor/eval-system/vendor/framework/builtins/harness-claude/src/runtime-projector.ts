import type { AgentFailure, AgentRuntimeEvent, RuntimeUsage } from "../../routekit-eval/src/index.ts";

import { agentFailure } from "../../routekit-eval/src/index.ts";
      ...
      runtimeEvent(
        AgentRuntimeEventTag.RuntimeError,
        {
          ...
          failure: agentFailure({
            code: "ROUTEKIT_EVAL_CLAUDE_PLUGIN_INSTALL_FAILED",
            kind: "configuration",
...
const claudeResultFailure = (event: ClaudeResultEvent): AgentFailure =>
  event.subtype === CLAUDE_TURN_LIMIT_SUBTYPE
    ? claudeFailure("ROUTEKIT_EVAL_CLAUDE_TURN_LIMIT", "", {
        stage: "harness",
        upstreamCode: event.subtype,
      })
    : claudeFailure("ROUTEKIT_EVAL_CLAUDE_SESSION_FAILED", extractError(event), {
        stage: "provider",
