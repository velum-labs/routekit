...
// `<node> <realpath(pi-cli.js)> ...` — `process.execPath` when routekit-eval is already
...
// upper cap.
//
// A user-provided `pi` (ROUTEKIT_EVAL_PI_BIN, or a global install) is commonly a bin
...
// `harness.ts`.
const ROUTEKIT_EVAL_PI_BIN_ENV = "ROUTEKIT_EVAL_PI_BIN";
...
const PI_L2S_HINT = `This looks like proot's link2symlink hard-link emulation (Android/Termux proot-distro) mangling a hardlink-installed pi: the script resolves into /.l2s/ where its relative imports cannot resolve. routekit-eval self-heals by keeping its own copy-installed pi under ~/.routekit-eval/pi-runtime — unset ${ROUTEKIT_EVAL_PI_BIN_ENV} to use it, or point ${ROUTEKIT_EVAL_PI_BIN_ENV} at a pi installed with \`npm install --ignore-scripts\`.`;
