// Reference material for `routekit-eval eval docs`. Keep this data close to the CLI so
...
export const EVAL_DOC_TOPICS: readonly EvalDocTopic[] = [
  {
    ...
    summary: "The `routekit/eval` API, run data, and code patterns.",
...
\`\`\`ts
import { assertModelIsLive } from "routekit/eval";
...
Human \`routekit-eval eval\` output prints blocks in order: one test line per \`test()\` with \`pass\`, \`FAIL\`, or \`skip\` and the test name, then one run line per candidate or judge with its resolved model and measurements, then the candidate and judge spend split when judge rows exist, and finally the baseline comparison when \`--baseline\` has one. Only the run block carries models, scores, context tokens, and costs. \`skip\` appears only on test lines. A run that has no resolved model falls back to its requested model, and otherwise prints \`unknown\`; an ungraded run omits \`score\` rather than printing \`score=0.00\`. Read the blocks in that order instead of treating every line as a model result.
...
Read the plain output from \`routekit-eval eval\`. A single run produced:
...
\`routekit-eval eval\` resolves its credential the same way \`routekit-eval code\` does: a key already in the environment first, then the workspace \`.routekit-eval/credentials.json\`, then the global \`~/.routekit-eval/credentials.json\` and \`~/.gateway/credentials.json\`. Anyone who has run \`routekit-eval login\`, locally or globally, is already set up, and the resolved key is exported into the \`node --test\` child. The environment check counts the credential the CLI resolves from the launch directory at startup, so a launch-directory or global key wins over a different workspace's when you run \`routekit-eval eval <path>\` from elsewhere. A run with no credential anywhere fails before any model call and says to run \`routekit-eval login\`, and in json mode it fails rather than opening a login flow, so the single JSON document stays parseable.

A failing eval fails the shell and CI, and the process exits \`1\` rather than forwarding Node's exact code, but the printed failure names the real one as \`\` \`node --test\` exited with code N \`\`, so you never need a second run to learn it. The per-test timeout is 120s, and a long agent loop that exceeds it fails as a bare Node timeout with no eval-level explanation, so raise it with \`--timeout <ms>\` or per-\`test()\` with \`{ timeout }\`, which wins. Discovery skips \`node_modules\`, \`.git\`, and \`.routekit-eval\`, so an eval parked under \`.routekit-eval/\` silently never runs.

Runs inside an RouteKitEval workspace append to \`.routekit/eval/history.jsonl\`, which is git-ignored and capped at 200 runs, and \`--no-history\` skips it. Scratch runs append to the history inside that scratch workspace, and their baseline series is local to that directory. \`--baseline last|best|model:<slug>\` is reporting only and never changes the exit code. \`--report <path>\` resolves against the eval directory rather than your cwd, so \`routekit-eval eval evals/support --report comparison.md\` writes \`evals/support/comparison.md\`, and a scratch run writes beside its eval. While iterating, re-run one file, not the whole comparison, because every full run spends real money.

## Commands

\`\`\`sh
routekit-eval eval                              # every *.eval.ts in the workspace
routekit-eval eval evals/support                # one directory or file
routekit-eval eval /tmp/tmp.AbC123              # a throwaway eval outside the workspace
routekit-eval eval --report comparison.md       # shareable markdown
routekit-eval eval --baseline best              # compare against the best earlier run
routekit-eval eval --timeout 300000             # raise the per-test ceiling from 120s
routekit-eval eval --list --allow-no-key        # list what would run, no key, no model call
\`\`\``),
  },
  {
    topic: "lifecycle",
...
In CI, evals make real model calls, so put them in a credentialed, opt-in job with the key as a secret rather than the default unit-test job. An \`routekit-eval eval --list --allow-no-key\` step needs no key and cheaply asserts the evals still exist.`),
  },
];
