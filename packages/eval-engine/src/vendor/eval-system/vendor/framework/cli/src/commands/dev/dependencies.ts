...
import { routeKitEvalChildArgvFrom } from "./split/child-argv.ts";
...
const ROUTEKIT_EVAL_DEV_DEPENDENCY_INSTALL_RESTARTED_ENV =
  "ROUTEKIT_EVAL_DEV_DEPENDENCY_INSTALL_RESTARTED";
...
const shouldRestartAfterDevDependencyInstall = (
  ...
  env[ROUTEKIT_EVAL_DEV_DEPENDENCY_INSTALL_RESTARTED_ENV] !== "1";

// Node lays out `process.argv` with the runtime at [0] and the entry script at
// [1], so user args always start at [2]. `routeKitEvalChildArgvFrom` rebuilds the child
...
const devDependencyRestartArgvFrom = (input: {
  ...
  return routeKitEvalChildArgvFrom(
    {
...
const restartAfterDevDependencyInstall = Effect.fn("DevDependencies.restart")(
  function* (decision: DevDependencyInstallDecision) {
    ...
    yield* Effect.logDebug("routekit-eval dev dependency-install restart", {
      argv,
      execPath: process.execPath,
    ...
    if (argv.length === 0) {
      return yield* new CliFailureError({
        detail:
          "Could not restart routekit-eval dev after installing dependencies: no current executable was available.",
      });
    }
    ...
    yield* writeProgressNotice(
      "Restarting routekit-eval dev after dependency install...\n"
    );
    const exitCode = yield* spawner.exitCode(
      ChildProcess.make(command, args, {
        env: {
          ...env,
          [ROUTEKIT_EVAL_DEV_DEPENDENCY_INSTALL_RESTARTED_ENV]: "1",
        },
        stderr: "inherit",
        stdin: "inherit",
        stdout: "inherit",
    ...
    // The dependency-install re-exec waits for the replacement `routekit-eval dev` to run
    // to completion, then this (now-thin) parent must exit with the child's exact
    // code. Rather than a manual exit syscall, fail with the `RouteKitEvalCliExit` marker:
    // it carries `Runtime.errorExitCode`, so `runRouteKitEvalCli` surfaces the code and the
...
export const resolveDevDependencyInstallDecision = Effect.fn(
   ...
   * (`routekit-eval start`) roots `featuresRoot` under `.routekit-eval/composed`, an internal
  ...
  if (Number(exitCode) !== SUCCESS_EXIT_CODE) {
    return yield* new CliFailureError({
      detail: `Could not install intern dependencies in ${decision.workspaceRoot}: npm ${INSTALL_ARGS.join(
        " "
      )} exited with code ${Number(exitCode)}. Run \`cd ${decision.workspaceRoot} && npm install --ignore-scripts\`, then \`routekit-eval dev\`.`,
    });
  }
...
export {
  ROUTEKIT_EVAL_DEV_DEPENDENCY_INSTALL_RESTARTED_ENV,
