...
interface InitProjectOptions {
   ...
   * in the same invocation. Unset for interactive `routekit-eval init <name>`, which
   * just warns and lets the user retry.
   */
  readonly failOnInstallError?: boolean;
  /**
   * Targets the global workspace at `~/.routekit-eval/global`. Only changes the
   * post-scaffold "next steps" message (the target path is encoded in
   * `cwd`/`name`); `routekit-eval dev`/`routekit-eval start` find it from any directory.
   */
  ...
  /**
   * Skip the Gateway credential gate. Set during `routekit-eval dev`/`routekit-eval start`
  ...
  /**
   * Skip the post-scaffold "next steps" banner. Set when `routekit-eval dev`/`routekit-eval start`
   * auto-scaffold the global workspace mid-flow, where a "Start it with: routekit-eval dev"
  ...
  /**
   * Also scaffold `deploy/routekit-eval.service` for a headless systemd deploy
...
const scaffoldWorkspace = Effect.fn("ProjectInit.scaffoldWorkspace")(
  function* (input: {
    ...
    return yield* Effect.gen(function* () {
      ...
      // Best-effort: mirror the embedded docs into `.routekit-eval/docs/` so the intern has
 ...
 * outcome of a headless `routekit-eval init`, not a failure.
 */
const runCredentialGate = Effect.fn("ProjectInit.credentialGate")(
  function* (input: {
    ...
    yield* ensureGatewayCredential({
      startDir: input.startDir,
    }).pipe(
      Effect.catch(() =>
        input.cliIo.writeStderr(
          "\nSign-in didn't finish. Run `routekit-eval login` before anything that calls a model.\n"
        )
      ),
      Effect.ignore
    );
...
/**
 * Existing-target path for `routekit-eval init`: refresh the `.routekit-eval/sdk` author-contracts
 * cache in place when the target is an RouteKitEval workspace, then ensure `.gitignore`
 * ignores the generated `.routekit-eval/` cache — so a `postinstall: routekit-eval init .` never
 * leaves the materialized cache commit-able. Both writes are idempotent. When
 * the directory exists but is not an RouteKitEval workspace, fail loudly so
 * `routekit-eval init <existing-non-workspace>` keeps its old "won't overwrite" behavior.
 *
 * When `install` is set (the default), run `npm install` after refreshing the
 * SDK cache: the workspace consumes `.routekit-eval/sdk` through `node_modules/routekit-eval`,
...
const syncExistingWorkspace = Effect.fn("ProjectInit.syncExistingWorkspace")(
  function* (input: {
    ...
    // keeps `.routekit-eval/docs/` current. Best-effort and idempotent.
    yield* writeDocsCache(input.projectRoot).pipe(Effect.ignore);

    yield* writeProgressNotice(
      `\nMaterialized the RouteKitEval SDK cache (.routekit-eval/sdk) in ${input.projectRoot}.\n`
    );

    // Refresh (or add) requested deploy/ artifacts on a sync too, so
    // `routekit-eval init . --with-docker` materializes them into an existing workspace.
    yield* writeDeployScaffold({
    ...
    // Install so the refreshed `.routekit-eval/sdk` lands in `node_modules/routekit-eval`. npm copies a
    ...
    //
    // Safe under a `"postinstall": "routekit-eval init ."` hook: the enclosing
    ...
    // recorded `routekit-eval.md` version against the running CLI, print what stopped
...
export const initProject = Effect.fn("ProjectInit.initProject")(function* (
  ...
  // When the target already exists, fall back to sync mode for an existing RouteKitEval
  // workspace: materialize the `.routekit-eval/sdk` author-contracts cache so typecheck
  // can resolve the bare `routekit-eval` specifier. This is what makes `routekit-eval init .` work,
  // and lets a generated project wire `"postinstall": "routekit-eval init ."`. A target
  // that exists but is *not* an RouteKitEval workspace still fails loudly.
