...
const writeAuthorContractsCache = Effect.fn(
  ...
  readonly routeKitEvalDirectory: RouteKitEvalDirectory["Service"];
  readonly projectRoot: string;
}) {
  const path = yield* Path.Path;
  const targetRoot = input.routeKitEvalDirectory.authorContractsCacheDir(
    input.projectRoot
  );
  const files = {
...
const syncAuthorContracts = Effect.fn("ProjectInit.syncAuthorContracts")(
  function* (
    ...
    FileSystem.FileSystem | RouteKitEvalDirectory | Path.Path
  > {
...
/**
 * Materialize (or refresh) the `.routekit-eval/sdk` author-contracts cache for an
 * *existing* workspace, but only when the project opts in by declaring the `routekit-eval`
 * author-contracts file dependency (`"routekit-eval": "file:.routekit-eval/sdk"`) in its root
 ...
 * source (where `.routekit-eval/sdk` never exists) install cleanly instead of failing
 * `npm install` outright on an unresolvable required dependency. This is the
 * safe, idempotent path shared by `routekit-eval dev` and the existing-workspace branch
 * of `routekit-eval init` (e.g. `postinstall: routekit-eval init .`): unrelated directories are
 * left untouched.
 *
 * Returns `true` when the directory is an RouteKitEval workspace and the cache is now
 ...
 * distinguish "synced an existing workspace" from "this is not an RouteKitEval
 * workspace".
 */
