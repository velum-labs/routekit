// `routekit-eval eval --dry-run`: prove an eval loads without spending a model call.
//
 ...
 */
const NEVER_MATCHES = "__ROUTEKIT_EVAL_DRY_RUN_NEVER__";
 ...
 *
 * `--test-name-pattern='__ROUTEKIT_EVAL_DRY_RUN_NEVER__'` still counts each *file*
...
const nodeUnavailable = (execPath: string): CliFailureError =>
  new CliFailureError({
    detail: `\`routekit-eval eval --dry-run\` could not run \`${execPath}\`. Evals load through \`node --test\`.`,
    hint: "Confirm Node.js >= 22.22 is on PATH, then re-run `routekit-eval eval --dry-run`.",
  });
