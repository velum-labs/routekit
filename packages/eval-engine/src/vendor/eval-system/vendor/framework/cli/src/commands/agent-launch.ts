...
/**
 * The flag config shared by `routekit-eval claude`, `routekit-eval codex`, `routekit-eval opencode`, and
 * `routekit-eval hermes`. Both `Command.make` calls and {@link splitAgentLaunchArgv}
...
export const agentLaunchFlags = {
  globalAuth: Flag.boolean("global-auth").pipe(
    ...
    Flag.withDescription(
      "Allow inherited and global Gateway auth (default: on; use --no-global-auth for workspace-only auth)"
    )
  ),
  model: Flag.string("model").pipe(
    Flag.withDescription("Model to use (any Gateway model id)"),
    Flag.optional
  ),
};

/**
 * Cuts `routekit-eval claude` / `routekit-eval codex` / `routekit-eval opencode` / `routekit-eval hermes` argv into the
...
export const formatAuthSource = (source: GatewayAuthSource): string => {
  switch (source.kind) {
    case "project": {
      return `project dotenv (${source.location})`;
    }
    case "environment": {
      return "exported environment (ROUTEKIT_EVAL_BEARER_TOKEN)";
    }
    case "global":
    case "workspace": {
...
export const resolveGatewayKey = Effect.fn(
  "AgentLaunch.resolveGatewayKey"
)(function* (input: {
  readonly kind: AgentLaunchKind;
  readonly globalAuth: boolean;
}) {
  ...
  if (Option.isSome(authSource) && (yield* cliIo.isStdinTty)) {
    yield* cliIo.writeStderr(
      `Using Gateway credential from ${formatAuthSource(authSource.value)}\n`
    );
  }
  const secrets = yield* RuntimeSecretStore;
  const secret = yield* secrets.get(RuntimeSecretName.GatewayApiKey);
  if (Option.isNone(secret)) {
    return yield* new CliFailureError({
      detail: `No Gateway API key is available for \`routekit-eval ${input.kind}\`.`,
    });
  }
  return secret.value;
});
...
const launchFailed =
  ...
  ({ detail }) =>
    Effect.fail(
      new CliFailureError({
        detail: `Could not launch \`routekit-eval ${kind}\`: ${detail}`,
      })
    );
        ...
        HostProcessCommandNotFound: ({ command }) =>
          Effect.fail(
            new CliFailureError({
              ...
              hint: `Open a new shell so it picks up the installer's PATH change, then run \`routekit-eval ${input.kind}\` again.`,
            })
          ),
