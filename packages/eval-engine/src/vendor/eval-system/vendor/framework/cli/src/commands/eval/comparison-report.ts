...
const ABSENCE_LINES: Record<EvalBaselineAbsence, (selector: string) => string> =
  {
    "no-workspace": (selector) =>
      `\nNo baseline (--baseline ${selector}). A run history is only kept inside an RouteKitEval workspace, so this run was not recorded and there is nothing here to compare against; run "routekit-eval init" to start one.\n`,
    "none-yet": () => "",
    "recording-disabled": () => "",
    "scoped-out": (selector) =>
