// Turning what the caller named into the two directories `routekit-eval eval` needs. Split
  ...
  function* (requestedBy: {
    ...
    if (isFile && !resolved.endsWith(EVAL_SUFFIX)) {
      ...
      return yield* new CliFailureError({
        ...
        hint: `\`routekit-eval eval\` only runs ${EVAL_SUFFIX} files. Name one of those, or a directory containing them.`,
      });
    }
