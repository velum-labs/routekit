...
// The composed root lives under the workspace's own (gitignored) `.routekit-eval/`, beside
...
/**
 * True when `featuresRoot` is (or lives under) a materialized `.routekit-eval/composed`
 * root. A composed root is an internal artifact: it links each source's
 * `features/` plus the declaring workspace's `routekit-eval.md`. Callers that re-resolve a
 * features root (notably `routekit-eval start`, which hands its already-composed root back
 * to `prepareDevFeaturesRoot`) MUST NOT treat it as a fresh source — re-reading
 * its linked `routekit-eval.md` `features:` and composing again would resolve declared
 * paths against `.routekit-eval/composed` and crash. This predicate lets them short-circuit.
 *
 * Matches the `.routekit-eval` → `composed` pair on path-segment boundaries (splitting on
...
export const isComposedFeaturesRoot = (featuresRoot: string): boolean => {
  ...
  return segments.some(
    (segment, index) =>
      segment === ROUTEKIT_EVAL_DIRECTORY_NAME &&
...
const resolveSource = Effect.fn("ComposeFeatures.resolveSource")(function* (
  ...
  // Remote repo paths materialize into the `.routekit-eval/remote-features` cache; local
...
/**
 * The base `.routekit-eval/composed` directory to materialize the composed root under: the
 * enclosing workspace's `.routekit-eval/` when inside a workspace, else `~/.routekit-eval`. Mirrors
  ...
  for (const source of sources) {
    ...
    for (const name of names) {
      ...
      if (previous !== undefined) {
        ...
        yield* cliIo
          .writeStderr(
            `routekit-eval start: feature "${name}" from ${source.origin} shadows the one from ${previous}.\n`
          )
          .pipe(Effect.ignore);
      }
...
/**
 * Link the declaring workspace's root persona `routekit-eval.md` into the composed root,
 ...
 * composed root is `.routekit-eval/composed` — not any real workspace — so without this
 * the intern's declared model, harness preference, and persona body silently
 * vanish whenever ≥2 sources are composed.
 *
 * The persona owner is the anchor workspace: the project whose `routekit-eval.md`
 ...
 * else the first `--features` source's workspace for a pure `routekit-eval start
 ...
 * tree (a symlink per feature, plus the declaring workspace's `routekit-eval.md`) in a
...
const buildComposedRoot = Effect.fn("ComposeFeatures.buildComposedRoot")(
  function* (input: {
    ...
    // Stage the declaring workspace's `routekit-eval.md` beside the composed `features/` so
...
export interface ComposedFeatures {
   ...
   * option when given (used when `routekit-eval.md`'s own project must anchor, regardless
   ...
   * `dirname(<.routekit-eval>/composed/features)`, which would point resolution at the
   * cache dir.
   */
  readonly workspaceRoot: string;
  /**
   * True only when ≥2 sources were consolidated into a `.routekit-eval/composed` root.
...
export interface ComposeFeatureRootsOptions {
   ...
   * by a workspace's `routekit-eval.md` `features:` (the declaring project must anchor)
 ...
 * - ≥2 inputs → a composed root at `<.routekit-eval>/composed/features` containing a
...
export const composeFeatureRoots = Effect.fn("ComposeFeatures.compose")(
  function* (
    ...
    // The composed features root lives under `.routekit-eval/composed`, so its own parent
    // is NOT a real workspace. Anchor to the explicit workspace root when given
    // (e.g. the project whose `routekit-eval.md` declared the sources); else the FIRST
    // source's workspace root (the primary project). Never `.routekit-eval/composed`.
    ...
    yield* buildComposedRoot({
      ...
      // workspace owns `routekit-eval.md`, so a declared/remote source ordered first never
