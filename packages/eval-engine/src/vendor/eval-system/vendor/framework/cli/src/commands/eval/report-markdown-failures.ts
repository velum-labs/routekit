...
import { runModel } from "./report-markdown-sections.ts";
import {
  isFailedRun,
  runErrorText,
} from "./results.ts";
 ...
 * prompt (ROUTEKIT_EVAL-762) rather than merely for knowing the run was wrong.
 *
 * Filtering on only the first was the ROUTEKIT_EVAL-794 defect: a judged-failed run has no
