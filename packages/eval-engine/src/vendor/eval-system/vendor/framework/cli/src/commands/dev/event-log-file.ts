...
// verbatim makes `.routekit-eval/logs` run files grow ~O(turns * promptSize) instead of
...
// for real-time debugging; only the on-disk copy is bounded, so `routekit-eval logs`
...
// Retention knobs for `.routekit-eval/logs`. We keep logs workspace-local and prune them
...
// or workspace-scoped). Each bound accepts a runtime-neutral `ROUTEKIT_EVAL_LOG_*` env
// (the documented name for headless `routekit-eval start`) and the historical
// `ROUTEKIT_EVAL_DEV_LOG_*` alias; the dev-prefixed name wins when both are set so an
// existing `routekit-eval dev` setup keeps its behavior.
const ROUTEKIT_EVAL_LOG_MAX_RUNS_ENVS = [
  "ROUTEKIT_EVAL_DEV_LOG_MAX_RUNS",
  "ROUTEKIT_EVAL_LOG_MAX_RUNS",
] as const;
const ROUTEKIT_EVAL_LOG_MAX_AGE_DAYS_ENVS = [
  "ROUTEKIT_EVAL_DEV_LOG_MAX_AGE_DAYS",
  "ROUTEKIT_EVAL_LOG_MAX_AGE_DAYS",
] as const;
const ROUTEKIT_EVAL_LOG_MAX_TOTAL_MB_ENVS = [
  "ROUTEKIT_EVAL_DEV_LOG_MAX_TOTAL_MB",
  "ROUTEKIT_EVAL_LOG_MAX_TOTAL_MB",
] as const;
...
/**
 * Per-invocation retention overrides (friendly units) from `routekit-eval dev` flags.
 * Each takes precedence over the matching `ROUTEKIT_EVAL_DEV_LOG_*` env var, which in
...
interface StartDevEventLogFileInput {
  ...
  /** Flag overrides for `.routekit-eval/logs` retention (flag > env > default). */
  readonly retention?: DevLogRetentionOverrides;
  /**
   * Run-file label, i.e. the `<label>-<stamp>.jsonl` prefix. Defaults to `dev`;
   * `routekit-eval start` passes `start` so its runs are distinguishable on disk while
   * `routekit-eval logs` still reads both.
   */
...
// MAX_RAW_PAYLOAD_BYTES above) — the live journal a running `routekit-eval dev`/`routekit-eval
...
const writeRecord = Effect.fn("DevCommand.writeRecord")(function* (
  ...
  // against DevEventLogEntrySchema by the readers (`routekit-eval logs`, dev-log-store).
  ...
  // (`routekit-eval logs`, `dev-log-store.ts`) decode it instead of silently dropping it.
  ...
  const fallback = JSON.stringify({
    line: `[routekit-eval-runtime] dropped oversized ${record.type} record (${serializedBytes.byteLength} bytes > ${MAX_LINE_BYTES} byte cap)`,
  ...
  return yield* Effect.gen(function* () {
    const maxRuns =
      ...
      parseOptionalPositiveInt(
        yield* readFirstConfigString(ROUTEKIT_EVAL_LOG_MAX_RUNS_ENVS)
      ) ??
      DEFAULT_DEV_LOG_RETENTION.maxRuns;
    const maxAgeDays =
      positiveOverride(overrides.maxAgeDays) ??
      parseOptionalPositiveInt(
        yield* readFirstConfigString(ROUTEKIT_EVAL_LOG_MAX_AGE_DAYS_ENVS)
      );
    const maxTotalMb =
      positiveOverride(overrides.maxTotalMb) ??
      parseOptionalPositiveInt(
        yield* readFirstConfigString(ROUTEKIT_EVAL_LOG_MAX_TOTAL_MB_ENVS)
      );
    return {
      maxAgeMillis:
...
/**
 * Tee everything the `routekit-eval dev` events pane shows into an append-only NDJSON file
 * under `.routekit-eval/logs/`, copying Cursor's durable event-log model. The tails are
...
export const startDevEventLogFile = Effect.fn("DevCommand.eventLogFile")(
  function* (input: StartDevEventLogFileInput) {
    ...
    input.publishLog?.(`[routekit-eval-runtime] writing event log to ${filePath}`);
    return {
      path: filePath,
