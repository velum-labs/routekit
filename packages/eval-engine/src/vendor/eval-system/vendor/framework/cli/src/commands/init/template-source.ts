...
import { ROUTEKIT_EVAL_GITIGNORE_ENTRY } from "../../routekit-eval-directory.ts";
...
/** The baseline template; what a plain `routekit-eval init` (no `--template`) scaffolds. */
const DEFAULT_TEMPLATE = "default";

/** Point at a local checkout of the templates repository, skipping the download. */
const TEMPLATES_DIR_ENV = "ROUTEKIT_EVAL_TEMPLATES_DIR";
...
// ignores). `ROUTEKIT_EVAL_GITIGNORE_ENTRY` (`.routekit-eval/`) keeps the injected author-contracts
...
// and idempotent: a template without an `routekit-eval.md`, or a version that cannot be
  ...
  function* (targetDir: string, workspaceName: string) {
    ...
    const updated = {
      ...
      dependencies: {
        ...
        // The scaffolded project declares ONLY `routekit-eval` (`file:.routekit-eval/sdk`). `effect` and
        // `@effect/platform-node` are NOT direct project dependencies — they are
        // transitive deps of the `.routekit-eval/sdk` package (declared in its own
        ...
        // `routekit-eval`, never from `effect` directly, so `effect` must not appear in the
...
// so the existing-workspace sync path (`routekit-eval init .`) can keep `.gitignore`
// correct after materializing `.routekit-eval/sdk`, matching what the create path does.
 ...
 * source is a local checkout when `ROUTEKIT_EVAL_TEMPLATES_DIR` is set, otherwise the
 ...
 *
 * After copying, the template is turned into a ready-to-edit RouteKitEval workspace: the
 * root package.json is renamed and gains the `routekit-eval` author-contracts dependency,
 * the `.routekit-eval/sdk` author-contracts cache is written, and the `.routekit-eval/` (plus env and
