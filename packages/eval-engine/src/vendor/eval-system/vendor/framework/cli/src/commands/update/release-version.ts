...
/** The outcome of an `routekit-eval update --check`, rendered to stdout or `--json` output. */
 ...
 * `routekit-eval update`: it is deliberately stricter than `!updateAvailable`, which is
...
export const formatUpdateCheckResult = (result: UpdateCheckResult): string => {
  if (result.currentVersion === null) {
    return [
      ...
      "Run `routekit-eval update` to install the latest version.",
      "",
    ].join("\n");
  }

  if (!result.updateAvailable) {
    return `RouteKitEval is up to date (${result.currentVersion}).\n`;
  }

  return [
    `Update available: ${result.currentVersion} -> ${result.latestVersion}`,
    "Run `routekit-eval update` to install it.",
    "",
  ].join("\n");
};
