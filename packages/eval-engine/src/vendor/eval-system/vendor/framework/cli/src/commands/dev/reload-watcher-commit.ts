...
const materializeSnapshotEffect = Effect.fn(
  ...
  // untouched) on reload, matching the agent-runner path; otherwise `routekit-eval dev`
  // would silently drop feedback that `routekit-eval code`/`routekit-eval chat` show.
...
const materializeThenCommit = Effect.fn(
  ...
  // interrupted (a clean `routekit-eval dev` exit / process shutdown), the interruption
