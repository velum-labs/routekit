// The shapes the `routekit-eval eval` results channel carries, and folding them back into
 ...
 * event type an author sees, so they arrive on the daemon path `routekit-eval eval` uses and
 ...
 * is optional because a workspace can be running a generated `routekit/eval` older than
 ...
 * complete comparison of them (ROUTEKIT_EVAL-792).
 *
