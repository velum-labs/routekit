...
import { comparableEvalRuns } from "./baseline-scope.ts";
import { describeEvalBaselineSelector } from "./baseline-selector.ts";
...
const EvalRegressionKindSchema = Schema.Literals([
  "pass-rate",
  "failed-run-rate",
]);
...
const failedRunRate = (counts: {
  ...
  counts.runs === 0 ? undefined : counts.failedRuns / counts.runs;
...
const compareModels = (
  ...
  const baselineByModel = new Map(
    baseline.models.map((model) => [model.model, model])
  );
  return Arr.getSomes(
...
const modelsMissingFrom = (
  ...
  return present.models
    ...
    .filter((model) => !absentModels.has(model));
};
...
export const selectEvalBaseline = (input: {
  ...
  if (selector.kind === "model") {
    return pipe(
      history,
      Arr.findLast((entry) =>
        entry.models.some((model) => model.model === selector.model)
      )
    );
  }
 ...
 * `no-workspace` can never fix itself: no run is recorded outside an RouteKitEval
...
const absenceWithinWorkspace = (input: {
  ...
  comparableEvalRuns(input.history, input.files).length === 0
    ? "scoped-out"
    : "none-yet";
 ...
 */
export const evalBaselineComparison = Effect.fn("EvalBaseline.compare")(
  function* (input: {
    readonly current: EvalHistoryEntry;
    ...
    if (Option.isNone(workspaceRoot)) {
      return noBaseline(
        input.selector,
        input.recordEnabled ? "no-workspace" : "recording-disabled"
      );
    }
    const history = yield* readEvalHistory(
      routeKitEvalDirectory.evalHistoryPath(workspaceRoot.value)
    );
    const files = input.current.files ?? [];
    const comparison = Option.map(
      selectEvalBaseline({
        files,
        history,
    ...
    const outcome: EvalBaselineOutcome = {
      absence: Option.isSome(comparison)
        ? Option.none()
        : Option.some(
            absenceWithinWorkspace({
              files,
              history,
            })
          ),
  ...
  // The pipeline sees the call's own arguments, so a history that could not be
  // read still reports which selector went unanswered. It reports `none-yet`: the
  // workspace exists and the next run may well read cleanly, so this is the one
  // absence that stays silent rather than telling someone to fix something.
  (effect, input) =>
    Effect.catchCause(effect, () =>
      Effect.succeed(noBaseline(input.selector, "none-yet"))
    )
);

export {
  EvalBaselineAbsenceSchema,
