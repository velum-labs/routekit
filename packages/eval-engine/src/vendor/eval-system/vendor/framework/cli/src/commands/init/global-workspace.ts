...
const ROUTEKIT_EVAL_DIR = ".routekit-eval";

/**
 * The `.routekit-eval/` entries a boot or an install rebuilds. Left in, they dominate the
 ...
 *
 * Anything under `.routekit-eval/` that is NOT listed here is kept on purpose. The author
 ...
 */
const GENERATED_ROUTEKIT_EVAL_ENTRIES = new Set([
  "docs",
  "feature-apis.d.ts",
  "feature-hooks.d.ts",
  "linked-features",
...
/** Where a workspace archive lands: `~/.routekit-eval/backups/global-<timestamp>.zip`. */
...
const isArchivable = (segments: readonly string[]): boolean => {
  ...
  return !(
    top === ROUTEKIT_EVAL_DIR &&
    next !== undefined &&
    GENERATED_ROUTEKIT_EVAL_ENTRIES.has(next)
  );
};

/**
 * `stat` follows symlinks, so a dangling one throws. A real global workspace
 * chains `.claude/skills/*` and `.agents/skills/*` through `.routekit-eval/snapshot/current`,
...
/**
 * Whether `routekit-eval dev`/`routekit-eval start` should scaffold the global workspace before
...
/**
 * Scaffold `~/.routekit-eval/global` on demand so `routekit-eval dev`/`routekit-eval start` always have a
 ...
 *
 * `routekit-eval code` always boots against `~/.routekit-eval/global/features` (never the launch
...
export const ensureGlobalWorkspaceForCode = Effect.fn(
  ...
  yield* writeProgressNotice(
    `No global RouteKitEval workspace yet; creating one at ${globalRoot}...\n`
  );
  yield* scaffoldGlobalWorkspace(globalRoot);
  yield* writeProgressNotice(`Global workspace ready at ${globalRoot}.\n`);
  return globalFeaturesRoot;
