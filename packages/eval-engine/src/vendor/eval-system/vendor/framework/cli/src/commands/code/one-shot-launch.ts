...
// The launch decision for `routekit-eval code` (RFC 0004 code.md): which of the three
...
const EXPLICIT_JSON_HINT =
  "A headless `routekit-eval code -p` run streams prose, not an envelope. For structured machine output use `--output jsonl`, or the JSON sub-APIs (`routekit-eval sessions`, `routekit-eval logs`, `routekit-eval schedules`).";
 ...
 * `--json`/`ROUTEKIT_EVAL_OUTPUT=json` without it keeps the interactive refusal (a piped
 ...
 * `ROUTEKIT_EVAL_OUTPUT=human`, which previously booted the TUI and painted ANSI escapes
 * into the pipe.
 */
