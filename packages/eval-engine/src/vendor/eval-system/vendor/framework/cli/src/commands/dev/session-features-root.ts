...
import { ROUTEKIT_EVAL_INTERN_WORKSPACE_ROOT_ENV } from "../../../../contracts/internal/src/cli/intern-launcher-env.ts";
...
import { readRouteKitEvalMdFeatureSources } from "./routekit-eval-md-feature-sources.ts";
import {
 ...
 * `routekit-eval init` scaffolds a new project into a child directory named after the
 * workspace, so users frequently run `routekit-eval dev` from the parent and hit the
 ...
 * project (never `.routekit-eval/composed`); {@link workspaceOverride} is set only when a
...
export interface ResolvedDevFeatures {
  ...
  readonly workspaceRoot: string;
  /** Set to `workspaceRoot` only when a `.routekit-eval/composed` root was materialized. */
 ...
 * cwd, fall back to the global workspace at `~/.routekit-eval/global/features`.
 */
const resolveLocalFeaturesRoot = Effect.fn("DevCommand.localFeaturesRoot")(
  function* (features: Option.Option<string>) {
    ...
    // into the `.routekit-eval/remote-features` cache and its local directory stands in.
    const input =
      override === undefined
    ...
    if (Option.isNone(features)) {
      ...
      return yield* new CliFailureError({
        detail: `Features directory not found at ${resolved}, and no global workspace exists at ${path.dirname(globalFeaturesRoot)}. Run \`routekit-eval dev\` from a project root that contains a features/ directory, pass --features <dir>, or run \`routekit-eval init --global\` to create a global workspace.${hint}`,
      });
    }

    const hint = yield* nearbyWorkspaceHint(fs, path, path.dirname(resolved));
    return yield* new CliFailureError({
      detail: `Features directory not found at ${resolved}. Run \`routekit-eval dev\` from a project root that contains a features/ directory, or pass --features <dir>.${hint}`,
    });
  }
);

/**
 * Compose the `routekit-eval.md`-declared sources with the local `./features` and any
...
const composeDeclaredAndFlags = Effect.fn("DevCommand.composeDeclared")(
  function* (input: {
    ...
    // `routekit-eval.md` that declares it, NOT the process CWD — those differ whenever
 ...
 * source that applies:
 *   `[…routekit-eval.md declared, local ./features (or the single --features base), …extra --features flags]`
 * `composeFeatureRoots` is last-wins on a duplicate feature id, so the local
 * workspace shadows an `routekit-eval.md`-declared same-named feature, and repeated
 * `--features` flags shadow everything. The result is anchored to the LOCAL
 * project (its `routekit-eval.md`'s workspace) so credentials/config/descriptor resolve
 * there — never `.routekit-eval/composed` or a remote source. With no declared sources and
 * no extra flags this is byte-for-byte the old single-root behaviour (no
 * `.routekit-eval/composed`). Shared by `start`, `dev`, `code`, `eval`.
 *
 ...
 * (only `routekit-eval start` passes more than one today).
 */
const resolveDevFeaturesRoot = Effect.fn("DevCommand.featuresRoot")(function* (
  ...
  // An already-materialized `.routekit-eval/composed` root is an internal artifact, not a
  // fresh source. `routekit-eval start` composes up front and then hands the composed root
  // back through here (via `prepareDevFeaturesRoot`); since the compose step links
  // the declaring workspace's `routekit-eval.md` INTO `.routekit-eval/composed`, re-reading its
  // `features:` and composing again would resolve declared paths against
  // `.routekit-eval/composed` and crash. Short-circuit: the composition already happened.
  ...
  const declared = yield* readRouteKitEvalMdFeatureSources(localWorkspaceRoot);
...
const exportWorkspaceRootEnv = Effect.fn("DevCommand.exportWorkspaceRootEnv")(
  function* (workspaceRoot: string) {
    ...
    const existing = env[ROUTEKIT_EVAL_INTERN_WORKSPACE_ROOT_ENV]?.trim();
    if (existing !== undefined && existing.length > 0) {
      return;
    }
    yield* hostProcess.setEnv(ROUTEKIT_EVAL_INTERN_WORKSPACE_ROOT_ENV, workspaceRoot);
  }
);
...
const prepareDevFeaturesRoot = Effect.fn("DevCommand.prepareFeaturesRoot")(
  function* (config: {
     ...
     * `ROUTEKIT_EVAL_INTERN_WORKSPACE_ROOT`. `routekit-eval start` sets this when it consolidates
     * several `--features` sources: the composed features root lives under
     * `.routekit-eval/composed`, so `dirname(featuresRoot)` would anchor to the cache dir
    ...
    // Materialize the `.routekit-eval/sdk` author-contracts cache before installing. On a
    // fresh clone `.routekit-eval/sdk` is gitignored and absent, so `npm install` would fail
    // to resolve the workspace's `"routekit-eval": "file:.routekit-eval/sdk"` dependency before the
    ...
    // Anchor precedence: an explicit caller override (e.g. `routekit-eval start` composing
    // --features) > the routekit-eval.md-composition anchor > `dirname(featuresRoot)`.
    const workspaceRoot =
    ...
    // not just `routekit-eval start`. Without this, a workspace whose `routekit-eval.md` declares
    // `features:` composes under `.routekit-eval/composed`, and callers that derive the
    // descriptor root as `dirname(featuresRoot)` would write `.routekit-eval/dev.json`
    // into the cache dir, where `routekit-eval tui`/`routekit-eval logs` can never discover it.
    return {
      featuresRoot,
      workspaceRoot,
    };
