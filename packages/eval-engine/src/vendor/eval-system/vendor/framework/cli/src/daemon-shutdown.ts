 ...
 *   `cli-exit.ts`'s `RouteKitEvalCliExit`). Used for supervisor-restart (`mode: "exit"`)
...
export class DaemonShutdown extends Context.Service<
  DaemonShutdown,
  DaemonShutdownShape
>()("routekit-eval/cli/DaemonShutdown") {
 ...
 * with the {@link RouteKitEvalCliExit} marker so the `runMain` teardown applies the exit
 ...
 * any ROUTEKIT_EVAL_* overrides); the child is orphaned, not killed, by the subsequent
 * exit.
 */
export const performDaemonShutdown = (
  request: DaemonShutdownRequest
): Effect.Effect<never, RouteKitEvalCliExit> =>
  DaemonShutdownRequest.$match(request, {
    Exit: ({ code }) => new RouteKitEvalCliExit({ exitCode: code }),
    ...
    Relaunch: ({ args, command }) =>
      ...
      }).pipe(Effect.andThen(new RouteKitEvalCliExit({ exitCode: RELAUNCH_EXIT_CODE }))),
  });
