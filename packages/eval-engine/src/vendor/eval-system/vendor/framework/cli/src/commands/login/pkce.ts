...
import { GATEWAY_AUTH_URL } from "../../../../contracts/internal/src/gateway-auth.ts";

const ROUTEKIT_EVAL_KEY_LABEL = "routekit-eval";
...
export const buildAuthUrl = ({
  ...
  const url = new URL(GATEWAY_AUTH_URL);
  url.searchParams.set("callback_url", callbackUrl);
  url.searchParams.set("code_challenge", codeChallenge);
  url.searchParams.set("code_challenge_method", CODE_CHALLENGE_METHOD_S256);
  url.searchParams.set("key_label", ROUTEKIT_EVAL_KEY_LABEL);
  return url.toString();
};
