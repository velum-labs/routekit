// RFC 0009 author-eval-support.md (and RFC 0004 eval.md): the `routekit-eval eval` command.
...
// separate `routekit-eval dev` to babysit.
...
import { loadStoredGatewayKeyIntoEnvFrom } from "../login/credentials.ts";
 ...
 * through the same gate `routekit-eval code` uses in its default mode: a key already in
 * the environment, then the workspace `.routekit-eval/credentials.json`, then the global
 * `~/.routekit-eval/credentials.json` and `~/.gateway/credentials.json`.
 *
 ...
 * `routekit-eval code` rule, not an eval-specific one, and `--no-global-auth` on `routekit-eval
...
const resolveEvalCredentialEnv = Effect.fn("EvalCommand.resolveCredential")(
  function* (input: {
    ...
    yield* input.allowNoKey
      ? loadStoredGatewayKeyIntoEnvFrom({ startDir: input.startDir })
...
/**
 * The two ways `routekit-eval eval` finishes having run nothing: `--list`, and a directory
 * with no evals in it.
 *
...
> =>
  Command.make(
    ...
    Command.withDescription(
      "Run *.eval.ts agent evals through node --test against a real model, or read the version-matched reference with `routekit-eval eval docs`. Prints a per-file pass/fail summary and exits non-zero when any eval fails. Resolves auth exactly as routekit-eval code does: ROUTEKIT_EVAL_BEARER_TOKEN, then a stored credential from routekit-eval login, workspace-local or global. Unless --dry-run, --list, or --allow-no-key is passed, a run with no credential anywhere fails before any model call"
    ),
