...
const makeProductionAutoUpdateActions = (input: {
  ...
  // version fetch and the ROUTEKIT_EVAL-370 up-to-date short-circuit.
...
const applyUpdateTick = Effect.fn("applyUpdateTick")(function* <R>(
  ...
  yield* log(
    `[routekit-eval-update] Applying ${severity} update ${versionArrow(current, latest)}...`
  );
...
const holdUpdateTick = Effect.fn("holdUpdateTick")(function* <R>(
  ...
  if (shouldNotifyHeldUpdate(state, latest)) {
    ...
    yield* log(
      `[routekit-eval-update] Holding ${severity} update ${versionArrow(current, latest)} for approval.`
    );
...
const reportAutoUpdateError = <R>(
  ...
  return options.log(`[routekit-eval-update] Update check failed: ${detail}`).pipe(
...
export const runAutoUpdateLoop = <R>(
  ...
  if (options.config.level === "off") {
    return options.log(
      "[routekit-eval-update] Auto-update is off; skipping update checks."
    );
  }
    ...
    runAutoUpdateOnce(options).pipe(
      Effect.flatMap((outcome) => {
        ...
        if (outcome.kind === "unsupported") {
          return options.log(
            "[routekit-eval-update] Not an installed release-channel binary; skipping auto-update checks."
          );
        }
        return Effect.sleep(
