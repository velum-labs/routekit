 ...
 * the last successful check. Stored at `<workspace>/.routekit-eval/auto-update.json` so
 * the auto-update loop does not re-notify on every tick and so `routekit-eval update` can
...
export const autoUpdateStatePath = (
  ...
  path.join(workspaceRoot, ROUTEKIT_EVAL_DIRECTORY_NAME, AUTO_UPDATE_STATE_FILE_NAME);
...
/** Atomically write the held-update state next to the workspace's `.routekit-eval` directory. */
