...
import type { UpdateRestartMode } from "./routekit-eval-config.ts";
...
/** Build the argv that re-invokes the running `routekit-eval` with the same subcommand and flags. */
...
/**
 * Restart `routekit-eval start` after an update: drain in-flight runs (bounded by the
...
export const runUpdateRestart = Effect.fn("Restart.runUpdateRestart")(
  function* (options: RunUpdateRestartOptions) {
    yield* options.log(
      "[routekit-eval-update] Draining in-flight runs before restart..."
    );
    const result = yield* options.drain;
    yield* options.log(
      result.drained
        ? "[routekit-eval-update] In-flight runs drained; restarting."
        : "[routekit-eval-update] Drain timed out; restarting with runs still active."
    );

    const { launcher } = options;
    if (options.mode === "exit") {
      yield* options.log("[routekit-eval-update] Exiting for supervisor restart.");
      return yield* launcher.cleanExit();
    }

    const restartArgv = resolveRestartArgv();
    yield* Effect.logDebug("routekit-eval update/start restart", {
      execPath: process.execPath,
    ...
    if (restartArgv.length === 0) {
      yield* options.log(
        "[routekit-eval-update] No executable to re-exec; exiting for supervisor restart."
      );
      return yield* launcher.cleanExit();
    }
    const [command, ...args] = restartArgv;
    yield* options.log("[routekit-eval-update] Re-executing the updated RouteKitEval binary.");
