...
const reportTransportFailure = (
  ...
  cliIo
    ...
    .pipe(
      Effect.ignore,
      Effect.andThen(new RouteKitEvalCliExit({ exitCode: HEADLESS_FAILURE_EXIT }))
    );
  ...
  function* (input: {
    ...
    if (termination.outcome === "succeeded") {
      ...
      if (!sawText) {
        yield* cliIo
          .writeStderr(
            `${HEADLESS_WARNING_PREFIX}the turn completed without producing any output; see \`routekit-eval logs\` for the full stream${HEADLESS_NEWLINE}`
          )
          .pipe(Effect.ignore);
      }
      return;
    ...
    return yield* new RouteKitEvalCliExit({ exitCode: termination.exitCode });
  }
);
