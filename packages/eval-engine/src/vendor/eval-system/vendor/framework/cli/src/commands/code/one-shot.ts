...
// The structured one-shot for `routekit-eval code --output jsonl` (RFC 0004 code.md
...
const oneShotFailure = (outcome: OneShotOutcome): CliFailureError =>
  new CliFailureError({
    ...
    ...(outcome.sessionId === undefined
      ? {}
      : {
          hint: `Resume this session with \`routekit-eval code --session ${outcome.sessionId} -p "<follow-up>"\`.`,
        }),
  });
 ...
 * The process exits non-zero via `RouteKitEvalCliExit`, which the entry point honors
...
export const runCodeOneShotJsonlTurn = Effect.fn("CodeCommand.oneShotJsonl")(
  function* (input: {
    ...
    yield* cliIo
      ...
      .pipe(Effect.ignore);
    if (!ok) {
      return yield* new RouteKitEvalCliExit({ exitCode: HEADLESS_FAILURE_EXIT });
    }
  }
);
