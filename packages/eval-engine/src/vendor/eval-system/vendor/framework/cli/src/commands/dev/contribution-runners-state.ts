 ...
 * `routekit-eval/state` slot for the lifetime of the session scope, so contextless feature
 * code (`import { db } from "routekit-eval/state"`, a chat surface's `start()` before any
 ...
 * "routekit-eval/state"`/`Chat.stores` and `import { config } from "routekit-eval/config"`/
