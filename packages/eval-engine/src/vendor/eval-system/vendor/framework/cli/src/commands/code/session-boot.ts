...
/**
 * Boot the daemon for an `routekit-eval code` session, in the one shape both session
 * kinds need: prepare the features root, start the daemon with audit and boot
 * output suppressed, publish the descriptor so `routekit-eval tui`/`routekit-eval logs` can find
...
export const bootCodeSession = Effect.fn("CodeCommand.bootSession")(function* (
  ...
  const workspaceRoot = yield* publishDevDescriptor(featuresRoot, daemon, {
    ...
    // `routekit-eval code` anchors to the user's arbitrary launch cwd, not a feature
    // workspace — don't refresh (and litter) an `.routekit-eval/docs` cache there.
    skipDocsCache: true,
  });
 ...
 * `routekit-eval code -p "<prompt>"`. The features root comes from the boot rather than
