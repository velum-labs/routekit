...
import { spawnWorkflowInternals } from "../../src/spawn-workflow.ts";
...
describe("outer protocol parsing without substituting execution", () => {
  test("reads every measured field from a real ori code summary shape", () => {
    assert.deepEqual(
      parseSummary(
        [
          "assistant answer",
  ...
  test("copies the private tree without recreating external symlinks", async () => {
    const source = await mkdtemp(path.join(os.tmpdir(), "ori-eval-copy-src-"));
    const dest = await mkdtemp(path.join(os.tmpdir(), "ori-eval-copy-dst-"));
    const outside = await mkdtemp(path.join(os.tmpdir(), "ori-eval-copy-out-"));
    try {
