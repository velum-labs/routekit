...
import { DEFAULT_EVAL_INFERENCE_ORIGIN, EVAL_INFERENCE_ORIGIN_ENV } from "../../../src/vendor/eval-system/host-env.ts";

const packageRoot = path.resolve(import.meta.dirname, "../..");
const repoRoot = path.resolve(packageRoot, "../..");
const binary = path.join(packageRoot, "dist", "routekit-eval-engine.mjs");
...
const invoke = async (
  ...
  run([process.execPath, binary, ...args], options.cwd ?? repoRoot, {
    ...process.env,
    HOME: isolatedHome,
    ROUTEKIT_EVAL_RUNTIME_CACHE: isolatedRuntimeCache,
    ROUTEKIT_EVAL_TELEMETRY: "0",
    ...options.env,
  });
...
const makeRepository = async (): Promise<string> => {
  const root = await mkdtemp(path.join(os.tmpdir(), "routekit-eval-eval-product-case-"));
...
before(async () => {
  isolatedHome = await mkdtemp(path.join(os.tmpdir(), "routekit-eval-eval-product-home-"));
  isolatedRuntimeCache = await mkdtemp(path.join(os.tmpdir(), "routekit-eval-eval-product-runtime-"));
  repository = await mkdtemp(path.join(os.tmpdir(), "routekit-eval-eval-product-repo-"));
...
describe("production standalone eval product", () => {
  test("runs evals with PATH limited to the node directory and system bins", async () => {
    ...
    const fullRun = await invoke(
      ["--json", "eval", "--allow-no-key", "--no-history", "--path", evalFile],
      {
        cwd: scratch,
        env: {
          ROUTEKIT_EVAL_BEARER_TOKEN: undefined,
          PATH: strippedPath,
        },
      },
    );
  ...
  test("routes -v and --version to the version command", async () => {
    ...
    assert.match(String(dashed.stdout), /@routekit-eval\/eval-system\s+\S+/u);
    const short = await invoke(["--human", "-v"]);
    assert.equal(short.exitCode, 0);
    assert.match(String(short.stdout), /@routekit-eval\/eval-system\s+\S+/u);
    const json = await invoke(["--json", "--version"]);
    assert.equal(json.exitCode, 0);
    assertMatchObject(decodeEnvelope(json.stdout), {
      data: {
        name: "@velum-labs/routekit-eval-engine",
      },
    });
  });
  ...
  test("keeps spawn reachable through leading --json flags", async () => {
    const result = await invoke(["--json", "spawn", "manifest"], {
      env: { ROUTEKIT_EVAL_BEARER_TOKEN: undefined },
    });
    ...
    assertMatchObject(host, {
      inferenceOrigin: DEFAULT_EVAL_INFERENCE_ORIGIN,
      inferenceOriginEnv: EVAL_INFERENCE_ORIGIN_ENV,
      credential: "missing",
      credentialEnv: "ROUTEKIT_EVAL_BEARER_TOKEN",
    });
  });
  ...
  test("creates the production scratch SDK and loads it through real node --test", async () => {
    ...
    assert.ok(
      String(await readFile(path.join(scratch, "sdk", "routekit-eval", "eval.js"), "utf8")).includes(
        "setupAgent",
      ),
    );
  ...
  test("runs the real node --test/JUnit/result/report/history pipeline without fabricating a provider call", async () => {
    ...
    const history = await readFile(path.join(scratch, ".routekit-eval", "eval", "history.jsonl"), "utf8");
  ...
  test("writes structured scratch and eval-run artifacts for the outer workflow", async () => {
    const artifactRoot = await mkdtemp(path.join(os.tmpdir(), "routekit-eval-eval-product-artifacts-"));
    try {
      ...
      const result = await invoke(["--json", "eval", "--allow-no-key", "--path", evalFile], {
        cwd: scratch,
        env: { ROUTEKIT_EVAL_RUN_RECORD_FILE: runRecord },
      });
      assert.equal(result.exitCode, 0);
  ...
  test("uses the real credential gate and never starts a model run without auth", async () => {
    const auth = await invoke(["--json", "auth"], { env: { ROUTEKIT_EVAL_BEARER_TOKEN: undefined } });
    ...
    assert.notEqual(result.exitCode, 0);
    assert.ok(String(result.stdout).includes("not signed in to Gateway"));
  });
  ...
  test("owns the real outer run protocol and stops at the production auth boundary", async () => {
    ...
    const prepared = await invoke(
      ...
      { cwd: repository, env: { ROUTEKIT_EVAL_BEARER_TOKEN: undefined } },
    );
  ...
  test("normalizes resume requests and archives every prior artifact instead of deleting it", async () => {
    ...
    try {
      ...
      assert.ok((await readdir(archiveEnvelope.archived)).includes("routekit-eval"));
    } finally {
      await rm(repo, { recursive: true, force: true });
    }
  });
  ...
  test("reflects a host API origin and environment credential in spawn manifest", async () => {
    ...
    const missing = await invoke(["spawn", "manifest"], {
      env: { [EVAL_INFERENCE_ORIGIN_ENV]: gateway, ROUTEKIT_EVAL_BEARER_TOKEN: undefined },
    });
    assert.equal(missing.exitCode, SPAWN_EXIT.ok);
    assertMatchObject(decodeEnvelope(missing.stdout), {
      host: {
        inferenceOrigin: gateway,
        credential: "missing",
      },
    });
    const present = await invoke(["spawn", "manifest"], {
      env: { [EVAL_INFERENCE_ORIGIN_ENV]: `${gateway}/`, ROUTEKIT_EVAL_BEARER_TOKEN: "sk-or-v1-test" },
    });
  ...
  test("provides the real spawn skill and never bundles unrelated Slack or chat UI product code", async () => {
    ...
    assert.ok(String(skill.stdout).includes("routekit-eval-engine spawn prepare"));
    ...
    assert.equal(
      localInputs.every((input) => input.includes("standalone/eval-system/")),
      true,
    );
  });
});
...
describe("credential-gated live production qualification", () => {
  const enabled = process.env.ROUTEKIT_EVAL_LIVE === "1";
  ...
  liveTest(
    ...
    { timeout: 300_000 },
    async () => {
      const key = process.env.ROUTEKIT_EVAL_BEARER_TOKEN;
      const model = process.env.ROUTEKIT_EVAL_LIVE_MODEL;
      if (!key || !model)
        throw new Error("ROUTEKIT_EVAL_LIVE requires ROUTEKIT_EVAL_BEARER_TOKEN and ROUTEKIT_EVAL_LIVE_MODEL");

      const scratchResult = await invoke(["--json", "eval", "scratch"], {
        env: { ROUTEKIT_EVAL_BEARER_TOKEN: key },
      });
      ...
      await writeFile(
        evalFile,
        `import { setupAgent } from "routekit/eval";\nimport { test } from "node:test";\nconst agent = setupAgent({ model: ${JSON.stringify(model)} });\ntest("real model call", async () => {\n  const run = await agent.run("Reply with exactly ROUTEKIT_EVAL_LIVE_EVAL_OK");\n  run.toComplete();\n  run.toMention("ROUTEKIT_EVAL_LIVE_EVAL_OK");\n});\n`,
      );
      const result = await invoke(
        ["--json", "eval", "--no-history", "--report", "report.md", "--path", evalFile],
        { cwd: scratch, env: { ROUTEKIT_EVAL_BEARER_TOKEN: key } },
      );
      assert.equal(result.exitCode, 0);
      const envelope = decodeEnvelope(result.stdout) as {
