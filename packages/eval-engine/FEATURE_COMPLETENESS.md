...
| ------------------------- | ------------------------------------------------ | ------------------------------------------------------------------ | ---------------------------------------- |
| Independent source tree   | yes; complete copied standalone source tree | isolated copy installs, typechecks, verifies, builds, and runs     | publish as its own repository/package    |
| No Ori dependency         | yes; no Ori/workspace dependencies or imports    | metafile boundary test                                             | keep the gate in CI                      |
| Dist CLI                  | yes; `dist/ori-eval-system.mjs` (esbuild ESM, `packages: "external"`) | Linux x64 Node 22.22+ / 24                                     | Node SEA / signed binary                 |
...
| Candidate/judge          | extracted real OpenRouter path | cutoff/empty reconciliation        | paid candidate and judge rows |
...
- host contract: `HOST.md`, spawn `manifest.host`, exit-code constants, and
  `ORI_EVAL_API_BASE_URL` overlay for catalog, endpoints, Claude, and Pi;
...
240-line `spawn-ori-eval` protocol snapshot:
...
No OpenRouter credential is installed in this environment. These remain
unobserved:
...
```bash
ORI_EVAL_LIVE=1 \
ORI_EVAL_LIVE_MODEL=<openrouter-model-slug> \
...
env, JSON envelopes, exit codes, and an injectable `ORI_EVAL_API_BASE_URL`.
RouteKit itself is unchanged. A later host shells out to `ori-eval-system`; it
