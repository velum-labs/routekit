/**
 * OpenAI Responses codec. Pure request/response translation between the
 * Responses wire and the gateway's OpenAI Chat Completions core. Streaming
 * translation lives in `responses-stream.ts`; HTTP handling lives in
 * `responses.ts`.
 */
 ...
 */
export function isServerWebSearchTool(tool: ResponsesTool): boolean {
  return (
    typeof tool.type === "string" &&
...
function collectDiscovered(
  ...
  for (const entry of entries) {
    ...
    if (record.type === "namespace" && Array.isArray(record.tools)) {
      collectDiscovered(
        record.tools,
        typeof record.name === "string" ? record.name : namespace,
        out
      );
      continue;
    }
    ...
    for (const item of input) {
      ...
      if (item.type === "web_search_call") {
        ...
        const carrier: Record<string, unknown> = {
          ...
          content:
            query.length > 0
              ? `[searched the web for: ${JSON.stringify(query)}]`
              : "[searched the web]"
        };
        attachPendingReasoning(carrier);
        messages.push(carrier);
      ...
      if (typeof message.content !== "string" && !Array.isArray(message.content)) {
        if (pendingEncryptedReasoning.length > 0)
          rejectPendingReasoning(`item type ${JSON.stringify(item.type)}`);
        continue;
      }
  ...
  }
  {
    ...
    for (const tool of body.tools ?? []) {
      ...
      if (isClientTypedTool(tool)) {
        ...
        if (
          !tools.some(
            (t) => (t.function as { name?: string } | undefined)?.name === WEB_SEARCH_TOOL_NAME
          )
        ) {
          tools.push({
            type: "function",
