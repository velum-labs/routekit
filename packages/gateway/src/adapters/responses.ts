...
export function handleResponses(
  ...
  return Effect.gen(function* () {
    ...
    if (supportsNativeResponses && responsesPort.kind === "responses") {
      ...
      const response = yield* responsesPort.execute(prepared.body, signal, {
        ...backendOptions,
        modelCallId
      });
    ...
    const chatOrError = yield* Effect.try({
      try: () =>
        responsesToChat(translatedBody, upstreamModel, { serverTools, destinationWireShape }),
      catch: (cause) =>
        cause instanceof ResponsesTranslationError ? cause : toRouteKitFailure(cause)
    }).pipe(
      Effect.catch((error) => {
        if (!(error instanceof ResponsesTranslationError)) return Effect.fail(error);
    ...
    if (chatOrError instanceof Response) return chatOrError;
    const chat = chatOrError;
