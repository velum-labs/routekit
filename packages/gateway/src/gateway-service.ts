...
class GatewayRuntime extends Context.Service<GatewayRuntime, Gateway>()(
  "@velum-labs/routekit-gateway/GatewayRuntime"
) {}
...
/** Runtime-owned gateway listener for standalone published façades. */
function gatewayLive(options: GatewayOptions): Layer.Layer<GatewayRuntime, Error> {
  return Layer.effect(
    GatewayRuntime,
    Effect.acquireRelease(startGatewayEffect(options), (gateway) =>
...
export async function startGateway(options: GatewayOptions): Promise<Gateway> {
  const runtime = ManagedRuntime.make(gatewayLive(options));
  try {
    const gateway = await runtime.runPromise(GatewayRuntime);
    ...
    const close = gateway.close.pipe(Effect.ensuring(disposeRuntime));
