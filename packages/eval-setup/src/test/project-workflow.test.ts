...
test("approved plans remain runnable after revision bumps but reject evaluation changes", async () => {
  const root = await makeRepository();
  const plan = await createPlan(root, "full");
  ...
  const rerun = await Effect.runPromise(
    Effect.gen(function* () {
      ...
      const qualified = yield* workflow.finishRun(
        root,
        qualifiedReport(plan, first.runId, running!.state.projectId)
      );
      ...
      return { qualified, mismatch, second };
    }).pipe(Effect.provide(ProjectWorkflowTestLive))
  );

  assert.equal(rerun.qualified.state.stage, "qualified");
  assert.equal(rerun.qualified.state.revision, plan.projectRevision + 2);
  assert.equal(rerun.mismatch._tag, "Failure");
  if (rerun.mismatch._tag === "Failure") {
    assert.match(String(rerun.mismatch.cause), /execution plan is stale/u);
  }
