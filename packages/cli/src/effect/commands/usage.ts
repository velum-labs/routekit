...
import { CliLive, withCliClient } from "../../cli-client.js";
...
export const makeUsageCommand = (
  runtime: CliRuntime = processCliRuntime
): Command.Command.Any => {
  const usageEffect = (watch: string | undefined) =>
    Effect.gen(function* () {
      const ctx = contextForFlags(yield* routekitRoot, runtime);
      if (watch !== undefined && ctx.json) {
      ...
      if (watch !== undefined) {
        const interval = watchInterval(watch);
        const source = yield* openSubscriptionUsageSourceEffect().pipe(Effect.provide(CliLive));
        yield* watchEffect(
          ctx.presenter,
          interval,
      ...
      const result = yield* fetchSubscriptionUsageEffect()
        .pipe(Effect.provide(CliLive), Effect.result);
      if (result._tag === "Failure") {
        const error = result.failure;
        if (ctx.json || !(error instanceof CliError)) return yield* Effect.fail(error);
      ...
      if (ctx.json) ctx.emit(result.success);
      else for (const line of renderUsageLines(result.success)) ctx.presenter.line(line);
    });

  const redeem = Command.make(
    "redeem",
    {
      provider: Flag.choice("provider", ["codex"] as const).pipe(
        Flag.withDescription("subscription provider (only codex)")
      ),
      label: Flag.string("label").pipe(
        Flag.optional,
        Flag.map(Option.getOrUndefined),
        Flag.withDescription("enrolled account label")
      ),
      creditId: Flag.string("credit-id").pipe(
        Flag.optional,
        Flag.map(Option.getOrUndefined),
        Flag.withDescription("specific reset credit id to redeem")
      )
    },
    ({ creditId: explicitCreditId, label }) =>
      Effect.gen(function* () {
        const ctx = contextForFlags(yield* routekitRoot, runtime);
        if (!ctx.yes && (ctx.json || ctx.noInput)) {
          return yield* Effect.fail(
            new CliError({ message: "`usage redeem` requires --yes in non-interactive mode" })
          );
        }
        const snapshot = yield* fetchSubscriptionUsageEffect().pipe(Effect.provide(CliLive));
        const memberFromUsage = yield* cliTryPromise(() =>
          chooseCodexMember(snapshot, label, !ctx.yes && !ctx.json && !ctx.noInput)
        );
        const member = withResetSnapshot(
          memberFromUsage,
          (
            yield* withCliClient((client) =>
              client.call("accounts.resetCredits", {
                kind: "codex",
                label: memberFromUsage.label
              })
            )
          ).resetCredits
        );
        const creditId = yield* cliTryPromise(() =>
          chooseResetCreditId(member, explicitCreditId, ctx.yes || ctx.json || ctx.noInput)
        );
        const selected =
          creditId === undefined
            ? undefined
        ...
        if (!ctx.yes) {
          ...
          const accepted = yield* cliTryPromise(() =>
            confirm({
          ...
          if (!accepted) {
            return yield* Effect.fail(new CliError({ message: "redeem cancelled", exitCode: 1 }));
          }
        }
    ...
    Command.withDescription("redeem a banked Codex rate-limit reset for an enrolled account")
  );
  ...
  return Command.make(
    ...
    ({ watch }) => usageEffect(watch)
  ).pipe(
    Command.withDescription("show account rate limits, credits, and reset windows"),
    Command.withSubcommands([redeem])
  );
};
