...
export const makeCallsCommand = (runtime: CliRuntime = processCliRuntime): Command.Command.Any => {
  const inspect = Command.make("inspect", { callId: Argument.string("call-id") }, ({ callId }) =>
    Effect.gen(function* () {
      const ctx = contextForFlags(yield* routekitRoot, runtime);
      const call = yield* withCliClient((client) => client.call("calls.inspect", { callId })).pipe(
        Effect.catch((error) =>
      ...
      const lines = [
        ["call", call.callId],
        ["status", call.status],
        ...(call.stopReason === undefined ? [] : [["stop reason", call.stopReason]]),
  ...
  ).pipe(Command.withDescription("show routing, billing, retry, usage, and cost attribution"));
