...
- ce85644: Split the account application service into query, enroll, and mutation handler groups so enrollment transactions, inventory reads, and destructive updates cannot silently share one catch-all owner.
- 653530c: Extract daemon worker spawn and IPC into a HostWorkerSession so the host owns singleton ports and rolls while talking to workers through one typed session.
- 6e6d1f4: Bind token and telemetry control methods through application services so every daemon use case owns its handler group the same way lifecycle, doctor, and launcher already do.
