...
- 08bba7b: Declare the daemon control protocol as a single method table and bind remaining daemon use cases through owned application services so parameter contracts, authorization, mutation classification, idempotency, and product telemetry are no longer restated by the dispatcher or the product client.
...
- bc5e40f: Annotate daemon-only and CLI-plumbing control methods so a new protocol method cannot land without a CLI caller or an explicit surface.
