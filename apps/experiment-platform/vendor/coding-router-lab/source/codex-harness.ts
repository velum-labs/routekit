...
const createSnapshotOnlyRepository = async (
  ...
  if (fetched.exitCode !== 0) {
    ...
    await materializeSnapshotOnlyRepository(
      sourceRepository,
      snapshot,
      destination,
    ).catch((archiveError) => {
      const detail =
        archiveError instanceof Error
          ...
          : String(archiveError);
      throw new Error(
        `Could not fetch isolated snapshot ${snapshot}: ${fetched.stderr}\nArchive fallback failed: ${detail}`,
      );
    });
    return;
  }
  ...
  if (resolved.exitCode !== 0 || resolved.stdout.trim() !== snapshot) {
    throw new Error(
      `Isolated snapshot mismatch: expected ${snapshot}, got ${resolved.stdout.trim() || "unresolved"}`,
    );
  }
};

const pipeSnapshotTreeToObjectDatabase = (
  sourceRepository: string,
  snapshot: string,
  destination: string,
): Promise<void> =>
  new Promise((resolve, reject) => {
    ...
    let indexCode: number | null = null;
    let settled = false;
    const finish = (): void => {
      if (
        settled ||
      ...
      settled = true;
      if (packCode === 0 && indexCode === 0) resolve();
      else {
        reject(
          new Error(
            `git pack-objects exited ${packCode}: ${packError.slice(-4_000)}; git index-pack exited ${indexCode}: ${indexError.slice(-4_000)}`,
          ),
        );
      }
    };
    ...
    index.stdin.on("error", (error: NodeJS.ErrnoException) => {
...
export const materializeSnapshotOnlyRepository = async (
  sourceRepository: string,
  snapshot: string,
  destination: string,
): Promise<void> => {
  ...
  const gitDirectory = await run(
    "git",
    ["-C", sourceRepository, "rev-parse", "--absolute-git-dir"],
    {
      cwd: sourceRepository,
      env: { PATH: process.env.PATH },
      timeoutMs: 60_000,
  ...
  if (initialized.exitCode !== 0) {
    throw new Error(
      `Could not initialize archived snapshot: ${initialized.stderr}`,
    );
  }
  for (const [name, value] of [
    ...
    if (configured.exitCode !== 0) {
      throw new Error(
        `Could not configure archived snapshot ${name}: ${configured.stderr}`,
      );
    }
  }
  ...
  await pipeSnapshotTreeToObjectDatabase(
    sourceRepository,
    snapshot,
    destination,
  );
  const expectedTree = await run(
    "git",
    ["-C", sourceRepository, "rev-parse", `${snapshot}^{tree}`],
    {
      cwd: sourceRepository,
      env: { PATH: process.env.PATH },
      timeoutMs: 60_000,
  ...
  const commit = await run(
    "git",
    ["-C", sourceRepository, "cat-file", "commit", snapshot],
    {
      cwd: sourceRepository,
      env: { PATH: process.env.PATH },
      timeoutMs: 60_000,
  ...
  if (written.exitCode !== 0 || written.stdout.trim() !== snapshot) {
    throw new Error(
      `Archived snapshot commit mismatch: expected ${snapshot}, got ${written.stdout.trim()}`,
    );
  }
...
const run = (command: string, args: string[], options: { cwd: string; env: NodeJS.ProcessEnv; timeoutMs: number; stdin?: string }): Promise<CodexRunResult> => new Promise((resolve, reject) => {
    ...
    for (const line of stdout.split(/\r?\n/u)) {
      try {
        ...
        if (
          ...
          const itemType = (record.item as Record<string, unknown>).type;
          if (
        ...
        if (record.type === "response_item" && record.payload && typeof record.payload === "object" && (record.payload as Record<string, unknown>).type === "function_call") toolCalls += 1;
        if (record.type === "turn.completed" && record.usage && typeof record.usage === "object") {
          const turnUsage = record.usage as Record<string, unknown>;
...
export const runCodexReadOnly = async (input: { repository: string; snapshot: string; model: string; prompt: string; outputSchema: unknown; timeoutMs?: number }): Promise<CodexRunResult> => {
  ...
  const temporary = path.join(
    temporaryRoot,
    `coding-router-codex-${process.pid}-${Date.now()}`,
  );
  ...
  await createSnapshotOnlyRepository(
    input.repository,
    input.snapshot,
    workspace,
  ).catch(async (error) => {
  ...
  try {
    ...
    try { result.finalMessage = await readFile(outputFile, "utf8"); } catch {}
    return result;
  } finally {
    await rm(temporary, { recursive: true, force: true });
  }
