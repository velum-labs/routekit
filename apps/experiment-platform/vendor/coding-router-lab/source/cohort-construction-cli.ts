...
import {
  auditCohortSnapshotResolution,
  buildConversationalCodingCohort,
  buildDiversePublicCohort,
  buildNaturalHardCohort,
} from "./cohort-construction.ts";
...
if (command === "conversational") {
  await buildConversationalCodingCohort({
    labRoot,
    outputDirectory:
      valueAfter(args, "--output-directory") ??
  ...
  await buildNaturalHardCohort({
    publicCohortDirectory:
      valueAfter(args, "--public-cohort-directory") ??
      path.join(privateRoot, "public-issue-diverse-v1"),
    outputDirectory:
      valueAfter(args, "--output-directory") ??
  ...
  const repositoryPaths = {
    "kubernetes/kubernetes":
      valueAfter(args, "--kubernetes-repository") ??
      "/home/benjamin/repos/kubernetes-public-benchmark",
    "grafana/grafana":
      valueAfter(args, "--grafana-repository") ??
  ...
  const conversational = path.join(
    privateRoot,
    "real-conversational-coding-v1",
  );
  ...
  throw new Error(
    "Usage: cohort-construction-cli <conversational|public-diverse|hard|snapshot-audit|all> [--lab-root DIR] [--output-directory DIR]",
  );
}
