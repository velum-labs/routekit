export const l2Normalize = (values: readonly number[]): number[] => {
  const magnitude = Math.sqrt(values.reduce((sum, value) => sum + value * value, 0));
  if (magnitude === 0) throw new Error("Cannot normalize a zero vector");
  return values.map((value) => value / magnitude);
};

export const cosine = (left: readonly number[], right: readonly number[]): number => {
  if (left.length !== right.length || left.length === 0) throw new Error("Vector dimensions differ");
...
export const centroid = (vectors: readonly (readonly number[])[]): number[] => {
  ...
  if (!vectors.every((vector) => vector.length === dimensions)) throw new Error("Vector dimensions differ");
