...
export const PUBLIC_PR_BENCHMARK_VERSION =
  "public-pr-benchmark-backstage-v1" as const;
export const PUBLIC_PR_ISSUE_GROUNDED_VERSION =
  "public-pr-benchmark-backstage-issue-grounded-v2" as const;

export interface PublicPrAreaDefinition {
  areaId: string;
  name: string;
  githubLabels: string[];
  description: string;
...
export const BACKSTAGE_PRIMARY_AREAS: readonly PublicPrAreaDefinition[] = [
  {
    areaId: "catalog",
    name: "Software catalog and entity ingestion",
    githubLabels: ["area:catalog"],
    description:
      "Catalog entities, locations, providers, processors, relations, entity pages, ingestion, querying, and catalog backend behavior.",
    inclusions: [
  ...
  },
  {
    ...
    description:
      "Sign-in, identity resolution, OAuth providers, sessions, tokens, authentication services, and authenticated user identity.",
    inclusions: [
  ...
  },
  {
    ...
    description:
      "Permission policies, rules, conditional decisions, authorization metadata, permission clients, and access-control enforcement.",
    inclusions: [
  ...
  },
  {
    ...
    description:
      "Software templates, scaffolder actions, task execution, template forms, field extensions, and generated-project workflows.",
    inclusions: [
  ...
  },
  {
    ...
    description:
      "TechDocs generation, storage, publishing, rendering, navigation, readers, builders, and documentation-site integration.",
    inclusions: [
  ...
  },
  {
    ...
    description:
      "Search indexing, collators, query APIs, search result presentation, search engines, and cross-plugin discoverability.",
    inclusions: [
  ...
  },
  {
    ...
    description:
      "Backstage's Kubernetes plugin, cluster discovery, object fetching, proxying, frontend resources, and Kubernetes-specific configuration.",
    inclusions: [
  ...
  },
  {
    ...
    description:
      "Event routing, brokers, topics, event ingestion, event subscribers, and integrations that publish or consume Backstage events.",
    inclusions: [
...
export const BACKSTAGE_NATURAL_UNKNOWN_LABELS =
  BACKSTAGE_NATURAL_UNKNOWN_GROUPS.flatMap((group) => group.labels);
...
export interface GithubPullRequestCandidate {
  ...
  samplingKind: "known" | "natural_unknown";
}

export interface GithubPullRequestMaterialized
  extends GithubPullRequestCandidate {
  preTaskSnapshot: string;
  actualChangedPaths: string[];
  taskTextSource:
    | "linked_issue"
    | "title_only"
    | "title_and_problem_statement";
  taskText: string;
  linkedIssue?: {
    number: number;
    url: string;
...
const stopHeading =
  /^(?:#{1,6}\s*)?(?:what changed|changes?|implementation|solution|the fix|fix|testing|tests?|verification|checklist|screenshots?|before|after|surface area|files changed|release notes?|risk assessment)\s*:?\s*$/iu;

const boilerplateLine =
  /^(?:##\s+Hey,? .*(?:Pull Request|MR)!?|<!--.*|[-*]\s*\[[ xX]\]\s+|####?\s*:heavy_check_mark:\s*Checklist)/u;

export const extractPublicPrProblemStatement = (
  body: string,
  ...
  const headingPattern =
    /^(#{1,6})\s+(problem|why|context|motivation|background|existing behavio(?:u)?r|expected behavio(?:u)?r)\s*:?\s*$/gimu;
  ...
  for (const rawLine of source.split("\n")) {
    ...
    if (stopHeading.test(line.trim())) break;
    if (boilerplateLine.test(line.trim())) continue;
    if (/^[-*]\s*\[[ xX]\]/u.test(line.trim())) continue;
    kept.push(line);
  }
  const compact = kept
    .join("\n")
...
export const buildPublicPrTaskText = (
  title: string,
  body: string,
): {
  taskText: string;
  source: GithubPullRequestMaterialized["taskTextSource"];
} => {
...
const normalizeIssueHeading = (heading: string): string =>
  heading
...
export const extractPublicIssueProblemStatement = (
  body: string,
  maximumCharacters = 3_000,
): string => {
  const cleaned = body
  ...
  if (preamble) included.push(preamble);
  const includeHeading =
    /^(?:description(?: (?:and )?context)?|context|provide the context for the bug|need|summary|what happened|what is wrong|what s wrong|what is the chore|what s the chore|what would you like to be added|why is this needed|expected behavior|what did you expect to happen|actual behavior(?: with screenshots)?|reproduction steps?|how can we reproduce it(?: as minimally and precisely as possible)?|use case|impact|problem|problem statement|background|motivation)$/u;
  const excludeHeading =
    /^(?:issue labels|search terms|project area|external integration|proposal|proposed solution|solution|solution options|root cause|implementation|details|alternatives|anything else we need to know|environment|your environment|kubernetes version|grafana version|browser|os version|cloud provider|install tools|container runtime(?: cri)? and version(?: if applicable)?|related plugins(?: cni csi)? and versions(?: if applicable)?|reproduction repo|have you read the code of conduct|are you willing to submit pr|checklist)$/u;
  for (const [index, match] of headingMatches.entries()) {
    const heading = normalizeIssueHeading(match[2] ?? "");
    const start = (match.index ?? 0) + match[0].length;
    const end =
      headingMatches[index + 1]?.index ?? cleaned.length;
    const section = cleaned.slice(start, end).trim();
    if (!section || excludeHeading.test(heading)) continue;
...
export const buildPublicIssueTaskText = (
  title: string,
  body: string,
): string => {
  const cleanedTitle = redactText(title).text.trim();
  const problem = redactText(extractPublicIssueProblemStatement(body)).text.trim();
  if (!problem || problem === cleanedTitle) return cleanedTitle;
  return `${cleanedTitle}\n\nProblem context:\n${problem}`;
};

export const extractBackstageIssueNumbersFromPullBody = (
  body: string,
): number[] => {
  ...
  for (const match of body.matchAll(
    /https?:\/\/github\.com\/backstage\/backstage\/issues\/(\d+)/giu,
  )) {
    add(match[1]!);
  }
  for (const match of body.matchAll(
...
const excludedCandidate = (candidate: GithubPullRequestCandidate): boolean => {
  const login = candidate.authorLogin.toLowerCase();
  if (login.includes("bot") || login.includes("dependabot")) return true;
  if (candidate.labels.includes("dependencies")) return true;
  if (
    candidate.samplingKind === "natural_unknown" &&
    candidate.labels.some((label) =>
      BACKSTAGE_PRIMARY_AREAS.some((area) =>
        area.githubLabels.includes(
          label as (typeof area.githubLabels)[number],
        ),
      ),
    )
  ) {
  ...
  if (
    /(?:^|\b)(?:version packages|build\(deps\)|deps:|dependency update|release(?:\s|:)|bump(?:\s|:))/iu.test(
      candidate.title.trim(),
    ) ||
    /(?:^|\n)See \[docs\/releases\/.*changelog\.md\]/iu.test(candidate.body) ||
    /^(?:patch release|updated?\s+`?@?[^ ]+`?\s+to\s+`?v?\d|(?:upgrade|update)\b.*\bto\s+(?:latest|`?v?\d)|.*(?:duplicate|unused)\s+(?:dependency|dependencies)\b)/iu.test(
      candidate.title.trim(),
    ) ||
...
const qualityScore = (candidate: GithubPullRequestCandidate): number => {
  const bodyLength = candidate.body.trim().length;
  let score = 0;
  if (bodyLength >= 200 && bodyLength <= 8_000) score += 5;
  else if (bodyLength > 0) score += 2;
  if (/^(?:feat|fix|refactor|perf|test)(?:\([^)]+\))?!?:/iu.test(candidate.title)) {
    score += 2;
  }
  if (candidate.changedFiles <= 12) score += 2;
  if (candidate.changedFiles <= 5) score += 1;
  if (candidate.labels.filter((label) => label.startsWith("area:")).length > 1) {
    score += 1;
  }
  if (candidate.samplingKind === "known") {
    const primaryLabels = new Set(
      BACKSTAGE_PRIMARY_AREAS.flatMap((area) => area.githubLabels),
    );
    const knownAreaLabels = candidate.labels.filter((label) =>
      primaryLabels.has(label),
    );
    const unknownAreaLabels = candidate.labels.filter((label) =>
      (BACKSTAGE_NATURAL_UNKNOWN_LABELS as readonly string[]).includes(label),
    );
...
export const selectBalancedPublicPrCandidates = (input: {
  ...
  perKnownArea: number;
  naturalUnknowns: number;
}): GithubPullRequestCandidate[] => {
  const selected = new Map<number, GithubPullRequestCandidate>();
  ...
  for (const area of BACKSTAGE_PRIMARY_AREAS) {
    ...
    const spread = [...candidates].sort(
      (left, right) =>
        left.mergedAt.localeCompare(right.mergedAt) ||
        left.number - right.number,
    );
    const available = spread.filter(
      (candidate) => !selected.has(candidate.number),
    );
    if (available.length < input.perKnownArea) {
      throw new Error(
        `Not enough distinct eligible public PRs for ${area.areaId}: ${available.length}`,
      );
    }
    for (let index = 0; index < input.perKnownArea; index += 1) {
      const start = Math.floor(
        (index * available.length) / input.perKnownArea,
      );
      const end = Math.max(
        start + 1,
        Math.floor(((index + 1) * available.length) / input.perKnownArea),
      );
  ...
  const unknownGroups = new Map<string, GithubPullRequestCandidate[]>();
  ...
  while (
    ...
    if (!selected.has(candidate.number)) selected.set(candidate.number, candidate);
    cursor += 1;
  }
  const expected =
    BACKSTAGE_PRIMARY_AREAS.length * input.perKnownArea +
  ...
  if (selected.size < expected) {
    throw new Error(
      `Could select only ${selected.size} unique public PRs; expected ${expected}`,
    );
  }
...
export const assignPublicPrSplits = (
  candidates: readonly GithubPullRequestCandidate[],
): Map<number, Split> => {
  const groups = new Map<string, GithubPullRequestCandidate[]>();
  ...
  const result = new Map<number, Split>();
...
const ghJson = async <T>(args: readonly string[]): Promise<T> => {
  ...
  for (let attempt = 0; attempt < 4; attempt += 1) {
    try {
      ...
      if (
        attempt >= 3 ||
        !/(?:HTTP 5\d\d|502 Bad Gateway|timeout|connection reset|stream error|received from peer|\bCANCEL\b)/iu.test(
          message,
        )
      ) {
        throw error;
...
const collectLabelCandidates = async (input: {
  ...
  label: string;
  samplingAreaId: string;
  samplingKind: GithubPullRequestCandidate["samplingKind"];
  limit: number;
  linkedIssueOnly?: boolean;
}): Promise<GithubPullRequestCandidate[]> => {
  const pulls = await ghJson<GhListPull[]>([
    "pr",
    "list",
    "-R",
    input.repository,
        ...
        timelineItems: {
          nodes: Array<
            | {
                ...
                subject: LinkedIssue & { __typename: "Issue" };
              }
            | {
                __typename: "CrossReferencedEvent";
                source:
                  | (LinkedIssue & { __typename: "Issue" })
                  | { __typename: string };
              }
          >;
        };
...
interface RestIssueResponse extends LinkedIssue {
  html_url: string;
  pull_request?: unknown;
}

interface ResolvedLinkedIssue extends LinkedIssue {
  resolutionMethod: NonNullable<
    GithubPullRequestMaterialized["linkedIssue"]
  >["resolutionMethod"];
  candidateCount: number;
}

const issueFromRest = async (
  repository: string,
  issueNumber: number,
): Promise<LinkedIssue | undefined> => {
  try {
    const issue = await ghJson<RestIssueResponse>([
      "api",
    ...
    const message = error instanceof Error ? error.message : String(error);
    if (/(?:HTTP 404|Not Found)/iu.test(message)) return undefined;
    throw error;
  }
};

const linkedIssueForPull = async (
  repository: string,
  candidate: GithubPullRequestCandidate,
): Promise<ResolvedLinkedIssue | undefined> => {
  const [owner, name] = repository.split("/");
  if (!owner || !name) throw new Error(`Invalid GitHub repository ${repository}`);
  const result = await ghJson<ClosingIssueResponse>([
    "api",
    "graphql",
    "-f",
  ...
  const ranked = new Map<
    ...
    {
      issue: LinkedIssue;
      method: ResolvedLinkedIssue["resolutionMethod"];
      rank: number;
    }
  >();
  const offer = (
    ...
    if (!issue || issue.number === candidate.number) return;
  ...
  for (const event of pull.timelineItems.nodes) {
    if (event.__typename === "ConnectedEvent") {
      ...
      offer(event.source as LinkedIssue & { __typename: "Issue" }, "cross_reference", 3);
    }
  }
  for (const issueNumber of extractBackstageIssueNumbersFromPullBody(
    candidate.body,
  )) {
    if (ranked.has(issueNumber)) continue;
    offer(
      await issueFromRest(repository, issueNumber),
      "explicit_body_reference",
      2,
    );
  }
  ...
  return {
    ...
    candidateCount: candidates.filter(
      (candidate) => candidate.rank === selected.rank,
    ).length,
  };
};
...
const materializeCandidate = async (
  ...
  candidate: GithubPullRequestCandidate,
  split: Split,
): Promise<GithubPullRequestMaterialized> => {
  const commit = await ghJson<GitCommitResponse>([
    "api",
  ...
  const files = await ghJson<PullFileResponse[]>([
    "api",
    "--paginate",
...
export interface PublicPrTaskQualityFlag {
  code:
    ...
    | "path-or-symbol-clue"
    | "ambiguous-linked-issues"
    | "backport-or-cherry-pick";
  severity: "critical" | "warning" | "note";
  detail: string;
}

export interface PublicPrTaskQualityAudit {
  taskEpisodeId: string;
  pullRequestNumber: number;
  split: Split;
  samplingAreaId: string;
  taskTextSource: GithubPullRequestMaterialized["taskTextSource"];
  taskCharacters: number;
  linkedIssueNumber?: number;
  flags: PublicPrTaskQualityFlag[];
  automaticDisposition: "accept" | "manual-review" | "reject";
}

export interface PublicPrToolOpportunityAudit {
  taskEpisodeId: string;
  split: Split;
  samplingAreaId: string;
  directRegisteredAreaCues: string[];
  hasRepositoryLocatorCue: boolean;
  opportunity: "high" | "medium" | "low";
  rationale: string;
}

export const auditPublicPrToolOpportunity = (
  candidate: GithubPullRequestMaterialized,
): PublicPrToolOpportunityAudit => {
  ...
  const directRegisteredAreaCues = BACKSTAGE_PRIMARY_AREAS.filter((area) => {
    ...
    return terms.some((term) => {
      ...
      return new RegExp(`(?:^|[^\\\\p{L}\\\\p{N}])${normalized}(?:$|[^\\\\p{L}\\\\p{N}])`, "iu").test(
        task,
      );
    });
  }).map((area) => area.areaId);
  const hasRepositoryLocatorCue =
    /https?:\/\/github\.com\/backstage\/backstage\/blob\//iu.test(task) ||
    /(?:^|[\s`(])(?:packages|plugins|docs|beps|app|src|test)\/[\w./*-]+/iu.test(
      task,
    ) ||
    /@backstage\/plugin-[\w-]+/iu.test(task);
  const opportunity =
    directRegisteredAreaCues.length > 0
      ? "low"
...
export const auditPublicPrTaskQuality = (
  candidate: GithubPullRequestMaterialized,
): PublicPrTaskQualityAudit => {
  const flags: PublicPrTaskQualityFlag[] = [];
  ...
  if (
    ...
    /\b(?:implemented|this change (?:adds|updates|removes)|we changed|the fix is to)\b/iu.test(
      task,
    )
  ) {
    flags.push({
      ...
      severity: "warning",
      detail:
        "Task text may contain a proposed or completed implementation rather than only the problem.",
    });
  }
  if (
    /(?:^|[\s`(])(?:packages|plugins|docs|beps|app|src|test)\/[\w./*-]+/iu.test(
    ...
    /\b[A-Z][A-Za-z0-9]+(?:Service|Api|Provider|Router|Page|Client|Builder)\b/u.test(
      task,
    )
  ) {
    flags.push({
      ...
      severity: "note",
      detail:
        "Task contains a path or symbol-like clue; audit whether it is natural issue context or answer leakage.",
    });
  }
...
const deduplicateIssueLineages = (
  candidates: readonly GithubPullRequestMaterialized[],
): GithubPullRequestMaterialized[] => {
  const selected = new Map<string, GithubPullRequestMaterialized>();
  for (const candidate of candidates) {
...
const renderTaskQualityAudit = (
  audits: readonly PublicPrTaskQualityAudit[],
): string => {
  const evaluated = audits.filter(
    (audit) => audit.split === "validation" || audit.split === "test",
  );
...
const renderToolOpportunityAudit = (
  audits: readonly PublicPrToolOpportunityAudit[],
): string => {
  const evaluated = audits.filter(
    (audit) => audit.split === "validation" || audit.split === "test",
  );
  return [
    ...
    ...evaluated.map(
      (audit) =>
        `| ${audit.taskEpisodeId} | ${audit.split} | ${audit.samplingAreaId} | ${audit.opportunity} | ${audit.directRegisteredAreaCues.join(", ") || "none"} | ${audit.hasRepositoryLocatorCue ? "yes" : "no"} |`,
    ),
    "",
  ].join("\n");
};

const areaCards = (): AreaCardV1[] =>
  BACKSTAGE_PRIMARY_AREAS.map((area) => {
    return {
      schemaVersion: 1,
...
export const collectBackstagePublicPrBenchmark = async (input: {
  ...
  perKnownArea?: number;
  naturalUnknowns?: number;
  candidateLimitPerLabel?: number;
  issueGroundedOnly?: boolean;
}): Promise<void> => {
  ...
  const pools: GithubPullRequestCandidate[] = [];
  for (const area of BACKSTAGE_PRIMARY_AREAS) {
    for (const label of area.githubLabels) {
      pools.push(
        ...(await collectLabelCandidates({
          repository,
          label,
  ...
  const initiallyMaterialized: GithubPullRequestMaterialized[] = [];
  for (const candidate of initialCandidates) {
    initiallyMaterialized.push(
  ...
  let materialized: GithubPullRequestMaterialized[];
  if (issueGroundedOnly) {
    const resolved = deduplicateIssueLineages(
      initiallyMaterialized.filter((candidate) => candidate.linkedIssue),
    );
    ...
    const finalNumbers = new Set(
      finalCandidates.map((candidate) => candidate.number),
    );
  ...
  const profile: RepositoryProfileV1 = {
    ...
    name: "Backstage",
    purpose:
      "An open-source framework for building developer portals, with a plugin architecture spanning catalog, identity, authorization, templates, documentation, search, Kubernetes, events, and notifications.",
  ...
  const offlineEvidence = materialized.map((candidate) => ({
    ...
    githubAreaLabels: candidate.labels.filter((label) =>
    ...
    ...(candidate.linkedIssue
      ...
      : {}),
    policy:
      "Offline sampling and audit evidence only. Never provide GitHub labels, changed paths, merge commits, or post-task diffs to Sol or Luna.",
  }));
  ...
  await writeJson("task-quality-audit.json", {
    ...
    policy:
      "Deterministic triage only. Every validation/test warning requires manual review before paid labeling.",
    counts: {
      total: taskQualityAudits.length,
      accept: taskQualityAudits.filter(
        (audit) => audit.automaticDisposition === "accept",
      ).length,
      manualReview: taskQualityAudits.filter(
        (audit) => audit.automaticDisposition === "manual-review",
      ).length,
      reject: taskQualityAudits.filter(
        (audit) => audit.automaticDisposition === "reject",
      ).length,
    },
    audits: taskQualityAudits,
  });
  await writeFile(
    path.join(outputDirectory, "task-quality-audit.md"),
    renderTaskQualityAudit(taskQualityAudits),
    { mode: 0o600 },
  );
  const toolOpportunityAudits = materialized.map(
    auditPublicPrToolOpportunity,
  );
  await writeJson("tool-opportunity-audit.json", {
    schemaVersion: 1,
    generatedAt: new Date().toISOString(),
    policy:
      "Primary metrics use the full frozen set. Repository-tool treatment effects are additionally reported on the predeclared high-opportunity validation subset.",
    counts: {
      total: toolOpportunityAudits.length,
      high: toolOpportunityAudits.filter(
        (audit) => audit.opportunity === "high",
      ).length,
      medium: toolOpportunityAudits.filter(
        (audit) => audit.opportunity === "medium",
      ).length,
      low: toolOpportunityAudits.filter(
        (audit) => audit.opportunity === "low",
      ).length,
    },
    audits: toolOpportunityAudits,
  });
  await writeFile(
    path.join(outputDirectory, "tool-opportunity-audit.md"),
    renderToolOpportunityAudit(toolOpportunityAudits),
    { mode: 0o600 },
  );
  ...
  await writeJson("manifest.json", {
    ...
    counts: {
      ...
      taskTextSources: {
        linkedIssue: materialized.filter(
          (candidate) => candidate.taskTextSource === "linked_issue",
        ).length,
        titleOnly: materialized.filter(
          (candidate) => candidate.taskTextSource === "title_only",
        ).length,
        titleAndProblemStatement: materialized.filter(
          (candidate) =>
            candidate.taskTextSource === "title_and_problem_statement",
        ).length,
      },
      linkedIssueResolutionMethods: Object.fromEntries(
        (
          ...
          materialized.filter(
            (candidate) =>
              candidate.linkedIssue?.resolutionMethod === method,
          ).length,
        ]),
      ),
    },
