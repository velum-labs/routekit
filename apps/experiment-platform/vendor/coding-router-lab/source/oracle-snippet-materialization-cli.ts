...
import { extractLunaRepositoryRetrievalTerms } from "./luna-accuracy-repository-retrieval.ts";
...
const maximumSnippetCharacters = Number(
  value("--maximum-snippet-characters") ?? "2200",
);
if (
...
const sourceEpisodes = await readJsonl<TaskEpisode>(
  path.resolve(required("--episodes")),
);
const baseEpisodesFile = value("--base-episodes");
const baseEpisodes = baseEpisodesFile
  ? await readJsonl<TaskEpisode>(path.resolve(baseEpisodesFile))
  : sourceEpisodes;
const labels = await readJsonl<SilverLabelV1>(
  path.resolve(required("--labels")),
);
...
const readSnapshotPath = async (
  ...
  const result = await execFileAsync(
    "git",
    ["-C", repository, "show", `${snapshot}:${repositoryPath}`],
    { encoding: "utf8", maxBuffer: 2 * 1024 * 1024 },
  );
  return redactText(result.stdout.replaceAll("\r\n", "\n")).text;
};

const bestLine = (content: string, queryTerms: readonly string[]): number => {
...
const excerpt = (
  ...
  while (true) {
    ...
    const candidate = takeBefore
      ? `${before}\n${text}`
      : `${text}\n${after ?? ""}`;
    if (candidate.length > maximumSnippetCharacters) break;
...
const materialized: TaskEpisode[] = [];
...
for (const source of sourceEpisodes) {
  ...
  if (!label || !base) throw new Error(`Missing data for ${source.id}`);
  ...
  const paths: (typeof provenance)[number]["paths"] = [];
  for (const repositoryPath of label.relevantPaths.slice(0, maximumPaths)) {
    const content = await readSnapshotPath(
  ...
  JSON.stringify(
    {
      ...
      output,
      manifestOutput,
      totalPaths: provenance.reduce(
        (sum, item) => sum + item.paths.length,
        0,
      ),
    },
    null,
