...
import type { Split, TaskEpisode, TaskEpisodeV2 } from "./types.ts";
...
export interface CurationResult {
  ...
  excluded: Array<{ episodeId: string; reason: string }>;
  summary: { input: number; included: number; excluded: number; splits: Record<Split, number>; lineageGroups: number };
}

const isV2 = (episode: TaskEpisode): episode is TaskEpisodeV2 => episode.schemaVersion === 2;
const likelyMeta = (text: string): boolean =>
  /\b(?:which experiment|experiment sequence|what do you need from me|how (?:do|can) I (?:give|send|share)|give you access|can you access|export (?:the|this|my) thread|attach (?:a|the) file|what did you build|what do you see|start openrouter oauth|github cli device login)\b/iu.test(text);
const normalizedTaskEnvelope = (episode: TaskEpisode): string =>
  [
    ...
    .trim()
    .replace(/\s+/gu, " ");
const minimumTimestamp = (episodes: TaskEpisode[]): number => Math.min(...episodes.map((episode) => Date.parse(episode.timestamp)));
const maximumTimestamp = (episodes: TaskEpisode[]): number => Math.max(...episodes.map((episode) => Date.parse(episode.timestamp)));
const isChronologicalBoundary = (
  ordered: TaskEpisode[][],
  index: number,
): boolean => {
...
const chronologicalBoundaries = (
  ordered: TaskEpisode[][],
  targetReference: number,
  targetValidationEnd: number,
): { referenceEnd: number; validationEnd: number } => {
  let best: { referenceEnd: number; validationEnd: number } | undefined;
  let bestDistance = Number.POSITIVE_INFINITY;
  for (let referenceEnd = 1; referenceEnd <= ordered.length - 2; referenceEnd += 1) {
    if (!isChronologicalBoundary(ordered, referenceEnd)) continue;
    for (let validationEnd = referenceEnd + 1; validationEnd <= ordered.length - 1; validationEnd += 1) {
      if (!isChronologicalBoundary(ordered, validationEnd)) continue;
  ...
  if (!best) throw new Error("Unable to assign strictly chronological split boundaries");
  return best;
};

export const curateEpisodes = (
  episodes: TaskEpisode[],
  decisions: CurationDecision[] = [],
  splitFractions: { reference: number; validation: number } = { reference: 0.7, validation: 0.15 },
): CurationResult => {
  ...
  const candidates = episodes.filter((episode) => {
    ...
    if (decision?.action === "exclude") { excluded.push({ episodeId: episode.id, reason: decision.reason }); return false; }
    if (decision?.action === "include") return true;
    if (isV2(episode) && episode.provenance.turnStatus !== "complete") { excluded.push({ episodeId: episode.id, reason: "turn_not_complete" }); return false; }
    if (likelyMeta(episode.currentRequest)) { excluded.push({ episodeId: episode.id, reason: "likely_experiment_or_access_meta" }); return false; }
    if (isReferentialRequest(episode.currentRequest) && !episode.taskAnchor && !episode.precedingAssistant && !(episode.earlierUserContext?.length)) {
      excluded.push({ episodeId: episode.id, reason: "referential_without_visible_context" }); return false;
    }
    return true;
  });
  const firstByExactEnvelope = new Map<string, TaskEpisode>();
  for (const episode of [...candidates].sort((left, right) =>
  ...
  const lineageGroups = new Map<string, TaskEpisode[]>();
  for (const episode of deduplicated) lineageGroups.set(episode.lineageHash, [...(lineageGroups.get(episode.lineageHash) ?? []), episode]);
  ...
  const curated: TaskEpisode[] = [];
  ordered.forEach((group, index) => {
    const split: Split = index < referenceGroups ? "reference" : index < validationEnd ? "validation" : "test";
  ...
  const splits: Record<Split, number> = { reference: 0, validation: 0, test: 0 };
  for (const episode of curated) splits[episode.split] += 1;
