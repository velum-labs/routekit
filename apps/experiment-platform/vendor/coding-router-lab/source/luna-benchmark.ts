...
export const selectStratifiedLunaCanary = (
  ...
  const hasContext = (label: SilverLabelV1): boolean => {
  ...
  const inSplit = (
    label: SilverLabelV1,
    split: TaskEpisode["split"],
  ): boolean => episodeById.get(label.taskEpisodeId)?.split === split;
  const orderedBuckets: Array<(label: SilverLabelV1) => boolean> = [
    (label) =>
      inSplit(label, "validation") &&
      label.known &&
  ...
  for (const bucket of orderedBuckets) {
    ...
    if (match) selected.add(match.taskEpisodeId);
    if (selected.size >= maximumCases) break;
  }
  ...
  return [...selected]
    ...
    .filter((episode): episode is TaskEpisode => Boolean(episode));
};

export interface LunaBenchmarkVariantSummary {
  ...
  confidencePolicy: Array<{
    ...
    routedCorrect: number;
    correctnessAmongRouted: number | null;
  }>;
  projected: {
    costPer1kTasksUsd: number | null;
    serialTasksPerMinute: number | null;
  };
}

export interface LunaBenchmarkReport {
  schemaVersion: 1;
  generatedAt: string;
  model: string;
  cases: number;
  labels: number;
  variants: LunaBenchmarkVariantSummary[];
  recommendations: {
    lowestCostVariant: string | null;
    lowestP95LatencyVariant: string | null;
    highestSafeScoreVariant: string | null;
  };
  limitations: string[];
}

export type LunaEnsembleMode = "majority" | "unanimity";

export const representativeLunaPredictions = (
  predictions: ClassifierPredictionV1[],
): ClassifierPredictionV1[] => {
  const seen = new Set<string>();
  const representative: ClassifierPredictionV1[] = [];
  for (const prediction of predictions) {
    if (seen.has(prediction.taskEpisodeId)) continue;
...
const lunaDecisionKey = (prediction: ClassifierPredictionV1): string =>
  JSON.stringify({
 ...
 */
export const aggregateLunaEnsemble = (
  predictions: ClassifierPredictionV1[],
  mode: LunaEnsembleMode,
): ClassifierPredictionV1[] => {
  const byCase = new Map<string, ClassifierPredictionV1[]>();
  for (const prediction of predictions) {
  ...
  return [...byCase.entries()].map(([taskEpisodeId, values]) => {
    if (values.length < 2) {
      throw new Error(
        `Luna ensemble requires repeated predictions for ${taskEpisodeId}`,
      );
    }
    ...
    const [winningKey, winningCount] = [...counts.entries()].sort(
    ...
    const winner = values.find(
      (prediction) => lunaDecisionKey(prediction) === winningKey,
    )!;
    const total = (key: keyof ClassifierPredictionV1): number | undefined => {
    ...
    const base: ClassifierPredictionV1 = accepted
      ? {
          ...winner,
          classifier: `ensemble:${mode}:${winner.classifier}`,
          confidence: Math.min(
            ...values.map((prediction) => prediction.confidence),
          ),
        }
      : {
          ...winner,
    ...
    return {
      ...
      taskEpisodeId,
      durationMs: Math.max(
        ...values.map((prediction) => prediction.durationMs),
      ),
...
const repeatability = (
  predictions: ClassifierPredictionV1[],
): LunaBenchmarkVariantSummary["repeatability"] => {
  const byCase = new Map<string, ClassifierPredictionV1[]>();
  for (const prediction of predictions) {
    const values = byCase.get(prediction.taskEpisodeId) ?? [];
    values.push(prediction);
...
const routingConfidencePolicy = (
  labels: SilverLabelV1[],
  predictions: ClassifierPredictionV1[],
): LunaBenchmarkVariantSummary["confidencePolicy"] => {
  ...
  return [0, 0.8, 0.9, 0.95, 0.97, 0.99].map(
    (minimumConfidence) => {
      let routedCases = 0;
      let falseKnown = 0;
      let falseUnknown = 0;
      ...
      for (const label of labels) {
        ...
        if (!prediction) {
          throw new Error(
            `Missing confidence-policy prediction for ${label.taskEpisodeId}`,
          );
        }
        const routed =
          prediction.known &&
        ...
        if (routed) {
          ...
          if (!label.known) {
            ...
            label.selectedAreaIds.every((area) =>
              prediction.selectedAreaIds.includes(area),
            )
          ) {
            routedCorrect += 1;
          }
...
const safeScore = (
  summary: LunaBenchmarkVariantSummary,
...
const lexicographicallyGreater = (
  left: number[],
  right: number[],
): boolean => left.some(
  (value, index) =>
...
export const buildLunaBenchmarkReport = (
  model: string,
  matrix: LunaBenchmarkMatrix,
  labels: SilverLabelV1[],
  predictionsByVariant: Record<string, ClassifierPredictionV1[]>,
): LunaBenchmarkReport => {
  ...
  const summaries = matrix.variants.map((variant) => {
    ...
    for (const prediction of predictions) {
      if (!expectedIds.has(prediction.taskEpisodeId)) {
        throw new Error(
          `Luna variant ${variant.id} contains a prediction without a label`,
        );
      }
      predictionCounts.set(
        prediction.taskEpisodeId,
        (predictionCounts.get(prediction.taskEpisodeId) ?? 0) + 1,
      );
    }
    for (const id of expectedIds) {
      ...
      if (count !== expectedRepetitions) {
        throw new Error(
          `Luna variant ${variant.id} has ${count} predictions for ${id}; expected ${expectedRepetitions}`,
        );
      }
    }
    if (predictions.length !== labels.length * expectedRepetitions) {
      throw new Error(
        `Luna variant ${variant.id} prediction count does not match the complete benchmark`,
      );
    }
    ...
    return {
      ...
      confidencePolicy: routingConfidencePolicy(
        labels,
        representative,
      ),
      projected: {
  ...
  const lowestCost = summaries
    ...
    .sort(
      (left, right) =>
        Number(left.metrics.costUsd.perCase) -
        Number(right.metrics.costUsd.perCase),
    )[0];
  const lowestLatency = summaries
    .filter((summary) => summary.metrics.latencyMs.p95 !== null)
    .sort(
      (left, right) =>
        Number(left.metrics.latencyMs.p95) -
        Number(right.metrics.latencyMs.p95),
    )[0];
  let safest: LunaBenchmarkVariantSummary | undefined;
...
export const buildLunaEnsembleReport = (
  labels: SilverLabelV1[],
  predictionsByVariant: Record<string, ClassifierPredictionV1[]>,
): Record<
  string,
  Record<
    ...
    {
      metrics: ReturnType<typeof calculateExtendedMetrics>;
      projectedCostPer1kTasksUsd: number | null;
    }
  >
> => Object.fromEntries(
  Object.entries(predictionsByVariant).map(([variant, predictions]) => [
    variant,
    Object.fromEntries(
      (["majority", "unanimity"] as const).map((mode) => {
        ...
        return [
          ...
          {
            metrics,
            projectedCostPer1kTasksUsd:
              metrics.costUsd.perCase === null
                ? null
                : metrics.costUsd.perCase * 1_000,
          },
        ];
