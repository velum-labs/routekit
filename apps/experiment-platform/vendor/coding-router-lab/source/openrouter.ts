...
interface StoredCredential { key?: string }
export const resolveOpenRouterKey = async (): Promise<string> => {
  const inherited = process.env.OPENROUTER_API_KEY?.trim();
  if (inherited) return inherited;
  const file = path.join(process.env.HOME ?? "", ".ori", "credentials.json");
  const stored = JSON.parse(await readFile(file, "utf8")) as StoredCredential;
  if (!stored.key?.trim()) throw new Error("No OpenRouter credential found");
  return stored.key;
};


interface KeyStatusResponse { data?: { usage?: number; limit?: number | null; limit_remaining?: number | null; label?: string; is_free_tier?: boolean } }
interface CreditsResponse { data?: { total_credits?: number; total_usage?: number } }
export const getOpenRouterKeyStatus = async (): Promise<{ usageUsd: number; limitUsd: number | null; limitRemainingUsd: number | null; accountUsageUsd: number | null; accountRemainingUsd: number | null; isFreeTier: boolean | null; label?: string }> => {
  const response = await fetch(`${API_BASE}/key`, { headers: { authorization: `Bearer ${await resolveOpenRouterKey()}` } });
  const text = await response.text();
  if (!response.ok) throw new Error(`OpenRouter key status HTTP ${response.status}: ${text.slice(0, 1000)}`);
  const payload = JSON.parse(text) as KeyStatusResponse; const data = payload.data ?? {};
  let accountUsageUsd: number | null = null, accountRemainingUsd: number | null = null;
  try {
    ...
    if (creditsResponse.ok) {
      ...
      if (Number.isFinite(totalUsage)) accountUsageUsd = totalUsage;
      if (Number.isFinite(totalCredits) && Number.isFinite(totalUsage)) accountRemainingUsd = totalCredits - totalUsage;
    }
  } catch {}
  return {
...
const requestJson = async <T>(url: string, body: unknown): Promise<T> => {
  ...
  const text = await response.text();
  if (!response.ok) throw new Error(`OpenRouter HTTP ${response.status}: ${text.slice(0, 1000)}`);
  return JSON.parse(text) as T;
};

export interface EmbeddingResponse { data: Array<{ embedding: number[]; index: number }>; usage?: { prompt_tokens?: number; total_tokens?: number } }
export const embedTexts = async (model: string, inputs: Array<{ id: string; text: string }>): Promise<{ vectors: DenseVector[]; usageTokens?: number }> => {
  const payload = await requestJson<EmbeddingResponse>(`${API_BASE}/embeddings`, { model, input: inputs.map((item) => item.text), encoding_format: "float" });
...
const directSystemPrompt =
  "You are the runtime task-area classifier for a personalized coding router. Select zero, one, or two known area IDs. Return unknown rather than forcing a weak match. Do not choose a coding model. Use only IDs supplied by the user.";
...
const outputInstruction = (mode: LunaOutputMode): string =>
  mode === "minimal"
...
export interface LunaClassificationInput {
...
export const classifyWithLuna = async (
  input: LunaClassificationInput,
): Promise<ClassifierPredictionV1> => {
  ...
  const response = await requestJson<ChatResponse>(`${API_BASE}/chat/completions`, {
    model: input.model,
    temperature: 0,
  ...
  if (!content) throw new Error("Luna returned no content");
  const parsed = JSON.parse(content) as ParsedLunaDecision;
...
export const classifyTaskKindWithLuna = async (input: {
  ...
  classifierLabel?: string;
  seed?: number;
}): Promise<LunaTaskKindPredictionV1> => {
  const messages = [
  ...
  const content = response.choices[0]?.message.content;
  if (!content) throw new Error("Luna task-kind gate returned no content");
  ...
  return {
    ...
    taskEpisodeId: input.taskEpisodeId,
    classifier:
      input.classifierLabel ?? `llm:${input.model}:task-kind-gate`,
