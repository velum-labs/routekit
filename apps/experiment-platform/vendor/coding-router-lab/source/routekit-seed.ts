...
import type { AreaCardV1, RepositoryProfileV1, Split, TaskEpisodeV1 } from "./types.ts";
...
const starts = (...prefixes: string[]) => (file: string): boolean => prefixes.some((prefix) => file === prefix || file.startsWith(`${prefix}/`));
const includesSegment = (...segments: string[]) => (file: string): boolean => segments.some((segment) => file.includes(segment));

export const ROUTEKIT_AREAS: AreaDefinition[] = [
  {
...
const git = async (repository: string, args: string[]): Promise<string> => (await execFileAsync("git", ["-C", repository, ...args], { maxBuffer: 10 * 1024 * 1024 })).stdout.trim();

const scoreAreas = (files: string[]): Array<{ areaId: string; score: number }> => ROUTEKIT_AREAS.map((area) => ({ areaId: area.areaId, score: files.filter(area.matches).length })).filter((item) => item.score > 0).sort((a, b) => b.score - a.score || a.areaId.localeCompare(b.areaId));

const assignedAreas = (files: string[]): string[] => {
  const scores = scoreAreas(files); const top = scores[0]; if (!top) return [];
  const selected = [top.areaId]; const second = scores[1];
  if (second && second.score >= Math.max(2, Math.ceil(top.score * 0.45))) selected.push(second.areaId);
  return selected;
};

export interface RouteKitSeedResult { profile: RepositoryProfileV1; cards: AreaCardV1[]; episodes: TaskEpisodeV1[]; areaAssignments: Map<string, string[]> }

export const buildRouteKitSeed = async (pullsFile: string, repository: string): Promise<RouteKitSeedResult> => {
  const excludedTitle = /^(?:chore:\s*version packages|build\(deps\):|bump (?:the )?npm-production group|release \d)/iu;
  const pulls = (JSON.parse(await readFile(pullsFile, "utf8")) as PullRequestRecord[])
  ...
  const episodes: TaskEpisodeV1[] = []; const areaAssignments = new Map<string, string[]>();
  for (const [index, pr] of pulls.entries()) {
    ...
    const split: Split = fraction < 0.65 ? "reference" : fraction < 0.82 ? "validation" : "test";
...
export const writeRouteKitSeed = async (result: RouteKitSeedResult, publicDir: string, privateDir: string): Promise<void> => {
  await mkdir(publicDir, { recursive: true }); await mkdir(privateDir, { recursive: true, mode: 0o700 });
