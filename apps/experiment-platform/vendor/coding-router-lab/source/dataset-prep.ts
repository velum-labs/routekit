...
import type { CanonicalTraceSource } from "./oracle-trace-audit.ts";
import type { LunaAccuracyCodingDecision } from "./luna-accuracy-coding-annotations.ts";
import type { AreaCardV1, SilverLabelV1, Split, TaskEpisode } from "./types.ts";
...
const likelyCodingPattern =
  /\b(?:implement|fix|refactor|test|build|lint|typecheck|bug|error|code|function|class|api|cli|daemon|gateway|router|provider|package|module|file|repo|commit|branch|typescript|javascript|python|rust|go|docker|ssh|deploy|config|docs?|readme|release|dependency|endpoint|database|schema|migration|stream|token|model)\b/iu;

export const selectEpisodeSplit = (
  episodes: TaskEpisode[],
  split: Split,
): TaskEpisode[] => episodes.filter((episode) => episode.split === split);

export interface ResolvedDevelopmentSubset {
  episodes: TaskEpisode[];
  ...
  report: {
    ...
    unresolvedEpisodeIds: string[];
    representedTaskStrata: Record<string, number>;
    warning: string;
  };
}

const oracleDecisionStratum = (label: SilverLabelV1): string =>
  label.known
 ...
 */
export const buildResolvedDevelopmentSubset = (
  episodes: TaskEpisode[],
  adjudicatedLabels: SilverLabelV1[],
): ResolvedDevelopmentSubset => {
  ...
  for (const episode of episodes) {
    ...
    if (episode.split !== "validation") {
      throw new Error(
        `Resolved development source must be validation data: ${episode.id}`,
      );
    }
  ...
  for (const label of adjudicatedLabels) {
    if (labelById.has(label.taskEpisodeId)) {
      throw new Error(
        `Duplicate adjudicated development label: ${label.taskEpisodeId}`,
      );
    }
    if (!episodeIds.has(label.taskEpisodeId)) {
      throw new Error(
        `Adjudicated label has no source episode: ${label.taskEpisodeId}`,
      );
    }
    if (!label.oracle.adjudicated) {
      throw new Error(
        `Development label is not adjudicated: ${label.taskEpisodeId}`,
      );
    }
    labelById.set(label.taskEpisodeId, label);
  }
  const resolvedEpisodes = episodes.filter((episode) =>
    labelById.has(episode.id)
  );
  const labels = resolvedEpisodes.map((episode) =>
    labelById.get(episode.id)!
  );
  const unresolvedEpisodeIds = episodes
    .filter((episode) => !labelById.has(episode.id))
    .map((episode) => episode.id)
    .sort();
  const representedTaskStrata: Record<string, number> = {};
  ...
  return {
    ...
    report: {
      ...
      warning:
        "This exact resolved join is for canary and development runs only. It omits unresolved source cases and must not be reported as the complete validation estimate.",
    },
  };
};

export interface LunaAccuracyCanarySelection {
  episodes: TaskEpisode[];
  labels: SilverLabelV1[];
  report: {
    schemaVersion: 1;
    ...
    noncodingCases: number;
    excludedAmbiguousCases: number;
    unknownCases: number;
    taskStrata: Record<string, number>;
    knownAreaCounts: Record<string, number>;
    contextualCases: number;
    warning: string;
  };
}

const episodeHasTaskContext = (episode: TaskEpisode): boolean =>
  Boolean(
    episode.taskAnchor ||
      episode.precedingAssistant ||
 ...
 */
export const selectLunaAccuracyDevelopmentCanary = (
  episodes: TaskEpisode[],
  labels: SilverLabelV1[],
  codingDecisions: ReadonlyMap<string, LunaAccuracyCodingDecision>,
  maximumCases = 10,
): LunaAccuracyCanarySelection => {
  ...
  if (source.report.unresolvedEpisodes > 0) {
    throw new Error(
      "Luna accuracy canary requires an exact resolved episode/label join",
    );
  }
  const episodeById = new Map(
    source.episodes.map((episode) => [episode.id, episode]),
  );
  const missingCodingDecisions = source.episodes
    .map((episode) => episode.id)
    .filter((id) => !codingDecisions.has(id));
  const unknownCodingDecisions = [...codingDecisions.keys()].filter(
    (id) => !episodeById.has(id),
  );
  if (missingCodingDecisions.length || unknownCodingDecisions.length) {
    throw new Error(
      `Luna accuracy canary coding annotations do not exactly cover its source: missing=${missingCodingDecisions.join(",")}; unknown=${unknownCodingDecisions.join(",")}`,
    );
  }
  ...
  const knownCoding = known.filter(
    (label) => codingDecisions.get(label.taskEpisodeId) === "coding",
  );
  const unknown = orderedLabels.filter((label) => !label.known);
  const selected = new Map<string, SilverLabelV1>();
  const add = (label: SilverLabelV1 | undefined): void => {
  ...
  for (const areaId of knownAreaIds) {
    add(
      knownCoding.find(
        (label) =>
          label.selectedAreaIds.includes(areaId) &&
  ...
  for (const label of [...knownCoding].sort((left, right) => {
    ...
    const rightEpisode = episodeById.get(right.taskEpisodeId)!;
    return (
      right.selectedAreaIds.length - left.selectedAreaIds.length ||
  ...
  const targetKnownDecisions = Math.min(
    known.length,
    Math.max(0, maximumCases - Math.min(3, unknown.length)),
    5,
  );
  const selectedKnownCount = (): number =>
    [...selected.values()].filter((label) => label.known).length;
  for (const label of [...knownCoding, ...known]) {
    if (selectedKnownCount() >= targetKnownDecisions) break;
    add(label);
  }
  ...
  for (const label of [...knownCoding, ...known, ...unknown]) add(label);

  const selectedLabels = [...selected.values()];
  const selectedEpisodes = selectedLabels.map((label) =>
    episodeById.get(label.taskEpisodeId)!
  );
  const taskStrata: Record<string, number> = {};
  const knownAreaCounts: Record<string, number> = {};
  ...
  const decisionFor = (
    label: SilverLabelV1,
  ): LunaAccuracyCodingDecision =>
    codingDecisions.get(label.taskEpisodeId)!;
  const codingCases = selectedLabels.filter(
    (label) => decisionFor(label) === "coding",
  ).length;
  const knownCodingCases = selectedLabels.filter(
    (label) => label.known && decisionFor(label) === "coding",
  ).length;
  const noncodingCases = selectedLabels.filter(
    (label) => decisionFor(label) === "noncoding",
  ).length;
  const excludedAmbiguousCases = selectedLabels.filter(
    (label) => decisionFor(label) === "excluded_ambiguous",
  ).length;
  return {
    ...
    report: {
      ...
      warning:
      "Use this non-selective subset only to validate prompts, parsing, persistence, and cost. Coding intent is reported from explicit annotations. Never rank product configurations from the canary.",
    },
  };
};

export const readUnlockedEpisodeSplits = async (
  file: string,
): Promise<{
  reference: TaskEpisode[];
  validation: TaskEpisode[];
  lockedTestCount: number;
}> => {
  const reference: TaskEpisode[] = [];
  const validation: TaskEpisode[] = [];
  ...
  for await (const line of lines) {
    ...
    const split = match[1] as Split;
    if (split === "test") {
      lockedTestCount += 1;
      continue;
    }
    const episode = JSON.parse(line) as TaskEpisode;
...
export interface ValidationEscalationPlan {
  schemaVersion: 1;
  policy: "one-pass-then-targeted-review-v1";
  primaryPassEpisodeIds: string[];
  mandatorySecondPassEpisodeIds: string[];
  postPassEscalationRules: string[];
  counts: {
    primaryPass: number;
    mandatorySecondPass: number;
    likelyCoding: number;
    contextual: number;
    diagnostic: number;
 ...
 */
export const buildValidationEscalationPlan = (
  episodes: TaskEpisode[],
): ValidationEscalationPlan => {
  const validation = selectEpisodeSplit(episodes, "validation");
  const contextual = (episode: TaskEpisode): boolean =>
    Boolean(
      episode.taskAnchor ||
        episode.precedingAssistant ||
        (episode.earlierUserContext?.length ?? 0) > 0,
    );
  const likelyMultiArea = (episode: TaskEpisode): boolean =>
    /\b(?:and|both|across|end-to-end|integrat|migration|release|deploy)\b/iu.test(
      episode.currentRequest,
    );
  ...
  return {
    ...
    counts: {
      ...
      likelyCoding: validation.filter((episode) =>
        likelyCodingPattern.test(taskIdentityText(episode)),
      ).length,
...
export const selectSecondPassEpisodes = (
  episodes: TaskEpisode[],
  firstPassLabels: SilverLabelV1[],
  plan: ValidationEscalationPlan,
): TaskEpisode[] => {
  const validation = selectEpisodeSplit(episodes, "validation");
  const labelById = new Map<string, SilverLabelV1>();
  ...
  return validation.filter((episode) => {
    ...
    const requiresRepositoryInspection =
      label.known || label.unknownType === "new_repository_area";
...
export const selectDisagreementEpisodes = (
  episodes: TaskEpisode[],
  firstPassLabels: SilverLabelV1[],
  secondPassLabels: SilverLabelV1[],
): TaskEpisode[] => {
  ...
  const firstById = new Map(
    firstPassLabels.map((label) => [label.taskEpisodeId, label]),
  );
  ...
  for (const second of secondPassLabels) {
    ...
    if (!first) {
      throw new Error(
        `Second-pass label has no first-pass label: ${second.taskEpisodeId}`,
      );
    }
  ...
  return [...disagreements]
    .map((id) => {
      ...
      if (!episode) throw new Error(`Disagreement has no episode: ${id}`);
      return episode;
    })
    .sort(
      (left, right) =>
...
export interface CodingChallengeLabelV1 {
  schemaVersion: 1;
...
export interface CodingChallengeSuite {
  episodes: TaskEpisode[];
  evidenceLabels: CodingChallengeLabelV1[];
  report: {
    schemaVersion: 1;
 ...
 */
export const buildCodingChallengeSuite = (
  episodes: TaskEpisode[],
  assignments: Array<{
    taskEpisodeId: string;
    selectedAreaIds: string[];
  ...
  const runtimeEpisodes: TaskEpisode[] = [];
  const evidenceLabels: CodingChallengeLabelV1[] = [];
  ...
  for (const episode of sourceEpisodes) {
    ...
    runtimeEpisodes.push(runtimeEpisode as TaskEpisode);
    evidenceLabels.push({
      schemaVersion: 1,
      ...
      actualChangedPaths,
      note:
        "Post-task changed paths are label evidence only and were removed from the runtime episode.",
    });
  }
  ...
  if (evidenceLabels.some((label) => !label.known)) {
    warnings.push(
      "Unassigned cases are retained but require Sol or human adjudication before accuracy scoring.",
    );
  }
  warnings.push(
    "PR titles are coding-heavy stress prompts, not production-like Codex conversations; report this suite separately.",
  );
  warnings.push(
    "Changed-path assignments are heuristic evidence, not independent Sol silver labels.",
  );
  return {
    ...
    report: {
      ...
      singleArea: evidenceLabels.filter(
        (label) => label.selectedAreaIds.length === 1,
      ).length,
      multiArea: evidenceLabels.filter(
...
export const selectLabelsByEpisodeIds = (
  labels: SilverLabelV1[],
...
interface RecoverableOracleTrace {
  pass: number;
  finalMessage: string;
  toolCalls: number;
}

export interface RecoveredOraclePasses {
  labels: SilverLabelV1[];
...
const parseRecoveredOracleDecision = (
  trace: RecoverableOracleTrace,
  taskEpisodeId: string,
  ...
  for (const areaId of selectedAreaIds) {
    if (!allowedAreaIds.has(areaId)) {
      throw new Error(
        `Oracle trace invented area ${areaId}: ${taskEpisodeId}`,
      );
    }
  }
  ...
  if (
    known
      ? unknownType !== null
      : !unknownTypes.includes(
          unknownType as (typeof unknownTypes)[number],
        )
  ) {
  ...
  if (
    ...
    !confidences.includes(
      value.confidence as (typeof confidences)[number],
    )
  ) {
  ...
  if (
    !Number.isInteger(trace.toolCalls) ||
    trace.toolCalls < 0 ||
  ...
  return {
    ...
    ...(!known
      ? {
          unknownType:
            unknownType as NonNullable<SilverLabelV1["unknownType"]>,
        }
      : {}),
    difficulty: value.difficulty as SilverLabelV1["difficulty"],
    confidence: value.confidence as SilverLabelV1["confidence"],
    reason: value.reason,
...
export const recoverOraclePassesFromTraces = async (input: {
  ...
  cards: AreaCardV1[];
  model: string;
  isolationMode?: CanonicalTraceSource["isolationMode"];
}): Promise<RecoveredOraclePasses> => {
  ...
  const recovered: Array<{
    label: SilverLabelV1;
    trace: CanonicalTraceSource;
  }> = [];
  const unexpectedEpisodeIds = new Set<string>();
  ...
  for (const entry of traceEntries) {
    ...
    const raw = JSON.parse(await readFile(sourceTrace, "utf8")) as unknown;
    ...
    const trace: RecoverableOracleTrace = {
      pass: sourcePass,
      finalMessage: raw.finalMessage,
      toolCalls: raw.toolCalls,
    };
    recovered.push({
      label: parseRecoveredOracleDecision(
        trace,
        taskEpisodeId,
        allowedAreaIds,
        input.model,
  ...
  recovered.sort(
    (left, right) =>
      left.label.taskEpisodeId.localeCompare(right.label.taskEpisodeId) ||
      left.trace.sourcePass - right.trace.sourcePass,
  );
  const recoveredEpisodeIds = new Set(
    recovered.map((entry) => entry.label.taskEpisodeId),
  );
  return {
...
export interface OraclePassAuditSummary {
  schemaVersion: 1;
  expectedEpisodeIds: string[];
  passCounts: Record<string, number>;
  missingEpisodeIds: string[];
  unexpectedEpisodeIds: string[];
  duplicatePasses: Array<{
    taskEpisodeId: string;
    count: number;
  }>;
  decisionDisagreementEpisodeIds: string[];
  readyForAdjudication: boolean;
}

export const auditOraclePassCoverage = (
  episodes: TaskEpisode[],
  passes: SilverLabelV1[],
): OraclePassAuditSummary => {
  const expectedEpisodeIds = episodes.map((episode) => episode.id);
  if (
    expectedEpisodeIds.some((id) => !id.trim()) ||
  ...
  const grouped = new Map<string, SilverLabelV1[]>();
  ...
  const passCounts = Object.fromEntries(
    [...expected].sort().map((id) => [id, grouped.get(id)?.length ?? 0]),
  );
  ...
  const decisionDisagreementEpisodeIds = duplicatePasses
    .filter(({ taskEpisodeId }) =>
      new Set((grouped.get(taskEpisodeId) ?? []).map(signature)).size > 1
    )
...
export const mergeOraclePassArtifacts = (
  ...
  const merged: Array<{
    label: SilverLabelV1;
    traceSource: CanonicalTraceSource;
  }> = [];
  const keys = new Set<string>();
  for (const [artifactIndex, artifact] of artifacts.entries()) {
    if (artifact.labels.length !== artifact.traceSources.length) {
      throw new Error(
        `Oracle artifact ${artifactIndex + 1} has mismatched labels and trace sources`,
      );
    }
    ...
    for (const label of artifact.labels) {
      taskCounts.set(
        label.taskEpisodeId,
        (taskCounts.get(label.taskEpisodeId) ?? 0) + 1,
      );
    }
    const containsRepeatedTask = [...taskCounts.values()].some(
      (count) => count > 1,
    );
    const paired: Array<{
      label: SilverLabelV1;
    ...
    if (containsRepeatedTask) {
      ...
      for (const [index, label] of artifact.labels.entries()) {
        ...
        if (traceSource.taskEpisodeId !== label.taskEpisodeId) {
          throw new Error(
            `Oracle artifact ${artifactIndex + 1} has ambiguous repeated-task pairing at record ${index + 1}`,
          );
        }
        paired.push({ label, traceSource });
      }
    } else {
      const tracesByTask = new Map<string, CanonicalTraceSource[]>();
      ...
      for (const label of artifact.labels) {
        ...
        if (matches.length !== 1) {
          throw new Error(
            `Oracle artifact ${artifactIndex + 1} has ${matches.length} trace sources for label ${label.taskEpisodeId}`,
          );
        }
      ...
      if (tracesByTask.size > 0) {
        throw new Error(
          `Oracle artifact ${artifactIndex + 1} has trace sources without labels`,
        );
      }
    }
...
const oracleDecisionSignature = (label: SilverLabelV1): string =>
  JSON.stringify({
    known: label.known,
    selectedAreaIds: [...label.selectedAreaIds].sort(),
    unknownType: label.unknownType ?? null,
...
export interface CanonicalOraclePassSelection {
  canonicalPasses: SilverLabelV1[];
  canonicalTraceSources: CanonicalTraceSource[];
  summary: {
    schemaVersion: 1;
    adjudicatedLabels: number;
    availablePasses: number;
    ignoredUnadjudicatedPasses: number;
    selectedPasses: number;
    rejectedMinorityPasses: number;
    unanimousSelections: number;
    majoritySelections: number;
  };
}
...
export const selectCanonicalOraclePasses = (input: {
  ...
  passes: SilverLabelV1[];
  traceSources: CanonicalTraceSource[];
}): CanonicalOraclePassSelection => {
  if (
    input.passes.length !== input.traceSources.length
  ) {
    throw new Error(
      "Canonical oracle selection requires paired pass and trace-source records",
    );
  }
  const finalById = new Map<string, SilverLabelV1>();
  for (const label of input.adjudicatedLabels) {
    if (finalById.has(label.taskEpisodeId)) {
      throw new Error(
        `Duplicate adjudicated oracle label: ${label.taskEpisodeId}`,
      );
    }
    if (!label.oracle.adjudicated) {
      throw new Error(
        `Final oracle label is not adjudicated: ${label.taskEpisodeId}`,
      );
    }
  ...
  const candidatesById = new Map<
    string,
    Array<{ pass: SilverLabelV1; traceSource: CanonicalTraceSource }>
  >();
  ...
  for (const [index, pass] of input.passes.entries()) {
    ...
    if (traceSource.taskEpisodeId !== pass.taskEpisodeId) {
      throw new Error(
        `Oracle trace source does not match pass ${pass.taskEpisodeId}`,
      );
    }
    const key = [
  ...
  const confidenceRank: Record<SilverLabelV1["confidence"], number> = {
    low: 0,
    medium: 1,
    high: 2,
  };
  ...
  for (const [taskEpisodeId, finalLabel] of [...finalById.entries()].sort(
    ([left], [right]) => left.localeCompare(right),
  )) {
    const candidates = candidatesById.get(taskEpisodeId) ?? [];
    if (!candidates.length) {
      throw new Error(
        `Adjudicated oracle label has no source passes: ${taskEpisodeId}`,
      );
    }
    const finalSignature = oracleDecisionSignature(finalLabel);
    const winners = candidates.filter(
      ({ pass }) => oracleDecisionSignature(pass) === finalSignature,
    );
    if (winners.length <= candidates.length / 2) {
      throw new Error(
        `Adjudicated oracle label lacks a strict-majority source decision: ${taskEpisodeId}`,
      );
    }
    if (finalLabel.oracle.passCount !== candidates.length) {
      throw new Error(
        `Adjudicated oracle pass count mismatch for ${taskEpisodeId}: expected ${finalLabel.oracle.passCount}, found ${candidates.length}`,
      );
    }
    ...
    winners.sort((left, right) => {
      const leftInspected =
        left.pass.oracle.repositoryInspected === true ? 1 : 0;
      const rightInspected =
        right.pass.oracle.repositoryInspected === true ? 1 : 0;
      return (
        rightInspected - leftInspected ||
        ...
        left.traceSource.sourcePass - right.traceSource.sourcePass ||
        left.traceSource.sourceTrace.localeCompare(
          right.traceSource.sourceTrace,
        )
      );
    });
