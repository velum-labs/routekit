...
const normalizedTaskEnvelope = (episode: TaskEpisode): string =>
  [
...
export const validateRepositoryProfile = (profile: RepositoryProfileV1): void => {
  if (profile.schemaVersion !== 1) throw new Error("Unsupported repository profile schema");
  ...
  if (profile.components.length === 0) throw new Error("Repository profile requires components");
};

export const validateAreaCards = (
  cards: AreaCardV1[],
  profile: RepositoryProfileV1,
  options: { requirePositiveExamples?: boolean } = {},
): void => {
  if (cards.length < 2) throw new Error("At least two Area Cards are required");
  const ids = cards.map((card) => card.areaId);
  if (!unique(ids)) throw new Error("Area IDs must be unique");
  const known = new Set(ids);
  for (const card of cards) {
    if (card.schemaVersion !== 1) throw new Error(`Unsupported Area Card schema: ${card.areaId}`);
    if (card.repositoryId !== profile.repositoryId) throw new Error(`Area Card repository mismatch: ${card.areaId}`);
    assertText(card.name, `${card.areaId}.name`);
    assertText(card.description, `${card.areaId}.description`);
    if (card.inclusions.length < 1) throw new Error(`${card.areaId} requires inclusions`);
    if (
      (options.requirePositiveExamples ?? true) &&
      card.positiveExampleIds.length < 1
    ) {
      throw new Error(`${card.areaId} requires positive examples`);
    }
    for (const confusable of card.confusableAreaIds) if (!known.has(confusable)) throw new Error(`${card.areaId} references unknown confusable area ${confusable}`);
    const codeCharacters = card.codeSnippets.reduce((sum, item) => sum + item.length, 0);
    if (card.codeSnippets.length > 3 || codeCharacters > 3_000) throw new Error(`${card.areaId} exceeds code snippet limits`);
  }
};
...
export const validateEpisodes = (episodes: TaskEpisode[], cards?: AreaCardV1[]): void => {
  const ids = new Set<string>();
  const lineageSplit = new Map<string, string>();
  const contentSplit = new Map<string, string>();
  ...
  for (const episode of episodes) {
    ...
    if (managedContext.test(allText)) throw new Error(`${episode.id} contains managed context`);
    if (likelySecret.test(allText)) throw new Error(`${episode.id} contains a likely secret`);
    const priorSplit = lineageSplit.get(episode.lineageHash);
    if (priorSplit && priorSplit !== episode.split) throw new Error(`Lineage crosses splits: ${episode.lineageHash}`);
    ...
    if (duplicateSplit && duplicateSplit !== episode.split) throw new Error(`Exact task duplicate crosses splits: ${episode.id}`);
    contentSplit.set(normalized, episode.split);
    if (episode.split !== "reference" && referenceIds.has(episode.id)) throw new Error(`Validation/test episode leaks into Area Card: ${episode.id}`);
  }
  const byRepository = new Map<string, TaskEpisode[]>();
  for (const episode of episodes) byRepository.set(episode.repositoryId, [...(byRepository.get(episode.repositoryId) ?? []), episode]);
  for (const group of byRepository.values()) {
    ...
    if (maxReference > minValidation || maxValidation > minTest) throw new Error("Splits are not strictly chronological");
  }
};

export const validateBenchmarkDataset = (
  profile: RepositoryProfileV1,
  cards: AreaCardV1[],
  ...
  for (const episode of episodes) {
    if (episode.repositoryId !== profile.repositoryId) {
      throw new Error(
        `Episode repository mismatch: ${episode.id} belongs to ${episode.repositoryId}`,
      );
    }
  ...
  for (const label of labels) {
    ...
    if (!episodeIds.has(label.taskEpisodeId)) {
      throw new Error(
        `Benchmark label has no episode: ${label.taskEpisodeId}`,
      );
    }
  }
...
export const validateSilverLabels = (labels: SilverLabelV1[], cards: AreaCardV1[]): void => {
  const known = new Set(cards.map((card) => card.areaId));
  for (const label of labels) {
    if (label.selectedAreaIds.length > 2) throw new Error(`${label.taskEpisodeId} selects more than two areas`);
    if (!unique(label.selectedAreaIds)) throw new Error(`${label.taskEpisodeId} repeats an area`);
    for (const area of label.selectedAreaIds) if (!known.has(area)) throw new Error(`${label.taskEpisodeId} selects unknown area ${area}`);
    if (label.known !== (label.selectedAreaIds.length > 0)) throw new Error(`${label.taskEpisodeId} known flag conflicts with selected areas`);
    if (!label.known && !label.unknownType) throw new Error(`${label.taskEpisodeId} requires unknownType`);
    if (label.oracle.humanReviewed !== false) throw new Error("Silver labels must record humanReviewed=false");
  }
};

export interface SilverLabelRepositoryEvidenceAudit {
  schemaVersion: 1;
  repository: string;
  labels: number;
  labelsRequiringInspection: number;
  labelsWithInspection: number;
  pathsChecked: number;
  missingPaths: Array<{
  ...
  insufficientInspection: string[];
  ready: boolean;
}

const pathExistsAtSnapshot = async (
  repository: string,
  snapshot: string,
  ...
  try {
    await execFileAsync(
      "git",
      ["-C", repository, "cat-file", "-e", `${snapshot}:${repositoryPath}`],
      { maxBuffer: 1024 * 1024 },
    );
    return true;
  } catch {
...
export const auditSilverLabelRepositoryEvidence = async (
  labels: SilverLabelV1[],
  episodes: TaskEpisode[],
  repository: string,
): Promise<SilverLabelRepositoryEvidenceAudit> => {
  const episodesById = new Map(episodes.map((episode) => [episode.id, episode]));
  const missingPaths: SilverLabelRepositoryEvidenceAudit["missingPaths"] = [];
  const missingEpisodes: string[] = [];
  const insufficientInspection: string[] = [];
  ...
  for (const label of labels) {
    ...
    if (requiresInspection) {
      ...
      if (
        label.oracle.repositoryInspected === true &&
...
export const redactText = (input: string): { text: string; redactions: number } => {
  let redactions = 0;
  let text = input
    ...
    .replace(/(authorization\s*:\s*bearer\s+)[^\s"']+/giu, (_, prefix: string) => { redactions += 1; return `${prefix}[REDACTED]`; })
    .replace(/((?:api[_-]?key|password|secret|token)\s*[=:]\s*["']?)[A-Za-z0-9_\-./+]{16,}/giu, (_, prefix: string) => { redactions += 1; return `${prefix}[REDACTED]`; });
