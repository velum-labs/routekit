...
import { analyzeWithGitNexus, buildGitNexusOnlyResult, fuseCurrentHybridWithGitNexus, GITNEXUS_VERSION, materializeGitNexusSnippets, parseGitNexusDefinitions, queryGitNexus, } from "./gitnexus-retrieval-experiment.js";
import { contentHash } from "./hash.js";
import { readJsonl } from "./jsonl.js";
import { evaluateLunaPerformanceRetrieval, } from "./luna-performance-retrieval.js";
...
const readConfig = async (file) => JSON.parse(await readFile(path.resolve(file), "utf8"));
const checkoutSnapshot = async (input) => {
    if (!(await exists(path.join(input.worktree, ".git")))) {
        ...
        await execFileAsync("git", ["-C", input.worktree, "checkout", "--detach", "--force", input.snapshot], { maxBuffer: 64 * 1024 * 1024 });
    }
    const actual = (await execFileAsync("git", ["-C", input.worktree, "rev-parse", "HEAD"], {
        encoding: "utf8",
    })).stdout.trim();
...
const commitTimestamp = async (repository, snapshot) => {
    const output = (await execFileAsync("git", ["-C", repository, "show", "-s", "--format=%ct", snapshot], { encoding: "utf8" })).stdout.trim();
    return Number(output);
};
const retrievalFile = (directory, episodeId) => path.join(directory, "retrieval", "hybrid_rerank", `${episodeId}.json`);
const runMaterialization = async (args) => {
    ...
    const requested = requestedEpisodeIds
        ? new Set(requestedEpisodeIds
            .split(",")
            .map((value) => value.trim())
            .filter(Boolean))
        : undefined;
    const allEpisodes = await readJsonl(path.resolve(config.episodes));
    const episodes = allEpisodes.filter((episode) => development.has(episode.id) &&
        (!requested || requested.has(episode.id)));
    if (requested && episodes.length !== requested.size) {
        throw new Error(`Requested ${requested.size} episode IDs but found ${episodes.length}`);
    }
    const repositoryById = new Map(config.repositories.map((repository) => [
        repository.repositoryId,
        repository,
    ]));
    const cardsById = new Map();
    for (const repository of config.repositories) {
        cardsById.set(repository.repositoryId, await readJsonl(path.resolve(repository.areas)));
    }
    const scheduled = await Promise.all(episodes.map(async (episode) => {
        const repository = repositoryById.get(episode.repositoryId);
        if (!repository)
            throw new Error(`Unknown repository ${episode.repositoryId}`);
        return {
            episode,
            repository,
            commitTimestamp: await commitTimestamp(repository.repository, episode.repositorySnapshot),
        };
    }));
    scheduled.sort((left, right) => left.repository.repositoryId.localeCompare(right.repository.repositoryId) ||
    ...
    for (const [index, item] of scheduled.entries()) {
        const gitNexusOutput = retrievalFile(path.join(outputDirectory, "gitnexus-only"), item.episode.id);
        const fusionOutput = retrievalFile(path.join(outputDirectory, "fusion"), item.episode.id);
        ...
        const perEpisodeArtifacts = path.join(outputDirectory, "gitnexus-runtime", item.episode.id);
        ...
        const current = JSON.parse(await readFile(retrievalFile(baselineDirectory, item.episode.id), "utf8"));
...
const prepareDevelopment = async (args) => {
    ...
    await writeFile(path.join(directory, "episodes.jsonl"), `${episodes.map((episode) => JSON.stringify(episode)).join("\n")}\n`, { mode: 0o600 });
    await writeFile(path.join(directory, "labels.jsonl"), `${labels.map((label) => JSON.stringify(label)).join("\n")}\n`, { mode: 0o600 });
...
const analyzeRetrieval = async (args) => {
    ...
    const labels = (await readJsonl(path.resolve(config.labels)))
        .filter((label) => development.has(label.taskEpisodeId));
    const outputDirectory = path.resolve(required(args, "--output-directory"));
    const readResults = async (root) => Promise.all(episodes.map(async (episode) => JSON.parse(await readFile(retrievalFile(root, episode.id), "utf8"))));
    ...
    const diagnostics = (results) => ({
        ...
        meanUniquePaths: results.reduce((sum, result) => sum + new Set(result.selected.map((item) => item.path)).size, 0) / results.length,
        testOrDocNoiseRate: results.reduce((sum, result) => sum +
            result.selected.filter((item) => /(?:^|\/)(?:test|tests|docs?|examples?|fixtures?)(?:\/|$)|(?:_test|\.test|\.spec)\.[^/]+$/iu.test(item.path)).length, 0) / results.reduce((sum, result) => sum + result.selected.length, 0),
    });
    const report = {
        schemaVersion: 1,
...
const reselectMaterialized = async (args) => {
    const config = await readConfig(required(args, "--config"));
    const development = new Set(config.developmentEpisodeIds);
    const episodes = (await readJsonl(path.resolve(config.episodes)))
        .filter((episode) => development.has(episode.id));
    ...
    for (const episode of episodes) {
        const gitNexusFile = retrievalFile(path.join(outputDirectory, "gitnexus-only"), episode.id);
        if (!(await exists(gitNexusFile)))
            continue;
        const existing = JSON.parse(await readFile(gitNexusFile, "utf8"));
        const { source: _source, fusionVersion: _fusionVersion, ...provenance } = existing.gitNexusProvenance;
        const gitNexus = buildGitNexusOnlyResult({
            episode,
            snippets: existing.candidates,
            provenance,
        });
        const current = JSON.parse(await readFile(retrievalFile(baselineDirectory, episode.id), "utf8"));
        const fusion = fuseCurrentHybridWithGitNexus({
            episode,
            current,
            gitNexus,
        ...
        await writeAtomic(retrievalFile(path.join(outputDirectory, "fusion"), episode.id), fusion);
    }
...
const main = async () => {
    ...
    if (args.command === "prepare-development")
        return prepareDevelopment(args);
    if (args.command === "materialize")
        return runMaterialization(args);
    if (args.command === "reselect")
        return reselectMaterialized(args);
    if (args.command === "analyze-retrieval")
        return analyzeRetrieval(args);
    throw new Error("Usage: gitnexus-retrieval-experiment-cli <prepare-development|materialize|reselect|analyze-retrieval> ...");
};
main().catch((error) => {
