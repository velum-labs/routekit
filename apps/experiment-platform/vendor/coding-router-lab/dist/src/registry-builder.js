...
import { ROUTEKIT_AREAS } from "./routekit-seed.js";
const execFileAsync = promisify(execFile);
const MAX_EXAMPLES_PER_AREA = 12;
const MINIMUM_EXAMPLES_PER_AREA = 5;
const repositoryPaths = async (repository, snapshot) => {
    const result = await execFileAsync("git", ["-C", repository, "ls-tree", "-r", "--name-only", snapshot], { maxBuffer: 32 * 1024 * 1024 });
    return result.stdout.split(/\r?\n/u).filter(Boolean);
};
const normalized = (value) => value.toLowerCase().replace(/[^a-z0-9_./-]+/gu, " ");
const containsAnchor = (text, anchor) => {
...
const scoreEpisode = (episode, repositoryFileSet) => {
    ...
    for (const area of ROUTEKIT_AREAS) {
        ...
        for (const anchor of area.pathAnchors) {
            if (containsAnchor(text, anchor) &&
                [...repositoryFileSet].some((file) => file === anchor || file.startsWith(`${anchor}/`))) {
                score += 3;
...
export const buildReferenceRegistry = async (input) => {
    const referenceEpisodes = input.episodes.filter((episode) => episode.split === "reference");
    ...
    const supplementalEpisodes = (input.supplementalEpisodes ?? []).filter((episode) => episode.timestamp <= latestReferenceEpisode.timestamp &&
        episode.split === "reference");
    const supplementalAssignments = input.supplementalAssignments ?? [];
    const supplementalById = new Map(supplementalAssignments.map((assignment) => [
        assignment.taskEpisodeId,
        assignment.selectedAreaIds,
    ]));
    const eligibleSupplemental = supplementalEpisodes.filter((episode) => (supplementalById.get(episode.id)?.length ?? 0) > 0);
    const byId = new Map([...referenceEpisodes, ...eligibleSupplemental].map((episode) => [
        episode.id,
        episode,
    ]));
    const registryDatasetId = contentHash(referenceEpisodes.map((episode) => ({
        id: episode.id,
    ...
    const profile = {
        ...
        snapshot,
        name: "RouteKit",
        purpose: "A TypeScript CLI and singleton daemon exposing an authenticated, OpenAI-compatible gateway across coding subscriptions and API providers.",
    ...
    const cards = ROUTEKIT_AREAS.map((area) => {
        const realCandidates = assignments
            ...
            .sort((left, right) => {
            const leftScore = left.scores.find((score) => score.areaId === area.areaId)?.score ?? 0;
            const rightScore = right.scores.find((score) => score.areaId === area.areaId)?.score ?? 0;
            return (rightScore - leftScore ||
                (byId.get(left.taskEpisodeId)?.timestamp ?? "").localeCompare(byId.get(right.taskEpisodeId)?.timestamp ?? ""));
        });
        const realIds = new Set(realCandidates.map((candidate) => candidate.taskEpisodeId));
        const supplementalCandidates = eligibleSupplemental
            .filter((episode) => supplementalById.get(episode.id)?.includes(area.areaId))
        ...
        const candidates = [...realCandidates, ...supplementalCandidates].slice(0, MAX_EXAMPLES_PER_AREA);
        ...
        return {
            ...
            confusableAreaIds: ROUTEKIT_AREAS.filter((other) => other.areaId !== area.areaId &&
                (other.pathAnchors.some((anchor) => area.pathAnchors.includes(anchor)) ||
                    candidates.some((candidate) => candidate.selectedAreaIds.includes(other.areaId)))).map((other) => other.areaId),
            pathAnchors: area.pathAnchors.filter((anchor) => paths.some((file) => file === anchor || file.startsWith(`${anchor}/`))),
            componentAnchors: area.componentAnchors,
            symbolAnchors: area.symbolAnchors,
            codeSummaries: [
                ...area.codeSummaries,
                ...representativeTasks.map((task) => `Representative reference-period task: ${task}`),
            ],
            codeSnippets: [],
    ...
    return {
        ...
        excludedReferenceEpisodes: assignments.filter((assignment) => assignment.selectedAreaIds.length === 0).length,
