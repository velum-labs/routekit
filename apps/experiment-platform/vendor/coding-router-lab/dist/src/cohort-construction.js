...
import { buildPublicIssueTaskText, extractPublicIssueProblemStatement, } from "./public-pr-benchmark.js";
const execFileAsync = promisify(execFile);
export const CONVERSATIONAL_COHORT_VERSION = "real-conversational-coding-v1";
export const DIVERSE_PUBLIC_COHORT_VERSION = "diverse-public-issue-grounded-v1";
export const NATURAL_HARD_COHORT_VERSION = "natural-hard-cohort-v1";
const writeJson = async (directory, name, value) => {
    await mkdir(directory, { recursive: true, mode: 0o700 });
    await writeFile(path.join(directory, name), `${JSON.stringify(value, null, 2)}\n`, { mode: 0o600 });
};
const writeJsonl = async (directory, name, values) => {
    await mkdir(directory, { recursive: true, mode: 0o700 });
...
const normalizeText = (text) => text.replace(/\s+/gu, " ").trim().toLowerCase();
export const DEFAULT_CONVERSATIONAL_SOURCES = [
    {
...
const hasManagedContext = (episode) => Boolean(episode.taskAnchor ||
...
const excludedConversationalTopic = /\b(?:minecraft|skin|x algorithm|twitter algorithm|notion|gtm|go[- ]to[- ]market|sales prospect|fundrais|dating|restaurant|vacation|yc says|keeping close contact with people for feedback|today we.?re launching routekit|launch announcement|social media|x account|website skill|linear spec doc)\b/iu;
const classifyConversation = (episode) => {
    ...
    if (episode.relevantDiagnostic ||
        /\b(?:ci|test|build|typecheck|lint|deploy(?:ment)?)\b.{0,30}\b(?:fail|error|broken)\b|\b(?:fix it|fix them|can you fix|migrate and fix)\b/isu.test(combined)) {
        return "debugging_followup";
    }
    if (/\b(?:please implement this plan|implement this|create a pr|commit|push|hotfix)\b/iu.test(request)) {
        return "implementation_followup";
    }
    if (request.length <= 100 ||
        /^(?:continue|continue!|go ahead|do it|proceed|done|fix it|fix them|remove that|add this)\b/iu.test(request)) {
        return "short_referential";
    }
    return "incomplete_specification";
};
const conversationScore = (episode) => {
    const request = episode.currentRequest.trim();
    let score = 0;
    if (episode.taskAnchor)
        score += 4;
    if (episode.precedingAssistant)
        score += 4;
    if (episode.earlierUserContext?.length)
        score += 3;
    if (episode.relevantDiagnostic)
        score += 6;
    if (episode.provenance.context.isReferentialRequest)
        score += 4;
    if (request.length >= 10 && request.length <= 180)
        score += 5;
    else if (request.length <= 1_200)
        score += 3;
    if (/\b(?:fix|implement|test|debug|refactor|migrate|build|ci)\b/iu.test(request)) {
        score += 4;
    }
    if (/^(?:continue|done|go ahead|proceed)!?$/iu.test(request))
        score -= 2;
    return score;
};
const assignChronologicalSplits = (values) => {
    const sorted = [...values].sort((left, right) => left.timestamp.localeCompare(right.timestamp));
    const result = new Map();
    for (const [index, value] of sorted.entries()) {
        const fraction = index / Math.max(1, sorted.length);
        result.set(value, fraction < 0.5
            ? "reference"
            : fraction < 0.75
                ? "validation"
                : "test");
...
const selectConversationalCandidates = (candidates, target) => {
    const independentByLineage = new Map();
    for (const candidate of candidates) {
        ...
        if (!existing ||
            ...
            (candidate.score === existing.score &&
                candidate.episode.timestamp.localeCompare(existing.episode.timestamp) < 0)) {
            independentByLineage.set(key, candidate);
        }
    }
    const independentCandidates = [...independentByLineage.values()];
    const availableByEngineer = new Map(["alen", "benjamin"].map((engineer) => {
        const sessions = new Map();
        for (const candidate of independentCandidates) {
            if (candidate.engineerPseudonym !== engineer)
                continue;
            sessions.set(candidate.episode.sessionHash, (sessions.get(candidate.episode.sessionHash) ?? 0) + 1);
        }
        return [
            engineer,
            [...sessions.values()].reduce((sum, count) => sum + Math.min(2, count), 0),
        ];
    }));
    const effectiveTarget = Math.min(target, availableByEngineer.get("alen") +
        availableByEngineer.get("benjamin"));
    if (effectiveTarget < 40) {
        throw new Error(`Only ${effectiveTarget} quality-gated conversational coding tasks are available; need at least 40`);
    }
    const minimumMinorityShare = Math.min(15, availableByEngineer.get("benjamin"));
    const benjaminQuota = Math.min(Math.floor(effectiveTarget / 2), availableByEngineer.get("benjamin"));
    const desiredBenjamin = Math.max(minimumMinorityShare, benjaminQuota);
    const desiredAlen = Math.min(availableByEngineer.get("alen"), effectiveTarget - desiredBenjamin);
    ...
    for (const engineer of ["alen", "benjamin"]) {
        ...
        let cursor = 0;
        while (selected.filter((candidate) => candidate.engineerPseudonym === engineer).length < quota) {
            const stratum = strataOrder[cursor % strataOrder.length];
            const candidate = pool.find((item) => item.stratum === stratum &&
                !selectedIds.has(item.episode.id) &&
                (perSession.get(item.episode.sessionHash) ?? 0) < 2);
            const fallback = pool.find((item) => !selectedIds.has(item.episode.id) &&
                (perSession.get(item.episode.sessionHash) ?? 0) < 2);
            const chosen = candidate ?? fallback;
            if (!chosen) {
                throw new Error(`Only ${selected.filter((item) => item.engineerPseudonym === engineer).length} eligible independent conversational tasks for ${engineer}; need ${quota}`);
            }
            selected.push(chosen);
            selectedIds.add(chosen.episode.id);
            perSession.set(chosen.episode.sessionHash, (perSession.get(chosen.episode.sessionHash) ?? 0) + 1);
            cursor += 1;
        }
    }
    return selected.sort((left, right) => left.episode.timestamp.localeCompare(right.episode.timestamp) ||
        left.episode.id.localeCompare(right.episode.id));
};
export const buildConversationalCodingCohort = async (input) => {
    ...
    for (const source of sources) {
        ...
        const episodes = await readJsonl(absolute);
        let managedContextRows = 0;
        let eligibleCodingRows = 0;
        for (const episode of episodes) {
            if (!hasManagedContext(episode))
                continue;
            ...
            if (seenRepresentations.has(representationHash))
                continue;
    ...
    const splitMap = assignChronologicalSplits(selected.map((candidate) => candidate.episode));
    ...
    const counts = {
        ...
        engineers: Object.fromEntries(["alen", "benjamin"].map((engineer) => [
            engineer,
            selected.filter((candidate) => candidate.engineerPseudonym === engineer).length,
        ])),
        repositories: Object.fromEntries([...new Set(episodes.map((episode) => episode.repositoryId))]
            .sort()
            .map((repositoryId) => [
            repositoryId,
        ...
        split: Object.fromEntries(["reference", "validation", "test"].map((split) => [
            split,
            episodes.filter((episode) => episode.split === split).length,
        ])),
        context: {
            taskAnchor: episodes.filter((episode) => episode.taskAnchor).length,
            precedingAssistant: episodes.filter((episode) => episode.precedingAssistant).length,
            earlierUserContext: episodes.filter((episode) => episode.earlierUserContext?.length).length,
            relevantDiagnostic: episodes.filter((episode) => episode.relevantDiagnostic).length,
        },
    };
    ...
    await writeJson(output, "manifest.json", {
        ...
        gates: {
            ...
            multipleIndependentEngineers: new Set(selected.map((candidate) => candidate.engineerPseudonym)).size,
            note: "Only two independent engineers are present in the received transfers. This satisfies multi-engineer coverage but not a broader several-engineer claim.",
        },
        hashes: {
    ...
    await writeFile(path.join(output, "audit.md"), [
...
export const KUBERNETES_PUBLIC_DEFINITION = {
    repository: "kubernetes/kubernetes",
    name: "Kubernetes",
    purpose: "A large Go codebase for container orchestration, control-plane APIs, node agents, networking, scheduling, storage, authentication, and command-line operations.",
    ...
    registeredAreas: [
        {
            ...
            description: "API server behavior, API types, serialization, admission, discovery, storage machinery, clients, and custom resources.",
        ...
        },
        {
            ...
            description: "Deployments, StatefulSets, DaemonSets, Jobs, ReplicaSets, workload lifecycle, and their controllers.",
        ...
        },
        {
            ...
            description: "Authentication, authorization, service accounts, credentials, RBAC, certificates, and security policy enforcement.",
        ...
        },
        {
            ...
            description: "kubectl commands, flags, output, apply behavior, command-line UX, and client-side workflows.",
        ...
        },
        {
            ...
            description: "Services, endpoints, kube-proxy, DNS, ingress, network policy, IP addressing, and cluster network behavior.",
        ...
        },
        {
            ...
            description: "Kubelet, pod and container lifecycle, runtimes, resource management, node status, eviction, and host integration.",
        ...
        },
        {
            ...
            description: "Scheduler framework, placement, predicates, scoring, preemption, queues, topology, and scheduling policy.",
        ...
        },
        {
            ...
            description: "Volumes, persistent volume claims, CSI, attach/mount, storage classes, snapshots, and storage lifecycle.",
...
const grafanaArea = (areaId, name, githubLabels, description, confusableAreaIds, pathAnchors, componentAnchors) => ({
    areaId,
    name,
    githubLabels,
    description,
...
export const GRAFANA_PUBLIC_DEFINITION = {
    repository: "grafana/grafana",
    name: "Grafana",
    purpose: "A mixed Go and TypeScript observability application spanning dashboards, visualizations, data sources, alerting, plugins, authentication, and backend/frontend platform boundaries.",
...
const ghJson = async (args) => {
    ...
    for (let attempt = 0; attempt < 4; attempt += 1) {
        try {
            ...
            if (attempt === 3 ||
                !/(?:HTTP 5\d\d|timeout|connection reset|stream error|received from peer)/iu.test(message)) {
                throw error;
            }
...
const labelMatches = (label, configured) => label === configured ||
    (configured.endsWith("/*") &&
        label.startsWith(configured.slice(0, -1)));
const categorizeCandidate = (definition, pull) => {
    const labels = pull.labels.map((label) => label.name).sort();
    let mappedAreaIds = definition.registeredAreas
        .filter((area) => area.githubLabels.some((configured) => labels.some((label) => labelMatches(label, configured))))
        .map((area) => area.areaId);
    if (definition.repository === "grafana/grafana" && mappedAreaIds.length > 1) {
        if (mappedAreaIds.includes("frontend-navigation") &&
            ...
            mappedAreaIds = mappedAreaIds.filter((areaId) => areaId !== "frontend-navigation");
        }
        if (mappedAreaIds.includes("backend-platform") &&
            ...
            mappedAreaIds = mappedAreaIds.filter((areaId) => areaId !== "backend-platform");
        }
    }
    const unknownGroupIds = definition.naturalUnknownGroups
        .filter((group) => group.labels.some((configured) => labels.some((label) => labelMatches(label, configured))))
        .map((group) => group.id);
    const samplingKind = mappedAreaIds.length > 1
        ? "multi_area"
    ...
    if (login.includes("bot") ||
        ...
        !pull.mergedAt ||
        /\b(?:backport|cherry[- ]pick|dependency update|version packages)\b/iu.test(pull.title)) {
        return undefined;
    }
    return {
        number: pull.number,
        ...
        unknownGroupIds,
        samplingKind,
        samplingAreaId: samplingKind === "known"
            ? mappedAreaIds[0]
...
const publicCandidateScore = (candidate) => {
    let score = 0;
    const bodyLength = candidate.body.trim().length;
    if (bodyLength >= 100 && bodyLength <= 8_000)
        score += 5;
...
const collectPublicCandidatePool = async (definition, limitPerLabel) => {
...
const extractRepositoryIssueNumbers = (body, repository) => {
    const [owner, name] = repository.split("/");
    const numbers = new Set();
    const escapedRepository = `${owner}/${name}`.replace(/[.*+?^${}()|[\]\\]/gu, "\\$&");
    for (const match of body.matchAll(new RegExp(`https?://github\\.com/${escapedRepository}/issues/(\\d+)`, "giu"))) {
...
const linkedIssueForCandidate = async (definition, candidate) => {
    ...
    const offer = (issue, rank, method) => {
        if (!issue || issue.__typename === "PullRequest")
            return;
    ...
    for (const issueNumber of extractRepositoryIssueNumbers(candidate.body, definition.repository)) {
        ...
        try {
            ...
            if (!issue.pull_request) {
                offer({ ...issue, url: issue.html_url }, 2, "explicit_body_reference");
            }
        }
        catch {
    ...
    return {
        ...
        candidateCount: candidates.filter((item) => item.rank === selected.rank).length,
    };
};
const materializePublicCandidate = async (definition, candidate) => {
...
const candidateOrder = (candidates) => [...candidates].sort((left, right) => publicCandidateScore(right) - publicCandidateScore(left) ||
    right.mergedAt.localeCompare(left.mergedAt) ||
    right.number - left.number);
const collectRepositoryPublicTasks = async (definition, input) => {
    const pool = await collectPublicCandidatePool(definition, input.limitPerLabel);
    const selected = [];
    const issueNumbers = new Set();
    const pullNumbers = new Set();
    const take = async (candidates, quota, description) => {
        let count = 0;
        for (const candidate of candidateOrder(candidates)) {
            if (count >= quota)
                break;
            if (pullNumbers.has(candidate.number))
                continue;
            const materialized = await materializePublicCandidate(definition, candidate);
            if (!materialized ||
        ...
        if (count < quota) {
            throw new Error(`${definition.repository}: collected ${count}/${quota} ${description} tasks`);
        }
    };
    for (const area of definition.registeredAreas) {
        await take(pool.filter((candidate) => candidate.samplingKind === "known" &&
            candidate.samplingAreaId === area.areaId), input.knownPerArea, `known ${area.areaId}`);
    }
    await take(pool.filter((candidate) => candidate.samplingKind === "multi_area"), input.multiArea, "multi-area");
    await take(pool.filter((candidate) => candidate.samplingKind === "natural_unknown"), input.naturalUnknown, "natural-open-set");
    const chronological = [...selected].sort((left, right) => left.mergedAt.localeCompare(right.mergedAt) ||
        left.number - right.number);
    const splits = new Map();
    for (const [index, candidate] of chronological.entries()) {
        const fraction = index / Math.max(1, chronological.length);
        splits.set(candidate.number, fraction < 0.5
            ? "reference"
            : fraction < 0.75
                ? "validation"
                : "test");
...
const publicAreaCards = (definition) => definition.registeredAreas.map((area) => ({
    schemaVersion: 1,
...
const directAreaCues = (taskText, definition) => definition.registeredAreas
    .filter((area) => {
    const terms = [
        area.areaId,
        area.name,
    ...
    return terms.some((term) => {
        ...
        return new RegExp(`(?:^|[^\\p{L}\\p{N}])${normalized}(?:$|[^\\p{L}\\p{N}])`, "iu").test(taskText);
    });
})
    .map((area) => area.areaId);
const directRegisteredAreaNameCues = (taskText, definition) => {
    const normalizedTask = taskText
        .toLowerCase()
        .replace(/[^\p{L}\p{N}]+/gu, " ")
        .trim();
    return definition.registeredAreas
        .filter((area) => [area.areaId, area.name, ...area.githubLabels].some((term) => {
        ...
        return (normalizedTerm.length >= 4 &&
            new RegExp(`(?:^| )${normalizedTerm.replaceAll(" ", " +")}(?: |$)`, "iu").test(normalizedTask));
    }))
        .map((area) => area.areaId);
};
const hasExactRepositoryLocator = (taskText) => /(?:^|[\s`(])(?:pkg|cmd|staging|test|tests|vendor|public|packages|apps|conf|devenv|scripts?)\/[\w./*-]+/imu.test(taskText) ||
    /https?:\/\/github\.com\/[^/\s]+\/[^/\s]+\/blob\//iu.test(taskText);
const outputPublicRepository = async (definition, materialized, outputDirectory) => {
    const episodes = materialized.map((candidate) => ({
        ...
        sessionHash: contentHash(`github:${definition.repository}:issue:${candidate.linkedIssue.number}`),
        lineageHash: contentHash(`github:${definition.repository}:issue:${candidate.linkedIssue.number}`),
    ...
    const evidence = materialized.map((candidate, index) => ({
        ...
        policy: "Offline construction and audit evidence only. Never expose labels, changed paths, merge commits, or post-task diffs to Sol or Luna.",
    }));
    const profile = {
        schemaVersion: 1,
        repositoryId: definition.repository,
    ...
    await writeJson(outputDirectory, "task-audit.json", {
        ...
        counts: {
            total: audits.length,
            highToolOpportunity: audits.filter((audit) => audit.toolOpportunity === "high").length,
            mediumToolOpportunity: audits.filter((audit) => audit.toolOpportunity === "medium").length,
            lowToolOpportunity: audits.filter((audit) => audit.toolOpportunity === "low").length,
            possibleImplementationLeakage: audits.filter((audit) => audit.possibleImplementationLeakage).length,
        },
        audits,
    });
    await writeJson(outputDirectory, "manifest.json", {
        ...
        counts: {
            total: episodes.length,
            known: materialized.filter((candidate) => candidate.samplingKind === "known").length,
            multiArea: materialized.filter((candidate) => candidate.samplingKind === "multi_area").length,
            naturalUnknown: materialized.filter((candidate) => candidate.samplingKind === "natural_unknown").length,
            bySplit: Object.fromEntries(["reference", "validation", "test"].map((split) => [
                split,
                episodes.filter((episode) => episode.split === split).length,
            ])),
            byKnownArea: Object.fromEntries(definition.registeredAreas.map((area) => [
                area.areaId,
...
export const buildDiversePublicCohort = async (input) => {
    ...
    for (const definition of definitions) {
        ...
        const repositoryDirectory = path.join(path.resolve(input.outputDirectory), definition.repository.split("/")[1]);
        await outputPublicRepository(definition, materialized, repositoryDirectory);
        combinedEpisodes.push(...(await readJsonl(path.join(repositoryDirectory, "episodes.jsonl"))));
        combinedEvidence.push(...(await readJsonl(path.join(repositoryDirectory, "offline-evidence.jsonl"))));
        combinedCards.push(...(await readJsonl(path.join(repositoryDirectory, "area-cards.jsonl"))));
    }
    const output = path.resolve(input.outputDirectory);
    if (combinedEpisodes.length < 40 || combinedEpisodes.length > 60) {
        throw new Error(`Cohort C must contain 40–60 tasks; got ${combinedEpisodes.length}`);
    }
    ...
    await writeJson(output, "manifest.json", {
        ...
        counts: {
            ...
            byRepository: Object.fromEntries(definitions.map((definition) => [
                definition.repository,
                combinedEpisodes.filter((episode) => episode.repositoryId === definition.repository).length,
            ])),
        },
        policy: "Issue text is runtime input. Labels, changed paths, merge commits, and issue/PR relation metadata remain offline.",
        hashes: {
...
const plausibleAreasForHardCase = (evidence, cards) => {
    const repositoryCards = cards.filter((card) => evidence.taskEpisodeId.startsWith(card.repositoryId.replace("/", "-")));
    ...
    if (plausible.size < 2) {
        const pathScores = repositoryCards
            .map((card) => ({
            areaId: card.areaId,
            score: evidence.actualChangedPaths.filter((changedPath) => card.pathAnchors.some((anchor) => changedPath.startsWith(anchor.replace(/\*.*$/u, "")))).length,
        }))
            .sort((left, right) => right.score - left.score || left.areaId.localeCompare(right.areaId));
        for (const item of pathScores) {
            if (item.score > 0 || plausible.size === 0)
                plausible.add(item.areaId);
            if (plausible.size >= 2)
                break;
        }
    }
...
export const buildNaturalHardCohort = async (input) => {
    ...
    if (target < 24)
        throw new Error("Natural hard cohort needs at least 24 tasks");
    const episodes = await readJsonl(path.join(input.publicCohortDirectory, "episodes.jsonl"));
    const evidence = await readJsonl(path.join(input.publicCohortDirectory, "offline-evidence.jsonl"));
    const cards = await readJsonl(path.join(input.publicCohortDirectory, "area-cards.jsonl"));
    const evidenceById = new Map(evidence.map((item) => [item.taskEpisodeId, item]));
    ...
    const eligible = episodes
        ...
        if (!item || !definition)
            return undefined;
        const cues = directRegisteredAreaNameCues(episode.currentRequest, definition);
    ...
    const existingPulls = new Set(evidence.map((item) => `${item.taskEpisodeId.startsWith("kubernetes-") ? "kubernetes/kubernetes" : "grafana/grafana"}:${item.pullRequestNumber}`));
    const existingIssues = new Set(evidence.map((item) => `${item.taskEpisodeId.startsWith("kubernetes-") ? "kubernetes/kubernetes" : "grafana/grafana"}:${item.linkedIssue.number}`));
    for (const stratum of strata) {
        const quota = quotas.get(stratum);
        let current = eligible.filter((item) => item.evidence.samplingKind === stratum).length;
        if (current >= quota)
            continue;
        for (const definition of [
            ...
            const pool = candidateOrder((await collectPublicCandidatePool(definition, 100)).filter((candidate) => candidate.samplingKind === stratum &&
                !existingPulls.has(`${definition.repository}:${candidate.number}`) &&
                directRegisteredAreaNameCues(candidate.title, definition).length === 0 &&
            ...
            for (const candidate of pool) {
                ...
                if (existingIssues.has(issueKey))
                    continue;
                if (directRegisteredAreaNameCues(materialized.taskText, definition).length > 0 ||
                ...
                const supplementalEpisode = {
                    schemaVersion: 1,
                    id: taskEpisodeId,
                    repositoryId: definition.repository,
                    repositorySnapshot: materialized.preTaskSnapshot,
                    sessionHash: contentHash(`github:${definition.repository}:issue:${materialized.linkedIssue.number}`),
                    lineageHash: contentHash(`github:${definition.repository}:issue:${materialized.linkedIssue.number}`),
                    timestamp: new Date(materialized.mergedAt).toISOString(),
                    split: "validation",
                    currentRequest: materialized.taskText,
                    source: "github",
                };
                const plausibleAreaIds = plausibleAreasForHardCase(supplementalEvidence, cards);
                if (plausibleAreaIds.length < 2)
                    continue;
                eligible.push({
                ...
                existingPulls.add(`${definition.repository}:${materialized.number}`);
        ...
        if (current < quota) {
            throw new Error(`Only ${current}/${quota} natural ${stratum} hard cases could be collected`);
        }
    }
    const selected = [];
    for (const stratum of strata) {
        const candidates = eligible
            ...
            .sort((left, right) => left.episode.repositoryId.localeCompare(right.episode.repositoryId) ||
            left.episode.timestamp.localeCompare(right.episode.timestamp) ||
            left.episode.id.localeCompare(right.episode.id));
        const quota = quotas.get(stratum);
        ...
        let cursor = 0;
        while (selected.filter((item) => item.evidence.samplingKind === stratum).length < quota) {
            ...
            if (!candidate) {
                ...
                if (repositories.length === 0) {
                    throw new Error(`Only ${selected.filter((item) => item.evidence.samplingKind === stratum).length}/${quota} eligible ${stratum} hard cases`);
                }
                continue;
            }
    ...
    const provenance = selected.map((item) => ({
        ...
        offlineEvidencePolicy: "Plausibility and stratum use construction-only labels/paths. Do not expose this record to classifiers.",
    }));
    const counts = {
        ...
        multiArea: provenance.filter((item) => item.stratum === "multi_area")
            .length,
        naturalUnknown: provenance.filter((item) => item.stratum === "natural_unknown").length,
        byRepository: Object.fromEntries([...new Set(selectedEpisodes.map((episode) => episode.repositoryId))]
            .sort()
            .map((repositoryId) => [
            repositoryId,
            selectedEpisodes.filter((episode) => episode.repositoryId === repositoryId).length,
        ])),
    };
    const output = path.resolve(input.outputDirectory);
...
export const auditCohortSnapshotResolution = async (input) => {
    const episodes = await readJsonl(path.join(input.cohortDirectory, "episodes.jsonl"));
    ...
    for (const repositoryId of repositories) {
        ...
        const repositoryEpisodes = episodes.filter((episode) => episode.repositoryId === repositoryId);
        const snapshots = [
            ...new Set(repositoryEpisodes.map((episode) => episode.repositorySnapshot)),
        ].sort();
        ...
        const origin = await execFileAsync("git", ["-C", localRepositoryPath, "remote", "get-url", "origin"], { encoding: "utf8" });
        results.push({
            repositoryId,
            localRepositoryPath,
    ...
    const audit = {
        ...
        counts: {
            ...
            uniqueSnapshots: results.reduce((sum, result) => sum + result.uniqueSnapshots, 0),
            resolvedSnapshots: results.reduce((sum, result) => sum + result.resolvedSnapshots, 0),
            missingSnapshots: results.reduce((sum, result) => sum + result.missingSnapshots.length, 0),
        },
        repositories: results,
        allResolved: results.every((result) => result.missingSnapshots.length === 0),
    };
    await writeJson(path.resolve(input.cohortDirectory), "snapshot-resolution-audit.json", {
        ...audit,
        hash: contentHash(audit),
    });
    if (!audit.allResolved) {
        throw new Error(`${audit.counts.missingSnapshots} cohort snapshots are unresolved`);
    }
};
