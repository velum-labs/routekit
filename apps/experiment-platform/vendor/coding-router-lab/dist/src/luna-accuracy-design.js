import { DEFAULT_LUNA_ACCURACY_FIXED_SEED_LIST, validateLunaAccuracyMatrixV2, } from "./luna-accuracy-context.js";
import { validateLunaAccuracyAnalysisAttestation, } from "./luna-accuracy-attestation.js";
import { lunaAccuracyArmsHash, lunaAccuracyAreaRegistryHash, lunaAccuracyMatrixHash, lunaAccuracyModelHash, lunaAccuracyProfileHash, lunaAccuracyRuntimeEpisodesHash, validateLunaAccuracyRunManifestBinding, } from "./luna-accuracy-runner.js";
import { contentHash } from "./hash.js";
export const LUNA_ACCURACY_PROTOCOL_VERSION = "luna-accuracy-protocol-v1";
export const LUNA_ACCURACY_PHASE_ONE_VARIANT_COUNT = 28;
export const LUNA_ACCURACY_PHASE_ONE_FINALIST_COUNT = 6;
export const LUNA_ACCURACY_TRANSPORT_SAFE_MAX_OUTPUT_TOKENS = 4_096;
export const LUNA_ACCURACY_INCUMBENT_MAX_OUTPUT_TOKENS = 128;
export const LUNA_ACCURACY_PHASE_TWO_B_SCHEDULE_SEED = 19_871;
...
export const LUNA_ACCURACY_PHASE_THREE_SINGLE_REPETITIONS = 3;
export const LUNA_ACCURACY_PHASE_THREE_SELF_CONSISTENCY_MEMBERS = 3;
export const LUNA_ACCURACY_PHASE_THREE_PIPELINE_REPETITIONS = 1;
...
export const buildLunaAccuracyPhaseOneMatrix = () => {
    ...
    const matrix = {
        schemaVersion: 2,
        description: "Phase 1: 28-arm task-aware Luna main-effect screen with a transport-safe 4096-token allowance for every non-incumbent arm, the exact accepted runtime incumbent mapping, and pre-registered accuracy interactions; no latest-request-only representation.",
        variants,
    };
...
const repeatedCopy = (source, id) => ({
    ...source,
    id,
    repetitions: 3,
    fixedSeedList: [...SEEDS],
});
const safeFinalistId = (id) => id.replace(/^p1-/u, "").slice(0, 65);
 ...
 */
export const buildLunaAccuracyPhaseTwoMatrix = (phaseOne, rankedPhaseOneIds) => {
    validateLunaAccuracyMatrixV2(phaseOne);
    const byId = new Map(phaseOne.variants.map((variant) => [variant.id, variant]));
    ...
    if (uniqueRankedIds.length !== phaseOne.variants.length ||
        phaseOne.variants.some((variant) => !uniqueRankedIds.includes(variant.id))) {
        throw new Error("Phase 2 requires a complete ranking of every Phase 1 candidate");
    }
    const selected = uniqueRankedIds.slice(0, LUNA_ACCURACY_PHASE_ONE_FINALIST_COUNT);
    const interactionCandidates = PHASE_TWO_INTERACTION_IDS
        .filter((id) => byId.has(id))
        .sort((left, right) => uniqueRankedIds.indexOf(left) - uniqueRankedIds.indexOf(right))
        .filter((id) => !selected.includes(id))
        .slice(0, 2);
    const sourceIds = [...selected, ...interactionCandidates];
    const variants = sourceIds.map((sourceId, index) => repeatedCopy(byId.get(sourceId), `p2-${String(index + 1).padStart(2, "0")}-${safeFinalistId(sourceId)}`));
    const matrix = {
        schemaVersion: 2,
        description: "Phase 2: three fixed-seed repetitions of six validation finalists plus up to two pre-registered interaction confirmations.",
        variants,
    };
    validateLunaAccuracyMatrixV2(matrix);
    return matrix;
...
export const buildLunaAccuracyPhaseTwoBMatrix = () => {
    ...
    const matrix = {
        schemaVersion: 2,
        description: "Corrected Phase 2b: three provider-distinct, task-aware single-call treatments over seven paired seeds; schedule seed 19871. A is the conservative labeled-sections 6k medium-reasoning baseline, B changes only to chronological 16k context, and C changes only to high reasoning.",
        variants,
    };
    validateLunaAccuracyMatrixV2(matrix);
    return matrix;
 ...
 */
export const buildLunaAccuracyPhaseThreeDesign = (primaryContext, alternativeContext) => {
    ...
    const providerVisibleContextKey = (variant) => contentHash({
    ...
    if (providerVisibleContextKey(primaryContext) ===
    ...
    for (const [contextKey, source] of [
        ...
        variants.push({
            ...
            fixedSeedList: seeds.slice(0, LUNA_ACCURACY_PHASE_THREE_SINGLE_REPETITIONS),
        }, {
            ...source,
            id: selfConsistencyId,
            repetitions: LUNA_ACCURACY_PHASE_THREE_SELF_CONSISTENCY_MEMBERS,
            fixedSeedList: seeds.slice(0, LUNA_ACCURACY_PHASE_THREE_SELF_CONSISTENCY_MEMBERS),
        }, {
            ...source,
            id: pipelineId,
            repetitions: LUNA_ACCURACY_PHASE_THREE_PIPELINE_REPETITIONS,
            fixedSeedList: seeds.slice(0, LUNA_ACCURACY_PHASE_THREE_PIPELINE_REPETITIONS),
        });
        arms.push({
            id: singleId,
            variantId: singleId,
    ...
    const matrix = {
        schemaVersion: 2,
        description: "Phase 3: preregistered two-context architecture comparison. The frozen Phase 2b selection and strongest alternative each receive three single-call trials, one three-member self-consistency ensemble, and one proposal-verify-revise pipeline.",
        variants,
    };
    validateLunaAccuracyMatrixV2(matrix);
    return { matrix, arms };
...
const assertTransitionReportBase = (input) => {
    ...
    if (report.schemaVersion !== 2 ||
        ...
        throw new Error("Phase transition requires a real-user validation report for Luna");
    }
    const hashPattern = /^[a-f0-9]{64}$/u;
    if (!Object.values(report.provenance).every((hash) => typeof hash === "string" && hashPattern.test(hash)) ||
        report.provenance.modelHash !== lunaAccuracyModelHash(report.model) ||
        report.provenance.matrixHash !== lunaAccuracyMatrixHash(matrix)) {
        throw new Error("Phase transition report is not bound to the supplied matrix/model");
    }
    const expectedIds = [...input.requiredVariantIds].sort();
    const rankedIds = report.ranking.map((entry) => entry.variantId);
    if (report.ranking.length !== expectedIds.length ||
        new Set(rankedIds).size !== rankedIds.length ||
        [...rankedIds].sort().some((id, index) => id !== expectedIds[index]) ||
        report.ranking.some((entry, index) => entry.rank !== index + 1 ||
            !input.allowedArchitectures.includes(entry.architecture))) {
        throw new Error("Phase transition requires a complete, unique candidate ranking");
    }
    if (report.recommendation.winnerVariantId !== rankedIds[0] ||
        report.recommendation.winnerArmId !== report.ranking[0]?.armId) {
        throw new Error("Phase transition report recommendation is inconsistent with ranking");
    }
};
export const validateLunaAccuracyPhaseOneTransition = (input) => {
    ...
    if (verified.report.provenance.datasetHash !==
        ...
        throw new Error("Phase 1 report is not bound to the supplied experiment arms");
    }
};
export const validateLunaAccuracyPhaseTwoTransition = (input) => {
    ...
    if (input.runManifest.hashes.runtimeEpisodes !==
        ...
        throw new Error("Phase 2 report is not bound to the supplied experiment arms");
    }
};
/**
 ...
 */
export const validateLunaAccuracyFreezeRecord = (value) => {
    ...
    if (value.modelHash !== lunaAccuracyModelHash(value.model) ||
        ...
        value.selectedArmsHash !==
            lunaAccuracyArmsHash({ schemaVersion: 2, variants: [value.selectedVariant] }, [value.selectedArm])) {
...
export const assertLunaAccuracyFreezeInputs = (input) => {
    ...
    if (input.model !== input.freeze.model ||
        ...
        throw new Error("Locked-test runtime inputs do not match the frozen configuration");
    }
};
export const createLunaAccuracyFreezeRecord = (input) => {
    ...
    const selectedMatrixVariant = input.selectionMatrix.variants.find((variant) => variant.id === input.selectedVariant.id);
    const selectedMatrixArm = selectionArms.find((arm) => arm.id === input.selectedArm.id);
    const winningRanking = input.selectionReport.ranking[0];
    if (input.selectionReport.schemaVersion !== 2 ||
        ...
        throw new Error("Locked-test freeze selection does not match its validation report, matrix, arms, and run manifest");
    }
    const record = {
        schemaVersion: 2,
        ...
        validationDatasetHash: lunaAccuracyRuntimeEpisodesHash(input.validationEpisodes),
        lockedTestDatasetHash: lunaAccuracyRuntimeEpisodesHash(input.lockedTestEpisodes),
        ...
        selectedArmsHash: lunaAccuracyArmsHash(lockedMatrix, [input.selectedArm]),
        selectionRunInputHash: input.selectionRunManifest.inputHash,
        selectionRunConfigurationHash: input.selectionRunManifest.configurationHash,
        ...
        syntheticChallengeSpecificationHash: contentHash(input.syntheticChallengeSpecification),
        lockedTestPolicy: {
