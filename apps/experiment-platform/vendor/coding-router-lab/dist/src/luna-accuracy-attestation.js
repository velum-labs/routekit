...
import { auditLunaAccuracyCodingAnnotations, codingEpisodeIdsFromAnnotations, } from "./luna-accuracy-coding-annotations.js";
import { buildLunaAccuracySelectionReport, buildLunaAccuracyTreatmentSelectionReport, } from "./luna-accuracy-report.js";
...
import { assertLunaAccuracyRunComplete, buildLunaAccuracyArchitecturePrompt, buildLunaAccuracyJobSchedule, buildLunaAccuracyPredictionSets, lunaAccuracyCallKey, normalizeLunaAccuracyArms, validateLunaAccuracyRunManifestBinding, } from "./luna-accuracy-runner.js";
import { contentHash } from "./hash.js";
import { validateBenchmarkDataset } from "./validation.js";
export const LUNA_ACCURACY_ANALYSIS_ATTESTATION_VERSION = "luna-accuracy-analysis-v2-pinned-transport-treatments";
...
const stageFor = (calls, job, stage) => {
    const key = lunaAccuracyCallKey(job, stage);
    const matches = calls.filter((record) => record.key === key);
    if (matches.length !== 1) {
        throw new Error(`Expected one completed ${stage} call for attestation job ${job.key}`);
    }
    return matches[0];
};
const expectedPrompt = (base, calls, job, stage) => {
    if (stage === "single" || stage === "member")
        return base;
    if (stage === "proposal") {
...
const expectedStages = (architecture) => architecture === "single_call"
    ? ["single"]
    ...
    for (const job of input.schedule) {
        ...
        for (const stage of expectedStages(job.architecture)) {
            ...
            if (record.promptHash !== contentHash(prompt)) {
                throw new Error(`Completed call prompt hash does not match runtime inputs: ${record.key}`);
            }
            ...
            if (record.transport.policyVersion !==
                ...
                throw new Error(`Completed call transport does not match runtime inputs: ${record.key}`);
            }
        }
    }
};
const stableGeneratedAt = (input) => {
...
export const buildLunaAccuracyAttestedAnalysis = (input) => {
    validateBenchmarkDataset(input.profile, input.cards, input.episodes, input.labels);
    const codingAudit = auditLunaAccuracyCodingAnnotations(input.episodes, input.codingAnnotations);
    ...
    const attestation = {
        ...
        hashes: {
            ...
            codingAnnotations: contentHash(canonicalAnnotations(input.codingAnnotations)),
...
export const validateLunaAccuracyAnalysisAttestation = (input) => {
    ...
    if (contentHash(attestation) !== contentHash(rebuilt.attestation) ||
        ...
        throw new Error("Luna accuracy analysis attestation or report does not match raw inputs");
    }
    return rebuilt;
};
