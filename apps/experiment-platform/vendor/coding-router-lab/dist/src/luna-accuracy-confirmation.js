import { lunaAccuracySelectionComponents, lunaAccuracySelectionScore, } from "./luna-accuracy-report.js";
import { buildSessionLineageClusters, calculateLunaAccuracyMetrics, } from "./luna-accuracy-metrics.js";
import { assertLunaAccuracyRunComplete, buildLunaAccuracyJobSchedule, buildLunaAccuracyPredictionSets, lunaAccuracyCallKey, lunaAccuracyArmsHash, lunaAccuracyMatrixHash, lunaAccuracyModelHash, lunaAccuracyRuntimeEpisodesHash, normalizeLunaAccuracyArms, validateLunaAccuracyRunManifestBinding, } from "./luna-accuracy-runner.js";
...
import { auditLunaAccuracyTreatmentDistinctness, } from "./luna-accuracy-distinctness.js";
...
export const LUNA_ACCURACY_PHASE_TWO_B_BASELINE_VARIANT_ID = "p2b-a-labeled-6k-medium";
export const LUNA_ACCURACY_PHASE_TWO_B_EXPECTED_REPETITIONS = 7;
export const LUNA_ACCURACY_PHASE_TWO_B_BOOTSTRAP_ITERATIONS = 10_000;
export const LUNA_ACCURACY_PHASE_TWO_B_BOOTSTRAP_SEED = 19_871;
...
const exactCaseIds = (expected, set) => {
    ...
    if (ids.length !== expected.size ||
        ...
        throw new Error(`Phase 2b prediction set ${set.id} is not an exact episode join`);
    }
};
const lexicalCompare = (left, right) => left < right ? -1 : left > right ? 1 : 0;
const canonicalLabels = (labels) => [...labels].sort((left, right) => lexicalCompare(left.taskEpisodeId, right.taskEpisodeId));
const canonicalAnnotations = (annotations) => [...annotations].sort((left, right) => lexicalCompare(left.taskEpisodeId, right.taskEpisodeId));
const canonicalCalls = (calls) => [...calls].sort((left, right) => lexicalCompare(left.key, right.key));
const unanimityRate = (sets) => {
    ...
    const unanimous = caseIds.filter((caseId) => new Set(bySet.map((values) => values.get(caseId))).size === 1).length;
    return unanimous / caseIds.length;
};
const analyzeVariant = (input) => {
    if (input.predictionSets.length !==
        LUNA_ACCURACY_PHASE_TWO_B_EXPECTED_REPETITIONS) {
        throw new Error(`Phase 2b variant ${input.variantId} requires exactly seven prediction sets`);
    }
    const repetitionIndices = input.predictionSets.map((set) => set.repetitionIndex);
    if (repetitionIndices.some((value) => value === null) ||
        new Set(repetitionIndices).size !==
            LUNA_ACCURACY_PHASE_TWO_B_EXPECTED_REPETITIONS ||
        [...repetitionIndices]
            .sort((left, right) => left - right)
            .some((value, index) => value !== index)) {
        throw new Error(`Phase 2b variant ${input.variantId} needs repetition indices 0 through 6`);
    }
    const ordered = [...input.predictionSets].sort((left, right) => left.repetitionIndex - right.repetitionIndex);
    const seedResults = ordered.map((set) => {
        if (set.seeds.length !== 1) {
            throw new Error(`Phase 2b set ${set.id} must contain exactly one paired seed`);
        }
        ...
        if (components === null || knownCoding === null) {
            throw new Error(`Phase 2b set ${set.id} has an undefined required accuracy metric`);
        }
        return {
    ...
    const knownCodingMean = mean(seedResults.map((result) => result.knownCodingExactSetAccuracy));
    const knownCodingWorstSeed = Math.min(...seedResults.map((result) => result.knownCodingExactSetAccuracy));
    const repeatUnanimity = unanimityRate(ordered);
    const completedSeedBlocks = seedResults.length;
    const failures = [];
    if (completedSeedBlocks !==
        LUNA_ACCURACY_PHASE_TWO_B_STABILITY_GATES
            .requiredCompletedSeedBlocks) {
        failures.push(`Only ${completedSeedBlocks}/7 paired seed blocks were complete`);
    }
    if (knownCodingMean <
    ...
    if (knownCodingWorstSeed <
        LUNA_ACCURACY_PHASE_TWO_B_STABILITY_GATES
            .knownCodingWorstSeedMinimum) {
        failures.push("Worst-seed known-coding exact-set accuracy was below 0.625");
    }
    if (repeatUnanimity <
    ...
    return {
        ...
        meanSelectionScore: mean(seedResults.map((result) => result.selectionScore)),
    };
};
const selectLunaAccuracyPhaseTwoBCore = (input) => {
    ...
    if (byVariant.size !== expectedVariantIds.length ||
        expectedVariantIds.some((id) => !byVariant.has(id))) {
        throw new Error("Phase 2b selection requires exactly the three preregistered variants");
    }
    const variants = expectedVariantIds.map((variantId) => analyzeVariant({
        variantId,
        labels: input.labels,
    ...
    const comparisonByChallenger = new Map(comparisons.map((comparison) => [
        comparison.challengerVariantId,
        comparison,
    ]));
    if (comparisons.length !== 2 ||
        comparisonByChallenger.size !== 2 ||
        expectedVariantIds.slice(1).some((id) => !comparisonByChallenger.has(id))) {
        throw new Error("Phase 2b requires one cluster/seed-paired comparison for B versus A and C versus A");
    }
    for (const comparison of comparisons) {
        if (comparison.baselineVariantId !==
            LUNA_ACCURACY_PHASE_TWO_B_BASELINE_VARIANT_ID ||
            ...
            !Number.isFinite(comparison.bootstrap.probabilityChallengerBetter) ||
    ...
    const practicallyWinningChallengers = variants
        ...
        return (variant.stability.passed &&
        ...
        .sort((left, right) => right.meanSelectionScore - left.meanSelectionScore ||
    ...
    if (practicalWinner) {
        return {
            ...
            reason: "The selected challenger passed every stability gate, led A by at least 0.01 mean selection score, and reached at least 0.80 paired cluster-bootstrap superiority.",
            variants,
            comparisons,
        };
    }
    return {
        selectedVariantId: LUNA_ACCURACY_PHASE_TWO_B_BASELINE_VARIANT_ID,
        baselineVariantId: LUNA_ACCURACY_PHASE_TWO_B_BASELINE_VARIANT_ID,
...
const exactSemanticDecisionCorrect = (label, prediction) => {
    ...
    return (prediction.selectedAreaIds.length === 0 &&
...
const scoreSample = (input) => {
    const predictionById = new Map(input.predictions.map((prediction) => [
        prediction.taskEpisodeId,
        prediction,
    ]));
    const exact = [];
    ...
    for (const label of input.labels) {
        ...
        if (!prediction) {
            throw new Error(`Phase 2b bootstrap is missing prediction ${label.taskEpisodeId}`);
        }
    ...
    const averageBooleans = (values) => values.length
    ...
    const f1 = (truePositive, falsePositive, falseNegative) => {
...
export const compareLunaAccuracyPhaseTwoBPaired = (input) => {
    const ordered = (sets) => {
        if (sets.length !== LUNA_ACCURACY_PHASE_TWO_B_EXPECTED_REPETITIONS ||
            sets.some((set) => set.repetitionIndex === null)) {
            throw new Error("Phase 2b paired bootstrap requires seven indexed prediction sets");
        }
        const result = [...sets].sort((left, right) => left.repetitionIndex - right.repetitionIndex);
        if (result.some((set, index) => set.repetitionIndex !== index || set.seeds.length !== 1)) {
            throw new Error("Phase 2b paired bootstrap requires repetition indices 0 through 6");
        }
        return result;
    };
    ...
    if (baseline.some((set, index) => set.seeds[0] !== challenger[index].seeds[0])) {
        throw new Error("Phase 2b challenger and A must use paired seeds");
    }
    const labelsById = new Map(input.labels.map((label) => [label.taskEpisodeId, label]));
    const clusters = buildSessionLineageClusters(input.episodes);
    const fullScore = (sets) => mean(sets.map((set) => {
        const metrics = calculateLunaAccuracyMetrics(input.labels, set.predictions, { codingEpisodeIds: input.codingEpisodeIds });
        const components = lunaAccuracySelectionComponents(metrics);
        if (components === null) {
            throw new Error(`Phase 2b set ${set.id} has undefined full-set score`);
        }
    ...
    const bySeedAndCase = (sets) => sets.map((set) => new Map(set.predictions.map((prediction) => [
        prediction.taskEpisodeId,
        prediction,
    ])));
    ...
    for (let iteration = 0; iteration < LUNA_ACCURACY_PHASE_TWO_B_BOOTSTRAP_ITERATIONS; iteration += 1) {
        const sampledIds = [];
        ...
        const sampledCodingIds = new Set(sampledIds.filter((id) => input.codingEpisodeIds.has(id)));
        const sampledSeedIndices = Array.from({ length: LUNA_ACCURACY_PHASE_TWO_B_EXPECTED_REPETITIONS }, () => Math.floor(random() * LUNA_ACCURACY_PHASE_TWO_B_EXPECTED_REPETITIONS));
        const averageSampledScore = (predictions) => {
            const values = [];
    ...
    const nonPositive = differences.filter((difference) => difference <= 0).length;
    const nonNegative = differences.filter((difference) => difference >= 0).length;
    return {
...
const assertPhaseTwoBDesign = (matrix, arms, runManifest) => {
    const expectedIds = [
        LUNA_ACCURACY_PHASE_TWO_B_BASELINE_VARIANT_ID,
        "p2b-b-chronological-16k-medium",
        "p2b-c-labeled-6k-high",
    ];
    if (matrix.variants.length !== 3 ||
        expectedIds.some((id, index) => matrix.variants[index]?.id !== id) ||
        matrix.variants.some((variant) => variant.repetitions !==
            LUNA_ACCURACY_PHASE_TWO_B_EXPECTED_REPETITIONS) ||
        arms.length !== 3 ||
        arms.some((arm, index) => arm.id !== expectedIds[index] ||
            arm.variantId !== expectedIds[index] ||
            arm.architecture !== "single_call") ||
        runManifest.scheduleSeed !==
            LUNA_ACCURACY_PHASE_TWO_B_BOOTSTRAP_SEED) {
        throw new Error("Phase 2b analysis requires the exact preregistered matrix, arms, and schedule seed");
    }
};
/**
 ...
 */
export const buildLunaAccuracyPhaseTwoBSelection = (input) => {
    if (input.model !== "openai/gpt-5.6-luna") {
        throw new Error("Phase 2b selection requires the pinned openai/gpt-5.6-luna model");
    }
    if (input.episodes.length === 0 ||
        input.episodes.some((episode) => episode.split !== "validation")) {
        throw new Error("Phase 2b selection requires a non-empty validation-only dataset");
    }
    ...
    const jobByCallKey = new Map(schedule.map((job) => [lunaAccuracyCallKey(job, "single"), job]));
    for (const call of input.calls) {
        ...
        if (!job || !variant || !episode || call.stage !== "single") {
            throw new Error(`Phase 2b call is not a preregistered single-call job: ${call.key}`);
        }
        ...
        if (call.promptHash !== expectedPromptHash ||
            call.transport?.providerRequestHash !== expectedProviderRequestHash) {
            throw new Error(`Phase 2b call prompt or provider request hash does not match bound runtime inputs: ${call.key}`);
        }
    }
    ...
    const expectedDistinctness = auditLunaAccuracyTreatmentDistinctness({
        model: input.model,
        profile: input.profile,
        cards: input.cards,
        episodes: input.episodes,
    ...
    if (contentHash(distinctnessAudit) !== contentHash(expectedDistinctness) ||
        distinctnessAudit.equivalentPairs.length > 0 ||
        distinctnessAudit.equivalenceClasses.length > 0) {
        throw new Error("Phase 2b distinctness audit is stale or contains provider-identical treatments");
    }
    ...
    const baselineSets = setsByVariant.get(LUNA_ACCURACY_PHASE_TWO_B_BASELINE_VARIANT_ID);
    const comparisons = [
        "p2b-b-chronological-16k-medium",
        "p2b-c-labeled-6k-high",
    ].map((challengerVariantId) => compareLunaAccuracyPhaseTwoBPaired({
        episodes: input.episodes,
        labels: input.labels,
        codingEpisodeIds,
        baselineSets,
...
export const validateLunaAccuracyPhaseTwoBSelection = (input) => {
    ...
    if (contentHash(rebuilt) !== contentHash(input.selection)) {
        throw new Error("Phase 2b selection artifact does not match its bound raw inputs");
    }
    return rebuilt;
};
