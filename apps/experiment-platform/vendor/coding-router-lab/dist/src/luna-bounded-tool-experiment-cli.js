...
import { evaluateLunaDistributionalPredictions, } from "./luna-distributional.js";
import { runLunaBoundedRepositoryClassification, } from "./luna-bounded-tool-harness.js";
import { fetchModelPrices } from "./cost.js";
import { contentHash } from "./hash.js";
import { readJsonl } from "./jsonl.js";
import { LUNA_ACCURACY_MODEL, LUNA_ACCURACY_PROVIDER, LUNA_ACCURACY_PROVIDER_SLUG, } from "./luna-accuracy-openrouter.js";
import { getOpenRouterKeyStatus } from "./openrouter.js";
import { validateAreaCards, validateBenchmarkDataset, validateRepositoryProfile, } from "./validation.js";
const ALL_ARMS = [
    "no_repository",
    "static_snippets",
    "candidate_read",
...
const value = (args, name) => args.values.get(name)?.at(-1);
...
const commaList = (raw) => raw
    ? raw
        .split(",")
        .map((item) => item.trim())
        .filter(Boolean)
    : [];
const parseSeeds = (raw) => {
    const values = commaList(raw ?? "181081").map(Number);
    if (values.length < 1 ||
        values.length > 20 ||
        values.some((seed) => !Number.isSafeInteger(seed) || seed < 0 || seed > 2_147_483_647) ||
...
const parseArms = (raw) => {
    const arms = (commaList(raw).length > 0
        ? commaList(raw)
        : [...ALL_ARMS]);
    if (arms.some((arm) => !ALL_ARMS.includes(arm)) ||
...
const candidatePathsFromEpisode = (episode) => {
...
const sameIds = (expected, actual, label) => {
    const expectedIds = new Set(expected.map((episode) => episode.id));
    if (expectedIds.size !== actual.size ||
        [...expectedIds].some((id) => !actual.has(id))) {
        throw new Error(`${label} episode IDs differ from control episodes`);
    }
};
const recordFile = (outputDirectory, arm, seed, episodeId) => path.join(outputDirectory, "records", arm, String(seed), `${episodeId}.json`);
const callStateFile = (outputDirectory, arm, seed, episodeId) => path.join(outputDirectory, "call-state", arm, String(seed), `${episodeId}.json`);
const sumProviderCallCost = (calls) => calls.reduce((sum, call) => sum + call.costUsd, 0);
const costForJobs = async (outputDirectory, jobs) => {
    let costUsd = 0;
    for (const job of jobs) {
        const completedFile = recordFile(outputDirectory, job.arm, job.seed, job.episodeId);
        if (await exists(completedFile)) {
            const completed = JSON.parse(await readFile(completedFile, "utf8"));
            costUsd += completed.prediction.costUsd;
            continue;
        }
        const partialFile = callStateFile(outputDirectory, job.arm, job.seed, job.episodeId);
        if (await exists(partialFile)) {
            const partial = JSON.parse(await readFile(partialFile, "utf8"));
            if (partial.arm !== job.arm ||
                partial.seed !== job.seed ||
                partial.taskEpisodeId !== job.episodeId ||
                !Number.isFinite(partial.totalCostUsd) ||
                partial.totalCostUsd < 0) {
                throw new Error(`Invalid provider call state for ${job.arm}/${job.seed}/${job.episodeId}`);
            }
...
const readRecords = async (outputDirectory) => {
        ...
        for (const seed of await readdir(armDirectory)) {
            ...
            for (const file of await readdir(seedDirectory)) {
                if (!file.endsWith(".json"))
                    continue;
                result.push(JSON.parse(await readFile(path.join(seedDirectory, file), "utf8")));
            }
        }
    }
    return result;
...
const aggregateMetrics = (values) => ({
    ...
    meanReciprocalRank: mean(values.map((item) => item.areaRanking.meanReciprocalRank)),
    conditionalBrier: mean(values.map((item) => item.areaProbability.conditionalBrier)),
    conditionalLogLoss: mean(values.map((item) => item.areaProbability.conditionalLogLoss)),
    calibrationError10Bins: mean(values.map((item) => item.areaProbability.calibrationError10Bins)),
    thresholdExactSet: mean(values.map((item) => item.thresholdPointFive.exactSetAccuracy)),
    totalProviderCalls: values.reduce((sum, item) => sum + item.usage.providerCalls, 0),
    totalCostUsd: values.reduce((sum, item) => sum + item.usage.costUsd, 0),
...
const analyze = async (input) => {
    ...
    for (const arm of input.arms) {
        ...
        for (const seed of input.seeds) {
            const selected = records.filter((record) => record.arm === arm && record.seed === seed);
            if (selected.length !== input.labels.length) {
                throw new Error(`${arm}/${seed} has ${selected.length} records; expected ${input.labels.length}`);
            }
        ...
        const toolRecords = records.filter((record) => record.arm === arm && record.trace.toolSession);
        let toolCases = 0;
        ...
        for (const record of toolRecords) {
            ...
            if (!label)
                continue;
            const exposedPaths = new Set(record.trace.toolSession.toolCallRecords.flatMap((call) => call.resultPaths));
            const readPaths = new Set(record.trace.toolSession.toolCallRecords
                .filter((call) => call.name === "read_repository_excerpt" && call.ok)
                .flatMap((call) => call.resultPaths));
            const relevant = label.relevantPaths;
            toolCases += 1;
            if (relevant.length > 0) {
                const exposedMatches = relevant.filter((pathValue) => exposedPaths.has(pathValue)).length;
                const readMatches = relevant.filter((pathValue) => readPaths.has(pathValue)).length;
                oraclePathEligibleCases += 1;
                if (exposedMatches > 0)
                    casesWithAnyOracleExposedPath += 1;
                if (readMatches > 0)
                    casesWithAnyOracleReadPath += 1;
        ...
        perArm[arm] = {
            ...
            toolUse: toolCases === 0
                ...
                : {
                    ...
                    anyOracleRelevantPathRate: oraclePathEligibleCases === 0
                        ? 0
                        : casesWithAnyOracleExposedPath /
                            oraclePathEligibleCases,
                    meanOracleRelevantPathRecall: oraclePathEligibleCases === 0
                        ? 0
                        : totalOracleExposedPathRecall /
                            oraclePathEligibleCases,
                    anyOracleRelevantReadPathRate: oraclePathEligibleCases === 0
                        ? 0
                        : casesWithAnyOracleReadPath /
                            oraclePathEligibleCases,
                    meanOracleRelevantReadPathRecall: oraclePathEligibleCases === 0
                        ? 0
    ...
    await writeAtomic(path.join(input.outputDirectory, "analysis.json"), report);
    return report;
};
const preflightTools = async () => {
    const response = await fetch(`https://openrouter.ai/api/v1/models/${LUNA_ACCURACY_MODEL}/endpoints`);
    if (!response.ok) {
    ...
    const endpoint = raw.data.endpoints.find((candidate) => typeof candidate === "object" &&
        candidate !== null &&
        "tag" in candidate &&
...
const main = async () => {
    ...
    const profile = JSON.parse(await readFile(path.resolve(required(args, "--profile")), "utf8"));
    const cards = await readJsonl(path.resolve(required(args, "--areas")));
    const controlEpisodes = await readJsonl(path.resolve(required(args, "--control-episodes")));
    const labels = await readJsonl(path.resolve(required(args, "--labels")));
    validateRepositoryProfile(profile);
    const allowEmptyPositiveExamples = args.flags.has("--allow-empty-positive-examples");
    ...
    let selectedControl = controlEpisodes.filter((episode) => selectedIds.size === 0 || selectedIds.has(episode.id));
    if (caseLimit > 0)
        selectedControl = selectedControl.slice(0, caseLimit);
    ...
    const selectedLabels = labels.filter((label) => selectedIdSet.has(label.taskEpisodeId));
    ...
    if (args.command === "analyze") {
        console.log(JSON.stringify(await analyze({
            outputDirectory,
    ...
    const snippetEpisodes = arms.includes("static_snippets") ||
        arms.includes("integrated_contrastive")
        ? await readJsonl(path.resolve(required(args, "--snippet-episodes")))
        : [];
    const pathEpisodes = arms.includes("candidate_read")
        ? await readJsonl(path.resolve(required(args, "--path-episodes")))
        : [];
    ...
    if (arms.includes("static_snippets") ||
        arms.includes("integrated_contrastive")) {
    ...
    if (!price)
        throw new Error("Luna missing from OpenRouter model catalog");
    const preflight = await preflightTools();
    const scheduleVersion = "paired-hash-interleave-v1";
    const jobs = arms
        .flatMap((arm) => seeds.flatMap((seed) => selectedControl.map((episode) => ({
        arm,
        seed,
        episodeId: episode.id,
    }))))
        .sort((left, right) => contentHash({ scheduleVersion, ...left }).localeCompare(contentHash({ scheduleVersion, ...right })));
    const alreadyComplete = new Set();
    for (const job of jobs) {
        if (await exists(recordFile(outputDirectory, job.arm, job.seed, job.episodeId))) {
            alreadyComplete.add(`${job.arm}:${job.seed}:${job.episodeId}`);
        }
    }
    const pending = jobs.filter((job) => !alreadyComplete.has(`${job.arm}:${job.seed}:${job.episodeId}`));
    const plan = {
        schemaVersion: 1,
        ...
        maximumProviderCalls: pending.reduce((sum, job) => sum +
    ...
    const artifactsRoot = path.resolve(value(args, "--artifacts-root") ??
        path.join(process.cwd(), "artifacts"));
    const ledger = new BudgetLedger(path.join(artifactsRoot, "global-budget.json"), 200);
    ...
    try {
        ...
        const workers = Array.from({ length: Math.min(concurrency, pending.length) }, async () => {
            while (true) {
                ...
                const episode = job.arm === "static_snippets" ||
                    job.arm === "integrated_contrastive"
                    ...
                    : job.arm === "candidate_read"
                        ...
                        : control;
                const providerCalls = [];
                const stateFile = callStateFile(outputDirectory, job.arm, job.seed, job.episodeId);
                const writeCallState = async (status, error) => {
                ...
                await writeAtomic(recordFile(outputDirectory, job.arm, job.seed, job.episodeId), record);
        ...
        await writeAtomic(path.join(outputDirectory, "cost.json"), {
            ...
            meteringPolicy: "Per-call OpenRouter usage cost when present, otherwise API-equivalent catalog cost",
        });
