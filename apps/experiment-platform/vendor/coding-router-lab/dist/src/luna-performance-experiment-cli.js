...
import { analyzeLunaRetrievalClassificationFailures, buildEnrichedAreaCards, classifyLunaPerformanceCase, ensembleLunaDistributionalPredictions, fitLunaTemperatureCalibration, applyLunaTemperatureCalibration, summarizeLunaPerformanceArm, } from "./luna-performance-experiment.js";
import { evaluateLunaPerformanceRetrieval, LUNA_PERFORMANCE_EMBEDDING_MODEL, LUNA_PERFORMANCE_RETRIEVAL_VARIANTS, retrieveLunaPerformanceEvidence, } from "./luna-performance-retrieval.js";
import { callLunaOpenRouter, } from "./luna-bounded-tool-harness.js";
import { LUNA_ACCURACY_MODEL, } from "./luna-accuracy-openrouter.js";
import { serializeLunaAccuracyTaskContext } from "./luna-accuracy-context.js";
import { getOpenRouterKeyStatus, } from "./openrouter.js";
import { validateAreaCards, validateRepositoryProfile, } from "./validation.js";
...
const referenceExamples = async (repository, excludedIds) => {
    if (!repository.publicEpisodes || !repository.publicOfflineEvidence) {
        return {};
    }
    const episodes = await readJsonl(path.resolve(repository.publicEpisodes));
    const evidence = await readJsonl(path.resolve(repository.publicOfflineEvidence));
    const episodeById = new Map(episodes.map((episode) => [episode.id, episode]));
    const result = {};
    for (const item of evidence) {
        if (item.samplingKind !== "known" ||
            excludedIds.has(item.taskEpisodeId)) {
            continue;
        }
        const episode = episodeById.get(item.taskEpisodeId);
        if (!episode || episode.split !== "reference")
            continue;
...
const loadResources = async (config, episodeIds) => {
    const result = new Map();
    for (const repository of config.repositories) {
        const profile = JSON.parse(await readFile(path.resolve(repository.profile), "utf8"));
        const baselineCards = await readJsonl(path.resolve(repository.areas));
...
const retrievalFile = (outputDirectory, variant, episodeId) => path.join(outputDirectory, "retrieval", variant, `${episodeId}.json`);
const queryFile = (outputDirectory, episodeId) => path.join(outputDirectory, "queries", `${episodeId}.json`);
const classificationFile = (outputDirectory, armId, seed, episodeId) => path.join(outputDirectory, "classification", armId, String(seed), `${episodeId}.json`);
const querySchema = {
    type: "object",
...
const parseQuery = (content) => {
    ...
    return {
        searchQueries: [...new Set(raw.search_queries)],
        identifiers: [...new Set(raw.identifiers)],
        pathHints: [...new Set(raw.path_hints)],
    };
};
const generateQuery = async (input) => {
    ...
    if (!call.message.content)
        throw new Error("Luna returned no query");
...
const readQueryRecord = async (outputDirectory, episodeId) => {
    const file = queryFile(outputDirectory, episodeId);
    if (!(await exists(file)))
        return undefined;
    return JSON.parse(await readFile(file, "utf8"));
};
const totalRetrievalCost = async (outputDirectory, embeddingPrice) => {
    let total = 0;
    ...
    if (await exists(queryDirectory)) {
        for (const file of await readdir(queryDirectory)) {
            if (!file.endsWith(".json"))
                continue;
            const record = JSON.parse(await readFile(path.join(queryDirectory, file), "utf8"));
    ...
    if (await exists(retrievalRoot)) {
        for (const variant of await readdir(retrievalRoot)) {
            for (const file of await readdir(path.join(retrievalRoot, variant))) {
                if (!file.endsWith(".json"))
                    continue;
                const record = JSON.parse(await readFile(path.join(retrievalRoot, variant, file), "utf8"));
                total += maximumCallCost(embeddingPrice, record.provenance.embeddingInputTokens, 0);
            }
        }
    }
    return total;
};
const runRetrieval = async (args) => {
    ...
    const episodes = await readJsonl(path.resolve(config.episodes));
    const episodeIds = new Set(episodes.map((episode) => episode.id));
    const resources = await loadResources(config, episodeIds);
    const variants = (args.values.get("--variants")?.split(",").filter(Boolean) ??
        [...LUNA_PERFORMANCE_RETRIEVAL_VARIANTS]);
    if (variants.some((variant) => !LUNA_PERFORMANCE_RETRIEVAL_VARIANTS.includes(variant))) {
    ...
    const embeddingPrice = embeddingPrices.get(LUNA_PERFORMANCE_EMBEDDING_MODEL);
    if (!lunaPrice || !embeddingPrice)
        throw new Error("Missing model price");
    const maximumCostUsd = Number(required(args, "--maximum-cost-usd"));
    const previouslyRecordedCostUsd = await totalRetrievalCost(outputDirectory, embeddingPrice);
    const ledger = new BudgetLedger(path.resolve(args.values.get("--budget-ledger") ?? "artifacts/global-budget.json"), 200);
    ...
    if (!(await exists(path.join(outputDirectory, "retrieval-manifest.json")))) {
        await writeAtomic(path.join(outputDirectory, "retrieval-manifest.json"), manifest);
    }
    const before = await getOpenRouterKeyStatus();
    const sharedContext = {
        embeddingCache: new EmbeddingCache(path.resolve(args.values.get("--embedding-cache") ??
    ...
    try {
        ...
        const artifactTotalCostUsd = await totalRetrievalCost(outputDirectory, embeddingPrice);
        const incrementalCostUsd = Math.max(0, artifactTotalCostUsd - previouslyRecordedCostUsd);
        ...
        if (!settled) {
            const artifactTotalCostUsd = await totalRetrievalCost(outputDirectory, embeddingPrice);
            await ledger.settle(maximumCostUsd, Math.max(0, artifactTotalCostUsd - previouslyRecordedCostUsd));
        }
        throw error;
    }
};
const readMatrix = async (file) => {
    const matrix = JSON.parse(await readFile(path.resolve(file), "utf8"));
    ...
    for (const arm of matrix.arms) {
        if (ids.has(arm.id))
            throw new Error(`Duplicate arm ${arm.id}`);
        ids.add(arm.id);
        if (!LUNA_PERFORMANCE_RETRIEVAL_VARIANTS.includes(arm.retrievalVariant) ||
            arm.seeds.length < 1 ||
...
const classificationCost = async (outputDirectory, jobs) => {
    let total = 0;
    for (const job of jobs) {
        const file = classificationFile(outputDirectory, job.armId, job.seed, job.episodeId);
        if (!(await exists(file)))
            continue;
...
const runClassification = async (args) => {
    if (!args.flags.has("--confirm-external-run")) {
        throw new Error("Refusing classification calls without confirmation");
    }
    const config = await readConfig(required(args, "--config"));
    const matrix = await readMatrix(required(args, "--matrix"));
    const retrievalDirectory = path.resolve(required(args, "--retrieval-directory"));
    const outputDirectory = path.resolve(required(args, "--output-directory"));
    const episodes = await readJsonl(path.resolve(config.episodes));
    const episodeById = new Map(episodes.map((episode) => [episode.id, episode]));
    const resources = await loadResources(config, new Set(episodes.map((episode) => episode.id)));
    const price = (await fetchModelPrices()).get(LUNA_ACCURACY_MODEL);
    if (!price)
        throw new Error("Missing Luna price");
    const jobs = matrix.arms
        .flatMap((arm) => arm.seeds.flatMap((seed) => episodes.map((episode) => ({
        arm,
        seed,
        episodeId: episode.id,
    }))))
        .sort((left, right) => contentHash({
    ...
    const pending = [];
    for (const job of jobs) {
        if (!(await exists(classificationFile(outputDirectory, job.arm.id, job.seed, job.episodeId)))) {
            pending.push(job);
        }
    }
    const maximumCostUsd = Number(required(args, "--maximum-cost-usd"));
    ...
    const previouslyRecordedCostUsd = await classificationCost(outputDirectory, costJobs);
    const ledger = new BudgetLedger(path.resolve(args.values.get("--budget-ledger") ?? "artifacts/global-budget.json"), 200);
    await ledger.reserve(maximumCostUsd);
    await mkdir(outputDirectory, { recursive: true, mode: 0o700 });
    ...
    const before = await getOpenRouterKeyStatus();
    let settled = false;
    const concurrency = Math.max(1, Math.min(8, Number(args.values.get("--concurrency") ?? "4")));
    try {
        let cursor = 0;
        const workers = Array.from({ length: Math.min(concurrency, pending.length) }, async () => {
            while (true) {
                ...
                const repository = resources.get(episode.repositoryId);
                const retrieval = JSON.parse(await readFile(retrievalFile(retrievalDirectory, job.arm.retrievalVariant, episode.id), "utf8"));
                const cards = job.arm.areaCardVariant === "enriched"
                ...
                await writeAtomic(classificationFile(outputDirectory, job.arm.id, job.seed, episode.id), record);
                console.error(JSON.stringify({
                    completed: index + 1,
        ...
        const artifactTotalCostUsd = await classificationCost(outputDirectory, costJobs);
        const incrementalCostUsd = Math.max(0, artifactTotalCostUsd - previouslyRecordedCostUsd);
        await ledger.settle(maximumCostUsd, incrementalCostUsd);
        settled = true;
        const after = await getOpenRouterKeyStatus();
...
const readRetrievalResults = async (directory, variant, episodes) => Promise.all(episodes.map(async (episode) => JSON.parse(await readFile(retrievalFile(directory, variant, episode.id), "utf8"))));
const analyzeRetrieval = async (args) => {
    const config = await readConfig(required(args, "--config"));
    const directory = path.resolve(required(args, "--output-directory"));
    const episodes = await readJsonl(path.resolve(config.episodes));
    const labels = await readJsonl(path.resolve(config.labels));
    const development = new Set(config.developmentEpisodeIds);
    const classificationDirectory = args.values.get("--classification-directory");
    const classificationMatrixFile = args.values.get("--classification-matrix");
    if (Boolean(classificationDirectory) !== Boolean(classificationMatrixFile)) {
        throw new Error("Retrieval/classification diagnostics require both --classification-directory and --classification-matrix");
    }
    ...
    for (const variant of LUNA_PERFORMANCE_RETRIEVAL_VARIANTS) {
        ...
        const classificationArm = classificationMatrix?.arms.find((arm) => arm.retrievalVariant === variant &&
            arm.seeds.length === 1);
        if (classificationMatrix && !classificationArm) {
            throw new Error(`Classification matrix lacks a single-seed arm for ${variant}`);
        }
        const classificationPredictions = classificationArm && classificationDirectory
            ? (await readClassificationRecords(path.resolve(classificationDirectory), classificationArm, episodes)).map((record) => record.prediction)
            : undefined;
        const evaluate = (selected) => {
            const selectedResults = results.filter((result) => selected.has(result.taskEpisodeId));
            const selectedIds = new Set(selectedResults.map((result) => result.taskEpisodeId));
            const selectedLabels = labels.filter((label) => selectedIds.has(label.taskEpisodeId));
            return {
                ...
                ...(classificationPredictions
                    ? {
                        retrievalClassificationDiagnostics: analyzeLunaRetrievalClassificationFailures({
                            results: selectedResults,
                            labels: selectedLabels,
                            predictions: classificationPredictions.filter((prediction) => selectedIds.has(prediction.taskEpisodeId)),
                        }),
                    }
                    : {}),
            };
        ...
        const confirmation = new Set(episodes
        ...
        report[variant] = {
            ...
            byRepository: Object.fromEntries([...new Set(episodes.map((episode) => episode.repositoryId))].map((repositoryId) => [
                repositoryId,
                evaluate(new Set(episodes
...
const readClassificationRecords = async (outputDirectory, arm, episodes) => {
    const result = [];
    for (const seed of arm.seeds) {
        for (const episode of episodes) {
            result.push(JSON.parse(await readFile(classificationFile(outputDirectory, arm.id, seed, episode.id), "utf8")));
        }
    }
    return result;
};
const weightedMetrics = (values) => {
    const cases = values.reduce((sum, item) => sum + item.metrics.cases, 0);
    const known = values.reduce((sum, item) => sum + item.metrics.knownCases, 0);
    const byCases = (select) => cases === 0
        ? 0
        : values.reduce((sum, item) => sum + select(item.metrics) * item.metrics.cases, 0) / cases;
    const byKnown = (select) => known === 0
        ? 0
        : values.reduce((sum, item) => sum + select(item.metrics) * item.metrics.knownCases, 0) / known;
    const calls = values.reduce((sum, item) => sum + item.metrics.usage.providerCalls, 0);
    const cost = values.reduce((sum, item) => sum + item.metrics.usage.costUsd, 0);
    return {
        cases,
        knownCases: known,
        ...
        exactSetAtPointFive: byKnown((metrics) => metrics.thresholdPointFive.exactSetAccuracy),
        areaBrier: byKnown((metrics) => metrics.areaProbability.conditionalBrier),
        areaLogLoss: byKnown((metrics) => metrics.areaProbability.conditionalLogLoss),
        providerCalls: calls,
        totalCostUsd: cost,
        meanCostUsd: cases === 0 ? 0 : cost / cases,
    };
};
const metricsAcrossRepositories = (input) => {
    const values = [];
    for (const repositoryId of [
        ...new Set(input.episodes.map((episode) => episode.repositoryId)),
    ]) {
        const ids = new Set(input.episodes
            .filter((episode) => episode.repositoryId === repositoryId &&
        ...
        values.push({
            ...
            metrics: summarizeLunaPerformanceArm({
                labels: input.labels.filter((label) => ids.has(label.taskEpisodeId)),
                predictions: input.predictions.filter((prediction) => ids.has(prediction.taskEpisodeId)),
    ...
    return {
        ...
        byRepository: Object.fromEntries(values.map((value) => [value.repositoryId, value.metrics])),
    };
};
const calibrateAndEvaluateConfirmation = (input) => {
    ...
    for (const repositoryId of [
        ...
        const resource = input.resources.get(repositoryId);
        const repositoryIds = new Set(input.episodes
            .filter((episode) => episode.repositoryId === repositoryId)
            .map((episode) => episode.id));
        const developmentIds = new Set([...repositoryIds].filter((id) => input.development.has(id)));
        if (developmentIds.size === 0)
            continue;
        const calibration = fitLunaTemperatureCalibration({
            labels: input.labels.filter((label) => developmentIds.has(label.taskEpisodeId)),
            predictions: input.predictions.filter((prediction) => developmentIds.has(prediction.taskEpisodeId)),
            areaIds: resource.baselineCards.map((card) => card.areaId),
        });
        calibrations[repositoryId] = calibration;
        calibratedPredictions.push(...input.predictions
            .filter((prediction) => repositoryIds.has(prediction.taskEpisodeId))
            .map((prediction) => applyLunaTemperatureCalibration({
            prediction,
            calibration,
        })));
    }
...
const analyzeClassification = async (args) => {
    const config = await readConfig(required(args, "--config"));
    const matrix = await readMatrix(required(args, "--matrix"));
    const outputDirectory = path.resolve(required(args, "--output-directory"));
    const episodes = await readJsonl(path.resolve(config.episodes));
    ...
    const arms = {};
    for (const arm of matrix.arms) {
        const records = await readClassificationRecords(outputDirectory, arm, episodes);
        const predictionsBySeed = new Map(arm.seeds.map((seed) => [
            seed,
            records
                .filter((record) => record.seed === seed)
                .map((record) => record.prediction),
        ]));
        const perSeed = Object.fromEntries(arm.seeds.map((seed) => [
            String(seed),
            {
        ...
        if (arm.seeds.length >= 2) {
            const ensemblePredictions = episodes.map((episode) => ensembleLunaDistributionalPredictions({
                predictions: arm.seeds.map((seed) => predictionsBySeed
                    .get(seed)
                    .find((prediction) => prediction.taskEpisodeId === episode.id)),
...
const numericMetric = (aggregate, field) => {
...
const selectClassificationWinner = async (args) => {
    const config = await readConfig(required(args, "--config"));
    const matrix = await readMatrix(required(args, "--matrix"));
    const outputDirectory = path.resolve(required(args, "--output-directory"));
    const episodes = await readJsonl(path.resolve(config.episodes));
    const labels = await readJsonl(path.resolve(config.labels));
    const development = new Set(config.developmentEpisodeIds);
    const developmentEpisodes = episodes.filter((episode) => development.has(episode.id));
    const resources = await loadResources(config, new Set(episodes.map((episode) => episode.id)));
    const candidates = [];
    for (const arm of matrix.arms) {
        const records = await readClassificationRecords(outputDirectory, arm, developmentEpisodes);
        const perSeed = new Map(arm.seeds.map((seed) => [
            seed,
            records
                .filter((record) => record.seed === seed)
                .map((record) => record.prediction),
        ]));
        const predictions = arm.seeds.length === 1
            ? perSeed.get(arm.seeds[0])
            : developmentEpisodes.map((episode) => ensembleLunaDistributionalPredictions({
                predictions: arm.seeds.map((seed) => perSeed
                    .get(seed)
                    .find((prediction) => prediction.taskEpisodeId === episode.id)),
        ...
        candidates.push({
            armId: arm.id,
            predictionKind: arm.seeds.length === 1 ? "single_seed" : "ensemble",
            seeds: arm.seeds,
            aggregate,
            selectionLoss: areaBrier +
                0.35 * scopeBrier +
                0.15 * (1 - areaHitAt1) +
                0.25 * (1 - allGoldAt3),
            tieBreakers: {
                ...
                exactSetAtPointFive: numericMetric(aggregate, "exactSetAtPointFive"),
            },
        });
    }
    candidates.sort((left, right) => left.selectionLoss - right.selectionLoss ||
    ...
    const selection = {
        ...
        developmentCases: development.size,
        objective: "area_brier + 0.35*scope_brier + 0.15*(1-area_hit_at_1) + 0.25*(1-all_gold_top_3)",
        winner: candidates[0],
        candidates,
        matrixHash: contentHash(matrix),
    };
...
const plan = async (args) => {
    ...
    if (!matrixFile) {
        const estimatedTaskTokens = episodes.reduce((sum, episode) => sum + estimateTokens(episode.currentRequest), 0);
        console.log(JSON.stringify({
            command: "retrieve",
            cases: episodes.length,
    ...
    const calls = matrix.arms.reduce((sum, arm) => sum + arm.seeds.length * episodes.length, 0);
    console.log(JSON.stringify({
        command: "classify",
        cases: episodes.length,
        arms: matrix.arms,
...
const main = async () => {
    ...
    if (args.command === "retrieve")
        return runRetrieval(args);
    if (args.command === "classify")
        return runClassification(args);
    if (args.command === "analyze-retrieval")
        return analyzeRetrieval(args);
    ...
    throw new Error("Usage: luna-performance-experiment-cli <plan|retrieve|classify|select-classification|analyze-retrieval|analyze-classification> --config FILE ...; analyze-retrieval accepts optional paired --classification-directory and --classification-matrix");
};
main().catch((error) => {
