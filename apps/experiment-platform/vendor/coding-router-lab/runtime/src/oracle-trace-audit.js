...
const activeCommandSegments = (command) => splitShellCommand(commandProgram(command))
...
const isMutationCommand = (command) => {
    ...
    for (const segment of activeCommandSegments(command).filter(isExecutableCommandSegment)) {
        const name = commandName(segment);
        if (name && mutatingCommands.has(name))
            return true;
        if (/^(?:sed|perl)\s+-[^\s]*i/u.test(segment))
            return true;
        /*
        ...
        if (/^git\s+branch\b/u.test(segment)) {
            ...
            const mutatingOption = /(?:^|\s)(?:-[dDmMcCf]|--delete|--move|--copy|--edit-description|--set-upstream-to|--unset-upstream|--track|--no-track|--create-reflog|--force|--recurse-submodules)(?:[=\s]|$)/u.test(argumentsText);
            ...
            for (let index = 0; index < tokens.length; index += 1) {
                ...
                if (readOnlyOptionsWithValues.has(option) &&
                    !token.includes("=")) {
                    index += 1;
                }
            }
            if (mutatingOption || positionalArgument)
                return true;
        }
        if (/^git\s+(?:add|checkout|switch|reset|clean|commit|merge|rebase|cherry-pick|revert|fetch|pull|push|clone|init|gc|repack)\b/u.test(segment)) {
            return true;
        }
        if (/^git\s+tag\b/u.test(segment)) {
            const argumentsText = segment.replace(/^git\s+tag\b/u, "").trim();
            const mutatingOption = /(?:^|\s)(?:-[adfsuemF]|--annotate|--delete|--force|--sign|--local-user|--edit|--message|--file|--cleanup|--create-reflog)(?:[=\s]|$)/u.test(argumentsText);
            const explicitReadOnlyMode = /(?:^|\s)(?:-l|--list|-v|--verify)(?:[=\s]|$)/u.test(argumentsText);
            const readOnlyOptionsWithValues = new Set([
                "--list",
                "--contains",
                "--no-contains",
        ...
        if (/^(?:npm|pnpm|yarn|bun)\s+(?:install|publish|add|remove)\b/u.test(segment)) {
            return true;
        }
    }
    return false;
};
const referencesFutureRefs = (command) => /\bgit\s+(?:show|log|rev-list|for-each-ref|show-ref|branch|tag)\b[^;&|\n]*(?:--all|refs\/)|\bgit\s+(?:reflog|fsck)\b|\.git\/(?:objects|refs|logs)/iu.test(command);
const referencesExternalPath = (command) => /(?:^|[\s"'=(])(?:\/home\/|\/Users\/|~\/|\.\.\/)/u.test(commandProgram(command));
const isolatedAllRefsEvidence = (output, expectedSnapshot) => {
    ...
    if (commitLines.length === 0)
        return true;
    return commitLines.every((line) => expectedSnapshot.toLowerCase().startsWith((/^[0-9a-f]{7,40}/iu.exec(line)?.[0] ?? "").toLowerCase()));
};
...
const hashCandidates = (command) => [...command.matchAll(/\b[0-9a-f]{7,40}\b/giu)].map((match) => match[0]);
const isIsolatedWorkspacePath = (value) => /\/\.codex\/tmp\/coding-router-lab\/coding-router-codex-[^/]+\/workspace(?:\/|$)/u.test(value);
const observedWorkingDirectories = (commands) => commands
    .filter((command) => {
    ...
    .filter((value) => Boolean(value?.startsWith("/")));
const safeSnapshotOnlyExternalPath = (command, observedWorkingDirectoriesForTrace) => {
    if (observedWorkingDirectoriesForTrace.length === 0 ||
        observedWorkingDirectoriesForTrace.some((directory) => !isIsolatedWorkspacePath(directory))) {
        return false;
    }
    const program = commandProgram(command);
    if (/(?:^|[\s"'=(])\.\.\//u.test(program))
        return false;
    const externalPaths = program.match(/(?:\/home\/[^/\s"'=()]+|\/Users\/[^/\s"'=()]+|~)\/[^\s"'=()]*/gu) ?? [];
    return (externalPaths.length > 0 &&
        externalPaths.every((externalPath) => /^(?:\/home\/[^/]+|\/Users\/[^/]+|~)\/\.codex\/tmp\/coding-router-lab\/coding-router-codex-[^/]+\/workspace(?:\/|$)/u.test(externalPath)));
};
const parseCommandEvents = (trace, taskEpisodeId, sourceTrace, parseErrors) => {
    const commands = [];
...
export const auditOracleTraces = async (input) => {
    const episodesById = new Map(input.episodes.map((episode) => [episode.id, episode]));
    const findings = {
        missingTraceSources: [],
        missingEpisodes: [],
    ...
    if (input.traceSources.length !== input.canonicalPasses.length) {
        throw new Error(`Trace source map has ${input.traceSources.length} records for ${input.canonicalPasses.length} canonical passes`);
    }
    const labelIds = new Set(input.labels.map((label) => label.taskEpisodeId));
    const passIds = new Set(input.canonicalPasses.map((label) => label.taskEpisodeId));
    if (labelIds.size !== passIds.size ||
    ...
    for (const [index, source] of input.traceSources.entries()) {
        ...
        if (pass.taskEpisodeId !== source.taskEpisodeId) {
            throw new Error(`Trace source ${index + 1} maps ${source.taskEpisodeId} to canonical pass ${pass.taskEpisodeId}`);
        }
        ...
        let trace;
        try {
            trace = JSON.parse(await readFile(source.sourceTrace, "utf8"));
        }
        catch {
        ...
        const parsed = parseCommandEvents(trace, source.taskEpisodeId, source.sourceTrace, findings.parseErrors);
        ...
        const observedHead = completedOutputs
            .flatMap((output) => output.split(/\r?\n/u))
            .map((line) => line.trim())
            .map((line) => /^([0-9a-f]{7,40})\s+\(?(?:grafted,\s*)?HEAD\)?\b/iu.exec(line)?.[1])
            .find(Boolean) ?? null;
        if (observedHead &&
            !episode.repositorySnapshot.toLowerCase().startsWith(observedHead.toLowerCase())) {
        ...
        const observedWorkingDirectoriesForTrace = observedWorkingDirectories(parsed.commands);
        for (const command of parsed.commands) {
            ...
            if (referencesFutureRefs(command.command)) {
                findings.futureRefCommands.push({
                    ...
                    sourceTrace: source.sourceTrace,
                    command: command.command,
                    isolatedAllRefsEvidence: isolatedAllRefsEvidence(command.output, episode.repositorySnapshot),
                });
            }
            if (referencesExternalPath(command.command) &&
                !(source.isolationMode === "snapshot-only-clone" &&
                    safeSnapshotOnlyExternalPath(command.command, observedWorkingDirectoriesForTrace))) {
            ...
            if (source.isolationMode === "shared-object-worktree") {
                for (const hash of hashCandidates(command.command)) {
                    if (episode.repositorySnapshot.startsWith(hash))
                        continue;
                    try {
    ...
    const unsafeFutureRefs = findings.futureRefCommands.filter((finding) => !finding.isolatedAllRefsEvidence);
    const gates = [
        {
            gate: "all-canonical-traces-present",
            passed: tracesAudited === input.canonicalPasses.length &&
        ...
        },
        {
            ...
            passed: webSearchEvents === 0 && findings.networkEvents.length === 0,
        ...
        },
        {
            ...
            passed: findings.externalPathCommands.length === 0 &&
