...
export const GITNEXUS_EXPERIMENT_VERSION = "gitnexus-retrieval-experiment-v1";
export const GITNEXUS_VERSION = "1.6.10-rc.205";
export const GITNEXUS_FUSION_VERSION = "weighted-rrf-current-1.0-gitnexus-1.2-diverse-v1";
const asPositiveInteger = (value, fallback) => typeof value === "number" && Number.isFinite(value) && value > 0
    ? Math.floor(value)
    : fallback;
const taskAwareText = (episode) => redactText([
    episode.taskAnchor,
    ...
    .join("\n")
    .slice(-24_000)).text;
const meaningfulLine = (text) => text
    .split("\n")
    .map((line) => line.trim())
    .find((line) => line.length >= 3 &&
    !/^problem context:?$/iu.test(line) &&
    !/^#{1,6}\s/u.test(line)) ?? text.trim().slice(0, 240);
const identifierHints = (text) => {
    const found = new Set();
    for (const match of text.matchAll(/`([^`\n]{2,100})`/gu)) {
        found.add(match[1].trim());
    }
    for (const match of text.matchAll(/(?:--[a-z0-9][a-z0-9-]*|[A-Za-z_$][\w$]*(?:\.[A-Za-z_$][\w$]*)+|[A-Za-z_$][\w$]*\([^)\n]{0,30}\))/gu)) {
...
export const buildGitNexusSearchQuery = (episode) => {
...
export const buildGitNexusTaskContext = (episode) => taskAwareText(episode).slice(-12_000);
const collectLocations = (value, result) => {
    if (Array.isArray(value)) {
        for (const item of value)
            collectLocations(item, result);
        return;
    }
...
const safeRepositoryPath = (candidate) => {
...
export const parseGitNexusDefinitions = (output) => {
    const locations = [];
    ...
    for (const [rank, location] of locations.entries()) {
        ...
        const endLine = Math.max(startLine, asPositiveInteger(location.endLine, startLine + 20));
...
const readDirectoryBytes = async (directory) => {
    try {
        ...
        for (const entry of entries) {
            ...
            if (entry.isDirectory())
                total += await readDirectoryBytes(file);
...
const parseTimeVerbose = (text) => {
    const maximumResidentSet = text.match(/Maximum resident set size \(kbytes\):\s*(\d+)/u);
...
export const analyzeWithGitNexus = async (options, snapshot) => {
    const safeId = snapshot.slice(0, 12);
    await mkdir(options.artifactDirectory, { recursive: true, mode: 0o700 });
    const logFile = path.join(options.artifactDirectory, `analyze-${safeId}.log`);
    const resourceFile = path.join(options.artifactDirectory, `analyze-${safeId}.time`);
    ...
    const result = await execFileAsync("/usr/bin/time", [
        "-v",
        "-o",
        resourceFile,
        options.binary,
    ...
    await writeFile(logFile, [result.stdout, result.stderr].filter(Boolean).join("\n"), { mode: 0o600 });
...
export const queryGitNexus = async (input) => {
    ...
    const started = performance.now();
    const result = await execFileAsync(input.binary, [
        "query",
        "-r",
        input.indexAlias,
        "-q",
...
const likelyAreas = (filePath, symbols, cards) => {
    ...
    return cards
        ...
        const symbolMatches = card.symbolAnchors.filter((anchor) => symbolsLower.includes(anchor.toLowerCase())).length;
...
const clipLines = (text, startLine, endLine, maximumCharacters = 1_240) => {
    ...
    while (from < to) {
        ...
        if (to - requestedEnd > center - from)
            to -= 1;
        else
            from += 1;
    }
...
const pathNoisePenalty = (filePath) => /(?:^|\/)(?:testdata|fixtures?|examples?)(?:\/|$)|(?:^|\/)\.changelog|(?:^|\/)changelog|(?:^|\/)zz_generated|\.generated\.|generated\.openapi/iu.test(filePath)
    ? 0.15
...
export const materializeGitNexusSnippets = async (input) => {
    ...
    for (const [rank, candidate] of ranked.entries()) {
        ...
        const clipped = clipLines(source, definition.startLine, definition.endLine);
        const symbols = [
            ...new Set(candidate.definitions
                .map((item) => item.name)
...
const pathTokens = (filePath) => new Set(filePath.toLowerCase().split(/[^a-z0-9]+/u).filter(Boolean));
...
const diverseTop = (candidates, scores, limit) => {
    ...
    while (selected.length < limit && remaining.length > 0) {
        const next = remaining
            ...
            const redundancy = selected.length === 0
                ? 0
                : Math.max(...selected.map((item) => similarity(candidate.path, item.path)));
            return {
                candidate,
                score: 0.95 * ((scores.get(candidate.path) ?? 0) / maximum) -
                    0.05 * redundancy -
...
export const buildGitNexusOnlyResult = (input) => {
    const candidates = [...input.snippets]
        .sort((left, right) => right.scores.lexical -
...
export const fuseCurrentHybridWithGitNexus = (input) => {
    ...
    for (const filePath of byPath.keys()) {
        scores.set(filePath, (currentRank.has(filePath)
    ...
    const candidates = [...byPath.values()]
        .sort((left, right) => (scores.get(right.path) ?? 0) - (scores.get(left.path) ?? 0) ||
        left.path.localeCompare(right.path))
        .slice(0, 12)
        .map((candidate) => ({
        ...candidate,
