...
const gitPathExists = async (repository, snapshot, candidate) => {
    try {
...
export const freezeRegistry = async (profile, cards, episodes, repository) => {
    validateRepositoryProfile(profile);
    validateAreaCards(cards, profile);
    if (new Set(cards.map((card) => card.registryVersion)).size !== 1)
        throw new Error("Area Cards must share one registryVersion");
    ...
    for (const card of cards) {
        ...
        const existingPathAnchors = [], missingPathAnchors = [];
        for (const anchor of card.pathAnchors) {
            if (!repository || await gitPathExists(repository, profile.snapshot, anchor))
                existingPathAnchors.push(anchor);
            else
                missingPathAnchors.push(anchor);
        }
        areas.push({
