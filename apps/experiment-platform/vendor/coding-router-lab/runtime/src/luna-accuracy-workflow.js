...
import { estimateTokens, maximumCallCost } from "./cost.js";
import { buildLunaAccuracyPrompt, validateLunaAccuracyMatrixV2, } from "./luna-accuracy-context.js";
import { assertLunaAccuracyTreatmentsDistinct, auditLunaAccuracyTreatmentDistinctness, } from "./luna-accuracy-distinctness.js";
import { assertLunaAccuracyFreezeInputs, validateLunaAccuracyFreezeRecord, } from "./luna-accuracy-design.js";
import { contentHash } from "./hash.js";
import { preflightLunaAccuracyOpenRouter, } from "./luna-accuracy-openrouter.js";
import { buildLunaAccuracyJobSchedule, buildLunaAccuracyRunManifest, normalizeLunaAccuracyArms, normalizeLunaAccuracyConcurrency, runLunaAccuracyExperiment, } from "./luna-accuracy-runner.js";
import { validateAreaCards, validateEpisodes, validateRepositoryProfile, } from "./validation.js";
export const LUNA_ACCURACY_WORKFLOW_MODEL = "openai/gpt-5.6-luna";
/**
 ...
 */
export const LUNA_ACCURACY_REQUEST_OVERHEAD_TOKENS = 2_000;
export const LUNA_ACCURACY_VERIFY_PRIOR_TOKENS = 1_000;
export const LUNA_ACCURACY_REVISE_PRIOR_TOKENS = 2_000;
const stageCount = (architecture) => architecture === "proposal_verify_revise" ? 3 : 1;
const assertModel = (model) => {
    if (model !== LUNA_ACCURACY_WORKFLOW_MODEL) {
        throw new Error(`Luna accuracy workflow requires ${LUNA_ACCURACY_WORKFLOW_MODEL}`);
    }
};
export const validateLunaAccuracyDatasetRole = (episodes, role, freeze) => {
    if (!episodes.length) {
        throw new Error("Luna accuracy workflow requires at least one episode");
    }
    if (role === "validation") {
        if (episodes.some((episode) => episode.split !== "validation")) {
            throw new Error("Validation selection may contain only validation episodes");
        }
        if (freeze !== undefined) {
    ...
    if (role === "locked_test") {
        ...
        if (!freeze) {
            throw new Error("Locked-test evaluation requires a complete freeze record");
        }
    ...
    if (episodes.some((episode) => episode.split === "reference")) {
        throw new Error("Burned development evaluation cannot include reference examples");
    }
};
export const planLunaAccuracyWorkflow = (input) => {
    ...
    validateLunaAccuracyDatasetRole(input.episodes, input.datasetRole ?? "validation", input.freeze);
    ...
    for (const job of schedule) {
        ...
        const baseInputTokens = estimateTokens(`${prompt.system}\n${prompt.user}`) +
            LUNA_ACCURACY_REQUEST_OVERHEAD_TOKENS;
        const perJobInputTokens = job.architecture === "proposal_verify_revise"
            ? baseInputTokens * 3 +
        ...
        maximumEstimatedCallTokens = Math.max(maximumEstimatedCallTokens, baseInputTokens +
    ...
    const byArm = [...armPlans.values()]
        ...
        .map((arm) => ({
        ...arm,
        projectedMaximumCostUsd: maximumCallCost(input.price, arm.estimatedInputTokens, arm.reservedOutputTokens),
    }));
    const estimatedInputTokens = byArm.reduce((sum, arm) => sum + arm.estimatedInputTokens, 0);
    const reservedOutputTokens = byArm.reduce((sum, arm) => sum + arm.reservedOutputTokens, 0);
    const warnings = [];
    if (input.price.contextLength > 0 &&
        maximumEstimatedCallTokens > input.price.contextLength) {
        warnings.push("At least one conservatively estimated call exceeds the catalog context length.");
    }
    if (input.datasetRole === "burned_development") {
        warnings.push("Burned development results are for harness debugging only and cannot select the product configuration.");
    }
    if (input.datasetRole === "locked_test") {
        warnings.push("This is the one-time locked-test path; do not rerun it or tune from case-level results.");
    }
    return {
        schemaVersion: 1,
        ...
        projectedMaximumCostUsd: maximumCallCost(input.price, estimatedInputTokens, reservedOutputTokens),
        price: {
        ...
        byArm,
        assumptions: {
            requestOverheadTokensPerCall: LUNA_ACCURACY_REQUEST_OVERHEAD_TOKENS,
...
const providerUsage = (status) => status.accountUsageUsd ?? status.usageUsd;
const assertProviderCapacity = (status, reservationUsd) => {
    const remaining = [
    ...
    if (remaining.length > 0 &&
        ...
        throw new Error(`OpenRouter has only $${Math.min(...remaining).toFixed(6)} available; reservation requires $${reservationUsd.toFixed(6)}`);
    }
};
const readCallRecords = async (callStateDirectory) => {
    let entries;
    try {
    ...
    for (const entry of entries.sort()) {
        if (!entry.endsWith(".json"))
            continue;
        records.push(JSON.parse(await readFile(path.join(callStateDirectory, entry), "utf8")));
    }
    return records;
};
const recordedCallCost = (record, matrix, price) => {
    if (record.prediction.costUsd !== undefined &&
        Number.isFinite(record.prediction.costUsd)) {
        return record.prediction.costUsd;
    }
    const variant = matrix.variants.find((candidate) => candidate.id === record.variantId);
    if (!variant) {
        throw new Error(`Cannot cost completed call for missing variant ${record.variantId}`);
    }
    const inputTokens = record.prediction.inputTokens ??
        (record.prediction.inputCharacters === undefined
            ? LUNA_ACCURACY_REQUEST_OVERHEAD_TOKENS
            : Math.ceil(record.prediction.inputCharacters / 3.5) +
                LUNA_ACCURACY_REQUEST_OVERHEAD_TOKENS);
    const outputTokens = record.prediction.outputTokens ?? variant.maxOutputTokens;
    return maximumCallCost(price, inputTokens, outputTokens);
};
const lockedTestMarkerPath = (runDirectory) => path.join(runDirectory, "..", ".locked-test-evaluation.json");
const beginLockedTestEvaluation = async (input) => {
    ...
    try {
        ...
        if (marker.schemaVersion !== 1 ||
            ...
            throw new Error("A different locked-test evaluation has already been started");
        }
        if (marker.status === "completed") {
            const existingManifestPath = path.join(input.runDirectory, "manifest.lock.json");
            try {
                const existing = JSON.parse(await readFile(existingManifestPath, "utf8"));
                if (existing.configurationHash !== expected.runConfigurationHash ||
                    existing.inputHash !== expected.runInputHash) {
                    throw new Error("Completed locked-test marker does not match this run");
                }
            }
            catch (manifestError) {
                if (manifestError.code === "ENOENT") {
                    throw new Error("Locked-test evaluation is already complete and cannot be rerun");
                }
                throw manifestError;
            }
            const completed = await readCallRecords(path.join(input.runDirectory, "private", "call-state"));
            if (completed.length !== input.expectedManifest.counts.expectedCalls ||
                new Set(completed.map((record) => record.key)).size !==
                    completed.length ||
                completed.some((record) => record.inputHash !== input.expectedManifest.inputHash)) {
                throw new Error("Locked-test evaluation is already complete; incomplete artifacts cannot be re-executed");
            }
        }
...
const completeLockedTestEvaluation = async (markerPath, marker) => {
...
export const runBudgetedLunaAccuracyWorkflow = async (input) => {
    assertModel(input.model);
    if (!Number.isFinite(input.reservationUsd) ||
    ...
    if (input.reservationUsd + 1e-9 < plan.projectedMaximumCostUsd) {
        throw new Error(`Reservation $${input.reservationUsd.toFixed(6)} is below the projected maximum $${plan.projectedMaximumCostUsd.toFixed(6)}`);
    }
    const treatmentDistinctness = auditLunaAccuracyTreatmentDistinctness({
        model: input.model,
        profile: input.profile,
        cards: input.cards,
        episodes: input.episodes,
    ...
    assertLunaAccuracyTreatmentsDistinct(treatmentDistinctness, input.allowEquivalentTreatmentReplicates ?? false);
    const transportPreflight = await (input.preflightTransport ?? preflightLunaAccuracyOpenRouter)();
    ...
    const existingKeys = new Set((await readCallRecords(callStateDirectory)).map((record) => record.key));
    ...
    try {
        if (!before) {
            ...
            const newlyCompleted = completed.filter((record) => !existingKeys.has(record.key));
            const recordedCallUsd = newlyCompleted.reduce((sum, record) => sum + recordedCallCost(record, input.matrix, input.price), 0);
            let providerMeteredUsd = 0;
            try {
                const after = await input.getProviderStatus();
                providerMeteredUsd = Math.max(0, providerUsage(after) - providerUsage(before));
            }
            catch {
    ...
    if (lockedTestMarker) {
        await completeLockedTestEvaluation(lockedTestMarker.path, lockedTestMarker.marker);
    }
    return {
        run,
        transportPreflight,
