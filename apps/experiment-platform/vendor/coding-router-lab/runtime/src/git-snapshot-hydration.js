...
const execute = (command, args, input = "", environment = process.env, timeoutMs = 900_000) => new Promise((resolve, reject) => {
...
const missingObjectIds = async (repository, snapshot) => {
    ...
    if (result.exitCode !== 0) {
        throw new Error(`Could not enumerate missing snapshot objects: ${result.stderr}`);
    }
    return [
...
const promisorRemote = async (repository) => {
    const result = await execute("git", [
        "-C",
        repository,
        "config",
...
const transientGitTransportFailure = (detail) => /SSL connection timeout|could not fetch .* from promisor remote|RPC failed|remote end hung up|HTTP (?:408|429|5\d\d)/iu.test(detail);
/**
 ...
 */
export const hydratePromisedSnapshotBlobs = async (repository, snapshot) => {
    ...
    if (!remote) {
        throw new Error(`Snapshot ${snapshot} has ${missing.length} missing objects but no promisor remote`);
    }
    let lastError = "";
    ...
    throw new Error(`Could not hydrate promised objects for snapshot ${snapshot}: ${lastError}`);
};
