import { runCodexReadOnly } from "./codex-harness.js";
...
const renderCards = (cards) => cards.map((card) => JSON.stringify(card)).join("\n");
export const buildOraclePrompt = (episode, cards) => `You are labeling a coding task for a repository-specific runtime router.
...
[AREA CARDS]
${renderCards(cards)}`;
export const runSilverOracle = async (input) => {
    ...
    for (let pass = 0; pass < passes; pass += 1) {
        ...
        if (result.exitCode !== 0 || !result.finalMessage) {
            throw new Error(`Codex oracle failed (${result.exitCode}). stderr: ${result.stderr.slice(-4_000)} stdout: ${result.stdout.slice(-8_000)}`);
        }
        ...
        for (const id of parsed.selectedAreaIds)
            if (!allowed.has(id))
                throw new Error(`Oracle invented area ID ${id}`);
        if (parsed.known !== (parsed.selectedAreaIds.length > 0))
            throw new Error("Oracle known flag conflicts with selected areas");
        results.push({
