...
export const LUNA_REPOSITORY_RETRIEVAL_VERSION = "luna-repository-retrieval-v2-task-aware-lexical";
...
export const LUNA_REPOSITORY_RETRIEVAL_DEFAULT_MAXIMUM_SNIPPET_CHARACTERS = 640;
...
const compareLexically = (left, right) => left < right ? -1 : left > right ? 1 : 0;
const positiveInteger = (value, fallback, minimum, maximum, field) => {
...
const resolveOptions = (options) => {
    ...
    return {
        mode: options.mode,
        maximumCharacters: positiveInteger(options.maximumCharacters, LUNA_REPOSITORY_RETRIEVAL_DEFAULT_MAXIMUM_CHARACTERS, MINIMUM_MAXIMUM_CHARACTERS, MAXIMUM_MAXIMUM_CHARACTERS, "maximumCharacters"),
        maximumFiles: positiveInteger(options.maximumFiles, LUNA_REPOSITORY_RETRIEVAL_DEFAULT_MAXIMUM_FILES, MINIMUM_MAXIMUM_FILES, MAXIMUM_MAXIMUM_FILES, "maximumFiles"),
        maximumSnippetCharacters: positiveInteger(options.maximumSnippetCharacters, LUNA_REPOSITORY_RETRIEVAL_DEFAULT_MAXIMUM_SNIPPET_CHARACTERS, MINIMUM_SNIPPET_CHARACTERS, MAXIMUM_SNIPPET_CHARACTERS, "maximumSnippetCharacters"),
    };
};
const assertSafeSnapshot = (snapshot) => {
    if (!/^[0-9a-f]{7,64}$/u.test(snapshot)) {
        throw new Error("repositorySnapshot must be a lowercase hexadecimal object ID; symbolic revisions are not accepted");
    }
};
const runGitBuffer = async (repository, args, maximumBytes) => {
    let lastError;
...
const resolveExactCommit = async (repository, snapshot) => {
    assertSafeSnapshot(snapshot);
    const resolved = (await runGitBuffer(repository, ["rev-parse", "--verify", `${snapshot}^{commit}`], 1024 * 1024)).toString("utf8").trim();
    ...
    if (!resolved.startsWith(snapshot)) {
        throw new Error(`Resolved repository snapshot ${resolved} does not match requested ${snapshot}`);
    }
    return resolved;
};
const treeHash = async (repository, resolvedSnapshot) => {
    const resolved = (await runGitBuffer(repository, ["rev-parse", "--verify", `${resolvedSnapshot}^{tree}`], 1024 * 1024)).toString("utf8").trim();
    if (!/^[0-9a-f]{40,64}$/u.test(resolved)) {
        throw new Error(`Could not resolve tree for repository snapshot ${resolvedSnapshot}`);
    }
    return resolved;
};
const repositoryPaths = async (repository, resolvedSnapshot) => {
    const output = await runGitBuffer(repository, ["ls-tree", "-r", "-z", "-l", "--full-tree", resolvedSnapshot], 64 * 1024 * 1024);
    const files = output
        .toString("utf8")
        .split("\0")
        .filter(Boolean)
        ...
        if (!Number.isSafeInteger(size) || size < 0) {
            throw new Error(`Repository tree contains an invalid blob size: ${filePath}`);
        }
        return { path: filePath, size };
    })
        .sort((left, right) => compareLexically(left.path, right.path));
    if (files.some(({ path }) => path.includes("\u0000") || path.includes("\n"))) {
...
export const extractLunaRepositoryRetrievalTerms = (currentRequest) => [...new Set(rawQueryTerms(currentRequest))]
    .sort(compareLexically)
    .slice(0, MAXIMUM_QUERY_TERMS);
const taskAwareQueryText = (episode) => [
    episode.taskAnchor,
...
const buildTaskAwareQuery = (episode) => {
    ...
    for (const size of [4, 3, 2]) {
        for (let index = 0; index + size <= filtered.length; index += 1) {
            ...
            if (new Set(phrase.split(" ")).size > 1)
                phrases.add(phrase);
            if (phrases.size >= 64)
                break;
        }
...
const textTermMatches = (value, terms) => {
    const valueTerms = new Set(rawQueryTerms(value));
    return terms.filter((term) => valueTerms.has(term));
};
const pathScore = (path, terms) => {
    const pathTerms = new Set(identifierPieces(path));
    const basename = new Set(identifierPieces(path.slice(path.lastIndexOf("/") + 1)));
    let score = 0;
    for (const term of terms) {
        if (basename.has(term))
            score += 18;
        else if (pathTerms.has(term))
            score += 12;
    }
    return score;
};
const readSnapshotFile = async (repository, resolvedSnapshot, path, size) => {
    if (size > LUNA_REPOSITORY_RETRIEVAL_MAXIMUM_FILE_BYTES) {
        return { redactions: 0 };
    }
    const content = await runGitBuffer(repository, ["show", `${resolvedSnapshot}:${path}`], LUNA_REPOSITORY_RETRIEVAL_MAXIMUM_FILE_BYTES + 1024);
...
const countPhrase = (value, phrase) => {
    let count = 0;
    let offset = 0;
    while (count < 3) {
...
const scoreContent = (content, query) => {
    ...
    return {
        ...
        score: matched.size * 4 +
            ...
            [...matchedPhrases].reduce((sum, phrase) => sum + phrase.split(" ").length * 14, 0),
        bestLine,
    };
};
const clipSnippet = (value, maximumCharacters) => {
...
const buildSnippet = (content, bestLine, maximumCharacters) => {
    const lines = content.split("\n");
    let start = bestLine;
    let end = bestLine;
...
const extractSymbols = (content, query, bestLine) => {
    const lines = content.split("\n");
    const candidates = [];
    for (let index = 0; index < lines.length; index += 1) {
        const signature = lines[index].trim();
        if (!signature || !SYMBOL_PATTERNS.some((pattern) => pattern.test(signature)))
            continue;
...
const rankCandidates = async (repository, resolvedSnapshot, files, query, options) => {
    const pathCandidates = files.filter(({ path }) => isEligiblePath(path)).map((file) => ({
        path: file.path,
        size: file.size,
    ...
    for (let offset = 0; offset < scan.length; offset += readConcurrency) {
        ...
        const reads = await Promise.all(batch.map((candidate) => readSnapshotFile(repository, resolvedSnapshot, candidate.path, candidate.size)));
        for (const [index, candidate] of batch.entries()) {
            ...
            if (options.mode === "paths_and_symbols") {
                candidate.symbols = extractSymbols(read.content, query, scored.bestLine);
            }
        }
    }
    return {
...
const publicMatch = (candidate, options) => {
    const base = {
        path: candidate.path,
    ...
    if (options.mode === "paths_and_snippets" &&
        candidate.content !== undefined &&
        candidate.bestLine !== undefined) {
        base.snippet = buildSnippet(candidate.content, candidate.bestLine, options.maximumSnippetCharacters);
    }
    return base;
};
const renderMatch = (match, mode) => {
...
const renderDiagnostic = (matches, mode, maximumCharacters) => {
    const prefix = [
        DIAGNOSTIC_HEADER,
    ...
    for (const match of matches) {
        ...
        if (`${diagnostic}\n${block}`.length > maximumCharacters)
            break;
        diagnostic = `${diagnostic}\n${block}`;
        included.push(match);
    }
    if (included.length === 0) {
        const { symbols: _symbols, snippet: _snippet, ...fallback } = matches[0];
...
export const retrieveLunaRepositoryContext = async (input) => {
    ...
    const resolvedSnapshot = await resolveExactCommit(input.repository, input.episode.repositorySnapshot);
    ...
    const ranked = query.terms.length === 0
        ? { candidates: [], inspected: 0, redactions: 0 }
        : await rankCandidates(input.repository, resolvedSnapshot, files, query, options);
    const rendered = renderDiagnostic(ranked.candidates.map((candidate) => publicMatch(candidate, options)), options.mode, options.maximumCharacters);
    const resultBinding = {
...
export const augmentEpisodeWithLunaRepositoryContext = async (input) => {
    ...
    const relevantDiagnostic = [
        ...
        .join("\n\n");
    return {
        episode: { ...runtimeEpisode, relevantDiagnostic },
        retrieval,
    };
};
const writeImmutablePrivate = async (file, value) => {
...
export const materializeLunaRepositoryRetrievalEpisodes = async (input) => {
    ...
    for (const episode of input.episodes) {
        if (ids.has(episode.id))
            throw new Error(`Duplicate retrieval episode: ${episode.id}`);
        ids.add(episode.id);
    ...
    return {
        ...
        manifest: {
            ...
            hashes: {
                sourceEpisodes: contentHash(input.episodes.map(({ actualChangedPaths: _removed, ...episode }) => episode)),
            ...
            retrievalSummary: {
                ...
                renderedCharacters: retrievals.reduce((sum, item) => sum + item.provenance.renderedCharacters, 0),
                redactionsApplied: retrievals.reduce((sum, item) => sum + item.provenance.redactionsApplied, 0),
            },
        },
    };
};
...
export const writeLunaRepositoryRetrievalMaterialization = async (input) => {
    ...
    try {
        ...
        throw new Error(`Repository retrieval materialization already exists: ${outputDirectory}`);
    }
    catch (error) {
    ...
    try {
        ...
        created = true;
        await writeImmutablePrivate(files.episodes, `${input.materialization.episodes.map((episode) => JSON.stringify(episode)).join("\n")}\n`);
        await writeImmutablePrivate(files.retrievals, `${input.materialization.retrievals.map((retrieval) => JSON.stringify(retrieval)).join("\n")}\n`);
        await writeImmutablePrivate(files.manifest, `${JSON.stringify(input.materialization.manifest, null, 2)}\n`);
    }
    catch (error) {
        if (created) {
