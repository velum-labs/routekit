...
const metadataTurnId = (payload) => {
    const metadata = payload.internal_chat_message_metadata_passthrough;
    return metadata && typeof metadata === "object" ? asString(metadata.turn_id) : undefined;
};
...
const isValidSnapshot = (value) => Boolean(value && /^[0-9a-f]{7,64}$/iu.test(value));
const isManagedOrInstruction = (text) => /^\s*(?:#\s*AGENTS\.md\b|<INSTRUCTIONS>|<system_instruction>|<developer>|<environment_context>|<managed-context|<in-app-browser-context|You are \/root\b|You are a coding agent\b)/iu.test(text);
const isSubstantive = (text) => text.trim().length >= 18 && !/^(?:ok(?:ay)?|yes|no|done|proceed|continue|do it|try again|go ahead|sounds good|that one|the second one)[.!?\s]*$/iu.test(text.trim());
export const isReferentialRequest = (text) => /^(?:ok(?:ay)?|yes|no|done|proceed|continue|do it|try again|go ahead|sounds good|that one|the second one)[.!?\s]*$/iu.test(text.trim())
...
const commandLooksDiagnostic = (name, argumentsText) => /(?:exec_command|write_stdin|shell|terminal)/iu.test(name) &&
    /(?:\btest\b|typecheck|lint|build|check|verify|pytest|jest|vitest|cargo test|go test|tsc\b)/iu.test(argumentsText);
const outputLooksDiagnostic = (output) => /(?:process exited with code [1-9]|command failed|tests? failed|failing|assertionerror|type error|error TS\d+|FAIL\b|not ok\b|ELIFECYCLE)/iu.test(output);
const boundedDiagnostic = (command, output) => {
    const useful = output.split(/\r?\n/u).filter((line) => /(?:error|fail|assert|not ok|process exited|tests?:|passed|typecheck|build|lint|warning)/iu.test(line));
    const selected = (useful.length ? useful : output.split(/\r?\n/u).slice(-20)).join("\n").slice(0, 2_000);
    return `$ ${command.slice(0, 500)}\n${selected}`;
};
const parseSession = (text, fallbackTimestamp) => {
    const metadata = {};
    const turns = [];
    const byId = new Map();
    const calls = new Map();
    ...
    for (const [index, line] of text.split(/\r?\n/u).entries()) {
        ...
        if (record.type === "session_meta") {
            ...
            const repositoryUrl = asString(git.repository_url);
            if (repositoryUrl)
                metadata.repositoryUrl = repositoryUrl;
            const commitHash = asString(git.commit_hash);
            if (commitHash)
        ...
        if (record.type === "event_msg" && payload.type === "user_message") {
            ...
            if (active) {
                ...
                if (raw.trim())
                    active.user = { text: raw, timestamp, index };
            }
            continue;
        }
        if (record.type === "event_msg" && payload.type === "agent_message") {
            ...
            const raw = extractText(payload);
            if (active && raw.trim())
                active.assistants.push({ text: raw, timestamp, index });
            continue;
        }
        if (record.type === "response_item" && payload.type === "function_call") {
            ...
            if (callId)
                calls.set(callId, { name: asString(payload.name) ?? "tool", arguments: asString(payload.arguments) ?? "", ...(turnId ? { turnId } : {}) });
            continue;
        }
    ...
    if (active)
        active.endIndex = Math.max(active.endIndex, text.split(/\r?\n/u).length - 1);
    return { metadata, turns, messagesSeen, parseErrors };
};
export const collectCodexEpisodes = async (options) => {
    ...
    for (const file of files.sort()) {
        ...
        diagnostics.messagesSeen += parsed.messagesSeen;
        diagnostics.turnsSeen += parsed.turns.length;
        diagnostics.parseErrors += parsed.parseErrors;
        if (parsed.turns.length === 0) {
            diagnostics.sessionsRejectedNoTurns += 1;
            continue;
        }
        ...
        if (!urlMatch && !cwdMatch) {
            diagnostics.sessionsRejectedRepository += 1;
            continue;
        }
        const repositoryMatch = urlMatch ? "url" : "cwd";
        ...
        if (!repositorySnapshot) {
            diagnostics.sessionsRejectedSnapshot += 1;
            continue;
        }
        const snapshotSource = metadataSnapshot ? "session_meta" : "provided_fallback";
        if (metadataSnapshot)
            diagnostics.snapshotFromMetadata += 1;
        else
            diagnostics.snapshotFromFallback += 1;
        diagnostics.sessionsAccepted += 1;
        const source = parsed.metadata.source ?? "unknown";
        const originator = parsed.metadata.originator ?? "unknown";
        ...
        for (const turn of parsed.turns) {
            if (turn.status === "aborted" && !options.includeAborted) {
                diagnostics.abortedTurnsExcluded += 1;
                continue;
            }
            if (turn.status === "incomplete") {
                diagnostics.incompleteTurnsExcluded += 1;
                continue;
            }
            const message = turn.user;
            if (!message) {
                if (turn.status === "complete")
                    previousCompleted = turn;
                continue;
            }
            if (isManagedOrInstruction(message.text)) {
                diagnostics.managedMessagesExcluded += 1;
                continue;
            }
            const cleaned = redactText(message.text);
            diagnostics.redactions += cleaned.redactions;
            if (!cleaned.text.trim() || /^\[REDACTED MANAGED CONTEXT\]$/u.test(cleaned.text.trim())) {
                diagnostics.emptyAfterRedaction += 1;
                continue;
            }
            const timestamp = new Date(message.timestamp).toISOString();
            if (options.since && timestamp < new Date(options.since).toISOString())
                continue;
            ...
            if (assistantClean)
                diagnostics.redactions += assistantClean.redactions;
            ...
            if (diagnosticClean)
                diagnostics.redactions += diagnosticClean.redactions;
            ...
            if (referential)
                priorTaskUsers.push(cleaned.text);
            else
                priorTaskUsers = [cleaned.text];
            if (!referential && substantive)
                lastAnchor = { text: cleaned.text, turnId: turn.id, timestamp };
            if (turn.status === "complete")
                previousCompleted = turn;
        }
    }
