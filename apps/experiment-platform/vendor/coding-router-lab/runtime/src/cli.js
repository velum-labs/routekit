...
import { collectCodexEpisodes } from "./codex-collector.js";
import { buildDatasetQualityReport } from "./dataset-quality.js";
import { adjudicateSilverLabels } from "./adjudication.js";
import { freezeRegistry } from "./registry-freeze.js";
import { calculateExtendedMetrics, pairedBootstrapDifference } from "./experiment-metrics.js";
import { applyLunaConfidenceThreshold, buildAreaVectors, buildHiddenAreaVectors, classifyAllEmbeddingMethods, simulateAreaRefresh, simulateCascade, tuneLunaConfidence, tuneThresholds, withholdArea } from "./experiment-runner.js";
import { renderClassificationReport } from "./report.js";
import { curateEpisodes } from "./curation.js";
import { buildRepresentationEmbeddingInputs, embedRepresentationInputs } from "./embedding-run.js";
import { EmbeddingCache } from "./embedding-cache.js";
import { classifyTaskKindWithLuna, classifyWithLuna, } from "./openrouter.js";
import { renderLunaAreaCards, serializeRepresentation } from "./experiment-runner.js";
import { buildGroupedReport } from "./group-report.js";
import { captureEnvironment } from "./environment.js";
import { inventoryCodexRepositories } from "./codex-inventory.js";
import { estimateTokens, fetchEmbeddingModelPrices, fetchModelPrices, maximumCallCost } from "./cost.js";
import { readJsonl, writeJsonlPrivate } from "./jsonl.js";
import { buildOraclePrompt, runSilverOracle } from "./oracle.js";
import { buildRouteKitSeed, writeRouteKitSeed } from "./routekit-seed.js";
import { createRunDirectory, writeImmutable } from "./run-artifacts.js";
import { getOpenRouterKeyStatus } from "./openrouter.js";
import { BudgetLedger } from "./budget.js";
import { selectOracleCanary } from "./canary.js";
import { auditSilverLabelRepositoryEvidence, validateAreaCards, validateBenchmarkDataset, validateEpisodes, validateRepositoryProfile, validateSilverLabels, } from "./validation.js";
import { encryptTransfer, generateTransferKeyPair, importTransfer } from "./transfer.js";
import { CODEX_HARNESS_INPUT_OVERHEAD_TOKENS_PER_CALL, EXPERIMENT_BUDGET_CEILING_USD, TASK_CONTEXT_REPRESENTATION } from "./config.js";
import { buildReferenceRegistry } from "./registry-builder.js";
import { buildSliceReport } from "./slice-report.js";
import { auditOracleTraces } from "./oracle-trace-audit.js";
import { renderLunaAreaContext, serializeLunaTaskContext, validateLunaBenchmarkMatrix, } from "./luna-context.js";
import { buildLunaBenchmarkReport, buildLunaEnsembleReport, representativeLunaPredictions, selectStratifiedLunaCanary, } from "./luna-benchmark.js";
import { buildLunaCascadeVariantReport, combineLunaCascadePrediction, validateLunaCascadeMatrix, } from "./luna-cascade.js";
import { auditOraclePassCoverage, buildCodingChallengeSuite, buildResolvedDevelopmentSubset, buildValidationEscalationPlan, mergeOraclePassArtifacts, readUnlockedEpisodeSplits, recoverOraclePassesFromTraces, selectCanonicalOraclePasses, selectDisagreementEpisodes, selectSecondPassEpisodes, selectLunaAccuracyDevelopmentCanary, } from "./dataset-prep.js";
import { validateLunaAccuracyMatrixV2, } from "./luna-accuracy-context.js";
import { buildLunaAccuracyAttestedAnalysis, } from "./luna-accuracy-attestation.js";
import { buildLunaAccuracyPhaseOneMatrix, buildLunaAccuracyPhaseThreeDesign, buildLunaAccuracyPhaseTwoBMatrix, buildLunaAccuracyPhaseTwoMatrix, validateLunaAccuracyPhaseOneTransition, } from "./luna-accuracy-design.js";
import { buildLunaAccuracyPhaseTwoBSelection, validateLunaAccuracyPhaseTwoBSelection, } from "./luna-accuracy-confirmation.js";
import { selectLunaAccuracyPhaseThreeContexts, } from "./luna-accuracy-phase-transitions.js";
import { buildLunaAccuracyPhaseThreeSelection, } from "./luna-accuracy-phase3.js";
import { buildLunaCounterfactualChallengeSuite } from "./luna-accuracy-challenges.js";
import { compareLunaAccuracyTopTwo, LUNA_ACCURACY_DATA_SOURCES, } from "./luna-accuracy-report.js";
import { createLunaAccuracyOpenRouterExecutor, LUNA_ACCURACY_MODEL, } from "./luna-accuracy-openrouter.js";
import { planLunaAccuracyWorkflow, runBudgetedLunaAccuracyWorkflow, } from "./luna-accuracy-workflow.js";
import { buildLunaGroundedCodingDevelopmentSuite } from "./luna-accuracy-coding-development.js";
import { auditLunaAccuracyCodingAnnotations, buildGithubLunaAccuracyCodingAnnotations, buildReviewedLunaAccuracyCodingAnnotations, codingEpisodeIdsFromAnnotations, inheritLunaAccuracyCodingAnnotations, } from "./luna-accuracy-coding-annotations.js";
const args = process.argv.slice(2);
const command = args.shift();
const value = (name) => { const index = args.indexOf(name); return index >= 0 ? args[index + 1] : undefined; };
const required = (name) => { const result = value(name); if (!result)
    throw new Error(`Missing ${name}`); return result; };
...
const selectBootstrapPredictionSet = (sets, armId) => {
    ...
    if (ensemble.length > 1) {
        throw new Error(`Multiple ensemble prediction sets found for bootstrap arm ${armId}`);
    }
    if (ensemble.length === 1)
        return ensemble[0];
    return candidates
...
const main = async () => {
    ...
    if (command === "report-dataset-quality") {
        const episodes = await readJsonl(resolve(required("--episodes")));
        const cards = value("--areas") ? await readJsonl(resolve(value("--areas"))) : undefined;
    ...
    if (command === "curate-episodes") {
        const episodes = await readJsonl(resolve(required("--episodes")));
        const decisions = value("--decisions") ? await readJsonl(resolve(value("--decisions"))) : [];
    ...
    if (command === "freeze-registry") {
        const profile = JSON.parse(await readFile(resolve(required("--profile")), "utf8"));
        const cards = await readJsonl(resolve(required("--areas")));
        const episodes = await readJsonl(resolve(required("--episodes")));
    ...
    if (command === "adjudicate-labels") {
        const result = adjudicateSilverLabels(await readJsonl(resolve(required("--labels"))));
    ...
    if (command === "merge-labels") {
        const inputs = args.flatMap((arg, index) => arg === "--labels" && args[index + 1] ? [resolve(args[index + 1])] : []);
        if (inputs.length < 1)
            throw new Error("merge-labels requires at least one --labels file");
        const merged = [];
        const seen = new Set();
        for (const input of inputs) {
            for (const label of await readJsonl(input)) {
                if (seen.has(label.taskEpisodeId))
                    throw new Error(`Duplicate merged label: ${label.taskEpisodeId}`);
    ...
    if (command === "evaluate-predictions") {
        const labels = await readJsonl(resolve(required("--labels")));
        const predictions = await readJsonl(resolve(required("--predictions")));
    ...
    if (command === "compare-predictions") {
        const labels = await readJsonl(resolve(required("--labels")));
        const left = await readJsonl(resolve(required("--left")));
        const right = await readJsonl(resolve(required("--right")));
    ...
    if (command === "tune-luna-confidence") {
        const result = tuneLunaConfidence(await readJsonl(resolve(required("--labels"))), await readJsonl(resolve(required("--predictions"))), Number(value("--maximum-false-known-rate") ?? "0.1"));
        await writeFile(resolve(required("--output")), `${JSON.stringify(result, null, 2)}\n`, { mode: 0o600 });
    ...
    if (command === "apply-luna-confidence") {
        const document = JSON.parse(await readFile(resolve(required("--thresholds")), "utf8"));
        const predictions = applyLunaConfidenceThreshold(await readJsonl(resolve(required("--predictions"))), document.minimumConfidence);
    ...
    if (command === "classify-embeddings") {
        const cards = await readJsonl(resolve(required("--areas")));
        const vectors = JSON.parse(await readFile(resolve(required("--vectors")), "utf8"));
        const representation = required("--representation");
        if (representation !== TASK_CONTEXT_REPRESENTATION)
            throw new Error(`Invalid --representation; only ${TASK_CONTEXT_REPRESENTATION} is supported`);
        ...
        const thresholdsDocument = thresholdsFile
            ? JSON.parse(await readFile(resolve(thresholdsFile), "utf8"))
            : undefined;
        const thresholds = thresholdsDocument
            ? ("thresholds" in thresholdsDocument ? thresholdsDocument.thresholds : thresholdsDocument)
            : { minimumTopScore: -1, minimumMargin: 0, minimumSecondScoreForMultiArea: 2, maximumSelectedAreas: 2 };
        ...
        for (const [method, values] of Object.entries(predictions))
            await writeJsonlPrivate(`${outputDirectory}/${representation}-${method}.jsonl`, values);
    ...
    if (command === "plan-embedding" || command === "embed-dataset") {
        ...
        if (execute && !has("--confirm-external-run"))
            throw new Error("Refusing hosted calls without --confirm-external-run");
        const profile = JSON.parse(await readFile(resolve(required("--profile")), "utf8"));
        const cards = await readJsonl(resolve(required("--areas")));
        const episodes = await readJsonl(resolve(required("--episodes")));
        ...
        const totalInputs = inputs.tasks.length + inputs.overviews.length + inputs.examples.length;
        const price = (await fetchEmbeddingModelPrices()).get(model);
        if (!price)
            throw new Error(`Model not in OpenRouter embedding catalog: ${model}`);
        ...
        if (reservation + 1e-9 < projectedMaximum)
            throw new Error(`--maximum-cost-usd $${reservation} is below projected maximum $${projectedMaximum}`);
        ...
        try {
            ...
            if (before.limitRemainingUsd !== null && before.limitRemainingUsd < reservation)
                throw new Error("OpenRouter key limit is below the requested reservation");
            ...
            const providerMeteredCostUsd = Math.max(0, (after.accountUsageUsd ?? after.usageUsd) -
                (before.accountUsageUsd ?? before.usageUsd));
            const apiEquivalentCostUsd = maximumCallCost(price, embedded.usageTokens ?? 0, 0);
            const actual = Math.max(providerMeteredCostUsd, apiEquivalentCostUsd);
            const artifact = { schemaVersion: 1, embeddingModel: model, serializationVersion: "task-envelope-v1", taskVectors: embedded.taskVectors, overviewVectors: embedded.overviewVectors, exampleVectors: embedded.exampleVectors };
    ...
    if (command === "plan-luna" || command === "run-luna") {
        ...
        const profile = JSON.parse(await readFile(resolve(required("--profile")), "utf8"));
        const cards = await readJsonl(resolve(required("--areas")));
        const episodes = (await readJsonl(resolve(required("--episodes")))).filter((episode) => episode.split !== "reference");
        validateRepositoryProfile(profile);
        validateAreaCards(cards, profile);
        ...
        if (!price)
            throw new Error(`Model not in OpenRouter catalog: ${model}`);
        ...
        try {
            before = await getOpenRouterKeyStatus();
            if (before.limitRemainingUsd !== null &&
                before.limitRemainingUsd + 1e-9 < reservation) {
                throw new Error(`OpenRouter key has only $${before.limitRemainingUsd.toFixed(6)} remaining; reservation requires $${reservation.toFixed(6)}`);
            }
            ...
            const apiEquivalentCostUsd = reportedCosts.length === predictions.length
                ...
                : maximumCallCost(price, predictions.reduce((sum, prediction) => sum + (prediction.inputTokens ?? 0), 0), predictions.reduce((sum, prediction) => sum + (prediction.outputTokens ?? 0), 0));
            const actual = Math.max(providerMeteredCostUsd, apiEquivalentCostUsd);
            await writeJsonlPrivate(resolve(required("--output")), predictions);
            await writeImmutable(`${directory}/cost.json`, {
                maximumCostUsd: reservation,
                ...
                inputTokens: predictions.reduce((sum, prediction) => sum + (prediction.inputTokens ?? 0), 0),
                outputTokens: predictions.reduce((sum, prediction) => sum + (prediction.outputTokens ?? 0), 0),
            });
            await ledger.settle(reservation, actual);
            settled = true;
    ...
    if (command === "select-luna-canary") {
        const selected = selectStratifiedLunaCanary(await readJsonl(resolve(required("--episodes"))), await readJsonl(resolve(required("--labels"))), Number(value("--maximum-cases") ?? "10"));
        await writeJsonlPrivate(resolve(required("--output")), selected);
        console.log(JSON.stringify({
            ok: true,
            output: resolve(required("--output")),
            episodes: selected.length,
            split: Object.fromEntries(["validation", "test"].map((split) => [
                split,
                selected.filter((episode) => episode.split === split).length,
            ])),
            contextual: selected.filter((episode) => Boolean(episode.taskAnchor ||
                episode.precedingAssistant ||
                episode.earlierUserContext?.length ||
                episode.relevantDiagnostic)).length,
        }, null, 2));
        return;
    }
    if (command === "plan-luna-benchmark" ||
        ...
        const cards = await readJsonl(resolve(required("--areas")));
        const episodes = (await readJsonl(resolve(required("--episodes")))).filter((episode) => episode.split !== "reference");
        const matrix = JSON.parse(await readFile(resolve(required("--matrix")), "utf8"));
        validateRepositoryProfile(profile);
        validateAreaCards(cards, profile);
        validateEpisodes(episodes);
        ...
        const planVariants = matrix.variants.map((variant) => {
            ...
            const estimatedInputTokensPerPass = episodes.reduce((sum, episode) => sum +
                estimateTokens(serializeLunaTaskContext(episode, profile, variant.taskContext)) +
                estimateTokens(areaContext) +
                300, 0);
            const estimatedInputTokens = estimatedInputTokensPerPass * (variant.repetitions ?? 1);
            ...
            return {
                ...
                projectedMaximumCostUsd: maximumCallCost(price, estimatedInputTokens, reservedOutputTokens),
                configuration: variant,
            };
        });
        const projectedMaximum = planVariants.reduce((sum, variant) => sum + variant.projectedMaximumCostUsd, 0);
        const reservation = Number(value("--maximum-cost-usd") ?? String(projectedMaximum));
        if (reservation + 1e-9 < projectedMaximum) {
            throw new Error(`--maximum-cost-usd $${reservation} is below projected maximum $${projectedMaximum}`);
        }
        ...
        let hostedStarted = false;
        let settled = false;
        const predictionsByVariant = Object.fromEntries(matrix.variants.map((variant) => [variant.id, []]));
        try {
            before = await getOpenRouterKeyStatus();
            if (before.limitRemainingUsd !== null &&
                before.limitRemainingUsd + 1e-9 < reservation) {
            ...
            for (const variant of matrix.variants) {
                ...
                for (let repetition = 0; repetition < (variant.repetitions ?? 1); repetition += 1) {
                    for (const episode of episodes) {
                        hostedStarted = true;
                        predictionsByVariant[variant.id].push(await classifyWithLuna({
                            taskEpisodeId: episode.id,
                            model,
                            taskEnvelope: serializeLunaTaskContext(episode, profile, variant.taskContext),
                ...
                await writeJsonlPrivate(`${directory}/private/${variant.id}.jsonl`, predictionsByVariant[variant.id]);
            }
            ...
            const reportedCosts = allPredictions
                .map((prediction) => prediction.costUsd)
                .filter((cost) => cost !== undefined);
            const apiEquivalentCostUsd = reportedCosts.length === allPredictions.length
                ? reportedCosts.reduce((sum, cost) => sum + cost, 0)
                : maximumCallCost(price, allPredictions.reduce((sum, prediction) => sum + (prediction.inputTokens ?? 0), 0), allPredictions.reduce((sum, prediction) => sum + (prediction.outputTokens ?? 0), 0));
            const actualCostUsd = Math.max(providerMeteredCostUsd, apiEquivalentCostUsd);
            const report = buildLunaBenchmarkReport(model, matrix, labels, predictionsByVariant);
            const representativeByVariant = Object.fromEntries(Object.entries(predictionsByVariant).map(([variant, values]) => [
                variant,
                representativeLunaPredictions(values),
            ]));
            const sliceReport = buildSliceReport(episodes, labels.filter((label) => episodes.some((episode) => episode.id === label.taskEpisodeId)), representativeByVariant);
            const repeatedPredictions = Object.fromEntries(matrix.variants
            ...
            if (Object.keys(repeatedPredictions).length) {
                await writeImmutable(`${directory}/metrics/ensembles.json`, buildLunaEnsembleReport(labels.filter((label) => episodes.some((episode) => episode.id === label.taskEpisodeId)), repeatedPredictions));
            }
            await writeImmutable(`${directory}/cost.json`, {
                maximumCostUsd: reservation,
                actualCostUsd,
                ...
                inputTokens: allPredictions.reduce((sum, prediction) => sum + (prediction.inputTokens ?? 0), 0),
                cachedInputTokens: allPredictions.reduce((sum, prediction) => sum + (prediction.cachedInputTokens ?? 0), 0),
                outputTokens: allPredictions.reduce((sum, prediction) => sum + (prediction.outputTokens ?? 0), 0),
                reasoningOutputTokens: allPredictions.reduce((sum, prediction) => sum + (prediction.reasoningOutputTokens ?? 0), 0),
            });
            await ledger.settle(reservation, actualCostUsd);
            settled = true;
            console.log(JSON.stringify({
            ...
            for (const [variant, predictions] of Object.entries(predictionsByVariant)) {
                if (predictions.length) {
                    await writeJsonlPrivate(`${directory}/private/partial-${variant}.jsonl`, predictions);
                }
            }
            if (!settled) {
                try {
                    if (!hostedStarted || !before) {
                        ...
                        const reportedCosts = allPredictions
                            .map((prediction) => prediction.costUsd)
                            .filter((cost) => cost !== undefined);
                        const apiEquivalentCostUsd = reportedCosts.reduce((sum, cost) => sum + cost, 0);
                        const actualCostUsd = Math.max(providerMeteredCostUsd, apiEquivalentCostUsd);
                        await writeImmutable(`${directory}/partial-cost.json`, {
                            maximumCostUsd: reservation,
                            actualCostUsd,
    ...
    if (command === "analyze-luna-benchmark") {
        const matrix = JSON.parse(await readFile(resolve(required("--matrix")), "utf8"));
        validateLunaBenchmarkMatrix(matrix);
        const predictionSpecs = args.flatMap((arg, index) => arg === "--prediction" && args[index + 1]
            ? [args[index + 1]]
            : []);
        const predictions = {};
        for (const spec of predictionSpecs) {
            ...
            predictions[spec.slice(0, separator)] = await readJsonl(resolve(spec.slice(separator + 1)));
        }
        for (const variant of matrix.variants) {
        ...
        const labels = await readJsonl(resolve(required("--labels")));
        const report = buildLunaBenchmarkReport(required("--model"), matrix, labels, predictions);
        ...
        console.log(JSON.stringify({
            ...
            representativePredictions: Object.fromEntries(Object.entries(predictions).map(([variant, values]) => [
                variant,
    ...
    if (command === "plan-luna-cascade" ||
        ...
        const planVariants = matrix.variants.map((variant) => {
            const areaContext = renderLunaAreaContext(cards, variant.areaContext);
            const gateInputTokens = episodes.reduce((sum, episode) => sum +
                estimateTokens(serializeLunaTaskContext(episode, profile, variant.gateTaskContext)) +
                220, 0);
            const areaInputTokens = episodes.reduce((sum, episode) => sum +
                estimateTokens(serializeLunaTaskContext(episode, profile, variant.areaTaskContext)) +
                estimateTokens(areaContext) +
                300, 0);
            const maximumGateCostUsd = maximumCallCost(price, gateInputTokens, episodes.length * variant.gateMaxOutputTokens);
            const maximumAreaCostUsd = maximumCallCost(price, areaInputTokens, episodes.length * variant.areaMaxOutputTokens);
            return {
                id: variant.id,
                maximumCalls: episodes.length * 2,
                maximumGateCostUsd,
                maximumAreaCostUsd,
                projectedMaximumCostUsd: maximumGateCostUsd + maximumAreaCostUsd,
                configuration: variant,
            };
        });
        const projectedMaximum = planVariants.reduce((sum, variant) => sum + variant.projectedMaximumCostUsd, 0);
        ...
        if (!execute)
            return;
        const labels = await readJsonl(resolve(required("--labels")));
        const relevantLabels = labels.filter((label) => episodes.some((episode) => episode.id === label.taskEpisodeId));
        ...
        let hostedStarted = false;
        let settled = false;
        const gatesByVariant = Object.fromEntries(matrix.variants.map((variant) => [variant.id, []]));
        const areasByVariant = Object.fromEntries(matrix.variants.map((variant) => [variant.id, []]));
        const finalByVariant = Object.fromEntries(matrix.variants.map((variant) => [variant.id, []]));
        try {
            before = await getOpenRouterKeyStatus();
            if (before.limitRemainingUsd !== null &&
                before.limitRemainingUsd + 1e-9 < reservation) {
            ...
            for (const variant of matrix.variants) {
                ...
                for (const episode of episodes) {
                    ...
                    const gate = await classifyTaskKindWithLuna({
                        taskEpisodeId: episode.id,
                        model,
                        taskEnvelope: serializeLunaTaskContext(episode, profile, variant.gateTaskContext),
                    ...
                    if (gate.taskKind === "repository_task") {
                        area = await classifyWithLuna({
                            taskEpisodeId: episode.id,
                            model,
                            taskEnvelope: serializeLunaTaskContext(episode, profile, variant.areaTaskContext),
                            areaCards: areaContext,
                            allowedAreaIds: cards.map((card) => card.areaId),
                    ...
                    finalByVariant[variant.id].push(combineLunaCascadePrediction(gate, area, variant.id));
                }
                await writeJsonlPrivate(`${directory}/private/${variant.id}-gates.jsonl`, gatesByVariant[variant.id]);
                await writeJsonlPrivate(`${directory}/private/${variant.id}-areas.jsonl`, areasByVariant[variant.id]);
                await writeJsonlPrivate(`${directory}/private/${variant.id}-final.jsonl`, finalByVariant[variant.id]);
            }
            const reports = Object.fromEntries(matrix.variants.map((variant) => [
                variant.id,
                buildLunaCascadeVariantReport(variant, relevantLabels, gatesByVariant[variant.id], areasByVariant[variant.id], finalByVariant[variant.id]),
            ]));
            ...
            await writeImmutable(`${directory}/metrics/slices.json`, buildSliceReport(episodes, relevantLabels, finalByVariant));
            await writeImmutable(`${directory}/cost.json`, {
                maximumCostUsd: reservation,
                actualCostUsd,
                providerMeteredCostUsd,
            ...
            console.log(JSON.stringify({
                ...
                directory,
                actualCostUsd,
                variants: Object.fromEntries(Object.entries(reports).map(([id, report]) => [
                    id,
                    {
            ...
            for (const variant of matrix.variants) {
                if (gatesByVariant[variant.id].length) {
                    await writeJsonlPrivate(`${directory}/private/partial-${variant.id}-gates.jsonl`, gatesByVariant[variant.id]);
                }
                if (areasByVariant[variant.id].length) {
                    await writeJsonlPrivate(`${directory}/private/partial-${variant.id}-areas.jsonl`, areasByVariant[variant.id]);
                }
            }
            if (!settled) {
                try {
    ...
    if (command === "simulate-cascade") {
        const embedding = await readJsonl(resolve(required("--embedding")));
        const luna = await readJsonl(resolve(required("--luna")));
    ...
    if (command === "withhold-area") {
        const cards = await readJsonl(resolve(required("--areas")));
        const labels = await readJsonl(resolve(required("--labels")));
        const result = withholdArea(cards, labels, required("--area-id"));
        const prefix = resolve(required("--output-prefix"));
    ...
    if (command === "simulate-refresh") {
        const fullCards = await readJsonl(resolve(required("--full-areas")));
        const hiddenCards = await readJsonl(resolve(required("--hidden-areas")));
        const vectors = JSON.parse(await readFile(resolve(required("--vectors")), "utf8"));
        const representation = required("--representation");
        if (representation !== TASK_CONTEXT_REPRESENTATION)
            throw new Error(`Invalid --representation; only ${TASK_CONTEXT_REPRESENTATION} is supported`);
    ...
    if (command === "render-report") {
        ...
        for (const spec of predictionSpecs) {
            const separator = spec.indexOf("=");
            if (separator < 1)
                throw new Error("--prediction must be NAME=FILE");
            predictions[spec.slice(0, separator)] = await readJsonl(resolve(spec.slice(separator + 1)));
        }
    ...
    if (command === "report-slices") {
        ...
        const report = buildSliceReport(await readJsonl(resolve(required("--episodes"))), await readJsonl(resolve(required("--labels"))), predictions);
        await writeFile(resolve(required("--output")), `${JSON.stringify(report, null, 2)}\n`, { mode: 0o600 });
        console.log(JSON.stringify({
            ok: true,
            output: resolve(required("--output")),
    ...
    if (command === "build-reference-registry") {
        ...
        const result = await buildReferenceRegistry({
            ...
            ...(supplementalEpisodesFile && supplementalAssignmentsFile
                ? {
                    supplementalEpisodes: await readJsonl(resolve(supplementalEpisodesFile)),
                    supplementalAssignments: await readJsonl(resolve(supplementalAssignmentsFile)),
                }
                : {}),
        });
    ...
    if (command === "prepare-accuracy-dataset") {
        const source = await readUnlockedEpisodeSplits(resolve(required("--episodes")));
        const { reference, validation } = source;
        validateEpisodes([...reference, ...validation]);
        if (reference.length === 0 || validation.length === 0) {
            throw new Error("Accuracy dataset requires non-empty reference and validation splits");
        }
        await writeJsonlPrivate(resolve(required("--reference-output")), reference);
        await writeJsonlPrivate(resolve(required("--validation-output")), validation);
        const plan = buildValidationEscalationPlan([...reference, ...validation]);
        await writeFile(resolve(required("--validation-plan-output")), `${JSON.stringify(plan, null, 2)}\n`, { mode: 0o600 });
        console.log(JSON.stringify({
            ok: true,
            referenceEpisodes: reference.length,
            validationEpisodes: validation.length,
            lockedTestEpisodes: source.lockedTestCount,
            mandatorySecondPassEpisodes: plan.mandatorySecondPassEpisodeIds.length,
    ...
    if (command === "select-oracle-second-pass") {
        const episodes = await readJsonl(resolve(required("--episodes")));
        const plan = JSON.parse(await readFile(resolve(required("--validation-plan")), "utf8"));
        const selected = selectSecondPassEpisodes(episodes, await readJsonl(resolve(required("--first-pass-labels"))), plan);
        await writeJsonlPrivate(resolve(required("--output")), selected);
        console.log(JSON.stringify({
            ok: true,
    ...
    if (command === "select-oracle-third-pass") {
        const selected = selectDisagreementEpisodes(await readJsonl(resolve(required("--episodes"))), await readJsonl(resolve(required("--first-pass-labels"))), await readJsonl(resolve(required("--second-pass-labels"))));
        await writeJsonlPrivate(resolve(required("--output")), selected);
        console.log(JSON.stringify({
            ok: true,
    ...
    if (command === "recover-oracle-passes") {
        const result = await recoverOraclePassesFromTraces({
            ...
            episodes: await readJsonl(resolve(required("--episodes"))),
            cards: await readJsonl(resolve(required("--areas"))),
            model: required("--model"),
        });
        await writeJsonlPrivate(resolve(required("--passes-output")), result.labels);
        await writeJsonlPrivate(resolve(required("--trace-sources-output")), result.traceSources);
        await writeFile(resolve(required("--report-output")), `${JSON.stringify(result.summary, null, 2)}\n`, { mode: 0o600 });
        console.log(JSON.stringify({
            ok: result.summary.missingEpisodeIds.length === 0 &&
        ...
        if (result.summary.missingEpisodeIds.length > 0 ||
            result.summary.unexpectedEpisodeIds.length > 0) {
            process.exitCode = 1;
        }
        return;
    ...
    if (command === "audit-oracle-pass-coverage") {
        const report = auditOraclePassCoverage(await readJsonl(resolve(required("--episodes"))), await readJsonl(resolve(required("--passes"))));
        await writeFile(resolve(required("--output")), `${JSON.stringify(report, null, 2)}\n`, { mode: 0o600 });
        console.log(JSON.stringify({
        ...
        if (!report.readyForAdjudication)
            process.exitCode = 1;
        return;
    }
    if (command === "merge-oracle-pass-artifacts") {
        const passFiles = args.flatMap((arg, index) => arg === "--passes" && args[index + 1]
            ? [resolve(args[index + 1])]
            : []);
        const traceSourceFiles = args.flatMap((arg, index) => arg === "--trace-sources" && args[index + 1]
            ? [resolve(args[index + 1])]
            : []);
        if (passFiles.length < 1 ||
            passFiles.length !== traceSourceFiles.length) {
            throw new Error("merge-oracle-pass-artifacts requires paired --passes and --trace-sources files");
        }
        const artifacts = await Promise.all(passFiles.map(async (file, index) => ({
            labels: await readJsonl(file),
            traceSources: await readJsonl(traceSourceFiles[index]),
        })));
        const merged = mergeOraclePassArtifacts(artifacts);
        await writeJsonlPrivate(resolve(required("--passes-output")), merged.labels);
        await writeJsonlPrivate(resolve(required("--trace-sources-output")), merged.traceSources);
        console.log(JSON.stringify({
            ok: true,
    ...
    if (command === "select-canonical-oracle-passes") {
        const result = selectCanonicalOraclePasses({
            adjudicatedLabels: await readJsonl(resolve(required("--labels"))),
            passes: await readJsonl(resolve(required("--passes"))),
            traceSources: await readJsonl(resolve(required("--trace-sources"))),
        });
        const passesOutput = resolve(required("--passes-output"));
        const traceSourcesOutput = resolve(required("--trace-sources-output"));
        const reportOutput = resolve(required("--report-output"));
        await writeJsonlPrivate(passesOutput, result.canonicalPasses);
        await writeJsonlPrivate(traceSourcesOutput, result.canonicalTraceSources);
        await writeFile(reportOutput, `${JSON.stringify(result.summary, null, 2)}\n`, { mode: 0o600 });
        console.log(JSON.stringify({
            ok: true,
            ...result.summary,
            passesOutput,
    ...
    if (command === "prepare-coding-challenge") {
        ...
        const cards = await readJsonl(resolve(required("--areas")));
        const result = buildCodingChallengeSuite(await readJsonl(resolve(required("--episodes"))), await readJsonl(resolve(required("--assignments"))), cards, split);
        await writeJsonlPrivate(resolve(required("--episodes-output")), result.episodes);
        await writeJsonlPrivate(resolve(required("--evidence-labels-output")), result.evidenceLabels);
        await writeFile(resolve(required("--report-output")), `${JSON.stringify(result.report, null, 2)}\n`, { mode: 0o600 });
        console.log(JSON.stringify({
            ok: true,
            ...result.report,
            episodesOutput: resolve(required("--episodes-output")),
            evidenceLabelsOutput: resolve(required("--evidence-labels-output")),
            reportOutput: resolve(required("--report-output")),
        }, null, 2));
        return;
    }
    if (command === "prepare-luna-accuracy-development") {
        const result = buildResolvedDevelopmentSubset(await readJsonl(resolve(required("--episodes"))), await readJsonl(resolve(required("--labels"))));
    ...
    if (command === "prepare-reviewed-coding-annotations") {
        ...
        const result = buildReviewedLunaAccuracyCodingAnnotations({
            episodes,
            drafts: await readJsonl(resolve(required("--drafts"))),
            reviewer: required("--reviewer"),
        });
        const annotationsOutput = resolve(required("--annotations-output"));
        ...
        await writeFile(auditOutput, `${JSON.stringify(result.audit, null, 2)}\n`, { mode: 0o600 });
        console.log(JSON.stringify({
            ok: true,
            ...result.audit,
            annotationsOutput,
    ...
    if (command === "inherit-coding-annotations") {
        const derivedEpisodes = await readJsonl(resolve(required("--derived-episodes")));
        const provenance = await readJsonl(resolve(required("--provenance")));
        const sourceEpisodeIdByDerivedId = new Map();
        for (const item of provenance) {
            if (sourceEpisodeIdByDerivedId.has(item.derivativeEpisodeId)) {
                throw new Error(`Duplicate derivative provenance: ${item.derivativeEpisodeId}`);
            }
            sourceEpisodeIdByDerivedId.set(item.derivativeEpisodeId, item.sourceEpisodeId);
        }
        const result = inheritLunaAccuracyCodingAnnotations({
            sourceAnnotations: await readJsonl(resolve(required("--source-annotations"))),
            derivedEpisodes,
    ...
    if (command === "audit-coding-annotations") {
        const audit = auditLunaAccuracyCodingAnnotations(await readJsonl(resolve(required("--episodes"))), await readJsonl(resolve(required("--annotations"))));
        const auditOutput = resolve(required("--audit-output"));
        await mkdir(resolve(auditOutput, ".."), {
            recursive: true,
            mode: 0o700,
    ...
    if (command === "select-luna-accuracy-canary") {
        const episodes = await readJsonl(resolve(required("--episodes")));
        const annotations = await readJsonl(resolve(required("--coding-annotations")));
        const annotationAudit = auditLunaAccuracyCodingAnnotations(episodes, annotations);
        if (!annotationAudit.ready) {
            throw new Error("Canary coding annotations failed their audit");
        }
        const result = selectLunaAccuracyDevelopmentCanary(episodes, await readJsonl(resolve(required("--labels"))), new Map(annotations.map((annotation) => [
        ...
        await writeJsonlPrivate(annotationsOutput, annotations.filter((annotation) => selectedIds.has(annotation.taskEpisodeId)));
        await mkdir(resolve(reportOutput, ".."), {
            recursive: true,
            mode: 0o700,
        });
    ...
    if (command === "prepare-luna-coding-development") {
        const suite = buildLunaGroundedCodingDevelopmentSuite({
            episodes: await readJsonl(resolve(required("--episodes"))),
            assignments: await readJsonl(resolve(required("--assignments"))),
            cards: await readJsonl(resolve(required("--areas"))),
        });
        const baseEpisodesOutput = resolve(required("--base-episodes-output"));
        const baseLabelsOutput = resolve(required("--base-labels-output"));
        const derivedEpisodesOutput = resolve(required("--derived-episodes-output"));
        const derivedLabelsOutput = resolve(required("--derived-labels-output"));
        ...
        await writeJsonlPrivate(derivedEpisodesOutput, suite.derivedEpisodes);
        ...
        await writeFile(reportOutput, `${JSON.stringify(suite.report, null, 2)}\n`, { mode: 0o600 });
        console.log(JSON.stringify({
            ok: true,
            ...suite.report,
            baseEpisodesOutput,
    ...
    if (command === "generate-luna-accuracy-phase2") {
        const phaseOne = JSON.parse(await readFile(resolve(required("--phase1-matrix")), "utf8"));
        validateLunaAccuracyMatrixV2(phaseOne);
        const report = JSON.parse(await readFile(resolve(required("--phase1-report")), "utf8"));
        const runManifest = JSON.parse(await readFile(resolve(required("--phase1-manifest")), "utf8"));
        const attestation = JSON.parse(await readFile(resolve(required("--phase1-attestation")), "utf8"));
        const profile = JSON.parse(await readFile(resolve(required("--profile")), "utf8"));
        const cards = await readJsonl(resolve(required("--areas")));
        const episodes = await readJsonl(resolve(required("--episodes")));
        const labels = await readJsonl(resolve(required("--labels")));
        const codingAnnotations = await readJsonl(resolve(required("--coding-annotations")));
        const calls = await readJsonl(resolve(required("--calls")));
    ...
    if (command === "analyze-luna-accuracy-phase2b") {
        const matrix = JSON.parse(await readFile(resolve(required("--phase2b-matrix")), "utf8"));
        validateLunaAccuracyMatrixV2(matrix);
        const runManifest = JSON.parse(await readFile(resolve(required("--phase2b-manifest")), "utf8"));
        const profile = JSON.parse(await readFile(resolve(required("--profile")), "utf8"));
        const cards = await readJsonl(resolve(required("--areas")));
        const episodes = await readJsonl(resolve(required("--episodes")));
        const labels = await readJsonl(resolve(required("--labels")));
    ...
    if (command === "generate-luna-accuracy-phase3") {
        const phaseTwoB = JSON.parse(await readFile(resolve(required("--phase2b-matrix")), "utf8"));
        validateLunaAccuracyMatrixV2(phaseTwoB);
        const selection = JSON.parse(await readFile(resolve(required("--phase2b-selection")), "utf8"));
        const runManifest = JSON.parse(await readFile(resolve(required("--phase2b-manifest")), "utf8"));
        const profile = JSON.parse(await readFile(resolve(required("--profile")), "utf8"));
        const cards = await readJsonl(resolve(required("--areas")));
        const episodes = await readJsonl(resolve(required("--episodes")));
        ...
        const { primary, alternative } = selectLunaAccuracyPhaseThreeContexts({
            matrix: phaseTwoB,
            selection: verified,
        });
        const design = buildLunaAccuracyPhaseThreeDesign(primary, alternative);
        ...
        await writeFile(matrixOutput, `${JSON.stringify(design.matrix, null, 2)}\n`, { mode: 0o600 });
        await writeFile(armsOutput, `${JSON.stringify(design.arms, null, 2)}\n`, { mode: 0o600 });
        console.log(JSON.stringify({
            ok: true,
            matrixOutput,
            armsOutput,
    ...
    if (command === "analyze-luna-accuracy-phase3") {
        const matrix = JSON.parse(await readFile(resolve(required("--matrix")), "utf8"));
        validateLunaAccuracyMatrixV2(matrix);
        const arms = JSON.parse(await readFile(resolve(required("--arms")), "utf8"));
        const runManifest = JSON.parse(await readFile(resolve(required("--manifest")), "utf8"));
        const profile = JSON.parse(await readFile(resolve(required("--profile")), "utf8"));
        const cards = await readJsonl(resolve(required("--areas")));
        const episodes = await readJsonl(resolve(required("--episodes")));
        const labels = await readJsonl(resolve(required("--labels")));
        ...
        console.log(JSON.stringify({
            ...
            architectureAnalyses: selection.architectureAnalyses.map((analysis) => ({
                ...
                averageObservedSelectionScoreLead: analysis.averageObservedSelectionScoreLead,
            })),
        }, null, 2));
        return;
    }
    if (command === "prepare-luna-accuracy-challenges") {
        ...
        const safeTruncationIds = value("--safe-truncation-ids")
            ? JSON.parse(await readFile(resolve(value("--safe-truncation-ids")), "utf8"))
            : undefined;
        const freeze = value("--freeze")
            ? JSON.parse(await readFile(resolve(value("--freeze")), "utf8"))
            : undefined;
        const suite = buildLunaCounterfactualChallengeSuite({
            episodes: await readJsonl(resolve(required("--episodes"))),
            labels: await readJsonl(resolve(required("--labels"))),
            cards: await readJsonl(resolve(required("--areas"))),
        ...
        await writeJsonlPrivate(provenanceOutput, suite.provenance);
        await writeFile(scenariosOutput, `${JSON.stringify({
    ...
    if (command === "plan-luna-accuracy" ||
        ...
        const arms = value("--arms")
            ? JSON.parse(await readFile(resolve(value("--arms")), "utf8"))
            : undefined;
        const datasetRole = (value("--dataset-role") ?? "validation");
        if (![
            "burned_development",
            "validation",
            "locked_test",
        ...
        const dataSource = required("--data-source");
        ...
        const reservation = Number(value("--maximum-cost-usd") ??
        ...
        const result = await runBudgetedLunaAccuracyWorkflow({
            ...
            budgetLedger: new BudgetLedger(resolve(artifacts, "global-budget.json"), EXPERIMENT_BUDGET_CEILING_USD),
            ...
            datasetRole,
            ...(freeze ? { freeze } : {}),
            allowEquivalentTreatmentReplicates: has("--allow-equivalent-treatment-replicates"),
        });
        ...
        const { report, treatmentReport, distinctnessAudit, attestation, } = attested;
        const reportOutput = resolve(directory, "metrics", "accuracy.json");
        const treatmentOutput = resolve(directory, "metrics", "accuracy-treatments.json");
        const distinctnessOutput = resolve(directory, "metrics", "treatment-distinctness.json");
        const attestationOutput = resolve(directory, "metrics", "analysis-attestation.json");
    ...
    if (command === "analyze-luna-accuracy") {
        ...
        const labels = await readJsonl(resolve(required("--labels")));
        const codingAnnotations = await readJsonl(resolve(required("--coding-annotations")));
        const codingEpisodeIds = codingEpisodeIdsFromAnnotations(episodes, codingAnnotations);
        const matrix = JSON.parse(await readFile(resolve(required("--matrix")), "utf8"));
        validateLunaAccuracyMatrixV2(matrix);
        const arms = value("--arms")
            ? JSON.parse(await readFile(resolve(value("--arms")), "utf8"))
        ...
        const calls = await readJsonl(resolve(required("--calls")));
        const runManifest = JSON.parse(await readFile(resolve(required("--manifest")), "utf8"));
        const datasetRole = required("--dataset-role");
        const dataSource = required("--data-source");
        if (!LUNA_ACCURACY_DATA_SOURCES.includes(dataSource)) {
            throw new Error("Invalid --data-source");
        }
        ...
        const { report, treatmentReport, distinctnessAudit, attestation, predictionSets, } = attested;
        const output = resolve(required("--output"));
        ...
        let writtenBootstrapOutput;
        if (bootstrapOutput && report.ranking.length >= 2) {
            const topSets = report.ranking
                .slice(0, 2)
                .map((entry) => selectBootstrapPredictionSet(predictionSets, entry.armId))
            ...
            if (topSets.length === 2) {
                ...
                await writeFile(writtenBootstrapOutput, `${JSON.stringify(compareLunaAccuracyTopTwo({
                    report,
                    episodes,
                    labels,
                    predictionSets: topSets,
                    codingEpisodeIds,
                    iterations: Number(value("--bootstrap-iterations") ?? "5000"),
    ...
    if (command === "validate") {
        ...
        if (labelsFile)
            validateSilverLabels(await readJsonl(resolve(labelsFile)), cards);
        console.log(JSON.stringify({ ok: true, repositoryId: profile.repositoryId, areaCards: cards.length, episodes: episodes.length, labels: labelsFile ? (await readJsonl(resolve(labelsFile))).length : 0 }, null, 2));
        return;
    }
    if (command === "audit-label-evidence") {
        const report = await auditSilverLabelRepositoryEvidence(await readJsonl(resolve(required("--labels"))), await readJsonl(resolve(required("--episodes"))), resolve(required("--repository")));
        await writeFile(resolve(required("--output")), `${JSON.stringify(report, null, 2)}\n`, { mode: 0o600 });
        console.log(JSON.stringify({
            ok: report.ready,
            output: resolve(required("--output")),
    ...
    if (command === "audit-oracle-traces") {
        const report = await auditOracleTraces({
            labels: await readJsonl(resolve(required("--labels"))),
            canonicalPasses: await readJsonl(resolve(required("--passes"))),
            traceSources: await readJsonl(resolve(required("--trace-sources"))),
            episodes: await readJsonl(resolve(required("--episodes"))),
            repository: resolve(required("--repository")),
        });
    ...
    if (command === "run-oracle") {
        if (!has("--confirm-external-run"))
            throw new Error("Refusing hosted calls without --confirm-external-run");
        const cards = await readJsonl(resolve(required("--areas")));
        const episodes = await readJsonl(resolve(required("--episodes")));
        validateEpisodes(episodes);
        if (cards.length < 2)
            throw new Error("Oracle requires at least two Area Cards");
        const model = required("--model");
        const passes = Number(value("--passes") ?? "2");
        const maxOutput = Number(value("--max-output-tokens") ?? "800");
        const price = (await fetchModelPrices()).get(model);
        if (!price)
            throw new Error(`Model not in OpenRouter catalog: ${model}`);
        ...
        try {
            ...
            if (keyBefore.limitRemainingUsd !== null && keyBefore.limitRemainingUsd + 1e-9 < reservation)
                throw new Error(`OpenRouter key has only $${keyBefore.limitRemainingUsd.toFixed(6)} remaining; reservation requires $${reservation.toFixed(6)}`);
            ...
            if (!settled) {
                try {
                    if (!hostedStarted || !keyBefore) {
                        ...
                        const keyAfter = await getOpenRouterKeyStatus();
                        const providerMeteredCostUsd = Math.max(0, (keyAfter.accountUsageUsd ?? keyAfter.usageUsd) -
            ...
            console.error(`Partial results preserved in ${directory}`);
            throw error;
        }
        return;
    }
