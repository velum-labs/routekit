...
import { scopeTargetForLabel, } from "./luna-distributional.js";
...
const readAggregate = async (input) => {
    const analysis = JSON.parse(await readFile(path.resolve(input.analysisFile), "utf8"));
    const aggregate = analysis.arms[input.analysisArmId]?.perSeed["181081"]?.[input.dataPartition]?.aggregate;
    if (!aggregate) {
        throw new Error(`Missing ${input.analysisArmId}/181081/${input.dataPartition} in ${input.analysisFile}`);
    }
    return aggregateFrom(aggregate);
};
const readPredictions = async (input, episodes) => Promise.all(episodes.map(async (episode) => {
    const file = path.join(path.resolve(input.classificationDirectory), "classification", input.analysisArmId, "181081", `${episode.id}.json`);
...
const usage = (predictions) => {
    ...
    const totalCostUsd = predictions.reduce((sum, prediction) => sum + prediction.costUsd, 0);
    return {
        calls: predictions.reduce((sum, prediction) => sum + prediction.providerCalls, 0),
        meanDurationMs: durations.reduce((sum, duration) => sum + duration, 0) /
            durations.length,
        medianDurationMs: quantile(durations, 0.5),
        p95DurationMs: quantile(durations, 0.95),
        totalCostUsd,
        meanCostUsd: totalCostUsd / predictions.length,
        meanInputTokens: predictions.reduce((sum, prediction) => sum + prediction.inputTokens, 0) / predictions.length,
        meanCachedInputTokens: predictions.reduce((sum, prediction) => sum + prediction.cachedInputTokens, 0) / predictions.length,
        meanOutputTokens: predictions.reduce((sum, prediction) => sum + prediction.outputTokens, 0) / predictions.length,
    };
};
const selectionLoss = (aggregate) => aggregate.areaBrier +
    0.35 * aggregate.scopeBrier +
    0.15 * (1 - aggregate.areaHitAt1) +
    0.25 * (1 - aggregate.allGoldAt3);
const topScope = (prediction) => Object.entries(prediction.scopeProbabilities).sort((left, right) => right[1] - left[1] || left[0].localeCompare(right[0]))[0][0];
const classificationSuccess = (label, prediction) => {
    const result = {
        scope: topScope(prediction) === scopeTargetForLabel(label),
    };
    if (!label.known)
        return result;
    const ranked = prediction.areaProbabilitiesGivenKnown.map((item) => item.areaId);
    const gold = new Set(label.selectedAreaIds);
    result.areaTop1 = gold.has(ranked[0] ?? "");
    result.allGoldTop3 = [...gold].every((areaId) => ranked.slice(0, 3).includes(areaId));
    return result;
};
const pairedChanges = (input) => {
    const baseline = new Map(input.baseline.map((prediction) => [
        prediction.taskEpisodeId,
        prediction,
    ]));
    const candidate = new Map(input.candidate.map((prediction) => [
        prediction.taskEpisodeId,
        prediction,
    ]));
    const counters = {
    ...
    for (const label of input.labels) {
        const left = classificationSuccess(label, baseline.get(label.taskEpisodeId));
        const right = classificationSuccess(label, candidate.get(label.taskEpisodeId));
        if (!left.scope && right.scope)
            counters.scopeFixed += 1;
        if (left.scope && !right.scope)
            counters.scopeRegressed += 1;
...
const readRetrievalRuntime = async (retrievalDirectory, episodes) => {
    const records = await Promise.all(episodes.map(async (episode) => JSON.parse(await readFile(path.join(path.resolve(retrievalDirectory), "gitnexus-only", "retrieval", "hybrid_rerank", `${episode.id}.json`), "utf8"))));
    const analyzeDurations = records.map((record) => record.gitNexusProvenance.analyze.durationMs);
    const queryDurations = records.map((record) => record.gitNexusProvenance.queryDurationMs);
    const maxRss = records
        .map((record) => record.gitNexusProvenance.analyze.maximumResidentSetKb ?? 0)
        .filter((value) => value > 0);
    return {
        exactSnapshotIndexes: records.length,
        totalAnalyzeHours: analyzeDurations.reduce((sum, value) => sum + value, 0) /
            3_600_000,
        meanAnalyzeMinutes: analyzeDurations.reduce((sum, value) => sum + value, 0) /
        ...
        maximumResidentSetGb: (maxRss.length === 0 ? 0 : Math.max(...maxRss)) / 1024 / 1024,
        meanQueryMs: queryDurations.reduce((sum, value) => sum + value, 0) /
            queryDurations.length,
        medianQueryMs: quantile(queryDurations, 0.5),
        p95QueryMs: quantile(queryDurations, 0.95),
        meanDefinitions: records.reduce((sum, record) => sum + record.gitNexusProvenance.graphDefinitionsReturned, 0) / records.length,
        meanUniquePaths: records.reduce((sum, record) => sum + record.gitNexusProvenance.uniqueDefinitionPaths, 0) / records.length,
    };
};
const percent = (value) => `${(value * 100).toFixed(1)}%`;
const seconds = (milliseconds) => `${(milliseconds / 1_000).toFixed(2)}s`;
const main = async () => {
    const args = parseArguments(process.argv.slice(2));
    const config = JSON.parse(await readFile(path.resolve(required(args, "--config")), "utf8"));
    ...
    const baselineClassification = required(args, "--baseline-classification-directory");
    const gitNexusAnalysis = required(args, "--gitnexus-analysis");
    const gitNexusClassification = required(args, "--gitnexus-classification-directory");
    const fusionAnalysis = required(args, "--fusion-analysis");
    const fusionClassification = required(args, "--fusion-classification-directory");
    const armInputs = [
        {
    ...
    const retrieval = JSON.parse(await readFile(path.resolve(required(args, "--retrieval-analysis")), "utf8"));
    const runtime = await readRetrievalRuntime(required(args, "--retrieval-output-directory"), episodes);
    ...
    const matchedDirectImprovement = baselineDirect.selectionLoss - fusionDirect.selectionLoss;
    const matchedEvidenceFirstImprovement = baselineEvidenceFirst.selectionLoss -
        fusionEvidenceFirst.selectionLoss;
    const directMatchesEvidenceFirst = fusionDirect.selectionLoss <=
        baselineEvidenceFirst.selectionLoss + 0.01;
    const directLatencyReduction = 1 -
        fusionDirect.usage.meanDurationMs /
            baselineEvidenceFirst.usage.meanDurationMs;
    const go = matchedDirectImprovement >= 0.01 ||
        matchedEvidenceFirstImprovement >= 0.01 ||
        (directMatchesEvidenceFirst && directLatencyReduction >= 0.25);
    const decision = {
        goToFreshConfirmation: go,
        rule: "Go if a matched GitNexus fusion arm improves selection loss by at least 0.01, or fusion-direct is within 0.01 of current evidence-first while reducing Luna duration by at least 25%.",
        matchedDirectImprovement,
        matchedEvidenceFirstImprovement,
        directMatchesEvidenceFirst,
        directLatencyReduction,
        warning: "This is a burned-development screen. A positive result selects a candidate for a fresh 40–60-case confirmation; it is not a final product-performance estimate.",
    };
    const publicArms = Object.fromEntries(Object.entries(arms).map(([id, value]) => [
        id,
        {
    ...
    const rows = Object.entries(publicArms)
        .map(([id, value]) => {
        const arm = value;
        return `| ${id} | ${arm.selectionLoss.toFixed(4)} | ${percent(arm.aggregate.scopeHitAt1)} | ${percent(arm.aggregate.areaHitAt1)} | ${percent(arm.aggregate.allGoldAt3)} | ${arm.aggregate.areaBrier.toFixed(4)} | ${seconds(arm.usage.meanDurationMs)} | $${arm.usage.meanCostUsd.toFixed(6)} |`;
    })
        .join("\n");
    const retrievalRows = [
        ...
        .map(([name, value]) => {
        const arm = value;
        return `| ${name} | ${percent(arm.oraclePaths.anyRelevantPathAt4)} | ${percent(arm.oraclePaths.anyRelevantPathAt8)} | ${percent(arm.oraclePaths.meanRelevantPathRecallAt4)} | ${percent(arm.oraclePaths.meanRelevantPathRecallAt8)} | ${percent(arm.testOrDocNoiseRate)} |`;
    })
        .join("\n");
    const directPaired = report.pairedFusionDirectVersusCurrentDirect;
    const evidenceFirstPaired = report.pairedFusionEvidenceFirstVersusCurrentEvidenceFirst;
...
(${labels.filter((label) => label.known).length} registered-area and ${labels.filter((label) => !label.known).length} new-area tasks). It does not read or make a claim on the old held-out set.
...
- Fusion-direct is ${directMatchesEvidenceFirst ? "within" : "not within"} 0.01 loss of current evidence-first.
...
${decision.warning}
`;
    await writeAtomic(path.resolve(required(args, "--markdown-output")), markdown);
