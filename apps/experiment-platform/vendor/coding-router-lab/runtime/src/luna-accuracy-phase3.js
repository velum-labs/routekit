...
import { buildLunaAccuracyArchitecturePrompt, buildLunaAccuracyJobSchedule, buildLunaAccuracyPredictionSets, assertLunaAccuracyRunComplete, lunaAccuracyAreaRegistryHash, lunaAccuracyArmsHash, lunaAccuracyCallKey, lunaAccuracyMatrixHash, lunaAccuracyModelHash, lunaAccuracyProfileHash, lunaAccuracyRuntimeEpisodesHash, normalizeLunaAccuracyArms, validateLunaAccuracyRunManifestBinding, LUNA_ACCURACY_RUNNER_VERSION, } from "./luna-accuracy-runner.js";
...
import { assessLunaAccuracyDatasetSupport, lunaAccuracySelectionComponents, lunaAccuracySelectionScore, } from "./luna-accuracy-report.js";
import { codingEpisodeIdsFromAnnotations, } from "./luna-accuracy-coding-annotations.js";
import { LUNA_ACCURACY_PHASE_THREE_ARCHITECTURES, LUNA_ACCURACY_PHASE_THREE_CONTEXT_KEYS, LUNA_ACCURACY_PHASE_THREE_PIPELINE_REPETITIONS, LUNA_ACCURACY_PHASE_THREE_SELF_CONSISTENCY_MEMBERS, LUNA_ACCURACY_PHASE_THREE_SINGLE_REPETITIONS, LUNA_ACCURACY_PHASE_TWO_B_FIXED_SEED_LIST, LUNA_ACCURACY_PHASE_TWO_B_SCHEDULE_SEED, } from "./luna-accuracy-design.js";
import { buildLunaAccuracyProviderRequest, LUNA_ACCURACY_CANONICAL_MODEL, LUNA_ACCURACY_MODEL, LUNA_ACCURACY_PROVIDER, LUNA_ACCURACY_TRANSPORT_POLICY, } from "./luna-accuracy-openrouter.js";
import { validateBenchmarkDataset } from "./validation.js";
export const LUNA_ACCURACY_PHASE_THREE_PROTOCOL = "luna-accuracy-phase3-architecture-selection-v1";
export const LUNA_ACCURACY_PHASE_THREE_EXPECTED_CASES = 26;
export const LUNA_ACCURACY_PHASE_THREE_EXPECTED_CALLS = 468;
export const LUNA_ACCURACY_PHASE_THREE_BOOTSTRAP_ITERATIONS = 10_000;
export const LUNA_ACCURACY_PHASE_THREE_BOOTSTRAP_SEED = 19_871;
/**
...
const canonicalLabels = (values) => [...values].sort((left, right) => lexicalCompare(left.taskEpisodeId, right.taskEpisodeId));
const canonicalAnnotations = (values) => [...values].sort((left, right) => lexicalCompare(left.taskEpisodeId, right.taskEpisodeId));
const canonicalCalls = (values) => [...values].sort((left, right) => lexicalCompare(left.key, right.key));
const expectedArmId = (contextKey, architecture) => {
...
const sameNumberList = (left, right) => left.length === right.length &&
    left.every((value, index) => value === right[index]);
const assertExactPhaseThreeDesign = (input) => {
    if (input.episodes.length !== LUNA_ACCURACY_PHASE_THREE_EXPECTED_CASES) {
        throw new Error(`Phase 3 requires the frozen ${LUNA_ACCURACY_PHASE_THREE_EXPECTED_CASES}-case validation set`);
    }
    if (input.episodes.some((episode) => episode.split !== "validation")) {
        throw new Error("Phase 3 may analyze validation episodes only");
    }
    if (input.manifest.scheduleSeed !==
        LUNA_ACCURACY_PHASE_TWO_B_SCHEDULE_SEED ||
        input.manifest.counts.expectedCalls !==
            LUNA_ACCURACY_PHASE_THREE_EXPECTED_CALLS) {
        throw new Error("Phase 3 manifest does not use the preregistered schedule or 468-call design");
    }
    const expectedIds = LUNA_ACCURACY_PHASE_THREE_CONTEXT_KEYS.flatMap((contextKey) => LUNA_ACCURACY_PHASE_THREE_ARCHITECTURES.map((architecture) => expectedArmId(contextKey, architecture)));
    const variantsById = new Map(input.matrix.variants.map((variant) => [variant.id, variant]));
    const armsById = new Map(input.arms.map((arm) => [arm.id, arm]));
    if (input.matrix.variants.length !== expectedIds.length ||
        variantsById.size !== expectedIds.length ||
        input.arms.length !== expectedIds.length ||
        armsById.size !== expectedIds.length ||
        expectedIds.some((id) => !variantsById.has(id) || !armsById.has(id))) {
        throw new Error("Phase 3 requires exactly the preregistered two-context/six-arm design");
    }
    const firstThreeSeeds = LUNA_ACCURACY_PHASE_TWO_B_FIXED_SEED_LIST.slice(0, 3);
    ...
    for (const contextKey of LUNA_ACCURACY_PHASE_THREE_CONTEXT_KEYS) {
        ...
        for (const architecture of LUNA_ACCURACY_PHASE_THREE_ARCHITECTURES) {
            ...
            const seeds = architecture === "proposal_verify_revise"
                ? firstSeed
                : firstThreeSeeds;
            if (variant.repetitions !== repetitions ||
                !sameNumberList(variant.fixedSeedList, seeds)) {
                throw new Error(`Phase 3 arm ${id} does not use its preregistered repetitions and seeds`);
            }
            configurationHashes.push(contentHash(contextConfiguration(variant)));
        }
        if (new Set(configurationHashes).size !== 1) {
            throw new Error(`Phase 3 ${contextKey} arms changed context settings alongside architecture`);
        }
        contextHashes[contextKey] = configurationHashes[0];
    }
    if (contextHashes.primary === contextHashes.alternative) {
        throw new Error("Phase 3 primary and alternative contexts must be provider-distinct");
    }
    return contextHashes;
};
const stageForProvider = (stage) => stage === "single" || stage === "member" ? "classify" : stage;
const assertPinnedCall = (input) => {
    const { call } = input;
    if (call.schemaVersion !== 1 ||
        ...
        throw new Error(`Phase 3 call is not a complete provider-pinned record: ${call.key}`);
    }
    const expectedProviderRequestHash = contentHash(buildLunaAccuracyProviderRequest({
        model: input.model,
        prompt: input.prompt,
    ...
    if (call.transport.providerRequestHash !== expectedProviderRequestHash) {
        throw new Error(`Phase 3 provider request hash is stale or mismatched: ${call.key}`);
    }
};
/** Reconstructs every prompt/request hash, including dependent PVR stages. */
const assertPhaseThreeCallBindings = (input) => {
    const variantById = new Map(input.matrix.variants.map((variant) => [variant.id, variant]));
...
const taskStratum = (label) => label.known
    ? label.selectedAreaIds.length === 1
        ? "known_single"
        : "known_multi"
    : label.unknownType;
const ratio = (numerator, denominator) => denominator > 0 ? numerator / denominator : null;
const f1 = (truePositive, falsePositive, falseNegative) => ratio(2 * truePositive, 2 * truePositive + falsePositive + falseNegative);
/**
...
const selectionScoreForSample = (input) => {
    ...
    for (const label of input.labels) {
        ...
        if (!prediction) {
            throw new Error(`Phase 3 bootstrap is missing prediction ${label.taskEpisodeId}`);
        }
        ...
        if (label.known) {
            ...
            if (input.codingEpisodeIds.has(label.taskEpisodeId)) {
                knownCoding.push(prediction.known &&
    ...
    const booleanMean = (values) => values.length > 0 ? values.filter(Boolean).length / values.length : null;
    ...
    const macroTaskStratumAccuracy = stratumAccuracies.length > 0
        ...
        : null;
    const knownUnknownBalancedAccuracy = knownAccuracy === null || unknownAccuracy === null
        ? null
...
const falseKnownErrors = (labels, predictions) => {
    const byId = new Map(predictions.map((prediction) => [prediction.taskEpisodeId, prediction]));
    return labels.filter((label) => !label.known && byId.get(label.taskEpisodeId)?.known === true).length;
};
const candidateSummary = (input) => {
    ...
    const metricsByPredictionSet = input.predictionSets.map((set) => {
        const metrics = calculateLunaAccuracyMetrics(input.labels, set.predictions, { codingEpisodeIds: input.codingEpisodeIds });
        const components = lunaAccuracySelectionComponents(metrics);
        if (components === null ||
            metrics.knownCodingExactSet.accuracy === null) {
            throw new Error(`Phase 3 arm ${input.arm.id} has an undefined required selection metric`);
        }
        return {
    ...
    const codingCaseCounts = new Set(metricsByPredictionSet.map((value) => value.knownCodingCases));
    if (codingCaseCounts.size !== 1 || [...codingCaseCounts][0] === 0) {
        throw new Error(`Phase 3 arm ${input.arm.id} lacks a stable known-coding denominator`);
    }
    const unknownCases = input.labels.filter((label) => !label.known).length;
    const costs = input.predictionSets.flatMap((set) => set.predictions.map((prediction) => prediction.costUsd));
    const meanCostUsdPerDecision = costs.every((value) => typeof value === "number" && Number.isFinite(value) && value >= 0)
        ? mean(costs)
        : null;
    const correct = metricsByPredictionSet.map((value) => value.knownCodingCorrect);
    const falseKnown = metricsByPredictionSet.map((value) => value.falseKnownErrors);
    ...
    return {
        ...
        predictionSetIds: metricsByPredictionSet.map((value) => value.predictionSetId),
        evaluatedPredictionSets: metricsByPredictionSet.length,
        casesPerPredictionSet: input.labels.length,
        metricsByPredictionSet,
        meanSelectionScore: mean(metricsByPredictionSet.map((value) => value.selectionScore)),
        knownCoding: {
            cases: codingCases,
...
const predictionsById = (set) => new Map(set.predictions.map((prediction) => [
    prediction.taskEpisodeId,
    prediction,
]));
export const compareLunaAccuracyPhaseThreePaired = (input) => {
    if (input.singleCallSets.length !==
        LUNA_ACCURACY_PHASE_THREE_SINGLE_REPETITIONS ||
        input.singleCallSets.some((set) => set.architecture !== "single_call" ||
            set.repetitionIndex === null || set.seeds.length !== 1) ||
        input.challengerSet.architecture === "single_call") {
        throw new Error("Phase 3 bootstrap requires three indexed single-call sets and one multi-call challenger set");
    }
    const singleCallSets = [...input.singleCallSets].sort((left, right) => left.repetitionIndex - right.repetitionIndex);
    if (singleCallSets.some((set, index) => set.repetitionIndex !== index)) {
        throw new Error("Phase 3 single-call repetitions must be indexed 0 through 2");
    }
    const expectedIds = new Set(input.episodes.map((episode) => episode.id));
    const assertExactSet = (set) => {
        ...
        if (ids.length !== expectedIds.size ||
            ...
            throw new Error(`Phase 3 prediction set ${set.id} is not an exact episode join`);
        }
    };
    [...singleCallSets, input.challengerSet].forEach(assertExactSet);
    const labelById = new Map(input.labels.map((label) => [label.taskEpisodeId, label]));
    if (labelById.size !== expectedIds.size ||
    ...
    const fullScore = (predictionById) => {
    ...
    for (let iteration = 0; iteration < LUNA_ACCURACY_PHASE_THREE_BOOTSTRAP_ITERATIONS; iteration += 1) {
        const sampledLabels = [];
        for (let draw = 0; draw < clusters.length; draw += 1) {
            const cluster = clusters[Math.floor(random() * clusters.length)];
            sampledLabels.push(...cluster.episodeIds.map((id) => labelById.get(id)));
        }
        ...
        const sampledSingleScores = [];
        for (let draw = 0; draw < LUNA_ACCURACY_PHASE_THREE_SINGLE_REPETITIONS; draw += 1) {
            const repetitionIndex = Math.floor(random() * LUNA_ACCURACY_PHASE_THREE_SINGLE_REPETITIONS);
    ...
    const variance = differences.length > 1
        ? differences.reduce((total, value) => total + (value - bootstrapMean) ** 2, 0) / (differences.length - 1)
        : 0;
    ...
    return {
        estimand: "fixed_challenger_decision_set_minus_mean_three_single_call_repetitions",
        clusterDefinition: "session_or_lineage_connected_component",
        singleCallResampling: "three_repetition_indices_with_replacement",
        challengerResampling: "fixed_final_decision_set_reused_for_each_sampled_single_repetition",
        ...
        bootstrap: {
            ...
            probabilityChallengerBetter: (better + 0.5 * tied) / differences.length,
            twoSidedSignPValue: Math.min(1, 2 * Math.min(nonPositive / differences.length, nonNegative / differences.length)),
        },
        limitation: "The challenger has one frozen final decision set. The bootstrap resamples dependence clusters and the three single-call trials, but cannot estimate independent challenger run-to-run variability.",
    };
};
const compareContext = (input) => {
    ...
    const selectionScoreLead = input.challenger.meanSelectionScore - input.baseline.meanSelectionScore;
    const selectionScoreDegradation = Math.max(0, -selectionScoreLead);
    const knownCodingMeanAccuracyLead = input.challenger.knownCoding.meanAccuracy -
        input.baseline.knownCoding.meanAccuracy;
    const knownCodingCorrectDeficitFromBestSingle = Math.max(0, input.baseline.knownCoding.maximumCorrect -
        input.challenger.knownCoding.meanCorrect);
    const falseKnownIncreaseOverSingleMean = input.challenger.falseKnown.meanErrors -
        input.baseline.falseKnown.meanErrors;
    const completeBootstrap = bootstrap.iterationsCompleted /
        bootstrap.iterationsRequested >=
        LUNA_ACCURACY_PHASE_THREE_WIN_RULE.minimumCompletedBootstrapFraction;
    const minimumSelectionScoreLead = selectionScoreLead + Number.EPSILON >=
        LUNA_ACCURACY_PHASE_THREE_WIN_RULE
            .minimumObservedSelectionScoreLeadPerContext;
    const minimumBootstrapProbability = bootstrap.bootstrap.probabilityChallengerBetter + Number.EPSILON >=
        LUNA_ACCURACY_PHASE_THREE_WIN_RULE
            .minimumBootstrapProbabilityPerContext;
    const maximumObservedDegradation = selectionScoreDegradation <=
        LUNA_ACCURACY_PHASE_THREE_WIN_RULE
            .maximumObservedSelectionScoreDegradationPerContext +
            Number.EPSILON;
    const noKnownCodingMeanAccuracyLoss = knownCodingMeanAccuracyLead + Number.EPSILON >=
        -LUNA_ACCURACY_PHASE_THREE_WIN_RULE
            .maximumKnownCodingMeanAccuracyLossPerContext;
    const maximumKnownCodingCorrectDeficit = knownCodingCorrectDeficitFromBestSingle <=
        LUNA_ACCURACY_PHASE_THREE_WIN_RULE
            .maximumKnownCodingCorrectDeficitFromBestSinglePerContext +
            Number.EPSILON;
    const maximumFalseKnownIncrease = falseKnownIncreaseOverSingleMean <=
        LUNA_ACCURACY_PHASE_THREE_WIN_RULE
    ...
    if (!maximumKnownCodingCorrectDeficit) {
        failures.push("Known-coding correct count was more than one below the best single-call trial");
    }
...
const exactCostTieBreak = (left, right) => {
    if (left.meanCostUsdPerDecision !== null &&
...
export const buildLunaAccuracyPhaseThreeSelection = (input) => {
    if (input.model !== LUNA_ACCURACY_MODEL) {
        throw new Error(`Phase 3 requires ${LUNA_ACCURACY_MODEL}`);
    }
    validateBenchmarkDataset(input.profile, input.cards, input.episodes, input.labels);
    const codingEpisodeIds = codingEpisodeIdsFromAnnotations(input.episodes, input.codingAnnotations);
    const datasetSupport = assessLunaAccuracyDatasetSupport(input.labels, codingEpisodeIds);
    if (!datasetSupport.eligibleForSelection) {
        throw new Error(`Phase 3 dataset is ineligible for selection: ${datasetSupport.ineligibilityReasons.join("; ")}`);
    }
    ...
    for (const contextKey of LUNA_ACCURACY_PHASE_THREE_CONTEXT_KEYS) {
        for (const architecture of LUNA_ACCURACY_PHASE_THREE_ARCHITECTURES) {
            ...
            if (sets.length !== expectedSets) {
                throw new Error(`Phase 3 arm ${armId} produced ${sets.length}/${expectedSets} prediction sets`);
            }
    ...
    const candidateByArm = new Map(candidates.map((candidate) => [candidate.armId, candidate]));
    const architectureAnalyses = [
        "self_consistency_3",
        "proposal_verify_revise",
    ].map((architecture) => {
        const contexts = LUNA_ACCURACY_PHASE_THREE_CONTEXT_KEYS.map((contextKey) => {
        ...
        const costs = contexts.map((context) => context.challenger.meanCostUsdPerDecision);
        return {
            architecture,
            contexts,
            passedBothContexts: contexts.every((context) => context.gates.passed),
            averageObservedSelectionScoreLead: mean(contexts.map((context) => context.observed.selectionScoreLead)),
            minimumObservedSelectionScoreLead: Math.min(...contexts.map((context) => context.observed.selectionScoreLead)),
            averageChallengerSelectionScore: mean(contexts.map((context) => context.challenger.meanSelectionScore)),
            meanCostUsdPerDecision: costs.every((value) => value !== null)
                ? mean(costs)
                : null,
        };
    });
    const qualified = architectureAnalyses
        ...
        if (Math.abs(meanLead) > Number.EPSILON)
            return meanLead;
    ...
    const selectedArchitecture = winner?.architecture ?? "single_call";
    const selectedPrimaryArmId = expectedArmId("primary", selectedArchitecture);
    ...
    return {
        ...
        provenance: {
            ...
            codingAnnotationsHash: contentHash(canonicalAnnotations(input.codingAnnotations)),
        ...
        selection: winner
            ? {
                ...
                reason: "The selected multi-call architecture passed every preregistered accuracy, paired-bootstrap, coding-safety, and false-known gate in both frozen contexts. Among passing architectures it had the strongest accuracy result; cost was considered only after exact accuracy ties.",
            }
            : {
                ...
                reason: "No multi-call architecture passed every preregistered gate in both frozen contexts, so the single-call primary-context baseline wins by default.",
            },
        limitations: [
...
export const validateLunaAccuracyPhaseThreeSelection = (input) => {
    ...
    if (contentHash(rebuilt) !== contentHash(input.selection)) {
        throw new Error("Phase 3 selection artifact does not match its bound raw inputs");
    }
    return rebuilt;
};
