...
import { buildLunaAccuracyPrompt, getLunaAccuracyRepetitionSeed, validateLunaAccuracyMatrixV2, } from "./luna-accuracy-context.js";
import { contentHash, sha256 } from "./hash.js";
import { validateAreaCards, validateEpisodes, validateRepositoryProfile, } from "./validation.js";
import { buildLunaAccuracyProviderRequest, LUNA_ACCURACY_CANONICAL_MODEL, LUNA_ACCURACY_MODEL, LUNA_ACCURACY_PROVIDER, LUNA_ACCURACY_TRANSPORT_POLICY, LUNA_ACCURACY_PREFLIGHT_VERSION, LUNA_ACCURACY_PROVIDER_SLUG, LUNA_ACCURACY_REQUIRED_PROVIDER_PARAMETERS, } from "./luna-accuracy-openrouter.js";
export const LUNA_ACCURACY_RUNNER_VERSION = "luna-accuracy-runner-v2-pinned-transport";
export const LUNA_ACCURACY_FINALIZATION_VERSION = "luna-accuracy-finalization-v1";
export const LUNA_ACCURACY_MAX_CONCURRENCY = 32;
...
export const normalizeLunaAccuracyConcurrency = (value) => {
    const concurrency = value ?? 1;
    if (!Number.isInteger(concurrency) ||
        concurrency < 1 ||
        concurrency > LUNA_ACCURACY_MAX_CONCURRENCY) {
        throw new Error(`Luna accuracy concurrency must be an integer from 1 to ${LUNA_ACCURACY_MAX_CONCURRENCY}`);
    }
    return concurrency;
};
...
const numericTotal = (predictions, key) => {
    const values = predictions.map((prediction) => prediction[key]);
    return values.every((value) => typeof value === "number" && Number.isFinite(value))
        ? values.reduce((sum, value) => sum + value, 0)
        : undefined;
};
const clonePrediction = (prediction, classifierOverride) => {
...
const persistedTransportMetadata = (metadata, providerRequestHash) => {
    if (metadata?.provider?.name !== LUNA_ACCURACY_PROVIDER ||
        metadata?.response?.model !== LUNA_ACCURACY_MODEL) {
        throw new Error("Luna accuracy executor omitted or returned mismatched pinned transport metadata");
    }
    return {
...
const validatePrediction = (prediction, taskEpisodeId, allowedAreaIds) => {
    ...
    if (prediction.taskEpisodeId !== taskEpisodeId) {
        throw new Error(`Luna accuracy prediction case mismatch: expected ${taskEpisodeId}`);
    }
    if (typeof prediction.classifier !== "string" ||
    ...
    if (typeof prediction.known !== "boolean" ||
    ...
    if (prediction.known && prediction.unknownType !== undefined) {
        throw new Error("Luna accuracy prediction returned unknownType for known work");
    }
    if (!prediction.known &&
        (prediction.unknownType === undefined ||
            !UNKNOWN_TYPES.has(prediction.unknownType))) {
        throw new Error("Luna accuracy prediction requires a valid unknown subtype");
    }
    ...
    if (prediction.gateConfidence !== undefined &&
        ...
        throw new Error("Luna accuracy prediction gateConfidence must be 0..1");
    }
    if (prediction.areaConfidence !== undefined &&
        ...
        throw new Error("Luna accuracy prediction areaConfidence must be null or 0..1");
    }
    if (prediction.areaConfidence !== undefined &&
        ((prediction.known && prediction.areaConfidence === null) ||
            (!prediction.known && prediction.areaConfidence !== null))) {
        throw new Error("Luna accuracy prediction areaConfidence conflicts with known");
    }
    if (typeof prediction.durationMs !== "number" ||
...
const expectedStages = (architecture) => {
    if (architecture === "single_call")
        return ["single"];
    if (architecture === "self_consistency_3")
        return ["member"];
    return ["proposal", "verify", "revise"];
};
export const normalizeLunaAccuracyRetryPolicy = (input = {}) => {
    const policy = {
...
export const normalizeLunaAccuracyArms = (matrix, arms) => {
    ...
    const variants = new Map(matrix.variants.map((variant) => [variant.id, variant]));
    const ids = new Set();
    for (const arm of normalized) {
        ...
        if (!ARCHITECTURES.has(arm.architecture)) {
            throw new Error(`Invalid Luna accuracy architecture: ${String(arm.architecture)}`);
        }
        const variant = variants.get(arm.variantId);
        if (!variant) {
            throw new Error(`Luna accuracy arm ${arm.id} references missing variant ${arm.variantId}`);
        }
        if (arm.architecture === "self_consistency_3" &&
            variant.repetitions !== 3) {
            throw new Error(`Self-consistency arm ${arm.id} requires exactly three repetitions`);
        }
    }
    return normalized;
};
const uint32FromHash = (value) => Number.parseInt(contentHash(value).slice(0, 8), 16) >>> 0;
...
export const lunaAccuracyJobKey = (job) => contentHash({
    schemaVersion: 1,
    armId: job.armId,
...
export const lunaAccuracyCallKey = (job, stage) => contentHash({
    schemaVersion: 1,
    jobKey: job.key,
    armId: job.armId,
    variantId: job.variantId,
...
export const buildLunaAccuracyJobSchedule = (input) => {
    ...
    if (!Number.isInteger(scheduleSeed) ||
        scheduleSeed < 0 ||
    ...
    for (const arm of [...arms].sort((left, right) => lexicalCompare(left.id, right.id))) {
        const variant = variants.get(arm.variantId);
        const jobs = [];
        for (const taskEpisodeId of [...episodeIds].sort(lexicalCompare)) {
            for (let repetitionIndex = 0; repetitionIndex < variant.repetitions; repetitionIndex += 1) {
                const withoutKey = {
                    armId: arm.id,
        ...
        buckets.set(arm.id, shuffle(jobs, seededRandom(uint32FromHash({
            scheduleSeed,
            armId: arm.id,
            purpose: "within-arm",
        }))));
    ...
    while (result.length < [...buckets.values()].flat().length) {
        const active = [...buckets.keys()].filter((id) => (offsets.get(id) ?? 0) < (buckets.get(id)?.length ?? 0));
        const orderedArms = shuffle(active.sort(lexicalCompare), seededRandom(uint32FromHash({
            scheduleSeed,
            round,
...
const runtimeEpisodeProjection = (episode) => ({
...
const canonicalCards = (cards) => [...cards].sort((left, right) => lexicalCompare(left.areaId, right.areaId));
const canonicalEpisodes = (episodes) => [...episodes].sort((left, right) => lexicalCompare(left.id, right.id));
export const lunaAccuracyModelHash = (model) => contentHash(model);
export const lunaAccuracyProfileHash = (profile) => contentHash(profile);
export const lunaAccuracyAreaRegistryHash = (cards) => contentHash(canonicalCards(cards));
export const lunaAccuracyRuntimeEpisodesHash = (episodes) => contentHash(canonicalEpisodes(episodes).map(runtimeEpisodeProjection));
export const lunaAccuracyMatrixHash = (matrix) => contentHash(matrix);
export const lunaAccuracyArmsHash = (matrix, arms) => contentHash(normalizeLunaAccuracyArms(matrix, arms));
const bindLunaAccuracyTransportPreflight = (preflight) => {
...
const validateRunInputs = (profile, cards, episodes, matrix) => {
    ...
    for (const episode of episodes) {
        if (episode.repositoryId !== profile.repositoryId) {
            throw new Error(`Luna accuracy episode repository mismatch: ${episode.id}`);
        }
    }
...
export const buildLunaAccuracyRunManifest = (input) => {
    validateRunInputs(input.profile, input.cards, input.episodes, input.matrix);
    if (!input.model.trim()) {
    ...
    const preflight = bindLunaAccuracyTransportPreflight(input.transportPreflight);
    const transport = {
        policyVersion: LUNA_ACCURACY_TRANSPORT_POLICY.version,
        ...
        expectedCanonicalModel: LUNA_ACCURACY_TRANSPORT_POLICY.expectedCanonicalModel,
        expectedProviderName: LUNA_ACCURACY_TRANSPORT_POLICY.expectedProviderName,
        ...(preflight.binding
            ? {
    ...
    const expectedCalls = schedule.reduce((sum, job) => sum + expectedStages(job.architecture).length, 0);
...
export const validateLunaAccuracyRunManifestBinding = (input) => {
    ...
    const expectedTransport = {
        ...
        ...(manifest.transport.catalogPreflight !== undefined
            ? {
                ...
                catalogPreflightHash: contentHash(manifest.transport.catalogPreflight),
            }
            : {}),
    };
    if (contentHash(manifest.transport) !== contentHash(expectedTransport)) {
        throw new Error("Luna accuracy run manifest uses an unpinned or changed transport policy");
    }
    if (!Number.isInteger(manifest.scheduleSeed) ||
    ...
    if ((input.episodes !== undefined &&
        ...
        throw new Error("Luna accuracy run manifest does not match the supplied runtime dataset");
    }
    if (input.profile !== undefined &&
        manifest.hashes.profile !== lunaAccuracyProfileHash(input.profile)) {
        throw new Error("Luna accuracy run manifest does not match the repository profile");
    }
    if (input.cards !== undefined &&
        manifest.hashes.registry !==
            lunaAccuracyAreaRegistryHash(input.cards)) {
        throw new Error("Luna accuracy run manifest does not match the Area Registry");
    }
    if (input.arms !== undefined && input.matrix === undefined) {
        throw new Error("Luna accuracy manifest arm validation requires the matrix");
    }
    if (input.matrix !== undefined) {
        validateLunaAccuracyMatrixV2(input.matrix);
        const normalizedArms = normalizeLunaAccuracyArms(input.matrix, input.arms);
        const schedule = buildLunaAccuracyJobSchedule({
            matrix: input.matrix,
            episodes: input.episodes ??
                Array.from({ length: manifest.counts.cases }, (_, index) => ({
        ...
        const expectedCalls = schedule.reduce((sum, job) => sum + expectedStages(job.architecture).length, 0);
        if (manifest.hashes.matrix !== lunaAccuracyMatrixHash(input.matrix) ||
            ...
            throw new Error("Luna accuracy run manifest does not match the supplied matrix or arms");
        }
    }
...
const atomicWrite = async (file, text, mode = 0o600) => {
...
const atomicWriteJsonl = async (file, values) => {
...
const createOrValidateManifest = async (file, expected) => {
    try {
        ...
        if (!isRecord(existing) ||
            ...
            throw new Error("Existing Luna accuracy run manifest does not match this run");
        }
        return existing;
    }
    catch (error) {
...
const callStateFile = (callStateDirectory, callKey) => path.join(callStateDirectory, `${sha256(callKey)}.json`);
const validateCallRecordShape = (value) => {
    if (!isRecord(value) ||
...
const loadCallRecords = async (callStateDirectory) => {
    let entries;
    try {
...
const publicArchitectureDecision = (prediction) => ({
    known: prediction.known,
...
the closest competing registered areas before returning the requested JSON.`,
            serializationVersion: `${input.base.serializationVersion}/pvr/proposal-v1`,
        };
    }
    if (!input.proposal) {
...
${JSON.stringify(publicArchitectureDecision(input.proposal))}`,
            serializationVersion: `${input.base.serializationVersion}/pvr/verify-v1`,
        };
    }
    if (!input.verification) {
...
${JSON.stringify(publicArchitectureDecision(input.verification))}`,
        serializationVersion: `${input.base.serializationVersion}/pvr/revise-v1`,
    };
};
...
const retryAfterMsFromError = (error) => {
    if (!isRecord(error))
        return undefined;
...
const defaultBackoffMs = (context, policy) => {
    const base = Math.min(policy.maximumBackoffMs, policy.initialBackoffMs *
        policy.backoffMultiplier ** Math.max(0, context.attempt - 1));
    if (base === 0 || policy.jitterFraction === 0)
        return Math.round(base);
    const random = seededRandom(uint32FromHash({
        jobKey: context.job.key,
        stage: context.stage,
        attempt: context.attempt,
        purpose: "retry-jitter",
    }))();
    const multiplier = 1 - policy.jitterFraction + random * policy.jitterFraction * 2;
...
export const executeLunaAccuracyCallWithRetry = async (input) => {
    ...
    for (let attempt = 1; attempt <= policy.maxAttempts; attempt += 1) {
        ...
        try {
            const result = await withTimeout(policy.timeoutMs, async (signal) => input.executor({
                ...input.executorInput,
                attempt,
                signal,
            }));
            const prediction = (isRecord(result) &&
                "prediction" in result &&
                isRecord(result.prediction))
                ? result.prediction
                : result;
            const metadata = (isRecord(result) &&
                "prediction" in result &&
                "metadata" in result &&
                isRecord(result.metadata))
                ? result.metadata
                : undefined;
            validatePrediction(prediction, input.executorInput.taskEpisodeId, input.executorInput.allowedAreaIds);
            return {
                prediction: clonePrediction(prediction, input.executorInput.classifierLabel),
                ...(metadata ? { metadata } : {}),
                attempts: attempt,
            };
        }
            ...
            if (!retry)
                throw error;
            const backoffMs = input.hooks?.backoffMs?.(context, policy) ??
                defaultBackoffMs(context, policy);
            const milliseconds = Math.min(policy.maximumBackoffMs, Math.max(0, Math.round(Math.max(backoffMs, retryAfterMsFromError(error) ?? 0))));
            await sleep(milliseconds);
        }
    }
    throw lastError;
};
export const aggregateLunaAccuracySelfConsistency3 = (predictions, classifierLabel = "luna-accuracy:self-consistency-3") => {
    ...
    if (predictions.some((prediction) => prediction.taskEpisodeId !== taskEpisodeId)) {
    ...
    for (const prediction of predictions) {
        ...
        byDecision.set(key, {
            predictions: group,
            averageConfidence: group.reduce((sum, item) => sum + item.confidence, 0) /
                group.length,
        });
    }
    const winner = [...byDecision.entries()].sort(([leftKey, left], [rightKey, right]) => right.predictions.length - left.predictions.length ||
        right.averageConfidence - left.averageConfidence ||
        lexicalCompare(leftKey, rightKey))[0][1];
    const representative = [...winner.predictions].sort((left, right) => right.confidence - left.confidence ||
        lexicalCompare(left.classifier, right.classifier))[0];
    ...
    const reasoningOutputTokens = numericTotal(predictions, "reasoningOutputTokens");
    ...
    return {
        ...
        durationMs: Math.max(...predictions.map((prediction) => prediction.durationMs)),
        ...(inputCharacters !== undefined ? { inputCharacters } : {}),
        ...(inputTokens !== undefined ? { inputTokens } : {}),
        ...(cachedInputTokens !== undefined ? { cachedInputTokens } : {}),
        ...(outputTokens !== undefined ? { outputTokens } : {}),
...
export const combineLunaAccuracyProposalVerifyRevise = (proposal, verification, revision, classifierLabel = "luna-accuracy:proposal-verify-revise") => {
    const predictions = [proposal, verification, revision];
    if (predictions.some((prediction) => prediction.taskEpisodeId !== revision.taskEpisodeId)) {
    ...
    return {
        ...
        classifier: classifierLabel,
        durationMs: predictions.reduce((sum, prediction) => sum + prediction.durationMs, 0),
        ...(inputCharacters !== undefined ? { inputCharacters } : {}),
        ...(inputTokens !== undefined ? { inputTokens } : {}),
        ...(cachedInputTokens !== undefined ? { cachedInputTokens } : {}),
        ...(outputTokens !== undefined ? { outputTokens } : {}),
...
const expectedCallMetadata = (schedule) => {
...
const assertRecordMetadata = (record, expected, inputHash, allowedAreaIds) => {
    const { job, stage } = expected;
    if (record.key !== lunaAccuracyCallKey(job, stage) ||
...
export const assertLunaAccuracyRunComplete = (input) => {
    ...
    for (const record of input.records) {
        ...
        assertRecordMetadata(record, metadata, input.inputHash, input.allowedAreaIds);
    }
...
const exactCaseSet = (predictions, expectedCaseIds, label) => {
    ...
    if (new Set(ids).size !== ids.length ||
        ...
        throw new Error(`Luna accuracy prediction set ${label} is not a complete exact case set`);
    }
};
export const buildLunaAccuracyPredictionSets = (input) => {
    ...
    const recordFor = (armId, caseId, repetitionIndex, stage) => {
        const matches = input.records.filter((record) => record.armId === armId &&
            record.taskEpisodeId === caseId &&
            record.repetitionIndex === repetitionIndex &&
            record.stage === stage);
        if (matches.length !== 1) {
            throw new Error(`Expected one ${stage} call for ${armId}/${caseId}/${repetitionIndex}`);
        }
        return matches[0];
    };
    const sets = [];
    for (const arm of arms) {
        ...
        if (arm.architecture === "self_consistency_3") {
            const predictions = [...expectedCaseIds]
                .sort(lexicalCompare)
                .map((caseId) => aggregateLunaAccuracySelfConsistency3([0, 1, 2].map((repetitionIndex) => recordFor(arm.id, caseId, repetitionIndex, "member").prediction), `llm:accuracy:${arm.id}:self-consistency-3`));
        ...
        for (let repetitionIndex = 0; repetitionIndex < variant.repetitions; repetitionIndex += 1) {
            const predictions = [...expectedCaseIds]
                ...
                if (arm.architecture === "single_call") {
                    return clonePrediction(recordFor(arm.id, caseId, repetitionIndex, "single").prediction);
                }
                return combineLunaAccuracyProposalVerifyRevise(recordFor(arm.id, caseId, repetitionIndex, "proposal").prediction, recordFor(arm.id, caseId, repetitionIndex, "verify").prediction, recordFor(arm.id, caseId, repetitionIndex, "revise").prediction, `llm:accuracy:${arm.id}:proposal-verify-revise`);
            });
            exactCaseSet(predictions, expectedCaseIds, `${arm.id}:repeat-${repetitionIndex + 1}`);
            sets.push({
...
const canonicalCompletedCalls = (records) => [...records].sort((left, right) => lexicalCompare(left.armId, right.armId) ||
...
const canonicalPredictionSets = (predictionSets) => predictionSets.map((predictionSet) => ({
    ...predictionSet,
    seeds: [...predictionSet.seeds],
    predictions: [...predictionSet.predictions].sort((left, right) => lexicalCompare(left.taskEpisodeId, right.taskEpisodeId)),
}));
const completedAtFromCalls = (records) => {
    if (records.length === 0) {
...
export const runLunaAccuracyExperiment = async (input) => {
    ...
    const callStateDirectory = path.join(input.runDirectory, "private", "call-state");
    const callsJsonl = path.join(input.runDirectory, "private", "completed-calls.jsonl");
    const predictionSetsDirectory = path.join(input.runDirectory, "private", "prediction-sets");
    const summaryPath = path.join(input.runDirectory, "summary.json");
    const finalizationPath = path.join(input.runDirectory, "finalization.json");
    await mkdir(input.runDirectory, { recursive: true, mode: 0o700 });
    const manifest = await createOrValidateManifest(manifestPath, expectedManifest);
    ...
    for (const record of records.values()) {
        ...
        assertRecordMetadata(record, metadata, manifest.inputHash, allowedAreaIds);
    }
    const variants = new Map(input.matrix.variants.map((variant) => [variant.id, variant]));
    ...
    let resumedCalls = 0;
    let attempts = 0;
    const executeOrReuse = async (job, stage, prompt, variant) => {
        const key = lunaAccuracyCallKey(job, stage);
        const promptHash = contentHash(prompt);
        const providerRequestHash = contentHash(buildLunaAccuracyProviderRequest({
            model: input.model,
            prompt,
            variant,
            allowedAreaIds,
        ...
        if (prior) {
            assertRecordMetadata(prior, { job, stage }, manifest.inputHash, allowedAreaIds);
            if (prior.promptHash !== promptHash) {
                throw new Error(`Luna accuracy prompt changed for completed call ${key}`);
            }
            if (prior.transport.policyVersion !==
                LUNA_ACCURACY_TRANSPORT_POLICY.version ||
                ...
                throw new Error(`Luna accuracy transport changed for completed call ${key}`);
            }
            resumedCalls += 1;
            return prior;
        }
        ...
        const record = {
            ...
            transport: persistedTransportMetadata(executed.metadata, providerRequestHash),
        };
    ...
    const executeJob = async (job) => {
        ...
        const proposal = await executeOrReuse(job, "proposal", buildLunaAccuracyArchitecturePrompt({
            base,
            stage: "proposal",
        }), variant);
        const verification = await executeOrReuse(job, "verify", buildLunaAccuracyArchitecturePrompt({
            base,
            stage: "verify",
            proposal: proposal.prediction,
        }), variant);
        await executeOrReuse(job, "revise", buildLunaAccuracyArchitecturePrompt({
            base,
            stage: "revise",
            proposal: proposal.prediction,
    ...
    const predictionSets = canonicalPredictionSets(buildLunaAccuracyPredictionSets({
        records: completed,
        episodes: input.episodes,
        matrix: input.matrix,
        arms,
    ...
    for (const predictionSet of predictionSets) {
        const filename = `${safeFilename(predictionSet.armId)}-${predictionSet.repetitionIndex === null
            ? "ensemble"
            : `repeat-${predictionSet.repetitionIndex + 1}`}.jsonl`;
        await atomicWriteJsonl(path.join(predictionSetsDirectory, filename), predictionSet.predictions);
    }
    ...
    for (const file of [
        callsJsonl,
        ...completed.map((record) => callStateFile(callStateDirectory, record.key)),
        ...predictionSetFiles.map((entry) => path.join(predictionSetsDirectory, entry)),
    ]) {
