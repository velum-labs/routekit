...
<ClaudeCodeModel native /> still resolves to
<ClaudeCodeModel />, and <CodexModel native /> in Codex still resolves to
<CodexModel />. RouteKit then selects an eligible managed account and forwards
...
(<ClaudeCodeModel native suffix=":high" />, `routekit/`<OpenAIModel suffix=":high" />). Selecting a variant
