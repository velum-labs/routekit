...
Every subscription kind enrolls through the same command. Choose the account
...
configuration. After enrollment, remove the bootstrap API route with `routekit
providers remove openai` if you do not intend to keep it. You may enroll either
subscription kind independently or add the other kind later.
