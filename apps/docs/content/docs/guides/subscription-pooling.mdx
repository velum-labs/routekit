...
For a guided setup that can combine subscriptions with selected API routes,
run `routekit setup` instead. `config init --empty` is the deterministic
subscription-only primitive: it starts a provider-free daemon so the first
transactional account enrollment can enable its provider.
