...
In RouteKit 0.18.0 and newer, create a deterministic Bedrock starter with an
explicit approved default:
...
model picker. On RouteKit 0.17.4, add Bedrock to an existing working router
with `routekit providers add bedrock --default-model
bedrock/APPROVED_MODEL_OR_INFERENCE_PROFILE_ID`.
