...
## Preview the remote installation
...
SSH control relay. `start`, `stop`, `setup`, `config init`, and advanced
`daemon` operations remain local-only.
...
3. The remote daemon advertises `routekit.control.v2`.
...
[data, credentials, and privacy](/docs/concepts/privacy) before exposing a
shared gateway.
