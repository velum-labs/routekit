...
| Cursor | `GET /v1/cursor/models`, `POST /v1/cursor/chat/completions` |

Chat Completions, Messages, and Responses support streaming and tools. The
Cursor base URL is `<gateway>/v1/cursor`; its advertised IDs use the
`routekit/` prefix and include reasoning-effort variants where supported.
