---
title: "Package map"
description: "RouteKit TypeScript package map and ownership."
---
...
Large packages also publish narrow subpaths for their explicitly owned
capabilities; consumers should prefer them over broad composition roots.
...
[TypeScript API status](/docs/reference/api).
TypeDoc Markdown can be generated on demand with `pnpm docs:generate-code` for
local source review. It is not routed through the public Fumadocs site.

## Workspace directories and package names
...
Only the Codex and Claude packages correspond to current public client
support. Cursor and OpenCode integrations are retained internal surfaces;
package presence does not make either client supported. See
...
These exports are intentional review surfaces, not compatibility promises.
Dependency-cruiser also prevents production modules from importing their own
package through its root barrel.
