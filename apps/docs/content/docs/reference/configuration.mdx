...
The no-flag form creates the canonical OpenAI starter. Use `routekit setup` for
interactive, multi-route onboarding; it authenticates and discovers selected
API providers before writing a fresh configuration, enrolls selected
...
directory. It always uses the canonical global path. Import a complete project
...
The embeddable `@velum-labs/routekit-config` SDK loads the canonical path by
default, or one explicit complete document supplied by the embedding. It does
not discover or layer project configuration.
