...
<Callout type="info" title="Commands added in RouteKit 0.18.0">
  The guided `setup` command and the `config init --provider`,
  `--default-model`, and `--empty` options are merged for 0.18.0. They are
  marked below because the currently published 0.17.4 CLI does not include
  them. Always confirm with `routekit version` and `routekit <command> --help`.
</Callout>
...
| Command | Purpose |
| --- | --- |
| `routekit setup [--no-browser]` | Interactively select, verify, and configure one or more first-launch routes; local-only, 0.18.0+. |
| `routekit config init [--force]` | Create the backward-compatible OpenAI starter at `~/.config/routekit/router.yaml`; local-only. |
| `routekit config init --provider <provider> [--default-model <provider/model>]` | Create one deterministic API-provider starter; local-only, 0.18.0+. |
| `routekit config init --empty` | Start with no provider before the first subscription enrollment; local-only, 0.18.0+. |
...
Both commands launch the installed native client and accept `--gateway-url`, `--auth-token` or
