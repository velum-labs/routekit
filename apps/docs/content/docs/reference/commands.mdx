...
| `--config <path>` | Diagnostic input for `doctor`; it is not a daemon scope selector. |
...
remain stable; JSON reports the previous/new worker PIDs and generation. Legacy
combined daemons use one graceful hard restart to migrate to the host/worker
layout. Listener-setting and host-protocol-incompatible changes require a hard
