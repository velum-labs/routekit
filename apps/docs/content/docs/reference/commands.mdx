...
| `routekit config init [--force]` | Create the default OpenAI starter at `~/.config/routekit/router.yaml`; local-only. |
...
`[provider/model]` is optional. Without it, RouteKit chooses a suitable
configured or compatible startup model and makes the compatible RouteKit
catalog available through the native client's model picker. You can switch
models inside the client. Supplying an exact namespaced ID pins the startup
model, which is useful for automation, reproducible runs, and one-off
comparisons.

Both commands launch the installed native client in the current directory and
accept `--gateway-url`, `--auth-token` or `--auth-token-env`, `--effort`, and
`--cwd`. Use `--cwd` to launch in another directory. Authenticated non-loopback
gateway URLs must use HTTPS. Codex uses Responses-compatible routes only. See
[Models and catalog behavior](/docs/reference/model-catalog) for implicit Codex selection and
...
surfaces. See [supported coding tool versions](/docs/reference/client-compatibility).
...
Each native-client install also installs the product-owned `routekit` skill for
onboarding, configuration, recovery, and compositional eval routing. An
unchanged RouteKit-managed copy is updated in place; a user-owned or edited
...
## Inspect models, usage, and health
...
[Set up a remote gateway](/docs/guides/remote-gateway) for the trust and network model.
...
Use top-level `routekit status` to inspect daemon and gateway state.
...
`daemon restart` rolls the active worker using the current entrypoint.
`daemon upgrade` rolls to the invoking CLI entrypoint after an exact version
probe. On host protocol v1 the host PID, data URL, public port, and control port
remain stable; JSON reports the previous/new worker PIDs and generation.
Listener-setting and host-protocol changes that cannot roll require a hard
service reinstall or restart.

## Update and configure the CLI

| Command | Purpose |
| --- | --- |
| `routekit eval setup [--repository <path>]` | Initialize or resume one durable compositional-routing project. |
| `routekit eval status [--repository <path>]` | Show project state, approvals, plans, and runs. |
| `routekit eval answer --answer <text>` | Answer exactly one unresolved setup question. |
| `routekit eval propose dimensions` | Generate a reviewable workload-dimension basis on the configured RouteKit target. |
| `routekit eval propose evaluations` | Generate dimension, decomposition, and composition evaluations for review. |
| `routekit eval approve dimensions --digest <digest>` | Approve the exact reviewed routing-basis digest. |
| `routekit eval approve evaluations --digest <digest>` | Approve the exact reviewed evaluation digest. |
| `routekit eval validate` | Validate configuration, approvals, and artifacts without model calls. |
| `routekit eval estimate [--scope pilot|full]` | Materialize an immutable exact-call execution plan before spend. |
| `routekit eval run --plan <id>` | Execute through the configured RouteKit target. Optional external `--gateway-url` and `--token-file` are qualification-only. |
| `routekit eval results [--run <id>]` | Show sanitized qualification evidence, accounting, and cleanup outcomes. |
| `routekit eval publish --run <id>` | Atomically activate an already-qualified configured-target run. |
| `routekit policy show` | Print the eval isolation policy (dedicated token, no auto-router). |
