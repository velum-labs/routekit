...
environment that supplies the token separately; it cannot be combined with
`--rotate-token`. Use `routekit credentials shell --tool <codex|claude>` only
when a shell needs explicit credential exports for a configured integration.
