...
| Command | Purpose |
| --- | --- |
| `routekit eval run --spec <path> --url <gateway> --token <token>` | Run an offline evaluation suite with explicit model IDs. |
| `routekit eval show --run-id <id>` | Read an immutable raw evaluation run. |
