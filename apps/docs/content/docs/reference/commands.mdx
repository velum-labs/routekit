...
| `routekit calls show <call-id>` | Show routing, billing, retries, usage, cost, principal, and account attribution. |
...
| Command | Purpose |
| --- | --- |
| `routekit eval prepare --profile <id>` | Start or resume the durable eval-routing interview. |
| `routekit eval answer --profile <id> --answer <text>` | Answer exactly one interview question. |
| `routekit eval status --profile <id>` | Show the durable interview state and current question. |
| `routekit eval validate --profile <id>` | Dry-load the generated `*.eval.ts` suite without model calls. |
| `routekit eval estimate --profile <id> [--mode pilot\|full]` | Count candidate and judge calls before approving spend. Unknown pricing remains unmeasured. |
| `routekit eval run --profile <id> --url <gateway> --token <token>` | Run the approved generated suite through an injected RouteKit gateway. |
| `routekit eval publish --profile <id>` | Publish the approved winner under `$ROUTEKIT_HOME/eval/published-routing.v1.json`. |
