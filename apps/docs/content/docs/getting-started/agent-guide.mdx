---
title: "Use RouteKit from an agent"
...
If the product-owned `$routekit` skill is available, use it as the primary
workflow. It unifies installation, onboarding, configuration, operation,
recovery, and compositional eval routing through the public CLI. This page is
the bootstrap contract when that skill has not been installed yet.
...
Interactive human sessions can omit the model and switch routes from the
native picker. Automation should pin an exact namespaced ID so startup behavior
is deterministic.
