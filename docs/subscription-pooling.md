...
Codex presents its subscription models under bare native names in its picker.
Claude Code's installed picker uses reversible
`anthropic.routekit.<canonical-routekit-id>` custom entries derived from the
same `modelPolicy`; direct Claude commands may use a bare native ID only when
it is unambiguous. RouteKit then selects an eligible managed account and
forwards the unchanged provider-native request. The native relay is part of the
RouteKit-owned pooling path, not a bypass around it.

Claude Code uses its native effort selector, which RouteKit forwards as
adaptive thinking plus the selected effort. Cursor continues to expose effort
as model-name variants, while Codex uses its native effort picker levels.

Use canonical namespaced IDs in router YAML and generic gateway API requests;
the bare native spelling is only a supported Claude launch convenience. Do not
put account enrollment, provider policy, URLs, or keys in client application
