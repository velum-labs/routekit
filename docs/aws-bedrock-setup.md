...
`bedrock/openai.gpt-5.6-luna`) use the regional Mantle endpoint
`https://bedrock-mantle.<region>.api.aws/openai/v1` with
`AWS_BEARER_TOKEN_BEDROCK`. RouteKit does not advertise other OpenAI model IDs
only because a bearer token is present. Anthropic models stay on Converse and
the AWS SDK credential chain. The bearer token is optional when you only use
Anthropic models. Mantle availability is regional.
